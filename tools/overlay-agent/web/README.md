...
기존 Before/After 화면에서 로컬 YOLO와 SAM2를 사용해 1인 또는 커플의 정밀 윤곽과 수평 가이드를 생성합니다. 관리자는 After 캔버스에서 고정된 건물·무대 구조를 최대 5개의 배경선으로 직접 등록할 수 있습니다. 사진은 이 PC의 로컬 서버에서 처리되며 외부 Vision API로 전송되지 않습니다.
...
5. 수평선과 투명도를 조정합니다.
6. 투명 Overlay PNG 또는 `guide.json`을 다운로드합니다.

## 촬영 구도 비교

`/compare/` 페이지는 `예시 사진 + guide.json + 촬영 사진`을 받아 다음을 비교합니다.
