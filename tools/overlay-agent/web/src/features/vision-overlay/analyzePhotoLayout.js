...
import { loadPoseLandmarker } from "../../vision";
...
async function analyzeWithBrowser({ image, mode, onProgress }) {
  onProgress(1, "브라우저 Vision 모델을 준비하고 있습니다.");
 ...
 */
export async function analyzePhotoLayout({ file, image, mode, onProgress = () => {} }) {
  const result = browserAnalysisMode
    ? await analyzeWithBrowser({ image, mode, onProgress })
