...
export default function InfoBox({ data }: InfoBoxProps) {
  ...
  const wasteStable = top.avgWasteRate < 15;
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '20px', fontWeight: '800', color: '#0F172A', lineHeight: '1.35', marginBottom: '10px' }}>
        <span style={{ color: '#2563EB' }}>{top.category}</span>은 현재 가장 높은 수익성을 보이고 있습니다.
      </div>

      <div style={{ fontSize: '14px', fontWeight: '600', color: '#1D4ED8', lineHeight: '1.6', marginBottom: '14px' }}>
        최근 1개월 기준 가장 높은 순이익 기여와 {wasteStable ? '안정적인 폐기율' : '개선이 필요한 폐기율'}을 바탕으로 한 분석입니다.
      </div>
      ...
      <div style={{ display: 'flex', gap: '22px', flexWrap: 'wrap', marginBottom: '14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', fontWeight: '700', color: '#1D4ED8', whiteSpace: 'nowrap' }}>
          <span>✓</span>순이익 기여도 {top.netShare.toFixed(0)}%
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', fontWeight: '700', color: '#1D4ED8', whiteSpace: 'nowrap' }}>
          <span>✓</span>폐기율 {wasteStable ? '안정 유지' : '변동 있음'} ({top.avgWasteRate.toFixed(1)}%)
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', fontWeight: '700', color: '#1D4ED8', whiteSpace: 'nowrap' }}>
          <span>✓</span>평균 마진율 {top.avgMarginRate.toFixed(0)}%
        </div>
      </div>
