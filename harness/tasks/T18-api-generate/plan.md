...
> 최종 갱신일: 2026-07-20
>
> 현재 단계: provider 비종속 서버 기반·자동검증 완료
>
> 다음 행동: T19 프롬프트 자산을 준비하고 T20에서 실제 provider client·키·Preview 왕복을 연결
>
> CHECKLIST 항목: T18

## 1. 목표와 완료조건

- 해결할 사용자/제품 문제: 브라우저 밖에서 provider 키 보호, 요청 취소, 오류 정규화와 남용 제한을 수행할 서버 경계가 없다.
...
| AC-8 | 실제 provider client·키·프롬프트 structured output·Preview 왕복은 T20 완료조건으로 분리하고, T18 진입점은 미연결 provider를 명시적으로 실패시켜 실제 AI처럼 가장하지 않는다. | 진입점 테스트 + CHECKLIST 책임 감사 | 필수 |
| AC-9 | `tsconfig.api.json` 타입검사, 관련 테스트, 전체 test/lint/build/diff와 명시적 `any` 0건을 통과한다. | 최종 자동 검증 | 필수 |

## 2. 의존성·정본 확인

- T항목과 의존성 상태 또는 비-T 식별 근거: T3 완료. 2026-07-20 사용자가 백엔드 기반 구현과 제품·출시 gate를 분리하는 구조 변경을 승인했다. 따라서 T18은 provider 비종속 프록시 기반을 완료조건으로 하고, T17 배포와 T25/R2 제품 판정은 T20 실 provider 품질·출시 진행에 적용한다.
...
| 요소 | 계획 |
| --- | --- |
| 맥락 | React 19+TypeScript+Vite 프론트에 Vercel Node Function을 추가한다. provider 키는 서버에만 두고 기존 공용 생성 계약과 `any` 금지를 유지한다. T18은 provider 비종속 기반만 완료하며 실제 AI 동작은 T20 전까지 주장하지 않는다. |
...
- 구조·계약·의존성 승인이 필요한 지점: 공용 요청/응답 필드 변경, 새 런타임·서버 프레임워크, 실제 provider 추가, 클라이언트 라우팅 변경은 별도 승인과 T20 계획이 필요하다.
...
| 2026-07-20 | 승인됨 → 종료 | T18은 provider 비종속 프록시 기반으로 완료하고, 실 provider·키·Preview는 T20, Preview DB background write는 T31로 분리 | 사용자 “승인” |
...
- 현재 작업 중인 단계: 없음 — T18 종료.
- 다음 행동: T19 프롬프트 자산을 준비하고, T17·T19와 품질 gate 충족 뒤 T20에서 실제 provider client·키·Preview 왕복을 연결한다.
- 보류 사유와 재개 조건: 없음. 실제 AI 연동은 T18 재개가 아니라 T20 범위다.
...
| 2026-07-20 | 실제 provider·Preview를 T20으로 분리하고 T18 provider 비종속 기반을 완료 판정 | 백엔드 구현 gate와 제품·출시 gate를 분리한 사용자 승인 반영 |
