...
- 목표 대비 결과: provider 비종속 `/api/generate` 서버 기반과 자동검증을 완료했다. 실제 provider·키·프롬프트·Preview 연동은 T20으로 분리했으며 T18 완료조건이 아니다.
...
| AC-8 | 진입점 테스트 + CHECKLIST 책임 감사 | 통과 | 미연결 provider가 유효 요청을 명시적 500으로 반환해 실제 AI처럼 가장하지 않으며, 실제 provider client·키·프롬프트·Preview 왕복은 T20 완료조건으로 기록 |
| AC-9 | 전체 자동 검증 | 통과 | T18 관련 24개·전체 215개, API 타입검사, lint, build, diff, 명시적 `any`·금지 의존성 검사 통과 |

## 3. 자동 검증

| 항목 | 명령 또는 방법 | 결과 | 근거·관찰 요약 |
| --- | --- | --- | --- |
| 관련 테스트 | `npm test -- api/_lib/generation --reporter=dot` | 통과 | 3파일 24개 통과 |
| API 타입검사 | `npm run typecheck:api` | 통과 | `tsc -p tsconfig.api.json` 오류 0건 |
| 전체 테스트 | `npm test -- --reporter=dot` | 통과 | 23파일 215개 통과. jsdom 기존 `scrollTo` 미구현 로그만 발생 |
...
| --- | --- | --- | --- | --- |
| 실 provider `/api/generate` | Vercel Preview | 직접입력 요청이 provider 왕복하고 키·원문이 노출되지 않음 | T20에서 수행 | T18 비적용 — T20 인계 |
...
- 미치환 필수 항목 없음: 예.
- 남은 위험·알려진 한계: 운영 진입점은 의도적으로 unconfigured provider라 현재 유효 요청도 500을 반환하고 프론트는 계속 목을 사용한다. limiter는 서버리스 인스턴스별 best-effort이며 raw client key는 영구 로그·DB가 아니라 인스턴스 Map의 window counter로만 쓰이고 같은 key의 window 갱신 또는 인스턴스 재활용 때 교체된다. Vercel Fluid Compute 또는 함수 max duration이 18초보다 긴지는 T20 Preview에서 확인한다. 18초·1024 토큰은 T20 실측 전 잠정값이다.

## 6. 완료 판정·인계

- 필수 AC 전부 통과: 예 — provider 비종속 기반 AC-1~9 통과.
- 적용되는 자동·수동 검증 전부 통과: T18 범위의 자동 검증 전부 통과. 실 provider 수동 검증은 T20 범위다.
- 미해결 차단사항 없음: T18 범위에는 없음.
- `docs/CHECKLIST.md` 갱신 여부와 근거: 2026-07-20 체크 완료. 사용자 승인으로 실제 provider·Preview 책임을 T20으로 분리했고 provider 비종속 기반의 AC가 모두 통과했다.
- `docs/LOG.md` 기록 및 계획·검증 보고서 링크: 2026-07-15 T18 코드 우선 백엔드 기반 기록과 계획·검증 링크 추가.
- 후속 작업 또는 사용자 판단이 필요한 사항: T19 프롬프트 자산 준비 후 T20에서 provider adapter·키·프론트 HTTP 어댑터·Preview 왕복을 함께 검증한다.
