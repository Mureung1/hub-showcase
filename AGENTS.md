...
- Git·터미널 명령어는 사용자가 직접 실행한다. 제시할 때는 실행 순서, 짧은 설명, 예상 결과를 함께 제공한다. 명령은 PowerShell 기준으로 한 줄씩 제시한다(`&&` 미지원).
- AI 도구는 사용자 레포에서 git 명령을 실행하지 않는다(잠금 파일 충돌). git 상태 확인이 필요하면 사용자에게 명령을 제시한다.
...
- 공식 문서는 현재 설계가 무엇인지를 무시제·선언형으로만 기술한다. 작업 일지, 시행착오, 정정 과정, 해명("~가 아니라 ~", "예외 하나", "~를 완료했다", "남은 것은")을 쓰지 않는다.
