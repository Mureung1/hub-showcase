...
해당 노트는 메인 볼트 `{your-mothership-vault-name}` 의 9 system files (precedence 1~9, 2026-05-22 기준 DESIGN.md 추가) + 핵심 에세이 5편에서 추출된 사용자 맥락 snapshot 이다. 김규태의 정체성·철학·7 재활용 축·CMDS 9 categories 를 담고 있으며, 이 맥락 없이는 LLM Wiki 의 모든 operation 이 "목적 없는 자동 정리" 로 전락한다. 9 system files alias 표는 아래 표 및 [[Core Context]] §8 참조.
...
| 7 | `@CMDS-Brain` | `{your-mothership-vault-name}/BRAIN.md` | PERSONA — 김규태 brain profile (Gobi 앱 entry) | 비공개 |
...
| Satellite (this) | `CMDS_LLM_Wiki` | `C:\Users\user\Desktop\cmds-llm-wiki` |
