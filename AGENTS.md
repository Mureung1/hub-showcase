...
For this participant, `N180_하성욱` is the default integration branch. Use `N180_하성욱` for normal small changes, and create feature/spike branches only when the user asks for branch isolation or the work is easier to review separately. Feature branches must be based on `N180_하성욱`, not `main`, and their PRs must target `connect-AIAgentChallenge-26-1/hub:N180_하성욱`. The upstream `origin` repository is read-only for this contributor, so publish branches by pushing to the personal fork remote (`fork`) and opening PRs from `swh3467:<branch>` into `connect-AIAgentChallenge-26-1/hub:N180_하성욱`. Do not target `main`; the auto-merge workflow explicitly skips PRs whose base branch is `main`.
