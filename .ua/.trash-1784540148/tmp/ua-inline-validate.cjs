...
try {
  ...
  graph.nodes.forEach((node, index) => {
    ...
    if (!node.tags || !node.tags.length) {
      issues.push(`Node[${index}] '${node.id}' missing tags`);
    }
    if (seen.has(node.id)) {
      issues.push(
        `Duplicate node ID '${node.id}' at indices ${seen.get(node.id)} and ${index}`,
      );
    } else {
  ...
  graph.edges.forEach((edge, index) => {
    if (!nodeIds.has(edge.source)) {
      issues.push(`Edge[${index}] source '${edge.source}' not found`);
    }
    if (!nodeIds.has(edge.target)) {
      issues.push(`Edge[${index}] target '${edge.target}' not found`);
    }
  });
  ...
  const stats = {
    ...
    nodeTypes: graph.nodes.reduce((counts, node) => {
      counts[node.type] = (counts[node.type] || 0) + 1;
      return counts;
    }, {}),
    edgeTypes: graph.edges.reduce((counts, edge) => {
