...
try {
  ...
  const fanInRanking = ranked("fanIn");
  const fanOutRanking = ranked("fanOut");
  ...
  const entryPointCandidates = nodes
    .map((node) => {
      ...
      if (node.type === "file") {
        ...
        if (
          outgoing.get(node.id).size >= highFanOutThreshold &&
  ...
  const bfsEdges = structuralEdges.filter(
    (edge) => edge.type === "imports" || edge.type === "calls",
  );
  const adjacency = new Map(nodes.map((node) => [node.id, []]));
  for (const edge of bfsEdges) adjacency.get(edge.source).push(edge.target);
  ...
  const coupledEdges = structuralEdges.filter(
    (edge) => edge.type === "imports" || edge.type === "calls",
  );
  const relationKeys = new Set(
    coupledEdges.map((edge) => `${edge.source}\u0000${edge.target}`),
  );
  const mutualPairs = [];
  for (const edge of coupledEdges) {
    if (relationKeys.has(`${edge.target}\u0000${edge.source}`)) {
      ...
      const candidate = nodes
        ...
        .map((node) => ({
          ...
          connections: [...cluster].filter((member) =>
            undirectedNeighbors.get(node.id).has(member),
          ).length,
        }))
