...
```text
Task: dependency·container 공급망 hardening
Backlog ID: SEC-007
Parent Epic: EPIC-07
Type: security
Owner: N187_정현우
...
재현 가능한 dependency와 최소 권한 container runtime을 검증한다.

## 3. Scope

- frozen lock install
- image digest 또는 승인된 immutable tag
- non-root·read-only·capability 제한
- SBOM·취약점 검토와 예외 기록
...
- [ ] frozen install이 통과한다.
- [ ] container가 최소 권한으로 실행된다.
- [ ] 사용 image와 digest가 기록된다.
- [ ] 알려진 위험과 승인 예외가 문서화된다.
...
```text
chore(security): harden dependencies and containers
```
