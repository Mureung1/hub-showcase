...
4. 100/300/500/1000m 반경 결과와 URL 확인
...
- [ ] 실제 development Supabase로 검색과 nearby endpoint가 HTTP 200을 반환한다.
- [ ] 검색 결과 선택 후 지도 중심·목록·분석 panel이 같은 store/market ID를 사용한다.
- [ ] 위치 확정과 반경 변경이 URL·request·원·marker·수치에 동일하게 반영된다.
- [ ] loading, empty, validation, unsupported area와 503이 서로 다른 사용자 문구를 가진다.
- [ ] 이전 요청이 늦게 끝나도 최신 선택 state를 덮어쓰지 않는다.
- [ ] retry가 마지막 확정 request를 다시 실행하고 정상 화면으로 복구한다.
- [ ] 오류 응답·화면·console에 DB URL, SQL, stack trace와 credential이 없다.
- [ ] 지원 영역 밖 basemap과 지원 영역 안 Overlay가 함께 회귀 통과한다.
- [ ] 실행 명령과 결과를 Run Report에 기록한다.
...
- [ ] `.harness/runs/<date>-EVAL-002-front-api-smoke.md`에 환경·시나리오·결과 기록
- [ ] `docs/development/tasks.md`의 API-003, WEB-005, EVAL-002 상태 갱신
- [ ] GitHub #27과 Jira LT-10에 Run Report 연결
...
- [ ] mock test만 통과하고 실제 DB smoke를 생략하지 않았는가?
- [ ] API failure를 빈 결과로 숨기지 않았는가?
- [ ] secret이나 개인 PC 경로를 evidence에 남기지 않았는가?
- [ ] 늦은 응답의 stale state를 검증했는가?
- [ ] basemap·Overlay 회귀를 함께 확인했는가?
