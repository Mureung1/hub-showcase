...
- `memberCount`도 변경 가능(3. 파티원 참고) — 단, 현재 가입된 파티원 수보다 적은 값으로는 변경 불가.
...
**Error `400`**: 값 형식/범위 오류, 또는 현재 가입된 파티원 수보다 적은 `memberCount`로 변경 시도
...
- 모든 파티원은 로그인 회원.
- `memberCount`는 구독 등록/수정 시 파티장이 입력하는 고정 정산 인원수(파티장 본인 포함)이며, 실제 가입 완료 인원과 별개 — 초대는 보냈지만 아직 가입하지 않은 인원이 있으면 아래 파티원 목록 수가 `memberCount - 1`(파티장 제외)보다 적을 수 있음. 파티원 가입/삭제로 자동으로 바뀌지 않으며, 정산 인원수를 바꾸려면 파티장이 `PATCH /subscriptions/:id`로 직접 조정.
- 가입은 정원(`memberCount - 1`명, 파티장 제외) 안에서만 허용 — 이미 정원이 찬 상태에서 가입을 시도하면 `403`. `memberCount`가 실제 정산 분모로 쓰이기 때문에, 정원을 초과해서 받으면 파티장이 구독료보다 많은 금액을 걷게 되는 문제를 막기 위함.
...
**Error `403`**: 정원(`memberCount - 1`명)이 가득 참
...
**Error `400`**: 가입 완료된 `PartyMember`가 0명 (정산할 대상 없음)
