...
- 장소·블로그 검색, 조건 추출과 추천 이유 생성을 application port로 정의하고
  Naver·OpenAI DTO가 domain 밖에 머문다.
...
- Naver 결과 결합·영구 저장·OpenAI 전달은 약관·표시 의무 검토 전까지 차단한다.
...
Cloudflare, Vercel, Render, Neon, Upstash의 실제 리소스·secret·배포와 실제 OpenAI
호출은 이번 정책 작업의 완료 범위가 아니다. 실제 key, 응답 전문, 개인정보, 검색어,
장소명·주소·링크를 문서·fixture·로그와 artifact에 남기지 않는다.
...
OpenAI 기본 계약은 Responses API, strict JSON Schema, exact model configuration,
낮은 reasoning effort, `store=false`, tool 비활성화와 bounded output이다. 실제 model과
상한은 구현 시점의 공식 문서, Eval과 비용 검토 없이 자동 변경하지 않는다.
...
| 실제 Local Live | 검증 안 됨 | 교체 key로 Local·Blog 각 1회 2xx·schema, 총 호출 수 2의 safe summary |
...
사람은 노출 key 교체, Naver·OpenAI 약관·비용·quota, 승인 actor·SHA, 외부 secret,
실제 Local Live와 클라우드 배포를 승인한다. 자동 테스트 결과를 실제 호출·약관·배포
증거로 승격하지 않는다.

## 남은 위험과 학습

외부 API, model, OIDC claim, 약관과 무료 플랫폼 정책은 저장소보다 먼저 바뀔 수 있다.
401·403, schema drift, 비정상 사용량이나 약관 변경이 감지되면 live 경로를 먼저
비활성화하고 Mock 회귀와 분리해 원인을 분석한다.

이 Work Record의 `done`은 정책과 자동 강제 장치가 구현·검증됐다는 뜻이다. Local Live
canary와 cloud 배포는 별도 상태와 Task로 남아 있으므로 실제 서비스 사용 가능성이나
운영 가용성을 성과로 주장하지 않는다.
