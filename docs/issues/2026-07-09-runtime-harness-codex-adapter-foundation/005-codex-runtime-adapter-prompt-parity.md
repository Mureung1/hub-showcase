...
- [ ] CodexRuntimeAdapter가 raw client를 통해 `thread/start`로 thread를 만들고 `turn/start`로 prompt input을 보낸다.
- [ ] Codex App Server notification stream을 계속 읽고 raw/debug log에 남긴다.
- [ ] `item/agentMessage/delta` 등 agent output notification에서 transcript-like output으로 표시할 text를 추출한다.
- [ ] `turn/completed` 또는 이에 준하는 final notification을 normalized `completed` run state로 mapping한다.
- [ ] Inspector에서 Fake와 Codex adapter 모두 prompt run happy path를 비교할 수 있다.
- [ ] Runtime core consumer는 raw Codex type에 직접 의존하지 않는다.
- [ ] 관련 typecheck/build 또는 smoke 검증이 통과한다.
