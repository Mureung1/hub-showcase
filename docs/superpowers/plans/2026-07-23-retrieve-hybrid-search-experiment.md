...
Task 5에서는 실제 Desktop Chrome 150.0.7871.181과 Android Chrome 148.0.7778.215를 각각 CDP와 ADB reverse를 통해 같은 localhost COOP/COEP origin에서 측정했다. `Xenova/multilingual-e5-small`의 고정 revision q8을 Worker의 실제 WASM backend로 실행했으며, desktop cold/cache load는 30,272.365/2,315.080ms, Android cold/cache load는 65,525.245/11,694.190ms였다. cache warm p50/p95는 desktop 13.735/27.410ms, Android 55.505/116.780ms였고 Worker 취소 뒤 회복 및 `hidden → visible` 뒤 384차원 query 성공을 두 플랫폼에서 확인했다. `navigator.gpu` 지원과 실제 backend는 구분해 기록했다. 메모리 API는 강제 eager 측정 플래그가 있는 Desktop을 포함해 호출 실패했으므로 모든 경계 bytes와 peak를 `null`로 남겼으며, 이를 수치로 대체하거나 연속 peak로 해석하지 않는다. WASM 파일 크기도 progress callback에서 분리되지 않아 `null`로 기록했다. 결과는 `scripts/retrieve_experiment/results/browser_benchmark_result.json`과 `scripts/retrieve_experiment/results/browser_benchmark_report.md`에 보존한다.
