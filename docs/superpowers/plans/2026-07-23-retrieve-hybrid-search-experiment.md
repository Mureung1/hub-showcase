...
2026-07-23 후속 실측에서 Desktop Chrome 150.0.7871.181과 Android Chrome 148.0.7778.215 모두 측정에 성공했다. Worker 생성 직전부터 pipeline ready까지 재측정한 cold/cache load는 Desktop 27,052.550/2,207.335ms, Android 65,790.345/10,918.060ms다. 두 플랫폼의 고정 revision 필수 Cache Storage 항목과 cache 단계 CDP 원격 모델 요청 0건을 확인해 `cacheHitVerified: true`로 기록했다. cache 준비 뒤 첫 progress callback에서 Worker를 취소한 후 새 Worker의 ready, 첫 query, warm 20회, 384차원 복구도 모두 성공했다.
