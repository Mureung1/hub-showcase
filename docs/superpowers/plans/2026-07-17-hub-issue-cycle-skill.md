...
**구조:** `.agents/skills/hub-issue-cycle` 아래의 지침 중심 스킬로 구현한다. 실행 시 Git·GitHub와 저장소 문서를 조회하며 Project 필드 ID는 하드코딩하지 않고, 제품 판단과 PR 최종 승인·병합에서만 사람에게 멈춘다.
...
- 생성: `.agents/skills/hub-issue-cycle/SKILL.md`
- 생성: `.agents/skills/hub-issue-cycle/agents/openai.yaml`
...
예상 결과: `.agents/skills/hub-issue-cycle`이 생성되고 `SKILL.md`와 `agents/openai.yaml`이 존재한다.
...
```powershell
Get-ChildItem -Recurse .agents/skills/hub-issue-cycle
```
...
- 수정: `.agents/skills/hub-issue-cycle/SKILL.md`
...
- Project #3의 `시작 가능`이면서 열린 이슈만 후보로 삼는다.
- `blocked`, `needs-info`, `ready-for-human` 라벨은 자동 대상에서 제외한다.
- 우선순위, 제품 단계, 주차, 완료 목표일, 선행 관계, 이슈 번호 순으로 비교한다.
...
- 검증: `.agents/skills/hub-issue-cycle/SKILL.md`
- 검증: `.agents/skills/hub-issue-cycle/agents/openai.yaml`

- [ ] **1단계: 공식 구조 검증 실행**

```powershell
python C:\Users\cjh51\.codex\skills\.system\skill-creator\scripts\quick_validate.py .agents/skills/hub-issue-cycle
```
...
```powershell
rg -n "hub-issue-cycle|allow_implicit_invocation|자동 병합|원격 브랜치|검증 예산|검토 중|완료" .agents/skills/hub-issue-cycle
```
...
```powershell
npm exec prettier -- --check .agents/skills/hub-issue-cycle docs/superpowers/plans/2026-07-17-hub-issue-cycle-skill.md
git diff --check
```
