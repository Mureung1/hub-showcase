...
- [x] **Step 1: 실제 빈 보관함과 필터 결과 없음 구분**

`LibraryPage`에 필터링 전 전체 인사이트 개수를 전달한다. 전체 개수가 0이면 검색어나 카테고리가 남아 있어도 실제 빈 상태를 표시하고, 전체 개수는 있지만 카테고리 결과만 0개이면 `전체 보기` 행동이 있는 조건 결과 없음 상태를 표시한다.

- [x] **Step 2: 일부 손상과 전체 손상 구분**

`corrupted-entry` 경고와 함께 유효한 인사이트가 남으면 기존 카드와 부분 손상 안내를 유지한다. 유효한 인사이트가 하나도 남지 않으면 원격 보관함 실패로 분류하고 다시 불러오기 행동을 제공한다.

- [x] **Step 3: 리뷰 회귀 테스트**

실제 빈 보관함에 검색어가 남은 경우, 카테고리 필터 결과만 0개인 경우, 일부 원격 행만 손상된 경우와 전체 원격 행이 손상된 경우를 단위·통합 테스트로 고정한다.
