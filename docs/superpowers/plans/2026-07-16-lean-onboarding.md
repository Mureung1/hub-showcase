...
`LandingPage` 테스트에서 `링크를 저장하고`, `지금 하는 일로 꺼내보세요`, `아맞다로 시작해보세요`를 확인한다. 기존 Hero 보조 설명, 로그인 안내, 감성 문구, 결과 예시 박스, 문제·장점 제목은 존재하지 않아야 한다. H1 안에는 이모지 없는 Electric Blue `링크`와 Amber `다시` 하이라이트가 있어야 한다.
...
Hero에는 상단바, H1, CTA만 남긴다. H1은 이모지 없이 Blue `링크`, Amber `다시` 하이라이트를 사용한다. 내비게이션은 `저장`, `꺼내보기`만 남긴다.
