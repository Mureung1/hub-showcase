...
OAuth `state`는 발급 후 10분 동안 한 번만 사용할 수 있다. 연결 토큰은 완료·취소 시 즉시 삭제하며, 중단된 가져오기도 연결 후 24시간이 지나면 토큰과 미반영 분석 후보를 자동 삭제한다. 완료·Undo된 작업 기록은 사용자가 삭제할 때까지 유지한다. 작업 기록을 삭제하면 Undo 권한과 관련 후보·오류·컬렉션 정보가 함께 사라지지만 이미 가져온 인사이트는 유지된다.
