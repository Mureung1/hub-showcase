...
반영을 완료하면 후보·오류·컬렉션 정보는 commit 트랜잭션에서 즉시 삭제한다. 생성 인사이트 ID와 반영 직후 수정 시각만 완료 후 24시간까지 보관하고, DB 시각으로 기한을 넘긴 Undo 요청은 거부한다. Supabase Cron은 저장된 ID와 시각을 1분마다 확인해 만료된 행을 삭제한다. 완료·Undo된 요약 작업 기록은 사용자가 삭제할 때까지 유지하며, 기록을 삭제해도 이미 가져온 인사이트는 유지된다.
