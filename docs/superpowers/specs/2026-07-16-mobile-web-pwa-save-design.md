...
2. 매니페스트의 POST 공유 대상은 URL·텍스트·제목을 `/share-target`으로 전달한다.
3. 서비스 워커는 폼 데이터를 읽고 `/#share-target?shared_title=...&shared_text=...&shared_url=...`로 향하는 same-origin `303` 응답을 만든다. fragment는 이어지는 HTTP 요청에 포함되지 않는다.
4. 인증 확인이 끝난 뒤 로그인 상태에서만 fragment를 `android_share` 저장 초안으로 해석해 같은 저장 화면에 채운다. 로그인·로그아웃 상태 모두 해석 직후 fragment를 주소에서 제거한다.
5. 공유 URL은 사용자 확인 전 자동 저장하지 않는다. 사용자가 `저장하기`를 누를 때만 공통 캡처 API를 호출한다.
6. PWA 공유 진입은 `android_share`, 일반 웹 저장은 `web` 소스를 사용한다. 두 소스 모두 현재 저장 스키마를 분기하지 않고 공통 계약으로 처리한다.

공유 대상 기능은 설치된 PWA에서만 제공하며, Android Chrome을 지원 환경으로 한정한다. iPhone과 설치하지 않은 모바일 웹 사용자는 B 흐름을 사용한다.

## PWA 구성

- 웹 앱 매니페스트에 앱 이름·아이콘·표시 모드와 `POST /share-target` 공유 대상을 선언한다.
- 서비스 워커는 공유 대상 POST 폼을 네트워크로 전달하지 않고 same-origin `303` redirect로 바꾼다. redirect 주소에서는 공유 값을 `#share-target` fragment에만 둬 HTTP 요청 URL과 서버 접근 로그에 남기지 않는다.
- 앱은 `shared_url → shared_text → shared_title` 순서로 첫 URL 후보를 고르고 `http`와 `https`만 저장 초안 URL로 인정한다.
- 제목은 앞뒤 공백을 제거한다. 제목 자체가 선택된 URL과 같으면 별도 제목으로 보존하지 않는다.
- 인증 로딩 중에는 공유 fragment를 소비하지 않는다. 인증이 끝난 뒤 로그인 상태에서만 저장 초안을 만들고, 로그인 여부와 관계없이 `#share-target` fragment를 주소에서 제거한다. 다른 hash는 유지한다.
- 공유 초안은 API 요청을 만들지 않는다. 사용자가 `저장하기`를 누를 때만 기존 인증 토큰으로 공통 캡처 API를 호출한다.
- `android_share`는 진입 출처 표시에만 사용하며 인증, URL 검증, 정규화, 중복 처리, RLS를 우회하지 않는다.
- `beforeinstallprompt`는 React 렌더보다 먼저 shared 이벤트 저장소가 보관한다. 설치 안내 정책은 저장 성공과 설치 이벤트 도착 순서에 관계없이 두 조건이 모두 충족된 뒤에만 동작한다.
- 설치 안내는 Android Google Chrome에서 첫 저장에 성공한 뒤 한 번만 보인다. 안내를 표시하기 전에 로컬 기록을 남기며, 설치를 거부하거나 닫거나 저장소 접근이 실패해도 일반 웹 저장 기능은 변하지 않는다.
- 서비스 워커는 프로덕션에서만 등록한다. 캐시·오프라인 저장은 추가하지 않고 정확한 same-origin `POST /share-target`만 처리한다.
...
- 공유 대상 POST는 서비스 워커가 same-origin `303`으로 변환하고, 공유 값이 담긴 fragment는 서버 요청 URL에 포함되지 않는다.
- 로그인하지 않은 상태에서는 공유 초안을 만들지 않으며 인증 확인이 끝나면 `#share-target` fragment만 제거한다.
