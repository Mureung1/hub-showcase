...
손님은 모바일 웹에서, 사장님은 매장 컴퓨터나 태블릿의 넓은 화면 웹에서 같은 카페 스탬프 데이터를 사용합니다.
...
| 2 | 공통 장부 만들기 | 1주차 |
| 3 | 로그인과 역할 나누기 | 1주차 |
| 4 | 손님이 자기 회원번호와 스탬프 보기 | 2주차 |
| 5 | 사장님이 그 회원번호로 손님 찾기 | 2주차 |
| 6 | 사장님이 스탬프 1개 적립하기 | 2주차 |
...
flowchart LR
    ...
    W2["2주차 · 4~7단계<br/>7/20~7/24<br/><br/>손님 스탬프<br/>사장님 조회<br/>적립 RPC<br/>손님 반영"]
...
```mermaid
flowchart TD
    A["4단계<br/>손님 회원번호와 스탬프 보기"] --> B["여러 카페의 목표·보상 표시"]
    B --> C["5단계<br/>사장님이 회원번호로 손님 찾기"]
    C --> D["담당 카페의 고객 스탬프 조회"]
    D --> E["6단계<br/>스탬프 1개 적립 RPC"]
...
- 손님은 자기 회원번호와 여러 카페의 스탬프를 볼 수 있습니다.
- 사장님은 그 회원번호로 손님을 찾을 수 있습니다.
- 스탬프가 정확히 1개만 증가합니다.
...
| 4 | 4단계 | 손님 스탬프 홈 | `feat(customer): 회원번호와 스탬프 현황 연결` |
| 5 | 5단계 | 사장님 고객 조회 | `feat(owner): 회원번호 고객 조회 구현` |
...
쿠폰에는 향후 사용 처리를 위해 바코드 번호와 상태를 저장할 수 있지만, 이번 필수 MVP에서는 사장님이 바코드 번호를 입력해 쿠폰을 `used`로 바꾸는 기능까지는 만들지 않습니다. 화면에 QR 모양을 보여줄 수는 있지만 실제 적립은 회원번호 입력으로 진행합니다.

## 10. 한눈에 보는 성공 기준

```mermaid
flowchart TB
    A["1~3단계<br/>로그인과 역할 이동"] --> B["4단계<br/>손님 회원번호와 스탬프 확인"]
    B --> C["5단계<br/>사장님 고객 조회"]
