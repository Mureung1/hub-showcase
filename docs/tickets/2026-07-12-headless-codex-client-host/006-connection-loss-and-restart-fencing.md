...
- Active turn의 loss는 성공·실패·interrupted terminal로 꾸미지 않고 `connection_lost`와 unknown outcome을 표현한다.
...
- [ ] Process loss가 `recoverable: true`인 `failed` snapshot과 connection-loss event를 만들고 active turn terminal을 합성하지 않는다.
