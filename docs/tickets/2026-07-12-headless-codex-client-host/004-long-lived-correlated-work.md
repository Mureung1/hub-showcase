...
- Timed-out `turn/start`의 execution-slot reservation은 `ready`로 돌아가서 다음 mutation을 허용하는 신호로 해제하지 않는다. Generation failure cleanup이 해당 thread에 `outcome: unknown`인 reconciliation-needed marker를 남기고 slot을 종료하며, late response와 `turn/started`를 포함한 old-generation observation은 ref나 state를 만들지 않는다.
...
- [ ] Mutation timeout이 해당 thread에 `outcome: unknown`인 reconciliation-needed marker를 남기고, 이후 late response와 late `turn/started` notification을 전송해도 opaque ref·snapshot·event가 추가로 바뀌지 않으며 explicit restart 전에 새 mutation을 받지 않는다.
