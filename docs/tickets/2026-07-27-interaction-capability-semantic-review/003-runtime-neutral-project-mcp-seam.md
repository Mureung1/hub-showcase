...
`@ay-ple/codex-chat-runtime`이 capability 의미를 알지 않고 bounded child environment를 same persistent generation에 전달하며, native project config가 발견한 MCP server의 readiness를 `serverName + expectedTools`로 확인할 수 있게 한다. Target Workspace thread는 exact Git root와 project config만으로 built Adapter를 시작하고, thread-start private MCP override 없이 required tool roster를 증명한다.
...
- Exact SDK 변경이 필요하면 repository의 ordered patch·generation·provenance discipline을 지키고 public high-level seam과 bridge test를 함께 갱신한다.
...
- `../2026-07-27-user-owned-semester-workspace-lifecycle/004-native-git-project-context-and-permission-trust.md` — Native Git project context와 permission·trust
