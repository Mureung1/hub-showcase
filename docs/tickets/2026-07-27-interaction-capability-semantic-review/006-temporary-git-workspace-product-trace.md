# 006 — Temporary Git workspace product trace
...
Real built Adapter, shared Server listener, Broker, Runtime project context, inline Review와 installed First Assignment Skill을 temporary Git SemesterWorkspace에서 한 trace로 조합한다. User accept 전에는 actual file이 바뀌지 않고, accept 뒤 AY가 actual file을 변경해 meaningful Git checkpoint를 남기며 revise·reject와 continuity failure는 승인되지 않은 mutation을 만들지 않는다.
...
- Highest practical seam은 real built `@ay-ple/interaction-mcp`, real Server Broker, deterministic Runtime/UI Adapter와 actual temporary Git workspace다. Core behavior를 fake success로 건너뛰지 않는다.
- Temporary root는 blocker Workspace ticket이 설치한 root-relative project MCP declaration, workspace Skill copy와 `AGENTS.md`를 사용한다. Thread-start MCP override와 managed Skill injection을 되살리지 않는다.
- Trace는 one active `product_turn` lease 안에서 request → evidence preflight → inline Review → held result → AY-owned file mutation → Git checkpoint ordering을 관찰한다.
...
- [ ] Fresh temporary Git SemesterWorkspace에서 built Adapter handshake와 exact `ay_ple_interaction` tool readiness가 성공한다.
...
- Manual or live smoke: Temporary repository의 `git status`, `git log --stat`과 Review transcript를 함께 확인해 accept checkpoint와 revise/reject no-mutation을 검증한다.
...
- `../2026-07-27-user-owned-semester-workspace-lifecycle/005-semester-workspace-init-skill.md` — SemesterWorkspace init Skill
