...
- Runtime·Server·Chat Shell package README, Codex Chat implementation map과 Codex Runtime 격리 문서는 current implemented behavior, root/cwd ownership과 remaining deferred Chat capability를 owning-document-first로 갱신한다. Work order/status는 development backlog에서만 갱신한다.
- Final review는 fixed point 대비 repository Standards와 parent Spec 두 축을 모두 수행한다.

## Acceptance Criteria

- [ ] Full Browser E2E가 parent spec의 아홉 representative trace와 1440×900 desktop accessibility를 통과한다.
