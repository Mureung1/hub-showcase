...
| Descriptor/manifest admission | `npm test -w @ay-ple/runtime-release` — 46/46 green. Missing/extra/unknown/type/range, application·target·contract·resource identity, roster와 byte binding drift뿐 아니라 repository query/fragment/userinfo-like segment/noncanonical path를 effect 전에 fail closed한다. |
...
| Review state | Initial independent Runtime-delivery/security review RED, `site_packages` re-review P1, folded ancestor P1과 fixed-point fold P1을 교정했다. 새 fixed candidate에 대한 Runtime-delivery/security review와 C authority review 재실행 대기 |
