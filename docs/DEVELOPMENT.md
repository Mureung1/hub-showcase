...
- production 코드에 가짜 구현이나 mock을 넣지 않는다.
- 단위 테스트에서는 외부 I/O 경계(HTTP, DB, RPC)를 mock할 수 있다.
- DB 제약, transaction 원자성, RLS·RPC 권한, 동시성은 mock으로 완료 판정하지 않고
  local Supabase 기반 통합 테스트로 검증한다.
