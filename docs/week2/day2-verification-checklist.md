...
1. ~~**서버·mock 데이터 불일치**: 서버 샘플 2건 vs 클라이언트 mock 8건.~~ **해결 (2026-07-21, #4)** — `sample-subsidies.ts`를 8건으로 확장하고 Supabase 재시드 완료. 서버·mock·DB 개수 일치.
2. ~~**`getSubsidies()` 미사용**: 지금은 죽은 코드.~~ **해결 (2026-07-21, #6)** — 실사용처가 없어 제거함.
3. 위 `[UI 미검증]` 항목들은 브라우저 자동화 도구가 없어 실제 클릭 확인을 못했음 — 로컬에서 `npm run dev`로 한 번 훑어보는 걸 권장.
4. **(2026-07-21, #6) 추가 검증**: Vite proxy(`/api/*`)를 통해 `GET /api/subsidies`, `GET /api/subsidies/:id`, `POST /api/match`가 실제 Supabase 데이터를 반환하는 것을 curl로 확인. 서버 프로세스만 중지하면 프록시가 502를 반환해 `client.ts`의 fallback 경로(→ mock + `console.warn`)가 트리거되는 조건도 확인함. 브라우저 콘솔에 실제로 경고가 뜨는지는 `[UI 미검증]`으로 남겨둠.
