...
이 절차는 한국어 기본 제품 경로(`/`) 발표를 위한 운영 메모다. 제출용 영어 데모(`/en`)는 별도
산출물이므로 수정·복구 대상으로 취급하지 않는다.
...
3. Web 상단의 공식 데이터 연결 문구와 분기 선택기를 확인한다.
4. 연남동 골목상권 · 카페 · 직접 선택 · 300m에서 점포 목록과 분석 패널을 확인한다.

Render Free 인스턴스가 idle 상태였다면 `/ready` 첫 요청이 지연될 수 있다. 이때 Web은 지원 범위만
정적으로 보여 주고 분석 수치를 예시 값으로 바꾸지 않는다. `준비 중` 문구가 사라진 뒤 실제 API
요청이 자동 시작되는지 확인한다.
...
| 시간 | 조작 | 전달할 메시지 |
| --- | --- | --- |
| 0:00–0:40 | 홈 URL을 연다 | LocalTwin은 예비 소상공인이 상권·점포 데이터를 이해하기 쉽게 보는 도구다. |
| 0:40–1:30 | 연남·홍대·합정을 바꾼다 | 상권 단위 데이터와 지도 표현이 함께 바뀐다. |
| 1:30–2:20 | 카페·100m/300m/500m를 바꾼다 | 반경은 주변 점포용이며 매출·유동은 공식 상권 단위임을 구분한다. |
| 2:20–3:20 | 지도에서 위치 선택 후 확정한다 | 원하는 위치의 주변 경쟁을 확인한다. |
| 3:20–4:15 | `LocalTwin`·`실제 지도`와 건물 레이어를 전환한다 | 기본 지도는 유지하고 전용 표현만 겹치지 않게 추가한다. |
| 4:15–5:00 | 데이터 도움말 또는 근거 보기를 연다 | 지표는 기간·출처·공간 기준과 함께 해석한다. |

장애별 대응:

| 증상 | 발표 중 행동 |
| --- | --- |
| `분석 서버를 준비하고 있습니다`가 계속 표시 | `/ready`를 한 번 열고, 준비 뒤 자동 갱신되는 실제 데이터를 사용한다. |
| API 오류·다시 시도 버튼 | 오류를 빈 결과나 예시 분석으로 설명하지 않는다. `다시 시도` 후 복구되지 않으면 지도·지원 범위·데이터 기준만 시연하고 API 분석은 중단했다고 명확히 말한다. |
| 지도 타일 또는 WebGL 표시 실패 | `실제 지도` mode로 전환한다. 계속 실패하면 데이터 도움말·분석 근거 dialog로 전환한다. |
