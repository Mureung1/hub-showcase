...
| 탭 | 목적 | 표시 |
| --- | --- | --- |
| 선택 자료 탭 | 사용자가 선택한 원본 자료를 같은 화면 안에서 확인한다. | `lms-outline-notice.txt`, `problem-solving-syllabus.pdf` |
...
| 자료 목록 | 과목별 원본 자료를 펼쳐 보고 정리할 자료를 고른다. | `문제해결글쓰기`, `lms-outline-notice.txt`, `2개 선택됨` |
| 선택 자료 탭 | 선택한 원본 자료를 확장자별 미리보기로 전환한다. | `lms-outline-notice.txt`, `problem-solving-syllabus.pdf` |
| 원본 미리보기 | 선택 자료 원문 또는 추출본을 확인한다. 보기 모드는 editor toolbar에서 전환한다. | `편집하기`, `프리뷰 보기`, `lms-outline-notice.txt`, `problem-solving-syllabus.pdf` |
| 하단 근거 패널 | AY가 찾은 값과 원본 위치를 preview와 분리해서 보여준다. | `마감`, `제출 방식`, `line 3`, `Markdown 추출본` |
