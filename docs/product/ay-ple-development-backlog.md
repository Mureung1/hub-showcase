...
- [ ] First Assignment를 user-owned Git SemesterWorkspace와 InteractionCapability seam으로 재구성한다.
  ...
  - [ ] Capability contract test, deterministic Browser E2E와 exact local-provider trace로 accept·revise·reject·cancel·disconnect·Runtime terminal과 concurrent request의 `busy`에서 허위 apply·duplicate result·숨은 queue가 없음을 검증한다.
