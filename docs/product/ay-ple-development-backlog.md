...
- [x] User-owned Git SemesterWorkspace에서 AY-originated InteractionCapability vertical을 완성한다.
  - [x] [InteractionCapability 기반 Semantic Review Spec](../specs/2026-07-27-interaction-capability-semantic-review.md)에 따라 app-neutral Review seam과 joint cutover를 완성한다.
    ...
    - [x] `propose_state_patch` input에서 caller-supplied `requestKey`, workspace·Course identity, store revision과 native identity를 제거하고, 설명·순서 있는 semantic before/after change·선택적 `SourceCitation`만 받는 도메인 중립 capability contract로 축소한다. Assignment schema와 raw Git diff를 공개 contract에 넣지 않는다.
    ...
    - [x] Optional `SourceCitation { relativePath, excerpt, locationHint? }`을 AY의 source interpretation claim으로 Browser에 전달한다. Broker는 active Runtime binding의 exact SemesterWorkspace containment와 실제 regular non-symlink file 존재만 모든 citation에 대해 확인하고 하나라도 invalid면 partial card 없이 `citation_invalid`로 MCP call 전체를 실패시킨다. Excerpt·location hint를 content에서 parse·인증하거나 digest·typed locator를 요구하지 않고, registry·cache·snapshot과 durable citation history를 만들지 않는다.
...
- [x] Fresh built-in Skill catalog와 SemesterModeling vocabulary를 current product path에 반영한다.
  ...
  - [x] Codex 개발용 대표 dogfood 환경은 `../fixtures/**`를 seed로 `../workspace/**`에 만들고, smoke 시작 시 current fixture·built-in Skill catalog·baseline·Git 상태를 reconcile한다. Clean current target은 재사용하고 clean stale·missing target은 fixture 재복사와 fresh Bootstrap으로 준비하며 dirty·untracked·unexpected·identity-mismatched target은 덮어쓰지 않고 fail closed한다. Harness는 normal `../.ay-ple` registry를 고치지 않고 exact generated target을 explicit `--workspace`로 실행한다. Fresh activation으로 같은 canonical root의 `workspaceId`가 바뀌면 Product startup이 full readiness 뒤 stale binding을 새 verified identity로 CAS 교체하고, no-argument identity mismatch는 계속 fail closed한다. Smoke가 적용한 Review, workspace와 App·Browser 관찰 상태는 종료 뒤 보존하고 다음 run 시작에 다시 판단한다. Git 밖 fixture path는 generic helper에 hard-code하지 않으며 이 development harness를 existing user workspace의 refresh·migration lifecycle로 승격하지 않는다.
  - [x] 실제 smoke에서 발견한 Product Turn의 상시 Plan mode 강제를 제거하고 canonical Turn을 Default collaboration mode로 복원했다. Interaction Adapter의 held lifecycle은 private HTTP response-body timeout 없이 Runtime generation 동안 유지하고, native MCP tool timeout과 별도인 이 회귀를 real built Adapter test로 고정했다.
...
- [ ] 확인된 사용자 필요에 따라 나머지 post-Ready capability를 순서대로 추가한다.
  ...
  - [ ] Current Default collaboration mode에서 실제 command·file·network·user-input 등 additional-input request가 발생하거나 same-Turn clarification 필요가 확인되면 해당 native request family와 first-party behavior를 먼저 흡수하고 필요한 policy·UI만 추가한다.
  ...
  - [ ] 실제 사용자가 한 SemesterWorkspace를 장기 보존하면서 새 AY-PLE built-in Skill을 받아야 할 때 update adoption gate를 다시 연다. 그 전에는 generated development target에만 필요 시 fixture→fresh Bootstrap을 적용하고 refresh·replace mode, local edit merge, stale Skill prune, hash manifest·lock·bundle version과 기존 workspace migration을 만들지 않는다.
