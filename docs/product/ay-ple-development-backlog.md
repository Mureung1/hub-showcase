...
- [ ] User-owned SemesterWorkspace lifecycle과 canonical local roots를 완성한다.
  ...
  - [ ] Hub-owned Interaction MCP STDIO Adapter와 App-side Broker를 분리하고, Bootstrap이 Git-safe한 static command·env 이름·capability allowlist와 `required = true`를 SemesterWorkspace의 `.codex/config.toml`에 설치한다. Runtime은 endpoint·token·binding value를 child environment로만 공급하고 current thread-start MCP config injection을 제거한다.
