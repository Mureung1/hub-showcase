...
- [ ] First Assignment를 user-owned Git SemesterWorkspace와 InteractionCapability seam으로 재구성한다.
  ...
  - [ ] Adapter startup의 짧은 authenticated handshake 뒤 capability call 하나를 Broker HTTP POST 하나로 유지해 Browser의 once-only result를 같은 MCP call에 반환한다. MCP·HTTP·Browser·Runtime cancellation과 duplicate·late answer를 terminal 정산하고 `202`+poll, callback, durable outbox·response replay를 만들지 않으며 delivery ambiguity를 성공이나 workspace apply로 추정하지 않는다.
...
- [ ] User-owned SemesterWorkspace lifecycle과 canonical local roots를 완성한다.
  ...
  - [ ] `@ay-ple/interaction-mcp`의 built STDIO Adapter와 `apps/server`의 Broker를 연결하고, Bootstrap이 exact SemesterWorkspace root에서 `hub/packages/interaction-mcp` entrypoint까지 계산한 relative command·env 이름·capability allowlist, explicit human-decision `tool_timeout_sec`과 `required = true`를 SemesterWorkspace의 `.codex/config.toml`에 설치한다. MCP server `cwd`는 생략하고 Runtime은 endpoint·token·binding value를 child environment로만 공급하며 current thread-start MCP config injection을 제거한다. Current pinned default 300초를 제품 bound로 사용하지 않고 exact override seconds를 별도 결정·regression test로 고정한다.
