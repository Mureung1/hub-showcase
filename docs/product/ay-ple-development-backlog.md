...
- [ ] User-owned SemesterWorkspace lifecycle과 canonical local roots를 완성한다.
  ...
  - [ ] Hub-owned Interaction MCP STDIO Adapter와 App-side Broker를 분리하고, Bootstrap이 Git-safe한 static command·env 이름·capability allowlist를 SemesterWorkspace의 `.codex/config.toml`에 설치한다. Runtime은 endpoint·token·binding value를 child environment로만 공급하고 current thread-start MCP config injection을 제거한다.
  - [ ] Trusted·untrusted project에서 effective MCP discovery가 어떻게 정산되는지 검증하고, AY-PLE activation의 trust UX와 missing Interaction MCP fail behavior를 고정한다.
