...
- [ ] First Assignment를 user-owned Git SemesterWorkspace와 InteractionCapability seam으로 재구성한다.
  ...
  - [ ] Private workspace package `@ay-ple/interaction-mcp`를 추가해 built STDIO executable, `InteractionCapability<Request, Result>` schema·codec와 authenticated Adapter↔Broker transport contract를 소유하게 한다. `apps/server`는 package의 server-side port로 production Broker·Browser Adapter를 조합하고 in-memory Adapter로 exact binding, correlation, once-only answer, cancel·disconnect·terminal settlement를 독립 검증한다.
...
- [ ] User-owned SemesterWorkspace lifecycle과 canonical local roots를 완성한다.
  ...
  - [ ] STDIO Adapter가 current App Broker와 authenticated handshake를 완료한 뒤에만 initialize를 성공시키고, activation이 effective MCP status에서 expected server·handshake를 확인하게 한다. Env 누락·Broker offline·invalid token·capability mismatch·Adapter crash·explicit `untrusted`로 인한 project config 무시가 Workspace Runtime start 또는 activation을 fail closed하는지 검증하며 MCP 없는 degraded AY-PLE mode는 만들지 않는다.
