...
- [ ] First Assignment를 user-owned Git SemesterWorkspace와 InteractionCapability seam으로 재구성한다.
  ...
  - [ ] Review UI의 `accept | revise | reject`와 optional feedback만 custom MCP call 하나의 closed normal result로 반환한다. UI dismiss, `busy`, timeout, disconnect와 Runtime terminal은 네 번째 `cancel` result가 아니라 MCP failure path로 끝내고, 같은 결정을 built-in `request_user_input`에 다시 걸지 않는다.
  ...
  - [ ] Capability contract test, deterministic Browser E2E와 exact local-provider trace로 accept·revise·reject만 정상 result이며 UI dismiss·disconnect·timeout·Runtime terminal과 concurrent request의 `busy`는 MCP failure임을 고정하고, 허위 apply·duplicate result·숨은 queue가 없음을 검증한다.
