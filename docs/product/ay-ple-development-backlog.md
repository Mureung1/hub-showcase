...
[Scrum Guide](https://scrumguides.org/scrum-guide.html)는 Product Backlog를 제품 개선에 필요한 일을 순서대로 관리하며 계속 변화하는 목록으로 설명한다. 항목을 더 작고 명확하게 나누는 refinement 역시 지속적인 활동으로 본다. [Agile Alliance의 Product Backlog 설명](https://agilealliance.org/glossary/backlog/)도 기능뿐 아니라 버그 수정, 인프라 변경, 그 밖의 활동을 결과 달성을 위한 백로그 항목으로 다룬다.

AY-PLE에서는 이를 다음처럼 적용한다.

| 질문 | AY-PLE의 적용 방식 |
| --- | --- |
| 왜 필요한가? | 해야 할 일과 하지 않을 일을 드러내고, 가장 중요한 사용자 결과에 개발 시간을 집중하기 위해 필요하다. |
| 어떻게 생겼는가? | 각 항목에 목적, 유형, 우선순위, 완료 조건, 목표 주차, 상태를 기록한다. |
| 기능이 미정이면 어떻게 하는가? | 억지로 구현 Task를 확정하지 않고 조사, 설계, prototype, 의사결정 Task로 불확실성을 먼저 줄인다. |
| 백로그에 있으면 반드시 개발하는가? | 아니다. 후보 항목은 선택지이며, 핵심 목표에 기여하지 않으면 순서를 낮추거나 제외할 수 있다. |
| 어떻게 관리하는가? | 가까운 작업은 작고 구체적으로, 먼 작업은 큰 결과 단위로 둔다. 매주 검토 후 순서와 범위를 갱신한다. |

이 프로젝트가 Scrum 전체를 도입한다는 뜻은 아니다. 변화하는 작업 목록을 한곳에 두고, 가까운 일일수록 더 구체적으로 관리한다는 원칙만 사용한다.
