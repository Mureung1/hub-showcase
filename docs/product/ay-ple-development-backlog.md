...
- [ ] First Assignment를 user-owned Git SemesterWorkspace와 InteractionCapability seam으로 재구성한다.
  ...
  - [ ] Review UI를 AY Chat transcript의 inline card 하나로 표시하고 pending 동안 composer·새 Turn·steer를 잠근다. Card에는 `accept | revise | reject`와 optional feedback만 두어 custom MCP call 하나의 closed normal result로 반환하고, 전체 Turn interrupt만 별도 native control로 유지한다. Modal·별도 approval page·card dismiss·입력 queue를 만들지 않으며 interrupt, `busy`, timeout, disconnect와 Runtime terminal은 네 번째 `cancel` result가 아니라 MCP failure path로 끝내고, 같은 결정을 built-in `request_user_input`에 다시 걸지 않는다.
  ...
  - [ ] Capability contract test, deterministic Browser E2E와 exact local-provider trace로 inline card pending 중 composer·새 Turn·steer가 닫히고 accept·revise·reject만 정상 result이며 Turn interrupt·disconnect·timeout·Runtime terminal과 concurrent request의 `busy`는 MCP failure임을 고정한다. Modal·별도 page·card dismiss·허위 apply·duplicate result·숨은 queue가 없음을 검증한다.
...
- [ ] User-owned SemesterWorkspace lifecycle과 canonical local roots를 완성한다.
  ...
  - [ ] Shared listener bind와 Broker route·binding 준비 뒤에만 Workspace Runtime을 spawn하고, STDIO Adapter가 authenticated handshake를 완료한 뒤에만 initialize를 성공시키며, activation이 effective MCP status에서 expected server·handshake를 확인하게 한다. Bind·route 준비 실패 시 child zero, env 누락·Broker offline·invalid or stale token·binding mismatch·capability mismatch·Adapter crash·explicit `untrusted`로 인한 project config 무시는 Runtime start 또는 activation을 fail closed한다. Replacement·close에서 pending interaction과 credential을 terminal 정산하며 MCP 없는 degraded AY-PLE mode는 만들지 않는다.
