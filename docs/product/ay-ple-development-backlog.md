...
- [ ] User-owned SemesterWorkspace lifecycle과 canonical local roots를 완성한다.
  ...
  - [ ] Personal target에서는 repository 개발 harness와 AY-PLE 제품 Skill source를 `hub/.agents/skills/`에 함께 둔다. Init·Update Skill이 선택한 source directory를 SemesterWorkspace의 Git-tracked `.agents/skills/`로 복사하고, symlink나 Runtime `extraRoots` 없이 native discovery되는지 검증한다.
