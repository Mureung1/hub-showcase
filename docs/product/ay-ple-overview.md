...
이 구현은 AY-PLE의 학업 kernel을 증명한다. Public `npx`와 Landing은 현재 제품 목표에서 제외했다. In-app Browser OAuth, `Semester Ready`, `ready-relaunch`와 app-owned scaffold는 구현 감사 전까지 다음 목표나 제거 대상으로 확정하지 않는다.
