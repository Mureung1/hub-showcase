...
이 구현은 AY-PLE의 학업 kernel을 증명한다. Public `npx`, Landing, In-app Browser OAuth, `Semester Ready`, `ready-relaunch`와 app-owned scaffold를 잇던 Server·Browser graph, Runtime managed account·`auth-only` primitive는 제거했다. V3 workspace package kernel은 ADR 0014의 adopted target 기반으로 유지하되 current capability로 주장하지 않는다.
