...
전용 disposable-auth 자동화, account/workspace chooser, thread 목록·복원, interactive approval과 AY-PLE 학업 adapter는 아직 제품 흐름이 아니다. Current canonical clone의 승인된 local legacy residue는 별도 deletion handoff에서 영구 삭제됐고 Chat-only repository gate를 통과했다. 이 완료는 다른 clone이나 external app data의 정리를 뜻하지 않는다. 실제 작업 순서와 완료 조건은 [개발 백로그](ay-ple-development-backlog.md)가 소유한다. 제품 vertical에서는 학생이 학기 폴더에서 자료와 action을 고르면 AY-PLE가 ModelingInvocation을 실행하고, structured result를 변경 제안으로 보여주며, 학생의 결정만 확인된 학기 정보에 반영해야 한다.
