...
현재 프론트엔드는 T25 전수 검수를 통과한 288개 카드 문구를 T26 결정적 컴파일러가 만든 고정 TypeScript 산출물에서만 조회한다. 산출물은 approved manifest·SHA-256 checksum·문구별 provenance를 가지며 `templates:check`가 저작 소스와의 byte drift를 차단한다. 질문 없는 초안과 guided 장애 fallback은 같은 조회 키를 네트워크 없이 사용하고, 직접입력 경로는 T20에서 실제 Gemini API(`/api/generate`)에 연결됐다.
...
- 템플릿 전수 검수(T25), holdout 기반 실 AI 품질·모델 비교(T21), 결정적 템플릿 엔진 운영 전환(T26)은 완료했다. 기술 배포·실기기(T23)와 Vercel Preview DB 수명주기 통합(T31)은 완료 전이며, 외부 가치 검증(T22)은 기술 트랙과 분리한 `Pending` 상태다.
