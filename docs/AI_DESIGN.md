...
현재 프론트엔드는 288개 카드 문구 **초안**을 로컬에서 조회해 UI 흐름을 시연한다. T25 전수 검수와 T26 독립 템플릿 엔진·라우터는 미완료다. 직접입력 경로도 **개발용 고정 예시**만 반환하며 아직 외부 AI API를 호출하지 않는다. 포트폴리오와 사용자 문구에서 초안을 검수 완료 자산으로, 목을 실제 AI 생성으로 과장하지 않는다.
...
T32의 `speechStyleId`는 카드·AI 공통 입력 계약이며 모델 출력에 추가하지 않는다. `습니다체 / 요체 / 이다체 / 용용체` 중 사용자가 고른 하나를 세 후보 모두에 적용하되, 사실·안전 > 관계 적합성 > 개인 말투 > toneLevel 순서로 제약한다. 네 말투는 모든 관계에서 선택할 수 있지만 관계 적합성을 자동 합격 처리하지 않고 카드 초안은 T25, AI 결과는 T21에서 사람 검토한다. 선택값은 운영 메트릭이나 DB에 추가하지 않는다.
