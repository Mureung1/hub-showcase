...
| 사용자 말투 프리셋(음슴체·용용체·다나까체) | 첫 카드 경로의 선택 부담과 템플릿 조합 수를 늘린다. T22 수정 이유 코드(`awkward`·`tone`)에서 체 변경 수정 패턴이 확인되면 `직접 설명할게요` AI 경로의 선택 기능으로 먼저 검증하고, 관계별 허용 범위와 효과가 확인된 뒤 카드 경로 확장을 판단 |
...
신규 구현 항목: CHECKLIST T25(상황 카드 템플릿 72개) · T26(생성 경로 라우터) · T27(S0 화면) · T28(가이드형 챗 UI) · T29(Three.js 냥이) · T30(PostgreSQL 데이터 계층) · T31(확장 MVP 통합 DoD).
