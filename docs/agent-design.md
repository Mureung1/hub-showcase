...
| 통계 분석 | A2 + A0 | 청크, 활성 분류체계, 평가 세트 | mention, 후보, 승격 결정, 할당, 지표 | 후보별 독립 공고·회사 근거 | 승격 심사 완료, 집계 검증 통과 |
...
통계 분석 에이전트는 차원 발견에서 판단하고 집계에서는 판단하지 않는다. 발견과 승격 절차는 [통계 모델](statistics-model.md)에 있다.
...
원문 표현의 추출과 차원 후보 명명에 생성 모델을 사용한다. 빈도, 비율, 추세, 조합은 규칙으로 계산한다.
...
| --- | --- | --- |
| 자료 수집 | 미사용 | |
| 지식 구축 | 생성 | 정규화된 관계와 계보 기록 |
| 통계 집계 | 미사용 | 집계는 관계형 조회로 수행 |
| 채용공고 해석 | 사용 | 공고에서 요구·역량·공식 근거로 이어지는 연결 |
| 합격 전략 | 사용 | 요구에서 증명 산출물·활용처로 이어지는 연결 |
| 준비 로드맵 | 사용 | 역량에서 선수 역량·학습·체크리스트로 이어지는 연결 |
| 화면 조회 | 미사용 | 저장된 분석 결과를 직접 조회 |
