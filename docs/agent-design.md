...
이 문서는 CareerSignal의 여섯 에이전트가 받는 입력, 생성하는 출력, 사용하는 도구와 내부 제어 흐름을 정의한다. 시스템 배치와 런타임 경계는 [아키텍처](architecture.md), 자료의 출처와 신뢰도는 [데이터 전략](data-strategy.md)에서 다룬다.
...
| 유형 | 판단 범위 | 구성 예 |
| --- | --- | --- |
| 에이전트 | 검색·도구 선택, 충분성 판단, 재검색·재생성 | 데이터 수집, 지식 구축, 통계 분석, 채용공고 해석, 합격 전략, 준비 로드맵 |
...
웹 검색과 외부 수집은 데이터 수집 에이전트를 통해 수행한다. 다른 에이전트는 부족한 자료의 조사 목표와 범위를 전달하고 갱신된 지식 저장소를 다시 조회한다.
...
flowchart TD
    ...
    VERIFY -->|근거 부족| RESEARCH["조사 요청·지식 갱신"]
    RESEARCH --> RETRIEVE
...
| --- | --- | --- | --- |
| 데이터 수집 | 조사 목표, 출처 정책, URL·API 응답 | Raw 원문, 출처 메타·평가 | 지식 구축 |
| 지식 구축 | Raw 원문, 출처 평가 | Wiki, 청크, 임베딩, 노드·엣지 | 나머지 네 에이전트 |
| 통계 분석 | 직무별 공고 원문, 용어사전 | 정형 공고, 직무·기업군·기간 통계, 통계 해석 | 채용공고 해석·합격 전략·준비 로드맵 |
| 채용공고 해석 | 통계, 공고, 공식 자료, Wiki·벡터·그래프 | 직무 기준선, 기업군·공고 편차, 숨은 요구, 근거, 신뢰도 | 합격 전략·준비 로드맵·화면 |
| 합격 전략 | 공고 해석, 통계, 공식·외부 전략 자료 | 체크리스트, 포트폴리오·자소서·면접 전략 | 준비 로드맵·화면 |
| 준비 로드맵 | 합격 전략, 공고 해석, 통계, 선수 관계, 학습 자료 | 기본 프로젝트 로드맵, 학습 전략, 항목 연결 | Express 조합기·화면 |

여섯 에이전트의 내부 처리는 다음 구조를 공유한다. 각 단계의 상세 입력·출력과 반복 조건은 7~11장에서 정의한다.

```mermaid
flowchart LR
    subgraph CO["데이터 수집 에이전트"]
        CO1["자료 발견·수집"] --> CO2["출처·관련성 평가"]
    end

    subgraph KB["지식 구축 에이전트"]
        KB1["정제·중복 제거·청크"] --> KB2["주장·엔티티·관계 추출"]
        KB2 --> KB3["Wiki·벡터·원천 그래프"]
    end
    ...
    subgraph IP["채용공고 해석 에이전트"]
        IP1["통계·근거 검색"] --> IP2["직무 기준선 구성"]
        IP2 --> IP3["기업군·공고 편차·숨은 요구 해석"]
        IP3 --> IP4["근거·신뢰도 검증"]
    end

    subgraph SG["합격 전략 에이전트"]
        SG1["요구사항 통합"] --> SG2["증명 방법·활용처 배정"]
        SG2 --> SG3["체크리스트·지원 전략 생성"]
        SG3 --> SG4["항목 연결 검증"]
    end
...
## 7. 데이터 수집 에이전트와 지식 구축 에이전트

자료 수집과 지식 구축은 성격이 다른 작업이라 두 에이전트로 나눈다. 데이터 수집 에이전트는 자료를 발견·수집·평가해 원문과 출처 메타데이터를 저장하고, 지식 구축 에이전트는 저장된 원문을 정제·구조화해 재현 가능한 검색 자산을 만든다.

### 7.1 데이터 수집 에이전트

**목표**: 분석에 필요한 자료를 발견·수집·평가하고 원문과 출처 메타데이터를 지식 저장소에 넣는다.
...
**목표**: 수집된 원문을 정제·구조화해 다른 에이전트가 재현 가능한 방식으로 검색할 수 있는 지식 자산을 만든다.

**입력**

- 데이터 수집 에이전트가 저장한 Raw 원문과 출처 메타데이터
- 자료 계층과 데이터 버전
- 자료 부족을 보고한 에이전트와 필요한 근거 유형
...
→ 정제·중복 제거·청크 분할
→ 주장·엔티티·관계 추출
→ Wiki·벡터 인덱스·지식 그래프 구축
→ 충분성·출처 검증
→ 부족 시 데이터 수집 에이전트에 조사 요청
→ 지식 버전 저장
```
...
통계 결과는 채용공고 해석·합격 전략·준비 로드맵 에이전트가 모두 직접 조회한다.

## 9. 채용공고 해석 에이전트

### 9.1 목표

직무 공통 기준선과 기업군·개별 공고의 편차를 근거와 함께 해석한다.
...
공고 해석에서 도출한 요구사항을 준비 체크리스트와 포트폴리오·자소서·면접 전략으로 변환한다.
...
| 채용공고 해석 | 핵심 | 공고→요구사항→역량→공식 근거 연결 |
...
| 근거 | 주장별 원문 청크와 출처 존재 |
| 범위 | 직무·기업군·공고 식별자 일치 |
| 연결 | 공고 해석→체크리스트→합격 전략→준비 로드맵 식별자 연속성 |
...
| 공고 해석·합격 전략·준비 로드맵 생성과 LLM 검증 | `gpt-5.6-terra` | 복합 관계·근거 충실도가 기준에 미달하면 Sol |
