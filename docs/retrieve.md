...
`꺼내보기`는 사용자가 입력한 현재 상황과 Supabase 개인 보관함에서 불러온 인사이트의 최소 텍스트 신호를 비교해, 지금 다시 쓸 만한 자료를 최대 6개로 보여주는 기능이다.

## 이번 MVP에서 검증할 것

- 사용자는 정확한 제목을 몰라도 현재 하려는 일을 표현할 수 있다.
- 사용자가 저장 후 남긴 한 줄 메모는 일반 메타데이터보다 개인적인 활용 맥락을 잘 담는다.
- 원격 저장과 별개로, 자동 수집한 원문 메타데이터나 AI가 없어도 제목·메모·도메인·URL만으로 의미 있는 재발견이 가능한지 측정한다.
...
| 입력           | 제목, 메모, 사용자 카테고리 이름, 도메인, URL 단서 | 현재 상황, 목적, 작업 맥락                |
| 검색 대상 필드 | 제목, 메모, 사용자 카테고리 이름, 도메인, URL      | 제목, 메모, 도메인, URL                   |
| 출력           | 일치하는 전체 결과 목록                            | 관련 인사이트 최대 6개 꺼내보기 결과      |
| 연결 이유      | 필요 없음                                          | 사용자에게 보여주지 않음                  |
| 점수           | 사용자에게 보여주지 않음                           | 내부 정렬용으로만 사용                    |
| 검색 코어      | 공유하며 카테고리 이름 해석기를 전달한다.          | 공유하며 카테고리 이름을 전달하지 않는다. |

두 화면은 같은 순수 함수를 사용한다. 보관함 검색은 현재 사용자의 카테고리 이름을 `categoryId`로 해석해 결정적 키워드 검색에 포함하고, `꺼내보기`는 카테고리 이름을 제외한다. 사용자 의도와 결과 표현은 각 화면의 목적에 맞게 구분한다.

## 데이터와 검색 실행 경계

- 인증된 앱은 `InsightRepository.list()`로 현재 사용자의 원격 인사이트를 최신 저장순으로 불러온다.
- Supabase Postgres와 RLS는 저장·조회와 사용자 격리를 담당한다.
- 보관함 검색과 `꺼내보기`의 점수 계산·정렬은 불러온 데이터에 대해 브라우저의 순수 함수로 실행한다.
- 검색 점수는 내부 정렬용으로만 사용하며 사용자에게 보여주지 않는다.
- 검색 결과는 동일한 입력과 데이터에 대해 재현 가능해야 하며, 원격 저장 전환이 랭킹 규칙을 바꾸지 않는다.
...
- 인사이트에는 사용자 카테고리 하나만 연결할 수 있으며, 저장 화면과 카드 수정에서는 기존 카테고리를 선택하거나 관리 화면에서 새 카테고리를 만든다.
- 카테고리 이름과 색상은 사용자별 CRUD 대상으로 관리하고, 인사이트에는 자유 문자열이 아닌 `categoryId`를 저장한다.
...
서버 메타데이터 설명, 썸네일, AI 요약, embedding 값은 MVP 검색 문서에 넣지 않는다. 보관함 검색만 현재 사용자의 카테고리 저장소에서 `categoryId`에 대응하는 이름을 조회해 검색 시점에 사용한다.

## 결정적 랭킹 기준

현재 토큰 정규화와 필드 점수 방식은 작은 순수 함수로 유지한다. 새 검색 라이브러리나 서버 검색 인프라는 기준 검색이 부족하다는 증거가 생기기 전에는 도입하지 않는다.

| 필드          | 권장 weight | 이유                                               |
| ------------- | ----------: | -------------------------------------------------- |
| `memo`        |         4.0 | 저장 이유와 활용 상황을 직접 담는다.               |
| `title`       |         3.0 | 자료의 주제를 직접 설명한다.                       |
| `category`    |         2.0 | 보관함 검색에서만 사용자 카테고리 이름을 사용한다. |
...
- `category` 필드는 보관함 검색이 카테고리 이름 해석기를 전달했을 때만 점수에 포함하고, `꺼내보기`에서는 항상 빈 값으로 처리한다.
...
같은 fixture로 `꺼내보기`를 실행할 때 `개발`처럼 카테고리 이름에만 일치하는 쿼리는 결과가 없어야 한다.
...
- 추천 상황 예시는 입력·검색 결과 상태와 관계없이 항상 보여준다.
- 결과가 없으면 입력값을 유지하고 단어를 줄이거나 다른 상황을 시도하도록 안내한다.
- 원격 응답에서 계약에 맞지 않는 항목은 해당 항목만 제외하고 나머지 인사이트 검색을 유지한다.
- URL이 유효하지 않으면 새 탭을 열지 않고 카드에서 문제를 안내한다.
