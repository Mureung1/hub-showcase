...
- 레시피 데이터: `src/data/mockRecipes.js`
- 추천·재료 매칭: `src/utils/recipeUtils.js`
- 코치 반응 UI: `src/components/CoachReactionModal.jsx`

레시피에는 필수·선택 재료, 조리 시간, 난이도, 불 사용 여부, 노력 수준, 일반·스페셜·인스턴트 구분, 추천 이유, 코치 반응과 간단 조리 단계가 포함됩니다. 태그 필드는 사용하지 않습니다.

필수 재료를 냉장고 재료 이름과 공백을 제거한 값으로 비교하며 결과는 다음 세 상태로 계산합니다.

- `available`: 필수 재료를 모두 보유
- `oneMissing`: 필수 재료 1개 부족
- `shoppingNeeded`: 필수 재료 2개 이상 부족

추천 화면은 `noFire`, `quick`, `special` 세 상황을 React state로 관리합니다. 전자레인지 메뉴는 `noFire`로 분류하며, 각 상황에서 현재 만들기 쉬운 메뉴 3개를 우선 노출합니다. 일반 메뉴 선택 시 `coachReaction` 칭찬 모달을, 인스턴트 메뉴 선택 시 기존 라면 코치 모달을 표시한 뒤 상세 화면으로 이동합니다.
