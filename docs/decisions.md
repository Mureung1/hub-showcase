...
### 이슈 분석 기능: Claude API(LLM) 도입 — 지연 생성 + 캐싱
- 날짜: 2026-07-14
- 배경/문제: 추천 상세 화면의 "기여 시작 가이드"가 이슈와 무관한 고정 문구뿐. 이슈 본문·코멘트를 분석해 요약/필요 기술/시작 가이드를 제공해 "초보자가 이슈를 보고 겁먹지 않게" 하는 핵심 가치를 채우고 싶음.
- 선택한 방안: LLM(Claude API) 분석. Haiku 4.5로 시작하고 품질이 아쉬우면 Sonnet 5로 상향. structured outputs(`output_config.format`)로 `{issueSummary, requiredSkills, guide[]}` JSON 형태 보장.
...
- 이유: 비용이 이슈 1건당 ~$0.006(Haiku 기준) 수준으로 무시 가능하고 lazy+캐싱으로 실비용은 0에 수렴. 구현도 `services/`에 llmService 추가 정도로 가벼움. 포트폴리오 관점에서 LLM 연동 경험 어필 포인트.
- 주의: API 키는 `.env`로 관리(커밋 금지). LLM 호출 실패/429 시 분석 필드 없이 상세 화면이 정상 동작하도록 폴백(필드는 명세상 옵셔널).
