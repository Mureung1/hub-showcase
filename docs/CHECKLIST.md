...
- [ ] **T17. 실행 환경·Vercel 세팅 + 목 1차 배포** — 의존: T14. Node 22.12.0 고정, `npm ci` 재현, 클라이언트+API CI와 main required `verify`, Vercel Git 연동·Production Branch 배포는 완료했다. 2026-07-20 비프로덕션 `t17-preview-t19`의 Vercel Preview 배포 성공도 확인했다. 공개 Production과 Standard Protection Preview의 JavaScript 후 S0 실물 확인이 남아 있다. 절차와 근거는 docs/CICD.md·T17 하네스
