...
- [ ] **T28. 가이드형 챗 UI 전환** — 의존: T2, T5, T7~T14, T27. 내부 S0~S3·생성 계약은 유지하면서 냥이 헤더, 발자국형 진행 표시, 이전 선택 사용자 말풍선, 냥이 질문, 빠른 답변, 결과 3개 말 꾸러미로 표현한다. 선택 턴 AI 호출·전체 자유 대화·히스토리 저장·새 UI 라이브러리는 도입하지 않는다. 375×667에서 헤더·첫 질문·첫 선택지가 보이고, 320×568 무가로넘침, 화자 텍스트 구분, 단계 초점·키보드·reduced motion, 기존 템플릿/AI/복사/실패/이동 회귀 테스트를 통과해야 완료. 구현·자동 검증 완료(59개 테스트·lint·build·diff, 2026-07-15), 현재 세션 브라우저 미제공으로 320×568·375×667 실물 확인 후 체크 예정
