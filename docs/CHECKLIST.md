...
- [x] **T16. 시드 2차 12개 + 24개 전수 검수** — 의존: T15. 24개 초안 작성 상태. F-A 입력 없는 약속 등 전수 자체 스크리닝을 다시 하고, SEEDS_REVIEW 재생성→제3자 블라인드 정렬→전송 가능성까지 통과해야 완료. `decline` 목적 0건은 holdout/프롬프트 검수에서 보완
...
- [ ] **T18. 프록시 함수 `/api/generate`** — 의존: T3, T17. 착수 전 RESEARCH_REVIEW 완료, T25 대표 12문구 source preflight, COMPETITIVE_VALIDATION C1~C4 무참여자 모델 벤치마크의 `Provisional Go`를 확인한다. Pending이면 분모를 확보하고, Iterate/No-go면 taxonomy·템플릿부터 수정하며 백엔드를 보류한다. 긴 인터뷰·사용자 모집은 선행조건이 아니다. Provisional Go 후 키 환경변수, 서비스 원문 비저장, 콘텐츠 없는 운영 메타만 로그, 서버 deadline+`AbortController`, 출력 토큰 상한, 제한 재시도, 오류 매핑, 레이트리밋을 구현. `api/` 전용 TypeScript 설정·핸들러 테스트로 클라이언트 빌드 밖 서버 코드도 검증. **코드 우선 기반 완료(2026-07-15, 완료 체크 보류)**: Vercel fetch 진입점, provider/limiter/metrics 주입 핸들러, AI 전용 검증, 18초 취소, 출력 상한 1024, 10회/60초 제한, 재시도·공개 오류 정규화, 원문 없는 메타데이터 타입과 API 테스트 19개를 구현. 실 provider·키와 T17·T25/R2 게이트가 남아 T18은 미완료
- [ ] **T19. 프롬프트 구성** — 의존: T16, T18. SPEC 3장(few-shot 주입, 안전 가이드, 데이터 블록 격리, `output_config.format`, `stop_reason`, 후처리 검증). 데이터 블록은 주입 완화책으로만 표현. **코드 우선 골격 완료(2026-07-15, 완료 체크 보류)**: 서버 전용 6개 모듈로 관계·목적·안전 규칙, 동일 관계 예시 2세트 주입 검증, XML 이스케이프, JSON Schema, `end_turn`+공용 런타임 검증을 구현하고 합성 fixture 테스트 24개를 통과. `docs/SEEDS.md`의 실제 제3자 검수와 T18 실 provider 연결이 남아 T19는 미완료
...
- [ ] **T30. Neon PostgreSQL + Drizzle 데이터 계층** — 의존: T17, T18. `prompt_versions`·`template_versions`·`generation_runs`·`evaluation_runs` schema와 migration·repository를 구현한다. 관계/모드/목적 ID, model·버전, status, latency, token, 집계 평가만 허용하고 받은 메시지·상황 설명·생성 문구·IP·영구 사용자 ID 필드는 금지한다. 프롬프트/템플릿 본문은 Git 정본으로 유지하고 DB에는 배포 version/checksum을 연결한다. 브라우저 직접 DB 연결 금지, 환경별 DB 분리, 기록 실패가 생성 응답을 막지 않는 best-effort와 원문 비직렬화를 repository·handler 테스트로 검증
- [ ] **T31. 확장 MVP 통합 DoD·최종 배포** — 의존: T22, T23, T29, T30. 기존 템플릿/AI/복사/접근성 DoD에 Three.js 정적 폴백·모바일 성능, server-only provider/DB 비밀, migration 재현, DB 원문 비저장, 교수님 직접입력 단일 AI 호출→후보 3개를 합쳐 재검증한다. 전체 테스트·lint·build·API 타입검사·migration dry run·실기기 확인 뒤 최종 통합 배포하며 RAG·자율 agent loop·런타임 멀티에이전트가 런타임 의존성/코드 경로에 없음을 확인
