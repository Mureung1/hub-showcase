...
- [ ] **T16. 시드 2차 12개 + 24개 전수 검수** — 의존: T15. 24개 초안과 자체 스크리닝은 완료했지만, `docs/SEEDS.md`·`docs/SEEDS_REVIEW.md` 기준 제3자 블라인드 정렬 8/8과 전송 가능성 24개 전수 판정이 아직 없다. 검수 근거를 기록하고 불일치 문구를 재작성해야 완료. `decline` 목적 0건은 holdout/프롬프트 검수에서 보완
...
- [ ] **T17. 실행 환경·Vercel 세팅 + 목 1차 배포** — 의존: T14. Node 22.12.0 고정, `npm ci` 재현, 클라이언트+API CI와 main required `verify`, Vercel Git 연동·Production Branch 배포는 완료했다. 공개 Production의 JavaScript 후 S0 수동 확인과 비프로덕션 브랜치 Preview 배포 확인이 남아 있다. 절차와 근거는 docs/CICD.md·T17 하네스
