...
#15의 저장·보관함 핵심 흐름은 세 화면 폭과 로컬 오프라인 상태에서 완료됐다. 발견된 Windows 포맷 검사 결함도 수정·재검증했다. 브라우저 저장소 강제 오류는 자동화 회귀 테스트로 보완했다. Chrome 390px에서 키보드만으로 전체 흐름을 완주했고, 세 화면 폭 모두 원문 링크가 보존된 주소로 실제 새 탭에서 열리는 것을 확인했다.
