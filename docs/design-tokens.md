...
이 문서는 `prototype/style.css`의 `:root`와 레이아웃 기준을 기록합니다. 정적 프로토타입의 값이 바뀌면 이 문서도 함께 갱신합니다.
...
- `.reader-layout`은 보고서와 로드맵의 본문·목차 레이아웃입니다. 본문 최대 폭은 `980px`, 오른쪽 목차 폭은 `220px`입니다.
...
- `.combination-grid`: 반복되는 구현 범위 조합 3개를 비교합니다.
- `.split-grid`: 필수 요구사항과 우대사항을 나란히 보여 줍니다.
- `.roadmap-timeline`: 4단계 학습 로드맵을 세로 흐름으로 보여 줍니다.
