...
| article 없음 | `404` |
| 동일 사용자가 동일 article에 다시 제출 | **허용한다.** 같은 글을 다시 읽고 새 생각을 기록할 수 있도록 unique 제약을 걸지 않는다. 실수 중복은 프론트가 제출 후 버튼 비활성·화면 전환으로 막는다 |
| 토큰 없음·위조 | `401` |
...
읽기·미션 흐름을 **하이라이트 없이 글 전체 대상**으로 확정했다. 결정 근거와 저장 계약은 이 문서의 `이번 설계 결론`과 `POST /api/mission-records`에 통합되어 있다.
...
### 동일 글 재기록 (2026-07-15 확정)

같은 글에 대한 미션 기록을 **여러 번 허용한다.** 같은 글을 다시 읽고 바뀐 생각을 남길 수 있어야 하기 때문이다.
