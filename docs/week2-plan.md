...
### 남은 일
- **`0003_watchlists.sql` Supabase 적용** — 미적용 시 대시보드 관심종목·웹 조건 저장이 실동작하지 않음.
- **T3 관망(hold)** — 미착수(스키마 `'hold'` + Discord 버튼 + 웹 3-way 라벨).
- **T4 로그인 Discord 연결 UI** — 미착수.
- **라이브 end-to-end 검증** — 로그인 세션에서 대시보드 검색·시세·조건 저장·`/journal/:symbol` 차트·히스토리 그룹·네비 중앙정렬 확인(현재 빌드·lint·마운트까지만 확인됨).
...
| ➕ | 대시보드 (종목검색/관심종목/최근기록) | ✅ 코드 완료 | `DashboardPage`+`/journal/:symbol`+`lib/symbols.js`. `watchlists`(`0003`) **적용 대기** |
| ➕ | 웹 종목검색 → 조건 추가 | ✅ 코드 완료 | `ConditionForm` 구조화 폼 → `conditions` insert (계획 외 추가) |
| ➕ | 로그인 화면 Discord 연결 | ❌ 없음 | `LoginPage`는 이메일/비번만. `discord_links` 테이블은 존재 (T4) |
...
- 대상: `0004_hold.sql`(신규 — `0003`은 watchlists가 선점), `discord-interactions/index.ts`, `monitor/index.ts`(버튼), `mvp/src/pages/{JournalPage,ReviewPage,HistoryPage}.jsx`.
