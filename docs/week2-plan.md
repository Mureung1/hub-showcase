...
- 하위 결정: **관심종목(watchlist) 저장 방식** — 신규 `watchlists` 테이블 vs 파생/고정(§4).

### T3. 관망(hold) 반영 — **결정 필요(§4)**
채택 시:
...
1. **관망을 1급 개념으로 채택?** — "진입 안 함" 결정을 기록·복기 대상에 포함할지. 채택 시 T3 수행(스키마 변경 동반). 미채택 시 프로토타입에서 관망을 데모용으로만 유지.
2. **관심종목 저장 방식** — 신규 `watchlists` 테이블 vs 고정/파생(최근 조회 종목 등).
3. **알림 메모리 에이전트화(T5) 범위** — 2주차 포함 vs 이후.
