...
4. `temporary_idea`이면 `docs/workflows/temporary_idea.md`에 따라 등록,
   수정 또는 승인 제안 전환으로 분기한다.
5. 확정 반영이 명확하지 않으면 아이디어나 승인 큐 초안으로만 처리한다.
6. 필요한 workflow 문서를 읽고 해당 절차를 따른다.
7. 정보가 부족하면 바로 확정하지 말고 질문 또는 `TBD`를 남긴다.
