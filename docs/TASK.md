...
- [x] **BE 후속 확인 결과(7/29)** — 어제 남겨둔 24개 불일치를 하나씩 실제로 다시 확인한 결과, "RegionDistrict가 불완전하다"는 어제의 가설은 **틀렸음**. 실제 원인은 세 가지로 갈렸다:
  1. **전남/광주 21개 불일치 — 버그 아님, 기존의 의도된 설계였음.** `regionRuleService.ts:126-127`에 이미 주석으로 남아 있던 사실을 뒤늦게 발견: 행정안전부_생활쓰레기배출정보(`household_waste_info`, `RegionDistrict`/`RegionRule`의 원본)는 전라남도+광주광역시를 `"전남광주통합특별시"` 하나로 묶어 내려보내고, `getRegionRule`가 라이브 API 응답의 `CTPV_NM`과 정확히 문자열 비교하기 때문에 이 값을 표준 명칭으로 바꾸면 그 즉시 전남/광주 사용자의 `RulesPage`가 전부 깨진다. **RegionDistrict를 고치려다 멈추고(수정 스크립트 작성 후 실행 전에 발견해서 실행 안 함) 대신 내 범위(수거함 기능)만 고쳤다**: `collectionPointService.ts`에 `resolveCollectionPointCtpvNm` 추가 — 사용자가 지역 선택기에서 (전남/광주 사용자에게는 유일한 선택지인) `"전남광주통합특별시"`를 고르면, `sggNm`이 광주 5개 구(동구/서구/남구/북구/광산구) 중 하나인지로 판별해 `CollectionPoint` 조회 시에만 표준 명칭(전라남도/광주광역시)으로 변환 — 오늘 동기화한 수거함 데이터 3종은 이 표준 명칭을 쓰기 때문. curl로 강진군(폐의약품)/광주 남구(의류) 둘 다 검증 완료, 단위 테스트 3건 추가(`collectionPointService.test.ts`).
  2. **인천 서구/동구 2개 불일치 — 버그 아님, 2026-07-01 인천 행정구역 개편(2군 8구 → 2군 9구) 반영 결과.** `RegionDistrict`를 직접 까보니 인천은 이미 `제물포구`/`영종구`/`검단구`/`서해구`라는 새 구 이름을 갖고 있었음 — `RegionDistrict`가 최신이고 오히려 **동기화한 수거함 표준데이터(의류 등)가 개편 전 옛 이름(서구/동구)을 그대로 쓰고 있는 쪽**이 낡은 것. 실측으로 "동구" 검색 결과에 인천 데이터가 없음을 재확인(처음에 오검출된 "동구"는 사실 "남동구"의 부분 문자열 매칭이었음).
  3. **청주시(4개 구)/서울 강동구·도봉구/경기 동두천시/전북 남원시 — 실제 데이터 자체가 없음(버그 아님).** `household_waste_info` API를 해당 구/시로 직접 필터링해 호출한 결과 전부 `totalCount: 0` — 지자체가 이 특정 데이터셋에 아예 데이터를 제출하지 않은 것으로 확인(다른 데이터셋에서도 반복된 자율 제출 편차 패턴과 동일). 대체 데이터 소스 없이는 해결 불가 — `RulesPage`/`BulkyPage`/`PointsPage` 전부 이 지역은 지원 불가 상태.
- [x] **BE 후속 처리(7/29)** — 위 2번(인천) 실제로 조사 및 해결: 웹 검색으로 인천시 행정체제 개편 세부 내용 확인 — "중구+동구 통합 → 제물포구"(동구 전체가 예외 없이 흡수), "중구의 영종도 부분만 분리 → 영종구", "서구는 경인아라뱃길 기준 북/남으로 검단구·서해구 분할". 옛 동구는 분할 없이 전량 제물포구로 흡수되는 게 확실해 `scripts/relabelIncheonDonggu.ts`로 기존 255건 일괄 치환 + `syncCollectionPoints.ts`의 `normalizeIncheonSggNm`에도 반영해 재동기화 때마다 다시 어긋나지 않게 함. 옛 서구(252건)는 주소 텍스트만으로 검단구/서해구 중 어느 쪽인지 판단할 근거(동 단위 경계 목록 또는 좌표 기반 GIS)가 없어 **의도적으로 그대로 둠** — 잘못 갈라 배정하는 것보다 미해결로 남기는 쪽을 택함.
- [x] **BE 후속 처리(7/29)** — 위 리스크 메모의 `"전남광주통합특별시"` UX 이슈 실제로 처리: 저장값(API 매칭용, `RegionDistrict`/`RegionRule`)은 그대로 두고 **표시 라벨만** 분기하는 `frontend/src/features/region/mergedRegion.ts` 추가 — 시/도 드롭다운(구/군을 아직 모르는 시점)엔 "전라남도 · 광주광역시" 병기, 구/군까지 정해지면(`formatSelectedRegionLabel`) 광주 5개 구 여부로 판별해 "전라남도"/"광주광역시" 중 정확한 이름을 보여줌. `RegionSelectSheet`/`formatSelectedRegionLabel` 두 곳만 수정, 저장 로직·API 호출은 무변경이라 회귀 위험 없음(기존 `RegionSelectSheet.test.tsx` 통과 확인).
- [x] **BE 후속(7/29) — 소형가전/종이팩 실 데이터 재조사 결과**: 처음 찾았던 "한국환경공단_재활용 폐기물 수거회수처 정보"(15141652, 중소형폐가전 특화라고 설명돼 있던 API)를 사용자가 직접 브라우저로 확인한 결과, 실제로는 이미 알고 있던 "전국재활용센터표준데이터"(15021108, `tn_pubr_public_ruse_cnter_api`)와 완전히 동일한 엔드포인트로 확인됨 — 카탈로그 설명과 실제 등록이 다른, 문서만 믿었으면 몰랐을 불일치. 소형가전/종이팩 전용 수거함과 정확히 맞는 데이터는 여전히 못 찾음(15021108의 `TRTMNT_PRDLST`가 "가구+냉장고+TV 등"/"종이+고철+철캔+페트 등"처럼 일반적이라 소형가전/종이팩에 억지로 끼워 맞추면 오분류) — **소형가전/종이팩은 계속 데모 유지**, 대신 15021108을 정직하게 새 카테고리 **"재활용센터"**로 추가(사용자 확인 후 진행). `services/govCollectionPointApiClient.ts`/`types/govCollectionPointApi.ts`/`syncCollectionPoints.ts`에 반영 — 이 데이터셋은 CTPV_NM/SGG_NM 필드가 없고 주소 문자열만 줘서 `parseAddressRegion`으로 직접 파싱(시/도+시/군/구 앞 두 토큰만 취함, 창원시/수원시처럼 여러 구가 있어도 구는 버림 — 이 앱의 기존 관례와 실측 대조로 확인). 실측 183건 전량 파싱 성공(스킵 0건) 중 원본 데이터 자체의 오류 2건 추가 발견: "강원도"(2023-06-11 개칭 전 이름, 정확히는 "강원특별자치도"여야 함)와 "서을특별시"(명백한 오타, "서울특별시") — 둘 다 애매함 없는 확실한 오류라 `CTPV_NM_CORRECTIONS`로 정규화. 이 데이터셋은 유일하게 실제 운영시간(평일/공휴일)이 있어 `hours` 필드도 실제 값으로 채워짐(다른 4종은 hours 없어 null). 카테고리 6종 → 7종(`재활용센터` 추가), FE `COLLECTION_POINT_CATEGORIES`/ko·en i18n 반영.
...
**드롭됨(7/29, 팀 결정)** — 이 프로젝트 범위에서 지도 시각화는 진행하지 않기로 함. 아래 항목은 기록만 남기고 착수하지 않는다.

~~Day 10에서 만든 수거 장소 리스트 데이터에 지도를 얹는 후속 스프린트. 테스트 시작(7/25) 이후 진행하며 MVP 완성 기준에는 포함하지 않는다.~~

- [ ] ~~**BE** 지도 API(카카오맵/네이버 지도 등) 조사 + 키 발급~~
- [x] **BE** 수거 장소 데이터에 좌표(위경도) 필드 추가 — Day 10 후속(7/28~7/29)에서 실 데이터로 전환하며 채택한 표준데이터 4종에 이미 좌표가 포함돼 있어 예정보다 앞당겨 완료됨(지도 자체는 드롭이라 활용 계획 없음)
