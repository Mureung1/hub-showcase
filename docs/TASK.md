...
- [x] **BE 후속 확인 결과(7/29)** — 어제 남겨둔 24개 불일치를 하나씩 실제로 다시 확인한 결과, "RegionDistrict가 불완전하다"는 어제의 가설은 **틀렸음**. 실제 원인은 세 가지로 갈렸다:
  ...
  2. **인천 서구/동구 2개 불일치 — 버그 아님, 2025년 인천 행정구역 개편 반영 결과.** `RegionDistrict`를 직접 까보니 인천은 이미 `제물포구`/`영종구`/`검단구`/`서해구`라는 새 구 이름을 갖고 있었음(구 서구/동구/중구 개편) — `RegionDistrict`가 최신이고 오히려 **오늘 동기화한 수거함 표준데이터(의류 등)가 개편 전 옛 이름(서구/동구)을 그대로 쓰고 있는 쪽**이 낡은 것. 실측으로 "동구" 검색 결과에 인천 데이터가 없음을 재확인(처음에 오검출된 "동구"는 사실 "남동구"의 부분 문자열 매칭이었음). 인천 지역은 동 단위까지 내려가야 정확한 매핑이 가능해 주소만으로 자동 재매핑하지 않고 일단 보류 — 인천 사용자는 검단구/서해구/제물포구/영종구를 선택해도 오늘 동기화한 의류/폐의약품/건전지 데이터가 옛 이름(서구/동구) 기준이라 조회되지 않을 수 있음.
...
- [ ] **BE 후속(범위 밖, 다음에 결정)** — `"전남광주통합특별시"`가 `RegionSelectSheet` 드롭다운에 실제 사용자에게 그대로 노출되는 중(전남/광주 주민이 자기 지역을 찾으려면 이 낯선 이름을 골라야 함). 저장값(API 매칭용)은 그대로 두고 **표시 라벨만** "전라남도"/"광주광역시"로 보여주는 방식(값은 유지, 라벨만 분기)이 필요할지 팀 논의 필요 — 오늘은 내 기능(수거함) 범위만 우회 대응했고 전역 UX는 손대지 않음.
