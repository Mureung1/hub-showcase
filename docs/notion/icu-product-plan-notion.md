...
- 학습을 시작하면 IDE형 워크스페이스에서 AI 튜터, 커리큘럼, 코드 에디터, 실행 결과를 함께 사용합니다.
...
- 프로젝트는 MVP 개발 본편 전 단계입니다.
- 기획, 사용자 흐름, 제품 디자인, 기능별 화면 스펙은 정리되었습니다.
- 다음 단계는 React 화면 구현입니다.

다음 작업:

1. Today Learning Hub를 React 화면으로 구현합니다.
2. Learning Workspace IDE를 React 화면으로 구현합니다.
3. mock 학습 데이터를 구성합니다.
4. 이후 Monaco Editor, 코드 실행, Electron 구조를 순차적으로 연결합니다.
