...
- [ ] [P0] Web Push 실제 수신 확인 — 아직 미확인. 만료 구독(410/404) 자동 삭제는 구현했으나, 새 구독으로 실제 알림이 도착하는지는 미검증
...
- [ ] [P1] Lv3 "기억 기반 개입" 신뢰성 문제 — 현재 `getMemoryNudge`(`src/lib/memoryNudge.ts`)는 과거에 완료했던 실제 microTask를 재사용하지 않고, 매칭된 과거 task의 유형만 참고해 룰베이스로 새 행동을 생성한다. 그런데 메시지 문구는 "지난번엔 이렇게 해서 완료하셨어요"처럼 과거 행동을 그대로 인용하는 것처럼 표현돼, 실제로는 새로 만든 행동을 과거 성공 사례인 것처럼 보여줄 위험이 있다(`generationSource`는 `rule_based`로 정직하게 저장되고 있어 데이터 자체는 문제 없음, UI 문구가 오해를 유발할 수 있다는 뜻). 실제 과거 done 이벤트의 microTask를 조회해 재사용하거나(`history_reuse`), 재사용하지 않을 경우 문구에서 "그때 이렇게 했다"는 인용 표현을 빼는 방향으로 STEP C(Lv3 실제 과거 근거)에서 해결 예정 (2026-07-24 발견, 이슈 없음)
...
- [ ] 정리 필요 — 배포 전 전체 파일 구조 점검(2026-07-25) 중 발견한 미사용/미연결 파일 3건
  - `src/lib/memoryNudge.ts`(+`.test.ts`): 오늘(`e18d9f8`) Lv3 로직이 서버(`lv3MemoryCandidate.ts`+`geminiMicrotask.ts`)로 옮겨가면서 어떤 컴포넌트도 더 이상 import하지 않는 죽은 코드가 됨. 위 152번 항목("Lv3 기억 기반 개입 신뢰성 문제")은 이 파일이 실제 로직으로 쓰이던 시절 얘기라 이제 의미 없음 — 문구 수정이 아니라 파일 삭제로 해결
