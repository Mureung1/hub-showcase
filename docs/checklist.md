...
- [ ] 테스트코드 생성 Skill. `[P1]` `.claude/skills/test-writer/SKILL.md` 신설, design-review와 동일 포맷, 월요일 T9 TDD에서 실제로 쓴 테스트를 예시로 인용 (이슈 #15) — 수요일로 유지, 가벼운 회귀 스모크와 함께(정식 T15는 아님). 목/금은 완전한 버퍼로 남김
...
---
작성일: 2026-07-10 · 수정일: 2026-07-21 — **3주차 화요일, T9~T14 전부 완료.** 2주차(화~목=T2-a~T8, 금요일 공휴일)는 GitHub 이슈 #1~#8 전부 닫힘. 3주차는 planning-agent로 요일별 계획을 세우고 이슈 #9~#15 등록, Project 보드 "3주차 - 자소서 초안 생성 슬라이스"(#2)에 연결. 월요일에 vitest 도입(#10)+README 아키텍처 다이어그램(#9)+T9(#11)+T10(#12), 화요일에 T11(#13)+T12(#14)+T13(#16)+T14(#17)까지 끝남 — T13(stale draftError)·T14(경쟁 상태로 인한 화면 덮어쓰기)에서 각각 실제 버그를 발견해 수정. 남은 건 테스트코드 생성 Skill(#15)뿐이라 수요일에 Skill 제작+가벼운 회귀 스모크만 하면 3주차 본작업이 끝나고, 목/금은 완전한 버퍼. 정식 T15(E2E)·T16(데모 준비)은 "이후 변경 없음" 전제가 필요한 마무리 작업이라 그대로 4주차에 남김.