...
- [ ] 캘린더 히스토리 뷰(#43)가 완료일이 아니라 등록일(`createdAt`) 기준으로만 표시되는 제약 개선 — (2026-07-26 갱신) `completedAt` 필드는 이미 `/api/history` 응답에 존재함(스키마 문제 아님). 실제 원인은 배선 문제: `historyGrouping.ts`/`HistoryPage.jsx`의 캘린더 그루핑이 `/api/tasks`(completedAt 없음) 데이터만 쓰고, 같은 화면에서 이미 fetch해오는 `/api/history`(completedAt 있음) 데이터는 리스트 뷰 표시용으로만 쓰고 캘린더 그루핑엔 연결 안 돼 있음. 남은 작업은 스키마 추가가 아니라 이 배선 연결
