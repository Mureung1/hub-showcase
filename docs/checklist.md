...
- [x] T15. `[P0]` 전체 시나리오 End-to-End 점검 (plan.md 6단계 시나리오 기준) (이슈 #18) — 3주차 본작업(T9~T14)이 화요일 하루 만에 끝나서 코드가 안정된 김에 이번 주로 당김. Playwright 한 스크립트로 5케이스(입력 오류·정상 6단계 전체·추천 0건·네트워크 에러·stale-error/경쟁상태 회귀 확인) 검증, 콘솔 에러 없음. `npm test`+oxlint 최종 재확인. ANTHROPIC_API_KEY 없이 폴백 경로로만 검증 — 실제 Claude 호출 경로는 사용자가 키/결제 준비되면 별도 확인 예정
