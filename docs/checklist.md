...
- [ ] T2-b. `[P0]` Supabase 프로젝트·`profiles` 테이블 설정 (사용자 입력 저장용, T1 선행) — 목요일 (스키마 설계는 [docs/data-model.md](./data-model.md)로 수요일에 완료, 실제 생성만 목요일로 미룸 — 공식 데일리 미션 "설계는 오늘, 생성은 내일" 순서 반영)
...
---
작성일: 2026-07-10 · 수정일: 2026-07-15 (화=T2-a+T4+T3 완료, 수=화면 흐름 mock 완성(T7/T8/T11 화면)+데이터 모델 설계, 목 예정=T2-b+T5 · 공고 데이터는 Supabase가 아닌 postings.json으로 유지, Supabase는 사용자 프로필 저장 전용으로 스코프 명확화)