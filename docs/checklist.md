...
- [x] 인증 상태에 따른 페이지 분기
- [x] 닉네임 기능 추가
  - [x] User 모델에 nickname 필드 추가 (Prisma 스키마)
  - [x] 마이그레이션 실행
  - [x] 회원가입 시 닉네임 입력 필드 추가
  - [x] POST /auth/signup에 nickname 파라미터 추가
  - [x] 프로필 설정 기본정보에 닉네임 필드 추가
  - [x] 프로필 수정 페이지(SettingsPage)에 닉네임 필드 추가
  - [x] PATCH /api/profile에 nickname 포함
  - [ ] 닉네임 표시 UI (대시보드 등 필요한 곳) - 선택
...
### 캘린더 통합 개선
- [x] 대시보드/스크랩 페이지에서 "일정에 추가" 버튼으로 공고를 캘린더에 추가
- [x] 추가된 공고 일정은 버튼이 "삭제"로 자동 변경
- [x] 캘린더에서 삭제 시 대시보드로 돌아오면 버튼 자동 복원
- [x] 공고 일정 추적을 위해 relatedPostingId 필드 추가 (backend)
- [x] CalendarEvent API에 relatedPostingId 지원 추가

### 캘린더 페이지 기능 강화
- [x] 캘린더 추가/수정 모달에 "공고마감일" 버튼 추가
- [x] 사용자가 추가한 공고 일정 목록 자동 표시
- [x] 공고가 여러 개일 때 드롭다운으로 선택 가능
- [x] 공고 선택 시 제목/날짜 자동 입력
- [x] POSTING 타입 이벤트는 항상 all-day로 설정 (week view 호환성)

### 캘린더 표시 개선
- [x] allDay 이벤트를 YYYY-MM-DD 형식으로 변환 (시간 정보 제거)
- [x] POSTING 이벤트가 week/day view에서 올바르게 표시
- [x] 색상 정의 (POSTING: 핑크 #ec4899)

### 페이지네이션 개선
- [x] 대시보드 페이지네이션을 5단위 고정으로 변경
- [x] 1-5 → 6-10 → 11-15 ... 형태로 일정하게 표시
- [x] 스크랩 페이지에도 동일한 로직 적용

### 버그 수정
- [x] 기존 저장된 POSTING 이벤트가 시간대로 표시되는 문제 (오전 9시)
