...
- [ ] 정리 필요 — 배포 전 전체 파일 구조 점검(2026-07-25) 중 발견한 미사용/미연결 파일 3건
  - `src/lib/memoryNudge.ts`(+`.test.ts`): `e18d9f8`에서 Lv3 로직이 서버(`lv3MemoryCandidate.ts`+`geminiMicrotask.ts`)로 옮겨가면서 어떤 컴포넌트도 더 이상 import하지 않는 죽은 코드가 됨. 위 "Lv3 기억 기반 개입 신뢰성 문제" 항목은 이미 해결·[x] 처리됐고, 남은 건 이 파일 삭제뿐
