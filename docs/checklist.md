...
- [x] T5. `[P0]` 사용자 입력 → 공고 매칭/추천 로직 설계 (T3 선행) — 수요일 (T2-b에 이어 하루 앞당겨 완료). 키워드 매칭은 단순 부분 문자열 포함 검사라 "창업 동아리" vs "창업동아리"처럼 띄어쓰기가 다르면 안 겹치는 한계 있음 — T6 통합 테스트하면서 실제로 문제되면 개선 예정
