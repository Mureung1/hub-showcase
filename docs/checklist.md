...
- [ ] T13. `[P0]` 화면 간 흐름/상태관리 정리 (입력 → 추천 → 상세 → 초안 전체 연결) — 원래 4주차 예정이었으나, planning-agent 검토 결과 T11/T12만 선행하면 되고 4주차의 미정 신규 기능과 무관한 순수 정리 작업이라 안전하다고 판단해 당겨옴. 범위는 현재 useState/props drilling 구조 안에서의 정리로 한정(Context/상태관리 라이브러리 도입 없음) (이슈 #16) — 목요일에서 수요일로 하루 더 당김
- [ ] T14. `[P1]` 예외/로딩 처리 (추천 결과 없음, 생성 실패, 입력 오류 등) — 마찬가지로 4주차에서 당겨옴(T11만 선행하면 충족) (이슈 #17) — 금요일에서 목요일로 하루 더 당김
- [ ] 테스트코드 생성 Skill. `[P1]` `.claude/skills/test-writer/SKILL.md` 신설, design-review와 동일 포맷, 월요일 T9 TDD에서 실제로 쓴 테스트를 예시로 인용 (이슈 #15) — 목요일, T14와 함께 + 가벼운 회귀 스모크(정식 T15는 아님). 금요일은 완전한 버퍼로 남김

## 4주차 — 통합 및 마무리

- [ ] T15. `[P0]` 전체 시나리오 End-to-End 점검 (plan.md 6단계 시나리오 기준) — 3주차로 당기지 않기로 함(planning-agent 권장): "이후 변경이 없다"는 전제에서 의미 있는 마무리 검증이라, 미리 하면 4주차에 코드가 조금이라도 바뀔 때마다 재검증이 중복됨. 최종 코드 기준으로 1회만 수행
- [ ] T16. `[P0]` 데모/발표 준비 (README 갱신, 실행 방법 정리) — 마찬가지로 당기지 않음: 데모 임박 시점에 정리해야 이후 변경으로 다시 stale해지지 않음
- 4주차 스코프에 검증/데모 준비 외에 여유 시간 확장 작업(예: 실제 채용 공고 API 연동, 배포 등)을 넣을지는 아직 미정 — 3주차 마무리 시점에 논의해서 결정

---
작성일: 2026-07-10 · 수정일: 2026-07-21 — **3주차 화요일, 계획보다 사흘 앞서 진행 중.** 2주차(화~목=T2-a~T8, 금요일 공휴일)는 GitHub 이슈 #1~#8 전부 닫힘. 3주차는 planning-agent로 요일별 계획을 세우고 이슈 #9~#15 등록, Project 보드 "3주차 - 자소서 초안 생성 슬라이스"(#2)에 연결. 월요일에 vitest 도입(#10)+README 아키텍처 다이어그램(#9)+T9(#11)+T10(#12), 화요일에 T11(#13)+T12(#14)까지 끝나서, 원래 4주차였던 T13/T14도 planning-agent 재검토 후 이번 주로 당겨옴(이슈 #16, #17): 수=T13(#16), 목=T14(#17)+Skill(#15)+가벼운 회귀 스모크, 금=완전한 버퍼. 정식 T15(E2E)·T16(데모 준비)은 "이후 변경 없음" 전제가 필요한 마무리 작업이라 그대로 4주차에 남김.