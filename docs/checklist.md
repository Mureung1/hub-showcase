# 스펙핏(SpecFit) 작업 분해 체크리스트 (통합, 전부 완료)

> 원래 `checklist_2.md`(0~4단계 초기 작업 분해) / `checklist_3_login_bookmark.md`(로그인·북마크 확장 상세) / `checklist_4_week3_4_issues.md`(3~4주차 이슈 #15~28) 세 문서로 나뉘어 있던 걸 하나로 합쳤다. **#1~28 전 항목 완료** — 지금은 새 작업을 추가하는 문서가 아니라 어떤 결정을 왜 내렸는지 찾아볼 때 쓰는 기록용 문서다. 세 문서 각각의 "확정 사항"은 여전히 유효한 프로젝트 결정이니 재검토 없이 뒤집지 말 것.

---

# Part 1 — 초기 작업 분해 (구 `checklist_2.md`)

>
> `demo_10.html`은 디자인/화면 흐름/갭 분석 로직 검증용 정적 목업이다. 실제 구현 스택(React+Express+SQLite)이 아니지만, 이미 검증된 UX 패턴(포커스 유지, 이어하기 배너, 인사이트 팝업 분기, 탭/정렬, 상세 모달 문장 등)은 그대로 포팅한다. (주: 이후 `demo_11`→`demo_12`→`demo_13.html`로 프로토타입이 교체됐다 — `plan.md`/`CLAUDE.md` 참고. "이어하기 배너"도 이후 히어로 버튼으로 대체됨.)
...
- **로그인/회원가입/북마크는 MVP 범위가 아니었으나 확장 기능으로 추가 결정 (2026-07-16)**. 기존 게스트(비로그인) 플로우는 그대로 유지하고, 로그인은 북마크 저장에만 필요한 선택 기능. 아래 0~4단계 완료 후 여유 시 착수하는 별도 트랙 — 상세 작업 분해는 `checklist_3_login_bookmark.md` 참고. 1단계의 "user_specs 임시 저장 방식 결정 (로그인 없는 MVP)" 항목은 이 결정을 뒤집지 않음 — 그 시점 스코프에서는 여전히 맞는 선택이었고, 로그인은 그 위에 얹는 확장.
...
# Part 2 — 로그인·북마크 확장 상세 (구 `checklist_3_login_bookmark.md`)
...
# Part 3 — 3~4주차 이슈 목록 #15-28 (구 `checklist_4_week3_4_issues.md`)
...
**완료 기준**
- [x] 결과 화면 등 주요 화면이 모바일 폭에서 레이아웃 깨짐 없음 (데스크톱 ≥860px 2컬럼 / 모바일 세로 스택은 이미 구현돼 있음 — 실측 점검만 남음) — Playwright로 320~1280px 8단계 × 주요 화면 6개(랜딩/필터/스펙/로그인/회원가입/가이드) 전수 점검, 실제 가로 스크롤 버그 1건 발견+수정(아래 참고)
- [x] 모달(공고 상세/인사이트) ESC 키로 닫힘 — `useModalA11y` 훅으로 3개 모달(공고 상세/인사이트/참고링크) 전부 적용
- [x] 모달 오픈 시 포커스 트랩 적용 — 같은 훅에서 Tab 트랩 + 열릴 때 포커스 이동 + 닫히면 이전 포커스로 복원까지 처리

**점검 중 실제로 발견한 버그(완료 기준엔 없었지만 함께 수정)**: 푸터 소개 문구(`.footer-desc`)에 `white-space: nowrap`이 걸려있어서, 680px 이하 화면에서 그 문장 원래 폭(700px+)만큼 전체 페이지가 가로 스크롤되는 문제가 있었음(모든 페이지에 공통 노출된 푸터라 전 화면 영향). `@media (max-width: 680px)`에서 `white-space: normal`로 되돌려 해결.
...
- [x] 경력 미충족 시 → 링커리어(linkareer.com) 대외활동·공모전·인턴 모음 페이지로 고정 링크 (최초엔 사람인이었으나, 이용자 대부분이 재학생이라 채용공고 사이트보다 대외활동/공모전 모음 사이트가 경력을 실제로 채우는 데 더 현실적이라는 판단으로 변경)
...
**완료 기준**
- [x] 이메일 정규식 검증 추가 (`로컬파트@도메인.최상위도메인` 형태, 예: `/^[^\s@]+@[^\s@]+\.[^\s@]+$/`) — `LoginPage`/`SignupPage` 제출 시 클라이언트 단에서 우선 확인 (`src/lib/validateEmail.js`)
- [x] 형식이 맞지 않으면 Supabase Auth 호출 자체를 하지 않고, 기존 에러 메시지 영역에 "올바른 이메일 형식이 아닙니다" 안내
- [x] `student@university.ac.kr`처럼 흔한 정상 형식은 회귀 없이 그대로 통과 (정규식이 너무 빡빡해서 정상 이메일까지 막지 않는지 케이스별 확인) — `src/lib/validateEmail.test.js` 10케이스로 검증
- [x] Supabase Auth 자체 검증에만 기대지 않고 클라이언트에서 즉시 피드백 (네트워크 왕복 없이 바로 안내)
- [x] 기존 로그인/회원가입 정상 케이스, 게스트 플로우 회귀 없음 확인 — 게스트 플로우(e2e 6/6) + 로그인/회원가입 사용자 브라우저 확인 완료 (2026-07-28)
...
**완료 기준**
- [x] `LoginPage`에 "비밀번호를 잊으셨나요?" 진입점 추가
- [x] 이메일 입력 → `supabase.auth.resetPasswordForEmail(email, { redirectTo: ... })` 호출 → 발송 완료 안내. 계정 존재 여부와 무관하게 동일한 안내 문구 표시 (가입 여부가 외부에 노출되지 않도록)
- [x] 재설정 이메일의 링크로 들어오면 Supabase가 세션을 심어주는 리다이렉트 경로(`/reset-password`) 신설, 새 비밀번호 입력 폼 제공 — 세션 없이 접근했을 때의 가드는 e2e로, 실제 이메일 링크 진입은 사용자 브라우저로 확인 완료 (2026-07-28)
- [x] `supabase.auth.updateUser({ password })`로 실제 비밀번호 변경 처리 — 실제 링크 클릭 후 동작까지 사용자 확인 완료
- [x] 변경 완료 후 자동 로그인 상태로 전환 (사용자 결정) — `updatePassword` 성공 시 `/`로 이동, 재설정 링크 클릭 시점에 이미 심어진 세션을 그대로 사용
- [x] 기존 로그인 플로우·게스트 플로우 회귀 없음 확인 — 게스트 플로우(e2e 10/10) + 실제 이메일 링크→세션→비밀번호 변경 전체 플로우 사용자 브라우저 확인 완료 (2026-07-28)
- [x] Playwright e2e에 비밀번호 재설정 플로우 테스트 추가 (`e2e/password-reset.spec.js`) — 이메일 발송 API(`auth/v1/recover`)는 스텁 처리(무료 프로젝트의 낮은 발송 한도를 자동 테스트가 갉아먹지 않도록). 실제 이메일 클릭→세션→비밀번호 변경까지는 받은편지함 접근이 필요해 자동화 범위 밖 — `#16`이 로그인/북마크 플로우를 자체 스코프에서 제외한 것과 같은 이유
