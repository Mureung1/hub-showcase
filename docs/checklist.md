...
- [ ] T10. `[P0]` LLM 연동 자소서 초안 생성 API: 사용자 경험 + 문항 반영 프롬프트 설계 (T1, T9 선행) (이슈 #12) — 화요일로 하루 당김(T9가 월요일에 끝나서). profile은 프론트가 매 요청 재전송하는 방식으로 결정(Supabase 재조회 방식 대신 단순함 우선)
- [ ] T11. `[P0]` 자기소개서 초안 화면 UI: 문항별 분석 결과, 생성 초안, 편집 영역 (T8, T10 선행) — mock 버전(`src/screens/DraftEditor.jsx`)은 지난주 수요일에 완성. 실제 T10 LLM 응답으로 교체만 남음 (이슈 #13) — 수요일로 하루 당김
- [ ] T12. `[P1]` 초안 수정/저장 기능 — 저장 범위는 세션 내 로컬 확정 표시로 한정(Supabase 영속화는 스코프 밖, 4주차 이후로 미룸) (이슈 #14) — 목요일로 하루 당김
- [ ] 테스트코드 생성 Skill. `[P1]` `.claude/skills/test-writer/SKILL.md` 신설, design-review와 동일 포맷, 월요일 T9 TDD에서 실제로 쓴 테스트를 예시로 인용 (이슈 #15) — 금요일 예정(T12 완료 후 버퍼 겸)
...
---
작성일: 2026-07-10 · 수정일: 2026-07-20 — **3주차 월요일, 계획보다 하루 앞서 진행 중.** 2주차(화~목=T2-a~T8, 금요일 공휴일)는 GitHub 이슈 #1~#8 전부 닫힘. 3주차는 planning-agent로 요일별 계획을 세우고 이슈 #9~#15 등록, Project 보드 "3주차 - 자소서 초안 생성 슬라이스"(#2)에 연결. 월요일에 지난주 수직슬라이스 스모크 재확인 + vitest 도입(#10) + README 아키텍처 다이어그램(#9) + T9 자소서 문항 분석 TDD(#11)까지 전부 끝나서, 원래 화~금이었던 T10~T12/Skill 일정이 하루씩 당겨짐: 화=T10(#12), 수=T11(#13), 목=T12(#14), 금=Skill(#15)+전체 재검증(버퍼).