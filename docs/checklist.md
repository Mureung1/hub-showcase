...
- [ ] [P1] Gemini/룰베이스 마이크로태스크 품질(약한 행동) 문제 — Lv2 Gemini 생성 결과와 `src/lib/microtaskTemplates.js` 룰베이스 템플릿 일부가 "교재를 펼친다", "첫 문제의 조건만 읽기"처럼 결과물이 남지 않는 준비 행동 수준에 그친다. 유형별 "결과물이 남는 행동" 기준(ActionFrame)을 정의해 템플릿을 재작성하고, Gemini 프롬프트에도 반영 예정 — 룰베이스 부분은 이번 주(3주차 말) 안에 진행, Gemini 프롬프트/재생성 파이프라인은 다음 단계로 분리 (2026-07-24 발견, 이슈 없음)
