...
- [ ] T2-b. `[P0]` Supabase 프로젝트·`profiles` 테이블 설정 (사용자 입력 저장용, T1 선행) — 수요일
- [x] T3. `[P0]` 목업 공고 데이터셋 설계·작성 (채용/인턴십/공모전/대외활동 4개 카테고리, 총 10건: 채용 4·인턴십 2·공모전 2·대외활동 2, 자소서 문항 포함) — `backend/data/postings.json` 파일로 관리, Supabase와 무관 (선행 작업 없음, T2-b와 독립적으로 진행 가능) — 수요일
...
---
작성일: 2026-07-10 · 수정일: 2026-07-14 (화/수 순서를 공식 데일리 미션 기준으로 재배치: 화=T2-a+T4, 수=T2-b+T3 · 공고 데이터는 Supabase가 아닌 postings.json으로 유지, Supabase는 사용자 프로필 저장 전용으로 스코프 명확화)