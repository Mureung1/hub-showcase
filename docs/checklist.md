...
- [x] 네이버 쇼핑 검색 API 신청 및 테스트 호출 — 구조화된 가격 데이터(숫자) 제공 여부 확인
- [x] 공공데이터포털 레시피 API 신청 및 테스트 호출
- [x] Claude API 키 발급, Tool Use 최소 예제(하드코딩 Context) 동작 확인 — 배달비↑+예산↑ Context로 recipe_tool 정확히 선택 확인
