...
## C25 [T25] 같이 있어요 (장식용 동시접속 표시)
- [x] One-Focus View에 "N명 함께 집중 중" 문구와 작은 점 몇 개(그 중 하나는 "나"로 구분)가 보인다 — 캐릭터가 화면 하단을 크게 차지해 중앙 배치 대신 상단 좌측 알약으로 압축(스크린샷 확인)
- [x] 이름·채팅이 없다는 문구가 함께 보여 실제 동시접속처럼 오해되지 않는다 — hover 시 "이름도, 채팅도 없어요..." 툴팁으로 표시
- [x] "혼자 할래요"를 누르면 표시가 사라지고, 새로고침해도 계속 안 보인다 — × 버튼 클릭 후 사라짐, 새로고침 후에도 안 보이는 것 스크린샷으로 확인

## C26 [T26] 통계 화면 (이번 주 완료 현황)
- [x] 이번 주(최근 7일) 완료한 날 수(0~7)가 큰 숫자로 보인다 — 스크린샷으로 확인
- [x] 요일별 완료 여부가 점 7개로 보인다(완료한 날만 채워짐) — 임시 완료 기록 3개(오늘·2일 전·5일 전)를 넣어 정확한 위치에 점이 채워지는 것 확인
- [x] 실제 Notion 완료 기록(Done=true, CompletedAt) 기준으로 계산된다(mock 아님) — `/api/stats`가 실제 Steps DB를 조회, 실데이터(0/7) 및 테스트 데이터(3/7) 둘 다 정확히 반영 확인
- [x] 입력 화면에서 통계 화면으로 이동할 수 있고, 홈으로 돌아올 수 있다 — "이번 주 통계 보기" 클릭 → 통계 화면, 홈 버튼 클릭 → 입력 화면 복귀 확인
