...
- [x] 로그인/인증 처리 방향 결정 — 이메일/비밀번호+세션 방식으로 확정, #57~64에서 실제 구현 완료 (`SEED_USER_ID` 하드코딩도 #62에서 전부 교체됨)
- [x] `AICoachScreen.tsx` mock 데이터 → 실제 API 연동 — #47에서 완료 (`SurvivalModeScreen.tsx`는 #30/#31에서 완료됨)
...
- [x] 백엔드 Render 배포 설정 (Dockerfile, PostgreSQL 애드온, 환경변수 이관) (#70)
- [x] 프론트가 배포된 백엔드 API를 바라보도록 설정 변경 (#71)
