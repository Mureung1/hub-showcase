...
- [ ] 구독(고정비) 등록·수정·삭제 화면
  ...
  - [ ] 프론트 API 클라이언트 함수 작성 (#19)
  - [ ] MyPageScreen 구독 목록 mock→GET 연동 (#20)
  - [ ] SubscriptionManageModal 저장 → 실제 등록/수정 API 연결 (#21)
  - [ ] 구독 삭제 버튼 → DELETE API 연결 (#22)
  - [ ] 새로고침 후 데이터 유지 검증 (#23)
...
- [ ] 장기 신호 계산 (카테고리 구성/추세, 구독 상태, 최대 90일) (#33)
- [ ] 장기 신호 데이터 품질 단계 분기 구현 (5일 미만 / 5~13일 / 14~89일 / 90일 이상) (#34)
- [ ] 단기+장기 신호를 하나의 Context 객체로 통합 (`ContextService` + `GET /api/context`) (#35)

### AI Agent (2주차 멘토링 후 스코프 변경 — F11/외부 F15 제외, 내부 데이터 조회 Tool + 챗봇 구조로)
- [ ] 시스템 프롬프트 작성 (Goal + Context 설명 + Tool 목록 + 출력 형식 규칙) (#36)
...
- [ ] Reasoning Summary(선택 이유) 생성 확인 — 실제 Context 값을 인용하는지 검증 (#43)
- [ ] Tool 결과를 Agent가 사용자 친화적 메시지로 정리하는 2차 호출 구현 (#44)
- [ ] 동일 Context 5~10회 반복 실행 → 판단 일관성 확인 (temperature 낮게 설정) (#45)
- [ ] "일부 신호는 같지만 전체 Context가 다른" 케이스로 실제 판단이 달라지는지 검증 (#46)
