...
- [ ] 이름 시작 매칭 > 이름 포함 매칭 > 설명 매칭, 3단계 우선순위 정렬 구현
- [ ] 동일 우선순위 그룹 내 이름 알파벳순 정렬
- [ ] git 카테고리 하위 명령어(`git commit` 등) 접두사 분리 처리 — `command.category` 필드 기반으로 판정
- [ ] 26개 알파벳 x 2개 카테고리 전수조사로 회귀 없음 확인, lint 통과
