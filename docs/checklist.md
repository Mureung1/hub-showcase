...
## 워크플로우 정리 (목)
- [x] `docs/workflow.md`를 레시피 형태(입력→순서→확인 기준→결과물)로 재구성
- [x] 문제 생겼을 때 재확인할 단계(0번 항목) 추가
- [x] Agent·Skill 협업 구조도(mermaid) 작성 — 기획→설계→구현→검증→배포 단계별
- [x] 사람이 결정한 것 / AI가 수행한 것 구분 표 작성
- [x] 실제 작업과 안 맞는 항목 없는지 검토 (4주 요약과 대조)

## 최종 기능 보완과 배포 확인 (목)
- [x] 배포에 새로 고칠 문제 없음 확인 (`/api/health`, FE 둘 다 정상)
- [x] FE·BE 주소, 환경변수 재확인

## 데모 준비 (목~금)
- [x] 서비스 소개·시연 순서 정리 → [docs/video-script.md](video-script.md)
- [ ] 실제 배포 주소로 시연 리허설 (본인 진행)
- [ ] 데모 영상 녹화 + 업로드 + `showcase.json`에 `demoVideoUrl` 추가
