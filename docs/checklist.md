...
- [x] 쪼갠 마이크로스텝 전체 목록과 각 항목의 카테고리 태그가 화면에 보인다 — `MicrostepReview` 실제 렌더 스크린샷으로 확인
- [x] 전체 예상 시간 합계가 표시된다 — 삭제 시 합계가 즉시 재계산되는 것까지 확인
- [x] 항목을 삭제할 수 있고, 삭제 후 남은 항목만 Notion에 저장된다 — 12개 중 2개만 남기고 저장 → Notion에 정확히 2개만 추가된 것을 API로 확인
- [x] "전부 다시 쪼개기" 버튼을 누르면 Brain Dump API를 다시 호출해 새로 쪼갠 결과로 교체된다 — 화면에서 클릭해 목록·합계가 다른 결과로 바뀌는 것 확인
- [x] CompleteScreen이 전체 완료 개수를 보여준다 — localStorage로 complete 상태 재현해 "N개 해냈다" 렌더 확인

## C18 [T18] Zero-Input 온보딩 화면
- [x] 노션 템플릿이 웹에 공유되고 복제가 허용되어 있다 — `https://twilight-editor-6bf.notion.site/Kok-3acbe7e7125181588f68d69ebb8aa602` 게시 확인(200 OK), "템플릿으로 복제" 토글 켬
- [x] 앱 안에서 템플릿 복제 → 토큰/DB ID 붙여넣기 순서를 안내하는 화면을 볼 수 있다 — `OnboardingGuide`, `/api/notion-health`로 notionReady===false일 때 노출 확인(로컬에서 토큰 비운 상태로 스크린샷 검증)

## C19 [T19] 오늘 마감까지 남은 시간 표시 + 연장 버튼
- [x] 화면에 "오늘 마감까지 남은 시간"과 현재 시각이 함께 보인다
- [x] 연장 버튼을 누르면 마감 시각이 뒤로 밀리고, 그 값이 이후 remainingTimeMinutes 계산에 반영된다
...
## C21 [T21] Notion Public Integration + OAuth 연결
- [ ] "Notion으로 연결하기" 버튼을 누르면 Notion OAuth 인증 화면으로 이동한다
- [ ] 인증 완료 후 콜백에서 발급받은 토큰이 로그인한 사용자 것으로 세션/쿠키에 저장된다
- [ ] 로그인 상태에 따라 각 API(brain-dump, steps/*, struggle, timer-extend, agent-log)가 전역 `.env` 토큰이 아니라 로그인한 사용자의 토큰을 사용한다
- [ ] 로그인 전에는 기존 `OnboardingGuide`(T18)가 그대로 보이고, 로그인 후에는 더 이상 보이지 않는다

## C23 [T23] 다시 시작해도 괜찮아 (복귀 환영 메시지)
- [x] 마지막 방문일과 오늘 날짜 차이가 일정 기간(2일) 이상이면 입력 화면에 환영 문구가 보인다 — localStorage에 3일 전 방문 기록을 넣고 확인
- [x] 문구에 실제 며칠 만인지(예: "3일 만이네")가 반영된다 — 스크린샷으로 확인
- [x] 오늘 처음 접속이 아니고 방문 간격이 짧으면(당일 재방문 등) 문구가 보이지 않는다 — 문구 노출 후 같은 날 재접속 시 기본 문구로 돌아오는 것 확인

## C24 [T24] 감각 강도 조절 + 테마별 배경음
- [x] One-Focus View에 "차분하게 ↔ 생기있게" 다이얼이 보이고 조절할 수 있다 — 스크린샷으로 패널·다이얼 드래그 확인
- [x] 다이얼을 조절하면 One-Focus View·타이머 화면 양쪽에서 캐릭터·테마 장식 요소의 채도가 바뀐다 — intensity 10/95에서 캐릭터 채도 변화 확인
- [x] 다이얼을 차분한 쪽으로 낮추면 타이머 전환 애니메이션이 즉시 반영된다 — intensity 15에서 water-fill div의 `transition` 값이 `none`인 것 확인
- [x] 스피커 아이콘으로 소리를 켜고 끌 수 있고, 켜면 테마별 배경음이 반복 재생된다 — 토글 클릭 후 `<audio>` paused=false, 올바른 테마 파일 재생 확인
- [x] 소리 볼륨은 다이얼 값을 따라간다 — intensity 95/15에서 `audio.volume`이 0.95/0.15로 일치 확인
- [x] 다이얼 값·소리 on/off가 새로고침해도 유지된다 — localStorage 값 자체는 유지 확인. 단 새로고침 직후 오디오 자동재생은 브라우저 정책상 사용자 동작 없이는 막힘(코드 문제 아님, 스피커 아이콘 다시 누르면 재생됨) — component-tree.md에 알려진 제한으로 기록

## C22 [T22] 화이트노이즈 테마 (낮/밤 동기화·숲·카페)
- [x] One-Focus View·타이머 화면에서 낮/밤 동기화·숲·카페 세 가지 테마 중 하나를 토글로 선택할 수 있다 — 실제 화면에서 토글 클릭 후 스크린샷으로 확인
- [x] 테마를 바꿔도 캐릭터 위치·크기는 그대로 유지되고 배경색·주변 장식 요소만 바뀐다 — 숲 테마에서만 캐릭터 색이 forest로 바뀌고 위치·크기는 불변 확인
- [x] "낮/밤 동기화" 테마는 실제 현재 시각(주간/야간)에 따라 자동으로 낮/밤 톤이 바뀐다 — Date를 14시/22시로 모킹해 낮·밤 배경·텍스트 색 전환 확인(밤 배경에서 기본 텍스트色 대비가 나빠 themeTextColor로 별도 수정)
- [x] 선택한 테마가 새로고침해도 유지된다 — 숲 테마 선택 후 새로고침해도 유지되는 것 확인
