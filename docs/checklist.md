...
### QA & 데모 준비
- [x] 전체 플로우 버그 수정 (업로드 → OCR → 분류 → 예측 → Agent → Tool → 화면 출력) (#65) — QA데모 계정으로 실제 영수증 이미지 업로드→OCR(SUCCESS)→MART 분류→확정→Agent 판단(개입 안 함)→홈/통계 화면 반영까지 curl+브라우저로 end-to-end 검증 완료, 버그 없음
- [x] 응답 속도 확인 (결정론적 분석 5초, Agent 경로 10~15초 목표) (#66) — OCR 업로드 5.3초(경계 근접), 예산 등 순수 DB 조회 0.005초, Agent 판단(Tool 미사용 케이스) 3.7초로 목표 내. Tool 호출이 실제로 발생하는 케이스(2차 호출 포함)는 비용 문제로 별도 측정하지 않음
- [x] 데모 계정용 3개월치 시드 데이터 준비 (장기 Context 시연용) (#67) — `demo@spendmate.app`(닉네임 "데모") 계정 신설, 가입일 95일 전으로 백데이트 + 90일치 지출 118건 SQL로 직접 시드(OCR/Agent 비용 없이). 배달비 최근 3주 급증 트렌드(+356%), 예산 90만원 중 52% 사용(여유 있음), 구독 2건, 전체 예산/카테고리 구성/트렌드/AI 인사이트까지 홈·통계 화면에서 실제 반영 확인. 로그인: demo@spendmate.app / demo1234
- [x] 데모 시나리오 리허설 (#68) — demo@spendmate.app 계정으로 실제 Claude API 호출까지 포함해 4개 케이스 전부 실측 검증. 리허설 후 계정은 Case 2 시작 상태(배달비 +66%, 예산 사용률 35.5%)로 복원해둠 — 내일 라이브 데모 시 이 상태에서 시작하면 됨
  - [x] Case 1: 정상 소비 → 개입 안 함 — qademo 계정에 평범한 마트 지출 추가 시 agentMessage null 확인
  - [x] Case 2: 배달비 증가 + 예산 여유 → 개입 안 함 — 배달비 증가율 66%/예산사용률 35.5% 상태에서 배달 지출 추가 → agentMessage null 확인. (주의: 첫 시도 때 시드 데이터의 배달비 증가율이 356~409%로 너무 과했더니 예산 여유와 무관하게 모델이 개입해버림 — 시드 데이터를 완만한 증가율로 재보정한 뒤에야 의도대로 "개입 안 함"이 나옴)
  - [x] Case 3: 배달비 증가 + 예산 부족 → 지출 데이터 조회 Tool 호출 후 먼저 말을 건다 — 큰 배달 지출 추가로 예산사용률 93%까지 올린 뒤 재확정, Tool 조회 결과(예산 90만원, 누적 83.6만원 92.98%, 배달비 72%)를 실제 숫자로 인용하며 개입하는 것 확인
  - [x] Case 4: 챗봇에 "이번 달 얼마 썼어?" 질문 → Tool 호출 후 실제 숫자로 답변 — /api/agent/chat 호출 시 get_expense_summary Tool 결과(83.6만원, 카테고리별 비중)를 그대로 자연어로 재구성해 답변하는 것 확인
  - [x] (테스트 중 발견) 백엔드 예산 계산은 spentAt이 서버의 실제 현재 시각보다 미래면 "이번 달 누적 지출"에서 제외됨 — 데모/시드 데이터 만들 때 spentAt을 실제 서버 시각 이전으로 맞춰야 함(버그 아님, 의도된 동작)
