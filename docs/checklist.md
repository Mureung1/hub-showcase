...
- [x] 날짜 기반 연속 완료로 전환 — `AppState.streak` 증감 대신 Asia/Seoul 날짜별 done 이벤트로 연속 완료일을 계산하고, 같은 날 여러 완료는 하루로 인정하며 stopped는 연속 완료에 영향을 주지 않도록 변경 (2026-07-27)
...
- [x] [P1] Lv2 마이크로태스크 "유형 오염" — 2026-07-27 실제 Gemini 샘플링(9유형×4이유=36조합) 중 발견. `리포트/글쓰기`에 "리포트 첫 슬라이드에 발표 제목 한 줄 입력하기", `조별과제`에 "첫 슬라이드에 발표 제목 한 줄 입력하기"처럼 해당 유형과 맞지 않는 매체(리포트인데 슬라이드)가 섞여 나오던 문제. 원인은 `buildPrompt()`의 정적 피하기/권장 예시 3쌍이 발표·문서 케이스에 쏠려 있던 것 — 9개 유형 중 3개만 대표되고 6개는 예시가 아예 없었음. 해결: `server/src/lib/geminiMicrotask.ts`에 `LV2_TYPE_REASON_EXAMPLES` 상수를 추가해 우선순위 8개 유형(문제확인된 리포트/글쓰기·발표/PT 준비 + 예시가 없던 6종)에 대해 유형×이유별 예시를 `microtaskTemplates.js`의 검증된 문구로 동적 주입, 유형과 무관한 형식 예시("표를 채우기")는 별도로 분리해 유지. `PROMPT_VERSION`을 `lv2-v2`→`lv2-v3`로 올려 캐시 무효화. 같은 36조합 재샘플링 결과 유형 오염 0건(어제 2건→0건), fallback 여전히 0%. `geminiMicrotask.test.ts`에 유형×이유별 동적 예시 검증 테스트 추가, 서버 52/52·typecheck 통과 확인
- [x] [P1] Lv2 마이크로태스크의 회피 이유별 차별화 부족 — 위와 같은 2026-07-27 샘플링에서 발견. `발표/PT 준비` 유형은 overwhelm/dislike/temptation/custom 4개 이유의 결과가 사실상 같은 문장("첫 슬라이드에 발표 제목 한 줄 입력하기")으로 나온다. plan.md 핵심 기능 ②("이유에 따라 개입 강도와 메시지 내용이 달라진다") 관점에서 남은 과제였음. **2026-07-27 최종 처리 — 구조적 커버리지는 100% 채움, 일부 유형의 근접 중복은 구조적 한계로 남겨둠(추가 투자 대비 효과 불확실)**: `문제풀이/암기` 3개 combo 추가(microtaskTemplates.js 값 그대로 재사용) + 유형별 custom 예시 9개 신규 작성(품질 정규식 전수 통과 확인) + `pickLv2DynamicExample` 로직 확장으로 9개 유형 전부에 동적 예시 커버리지 완성. `PROMPT_VERSION`을 `lv2-v3`→`lv2-v4`로 올려 캐시 무효화. 재샘플링(36조합) 결과 리포트/글쓰기·코딩 실습·시험공부·프로젝트·조별과제·개인공부·기타 7개 유형은 4개 이유가 뚜렷이 구별됨. 다만 `발표/PT 준비`는 매체 표현만 다양해졌을 뿐 여전히 "슬라이드 제목 입력"으로 수렴했고, `문제풀이/암기`도 overwhelm/temptation이 이번 샘플링에선 근접했음 — 이건 예시 부족이 아니라 (a) 해당 유형 자체의 첫 행동 공간이 좁은 구조적 특성, (b) 예시를 넣어도 LLM이 매번 그 차이를 반영한다는 보장이 없는 샘플링 변동성 때문으로 판단, 더 이상 투자하지 않기로 함. 부수 발견: `BOUNDED_SCOPE_PATTERN`이 한국어 숫자표현("세 개"/"세 줄")을 인식 못 해 fallback 2건 발생(안전망은 정상 작동) — 사소한 정규식 한계라 별도 대응 안 함. `geminiMicrotask.test.ts`에 문제풀이/암기 combo, 유형별 custom 9종, 미매핑 유형 폴백 테스트 추가, 서버 62/62·typecheck 통과 확인
...
- [ ] `tasks.test.ts:267`의 날짜 streak 테스트("60초 중단은 Lv4를 정확히 Lv3으로 완화하고 날짜 streak에 영향을 주지 않는다")가 실측 통합테스트 특성상 가끔 5초 타임아웃으로 실패할 수 있음 (2026-07-27 stash로 무관함 확인, 이번 작업과 무관, DB 왕복 지연 추정, 원인 조사는 범위 밖)
