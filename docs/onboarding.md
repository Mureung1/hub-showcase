...
| 3    | CTA          | 서비스를 직접 시작하는 행동                    |

`문제`, `장점`, `팀 소개`, `문의`를 독립 섹션으로 만들지 않는다. 핵심 기능 탭에서 이미 드러나는 내용을 카드 목록과 설명 문단으로 반복하지 않는다.
...
`🔖 링크`는 Electric Blue, `✦ 다시`는 Amber 책갈피 하이라이트로 표시한다. 두 기호는 하이라이트 안에서 텍스트 앞에만 둔다.
...
마지막 장면은 `아맞다로 시작해보세요` 제목과 `서비스 경험하기` CTA만 사용한다. 인증 정책은 다음 로그인 화면에서 안내한다.
...
기능 탭은 한 줄로 배치한다. 활성 탭은 Electric Blue 글자와 하단선으로 표시하고 `aria-selected`도 함께 제공한다. 제품 UI 안에서만 Blue, Amber, Coral을 제한적으로 사용한다.
