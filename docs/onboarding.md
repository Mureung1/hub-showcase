...
```text
인사이트를 저장한다 → 카테고리로 분류한다 → 필요한 순간 다시 꺼내 본다
```
...
| 순서 | 장면         | 전달할 내용                                         |
| ---- | ------------ | --------------------------------------------------- |
| 1    | Hero         | 저장한 인사이트를 필요한 순간 다시 꺼내 보는 서비스 |
...
`🔖 인사이트`는 Electric Blue, `🪄 다시`는 Amber 책갈피 하이라이트로 표시한다. 두 이모지는 하이라이트 안에서 텍스트 앞에만 둔다.

데스크톱에서는 첫 진입 때 일반 문구, `인사이트`, `다시`, CTA가 약 1초 동안 한 번 순서대로 나타난다. `인사이트`는 책갈피를 꽂는 방향으로 드러나고 `다시`는 아래에서 제자리로 돌아오며 반짝임을 한 번 보여준다. 반복하거나 스크롤에 따라 재생하지 않는다.
...
제목 아래에는 `01 저장`, `02 분류`, `03 꺼내보기` 탭과 하나의 공용 제품 무대를 둔다. 탭을 바꾸면 같은 위치에서 제품 장면만 전환한다. 실제 제품 컴포넌트를 축소하지 않고 현재 제품의 정보 구조를 반영한 랜딩 전용 미니 화면을 사용한다. 별도의 마케팅 설명 문단을 붙이지 않는다.

#### 저장

- URL 입력
- 저장 행동
- 저장 뒤 제목, 메모와 카테고리를 선택해서 더하는 흐름
...
검색 알고리즘, AI 추천, 랭킹 기준과 결과가 연결된 이유를 설명하지 않는다. 실제 제품 장면으로 작동 방식을 전달한다.
...
Hero, 핵심 기능, 소통 장면은 각각 최소 `84svh` 높이로 같은 전환 리듬을 만든다. Hero 높이는 상단바를 포함해 계산하며, 콘텐츠가 더 길면 장면 높이가 자연스럽게 늘어난다. 핵심 기능 섹션의 위아래 여백은 모바일 64px, 태블릿 80px, 데스크톱 96px을 유지한다.

## 모션 도입 순서

GSAP은 Hero의 첫 진입 시퀀스 한 곳에만 사용한다.

```text
일반 문구 → 인사이트 → 다시 → 아맞다 시작하기
```

기능 탭은 클릭과 키보드로 바뀌는 정적 미니 화면을 유지한다. 모바일과 `prefers-reduced-motion` 환경에서는 Hero 모션도 생략하고 처음부터 정적 최종 상태를 제공한다.
...
- 상단 `로그인`과 Hero `아맞다 시작하기`는 같은 로그인 화면으로 이어진다.
