...
```text
인사이트를 저장한다 → 카테고리로 정리한다 → 필요한 순간 다시 꺼내 본다
```
...
| 2    | 핵심 기능 탭 | 저장, 카테고리, 꺼내보기의 실제 제품 장면           |
...
제목 아래에는 `01 저장`, `02 카테고리`, `03 꺼내보기` 탭과 하나의 공용 제품 무대를 둔다. 탭을 바꾸면 같은 위치에서 제품 장면만 전환한다. 별도의 마케팅 설명 문단을 붙이지 않는다.
...
보관함 검색은 이 탭에 섞지 않는다. 카테고리 장면은 카테고리로 저장물을 묶는 기능에 집중한다.
