...
이 문서는 아맞다의 온보딩 흐름을 정의한다. 구현 전 기준 문서로 사용하며, 화면 문구나 진입 흐름이 바뀌면 이 문서를 먼저 갱신한다. 로그인 전 온보딩 모션의 세부 설계는 `docs/superpowers/specs/2026-07-09-onboarding-motion-design.md`를 함께 따른다.
...
- CTA는 첫 화면 안에서 즉시 보여야 하며, 기능 설명이나 긴 스크롤 영역 아래로 밀리지 않아야 한다.
...
- 섹션 이동 내비게이션이나 URL hash는 소개 영역이 길어질 때만 검토한다. MVP에서는 첫 화면 중심의 짧은 흐름을 우선한다.
...
- 로그인 전 화면에서 `꺼내보기`의 핵심 경험을 짧은 애니메이션 예고편으로 보여준다.
...
한 화면 안에서 소개 영역, 핵심 경험 미리보기, CTA 영역을 함께 보여준다. 실제 인증 버튼과 약관 안내는 CTA 이후의 로그인 화면에서 제공한다.

```text
┌────────────────────────────────────────────────────────────┐
│ 서비스 소개 영역      │ 핵심 경험 미리보기     │ CTA 영역          │
│                       │                        │                 │
│ 아맞다                │ 저장 카드가            │ [서비스 경험하기] │
│ 저장한 링크를         │ 상황 문장 기준으로     │ 짧은 신뢰 문구    │
│ 필요한 순간 다시...   │ 작업팩처럼 모이는 장면 │                 │
│ 핵심 기능 3~4개        │                        │                 │
└────────────────────────────────────────────────────────────┘
```
...
로그인 전 서비스 온보딩에는 GSAP 기반의 짧은 브랜드 시퀀스를 사용한다. 목적은 실제 `꺼내보기` 기능을 체험하게 하는 것이 아니라, "저장해둔 자료가 나중의 현재 상황과 다시 연결된다"는 핵심 경험을 직관적으로 보여주는 것이다.
...
- GSAP은 이 로그인 전 서비스 온보딩 시퀀스에만 사용한다.
- 앱 내부 화면 전환, 버튼 인터랙션, 카드 등장 같은 micro interaction은 Motion 또는 CSS transition을 우선한다.
- 애니메이션은 4~6초 안에 한 번의 의미가 전달되어야 한다.
- 반복은 가능하지만 사용자의 읽기와 CTA 클릭을 방해하지 않아야 한다.
- `prefers-reduced-motion` 환경에서는 정적 작업팩 미리보기로 대체한다.
...
- 미리보기는 저장 카드가 상황 문장에 의해 작업팩으로 모이는 축약 장면으로 제한한다.
...
- `useGSAP()`으로 timeline cleanup을 처리하고, reduced motion에서는 timeline을 만들지 않는다.
