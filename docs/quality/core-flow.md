...
- [ ] 나의 깸 캘린더는 선택한 월에 기록이 있는 날짜와 기록 개수를 표시한다
- [ ] 날짜를 선택하면 해당 날짜의 기록이 최신순으로 표시되고 같은 글의 여러 기록도 각각 보인다
- [ ] 기록 없는 월과 날짜는 빈 상태를 표시하며 화면이 멈추지 않는다
- [ ] 삭제되거나 깨진 원문의 기록은 유지되고 원문 버튼 상태가 구분된다
