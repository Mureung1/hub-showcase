...
목표: 계획 수립/검증 Agent와 백로그 체계를 만들고 FE/BE/DB 수직 슬라이스를 설계한다.
...
- [ ] `POST /api/quest-logs` 계약 작성
- [ ] `GET /api/quest-logs` 계약 작성
- [ ] React 개선 범위와 컴포넌트 분해안 확정
- [ ] Express + Supabase 연결 준비
...
목표: 퀘스트 기록 저장·조회 수직 슬라이스를 구현한다.

- [ ] Express 서버 골격 구현
- [ ] Supabase `quest_logs` 테이블 연결
- [ ] 완료 기록 저장 API 구현
- [ ] 실패 기록 저장 API 구현
- [ ] 복구 완료 기록 저장 API 구현
- [ ] 기록 조회 API 구현
- [ ] React 기록 노트와 API 연결
- [ ] API 실패 시 재시도/안내 UI 구현
