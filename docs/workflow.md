...
3주 동안 SpendMate를 만들며 Claude Code와 실제로 어떻게 일했는지 정리한 문서.
...
- **다음 주 목표**: `code-reviewer` 에이전트를 매 커밋 전 습관적으로 사용, 4주차는 회원가입/로그인부터 시작해 F16 나머지(#40~47)를 이어감
