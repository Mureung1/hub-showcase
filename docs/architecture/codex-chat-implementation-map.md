...
| First Assignment vertical은 닫혔는가? | 절반만 닫혔다. Prepared Git workspace의 normal AY Chat→Interaction MCP proposal→inline Semantic Review→AY-owned actual-file apply/checkpoint는 검증됐지만, GUI source selection→workspace Skill+file reference ActionInvocation은 현재 없다. |
| 채택된 public workflow는 무엇인가? | 현재 구현은 ADR 0018·0019·0020의 prepared Git workspace, read-only source explorer·preview, normal Chat, transient Interaction request/result와 AY-owned file apply다. ADR 0021은 여기에 명시적 ActionInvocation을 추가하는 target을 채택하며, old app-owned source registry·durable Run·patch·confirmation·apply는 복원하지 않는다. |
...
| `packages/product-contract` | Prepared-workspace lifecycle, bounded source list·text preview, normal Chat, Browser-safe semantic Review·interaction·interrupt request/response와 closed target operation frame을 위한 dependency-free exact type·decoder | Browser-safe `.` 하나. 현재 ActionInvocation contract는 없고 Chat request는 material·Skill input을 거절한다. Course·RawMaterial registry·academic history, PDF byte transport, raw MCP, private credential·binding, HTTP framing, persistence, Server domain과 native Runtime protocol은 포함하지 않음 |
...
| `apps/server` | Prepared launch·registry·startup coordinator, read-only active-root SourceProjection, target Product Router, Interaction Broker, generic text Product Turn, neutral NDJSON writer와 listener·Runtime close ordering | Canonical entrypoint의 `/api/product/*`는 lifecycle·settings·source list/text/PDF preview·normal Chat·semantic Review·general interaction·interrupt를 제공. ActionInvocation route·action→Skill composition은 없다. Private Broker Router는 same listener loopback+credential 경계이며 academic persistence·apply는 없음 |
...
| `npm run test:e2e` | Default Browser route의 prepared lifecycle, 3-pane source list·text/PDF/unsupported preview와 evidence navigation, normal Chat, general clarification·interrupt, inline Semantic Review accept·revise·reject. 현재 source action이 없고 removed app-owned academic route가 mount되지 않은 trace도 함께 검증 |
...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| App-originated ActionInvocation | Source selection은 preview-only이고 Browser contract·Server route·Runtime input은 text Chat만 지원한다. Workspace-local Skill과 선택 file reference를 명시적 GUI action으로 한 Turn에 전달하는 target은 미구현이다. | [AY–App Interaction Layer 아키텍처](ay-app-interaction-layer.md) |
