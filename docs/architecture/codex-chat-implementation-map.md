...
[ADR 0013](../adr/0013-adopt-product-only-public-surface-and-v2-store-compatibility-baseline.md)의 workspace-local v2 store는 cutover 전 durable baseline이었다. Server의 app-owned academic store와 physical I/O source는 제거됐으며 이 contraction은 기존 v2/v3 bytes를 읽거나 변환·rewrite·삭제하지 않았다. `@ay-ple/semester-workspace`에 남은 decoder·legacy kernel의 제거와 bytes 처리 정책은 sibling Workspace ticket이 소유한다.
...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| Legacy SemesterWorkspace kernel contraction | Browser academic surface, Server academic orchestration·persistence와 Runtime private MCP·managed Skill override는 제거됐다. 기존 v2/v3 bytes는 untouched이며 `@ay-ple/semester-workspace`의 consumer 없는 decoder·v3 kernel 물리 제거만 sibling Workspace 범위로 남는다. | [User-owned Git SemesterWorkspace ADR](../adr/0018-adopt-user-owned-git-semester-workspaces.md) |
