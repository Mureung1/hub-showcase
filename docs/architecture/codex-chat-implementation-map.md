...
| 제품의 `ModelingRun`까지 구현됐는가? | Server의 First Assignment action은 exact admission 뒤 native call 전에 requested Skill path·version과 source digest를 포함한 durable `ModelingRun(starting)`을 current v2 product store에 기록하고 acceptance·terminal·validation·unknown outcome을 같은 receipt에 정산한다. Pre-006 v2 shape를 자동 migration하거나 rewrite하지 않는다. Public bootstrap은 terminal Run의 safe receipt만 반환하고 Browser는 transient action 단계와 학생용 activity를 cumulative transcript에 표시한다. Exact local-provider와 isolated live-provider product gate도 complete Assignment action·Review·confirmed outcome과 clean process-group shutdown을 증명했다. Canonical caller cutover와 first durable baseline은 009a가 소유한다. |
| Native Plan interaction은 어디까지 열렸는가? | Exact official SDK와 production wheel, private bridge와 Node `CodexProductCapableRuntime`이 native `thread/start`가 `configured → advertised default → first available`로 정한 effective model·reasoning effort를 보존해 native Plan `collaborationMode`, managed standalone Skill root, typed `request_user_input`, opaque answer/cancel과 curated activity를 제공한다. Server는 일반 clarification을 ephemeral binding으로 answer/cancel한다. Bound Review에서는 수락·거절을 product transaction 뒤 native answer로 보내고, 수정 요청은 bounded feedback과 Server-private fresh proposal key를 같은 Turn에 전달해 replacement를 기다린다. Browser는 일반 clarification과 세 Review 결정을 별도 UI·route로 연결한다. |
| 이 tracer가 제품 runtime으로 충분한가? | Deterministic Chromium→Vite→Express→product store/runtime trace가 explicit workspace, exact Skill+Text, private MCP proposal, 세 갈래 Review, replacement 뒤 수락, durable reject/no-apply, Review 결정 뒤 same-Turn 일반 clarification의 answer/cancel·explicit interrupt, answer 전 continuity loss의 `interrupted`·no-apply·explicit retry, default disconnect drain 뒤 final settled read, pending Review 중 실제 reload 수렴, apply 뒤 `continuation_lost`·no-reapply, source/store recovery와 fresh-run isolation을 증명한다. 별도 exact actual-child/local-provider와 isolated live-provider gate도 active `SemesterWorkspace` cwd, managed Skill·source, hosted MCP, Plan Review round trip, same-Turn terminal과 durable outcome을 증명했다. Tracer-only surface 제거와 canonical product cutover는 009a가 소유한다. |

## Tracked 구성

| 위치 | 책임 | 공개 경계 |
| --- | --- | --- |
| `packages/product-contract` | Coarse operation status를 포함한 Product bootstrap·workspace recovery·settled history·material preview, `refreshed | source_rebaselined` material outcome, explicit retry와 세 갈래 Review를 포함한 current mutation request·response, replacement/resolved/operation recovery activity를 포함한 closed operation frame의 dependency-free exact type·decoder | Browser-safe `.` 하나. HTTP framing, Server domain/store, React state와 native Runtime protocol은 포함하지 않음 |
...
| `apps/server` | Chat configuration·runtime lease와 별도로 주입 가능한 SemesterWorkspace activation, current-only v2 workspace store의 `Course`·`RawMaterial`·Assignment/StatePatch/UserConfirmation/ModelingRun aggregate와 registered-material execution guard, retry ancestry·recovery invariant, codec·invariant·physical I/O·exact opened-byte authority·compare-before-rename을 숨기는 internal persistence module, bytes-preserving incompatible 경계와 explicit source rebaseline/store reactivation, selected-source snapshot·lease, private `propose_state_patch` MCP host, Assignment/explicit retry/Chat product operation coordinator, source drift 시 native interrupt, ephemeral Plan clarification과 exact Review binding·accept/revise/reject·continuation-loss transaction, bounded text scan·digest-bound preview, loopback·Origin guarded HTTP/NDJSON, listener/runtime close ordering과 signal handling | `createServerApplication()`, `SemesterWorkspaceController`, product operation/Review coordinator, `/api/product/*`와 `/api/codex-chat/*` |
| `apps/chat-shell` | SourceSelection·자료 preview·workspace activation, Review exact response 뒤 control을 해제한 best-effort hydration과 response-loss confirmation fallback용 one-shot read, recovery용 status-aware final product bootstrap read와 read-generation freshness를 소유하는 source-centered 3-pane workbench, workspace recovery mutation guard와 explicit rebaseline/reactivation, cumulative Assignment/Chat activity, evidence-linked 세 갈래 Review와 replacement 전환, interrupted·unknown recovery와 explicit retry, apply 뒤 continuation-loss, 일반 clarification answer/cancel, interrupt와 settled-only reload를 유지하는 오른쪽 companion | Mounted product surface는 `@ay-ple/product-contract`를 strict decode하는 fetch/NDJSON adapter와 cross-frame reducer. Chat-only conformance source는 `@ay-ple/codex-chat-runtime/contract` |
...
| `GET /api/product/bootstrap` | Account readiness와 coarse product `operationStatus(active | idle)`, 활성 workspace의 root 비노출 product snapshot, optional `cleanup_required | source_conflict | store_conflict`, `Course`, `RawMaterial` registry, confirmed revision·settled product history를 `no-store`로 반환한다. Status를 snapshot보다 먼저 읽어 `idle` 응답을 앞선 lease 정산 뒤 final history read로 만든다. Readiness 조회 실패는 safe `unavailable`로 격리하고 pending patch·active Run, prompt identity, Skill path·native correlation, persistence version과 내부 진단값은 Browser에 노출하지 않는다. |
| `POST /api/product/workspaces/activate` | macOS chooser로 선택한 root를 정규화·검증한다. 새 store는 첫 bounded scan·write 뒤 authority를 교체하고 기존 current store는 original bytes를 rewrite하거나 material baseline을 자동 갱신하지 않고 연다. Active guard store conflict는 matching product operation release 뒤 같은 canonical root에만 recovery activation을 허용한다. Current bytes가 valid하면 새 authority로 채택해 guard·settled record·scratch를 reconcile하고, invalid·unsupported·non-regular이면 bytes-preserving `incompatible/readOnly`로 전환한다. 다른 invalid candidate나 operation-active switch 실패는 기존 workspace를 유지한다. |
| `POST /api/product/courses` | 활성 workspace에 opaque `Course` 하나를 생성하거나 기존 값을 다시 연다. |
| `POST /api/product/materials/refresh` | 일반 상태에서는 bounded scan 뒤 `refreshed`를 반환한다. `source_conflict`에서는 bounded cleanup과 current TXT scan을 explicit rebaseline으로 수행해 stable material ID와 새 digest를 기록하고 `source_rebaselined`를 반환한다. App-owned subtree·symlink·escape·과대·비텍스트 파일은 제외한다. |
...
| `POST /api/product/actions/first-assignment/retry` | Prior settled retryable Run과 exact Course·Recipe·arguments·source snapshot, one-child ancestry를 검증하고 새 Run·Turn·keys를 한 번만 만든다. |
...
| `POST /api/product/reviews/:interactionId` | Exact active Review binding과 수락·수정 요청·거절을 검증한다. 수락·거절은 product transaction 뒤, 수정 요청은 Server-private fresh proposal key와 bounded feedback으로 native same-Turn answer를 전달한다. |
...
`@ay-ple/codex-chat-runtime`은 official source commit `8c68d4c87dc54d38861f5114e920c3de2efa5876`, exact native runtime `0.144.4`, standalone CPython, patched SDK와 dependency closure를 canonical manifest로 검증한다. Ordered `0001`–`0008` patch stack의 Plan·pending request, native effective thread setting과 standalone Skill root seam을 private bridge와 additive Node product contract가 raw App Server request ID 없이 projection한다. Answer/cancel은 native resolution 뒤 nonterminal same-Turn continuation까지 확인된 경우에만 성공하며, Bridge는 one resolved activity를 그 continuation보다 먼저 내보낸다. 같은 native resolution이 lifecycle cleanup에서 왔고 terminal이 이어지면 synthetic success 없이 `interaction_not_pending`으로 끝난다. Package-private routing과 bound의 상세는 [runtime README](../../packages/codex-chat-runtime/README.md)와 [patch ledger](../../packages/codex-chat-runtime/upstream/PATCHES.md)가 소유한다. `readAccountReadiness`는 native work를 시작하지 않고, structured Turn은 acceptance-first identity와 authoritative terminal을 보존한다. Production factory는 complete verified bundle만 시작하고 system Python, source checkout, ambient environment나 network repair를 사용하지 않는다.

Node는 detached Python worker와 native child를 explicit controlled environment에서 supervise한다. Existing text path는 `deny_all + read_only`를 유지하고, additive product Turn은 bounded `TextInput`, optional exact `SkillInput`, native `thread/start`가 결정한 effective model·reasoning effort의 Plan mode와 `auto_review + workspace_write`를 사용한다. Product caller는 model·reasoning override를 전달하지 않는다. Turn start는 process-wide Skill root replacement와 함께 직렬화해 text Turn에는 empty roots, managed product Turn에는 exact `SKILL.md` directory를 적용한다. Product activity는 requested Skill, Plan, allowlisted `propose_state_patch` MCP lifecycle, opaque user-input, Agent message와 terminal만 projection한다. Operation·stream·queue·stderr bound와 deadline을 적용하며 fatal·disconnect·shutdown은 pending operation, interaction과 stream을 한 번 정산한 뒤 process group disappearance까지 확인한다.
...
| `npm run test:e2e` | 실제 Vite·Express·product store·hosted MCP와 deterministic product-capable runtime을 통과하는 Chat Shell Assignment/Chat, 세 갈래 Review·replacement·settled reload, Review 결정 뒤 same-Turn 일반 clarification answer/cancel·interrupt, answer 전 continuity loss·explicit retry, default disconnect drain·실제 reload의 status-aware recovery, apply 뒤 continuation-loss, source drift→native interrupt→explicit rebaseline→fresh action, active guard store drift→operation release→same-session reactivation·reconciliation→fresh action, invalid store exact-byte read-only open, 연속 fresh harness 격리 desktop behavior 및 static camp browser flow |
...
| `npm run test:first-assignment-product-actual -w @ay-ple/server` | Exact local provider와 real Server product seam을 통한 distinct legacy/active workspace, managed Skill·source, Python/command, hosted MCP → Plan Review answer → same-Turn terminal, durable outcome과 process-group reap |
| `npm run trace:first-assignment-live -w @ay-ple/server -- --codex-home <isolated-auth-seed>` | 명시적 owner-only auth seed와 fresh·disjoint root에서 complete Assignment action·Review·confirmed outcome을 검증하고 prerequisite 부재를 `blocked`로 분리하는 opt-in live gate |
...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| 제품 작업 조합 | Server authority와 Browser product companion이 Assignment action, activity, 일반 clarification, evidence-linked 수락·수정 요청·거절, atomic replacement, answer 전 continuity loss의 recovery UI·explicit retry, apply 뒤 continuation-loss·no-reapply와 workspace source/store recovery까지 shared contract로 연결됐다. Exact local-provider와 isolated live-provider conformance는 complete action·Review·confirmed outcome과 process cleanup까지 green이다. Tracer-only caller·route 제거와 durable baseline 확정은 009a에 남아 있다. | [Codex-native 제품 작업 조합](codex-native-product-composition.md) |
