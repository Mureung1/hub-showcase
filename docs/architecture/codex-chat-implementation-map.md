...
| `packages/product-contract` | Current Product bootstrap·workspace recovery·settled history·material preview, Assignment·retry·Chat·old Review 옆의 Browser-safe semantic Review·interaction·interrupt request/response와 closed operation frame, user-owned Git target lifecycle의 dependency-free exact type·decoder | Browser-safe `.` 하나. Target lifecycle은 `TargetProductBootstrap`으로 current `ProductBootstrap` compatibility alias 옆에 expand되며 아직 current route를 교체하지 않음. Raw MCP, private credential·binding, HTTP framing, persistence, Server domain과 native Runtime protocol은 포함하지 않음 |
...
ADR 0018 target의 root v4 identity codec, external `WorkspaceRegistry` v1 codec·CAS store, exact Browser lifecycle codec과 process-local generic operation lease coordinator는 current graph 옆의 internal target seam으로 구현됐다. Registry는 canonical root와 active pointer만 보존하고 fresh root identity가 matching할 때만 available로 판정하며 malformed·future bytes를 reset하지 않는다. Generic coordinator는 lifecycle eligibility와 `workspace_transition | product_turn` claim을 한 critical section에서 수행하고 current Assignment·Chat admission도 같은 target product-turn lease를 소비한다. Candidate·activation·target Browser lifecycle은 아직 이 seam을 current public route·UI composition에 연결하지 않는다.
...
| User-owned Git workspace | Current chooser directory 하나를 곧바로 Runtime cwd로 쓰며 fixed `project_root_markers=[]`, process-wide managed Skill root, app-owned source registry와 `.ay-ple/workspace-state.json`을 사용한다. Root v4 identity codec, external `WorkspaceRegistry` v1 CAS·fresh reopen·reselect, target Browser lifecycle codec과 transition/product-turn atomic coordinator seam은 internal target으로 구현됐다. Hub-rooted Bootstrap→fresh Workspace Runtime 전환, exact Git project root에 고정된 thread cwd, native project context, `hub/.agents/skills/` Bootstrap→`hub/skills/` built-in source→workspace `.agents/skills/` copy, candidate/activation과 AY-owned commit flow는 아직 연결되지 않았다. | [User-owned Git SemesterWorkspace ADR](../adr/0018-adopt-user-owned-git-semester-workspaces.md) |
