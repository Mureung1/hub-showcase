...
| User-owned Git workspace | Current chooser directory, fixed `project_root_markers=[]`, process-wide managed Skill root, app-owned source registry와 `.ay-ple/workspace-state.json`을 사용한다. Durable `WorkspaceRegistry`, exact Git project root에 고정된 thread cwd, native project context, `hub/.agents/skills/` source→workspace `.agents/skills/` copy, init Skill, root `workspace-state.json`과 AY-owned commit flow가 아직 연결되지 않았다. | [User-owned Git SemesterWorkspace ADR](../adr/0018-adopt-user-owned-git-semester-workspaces.md) |
