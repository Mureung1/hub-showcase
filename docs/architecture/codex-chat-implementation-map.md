...
| `apps/server` | Current-v2 product graph, Account Readiness, Course·material·Assignment·Chat·Review HTTP, old private MCP, neutral NDJSON writer와 listener·Runtime close ordering. Target expand로 Runtime generation credential, active product Turn binding, single held call, atomic evidence resolver와 in-memory UI Adapter를 가진 Interaction Broker가 추가됨 | Side-effect-free `@ay-ple/server` root의 `createServerApplication()`, `listenToServerApplication()`, `bindServerApplicationListener()`와 `/api/product/*`. Target Broker Router는 아직 canonical Runtime·Browser composition에 연결되지 않은 internal seam |
...
Interaction expand 단계에서는 `@ay-ple/interaction-mcp`의 built Adapter·exact codecs와 `apps/server`의 held Interaction Broker·evidence resolver가 current graph 옆에 존재한다. Runtime은 bounded generic child environment와 thread-scoped exact MCP readiness를 제공하며 tracked trusted Git project에서 built Adapter handshake·`propose_state_patch` roster까지 검증한다. Broker와 Runtime seam은 canonical Server application·prepared-workspace startup, inline Review public composition에는 아직 연결되지 않았다. Current `/api/product-mcp`와 academic Review/apply path가 계속 active public behavior다.
