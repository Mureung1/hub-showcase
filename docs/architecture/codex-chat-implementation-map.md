...
| 이 tracer가 제품 runtime으로 충분한가? | Deterministic Chromium→Vite→Express→product store/runtime trace가 explicit workspace, exact Skill+Text, private MCP proposal, 세 갈래 Review, replacement 뒤 수락, durable reject/no-apply, answer 전 continuity loss의 `interrupted`·no-apply·explicit retry와 apply 뒤 `continuation_lost`·no-reapply를 증명한다. Workspace source/store recovery와 actual-child·local/live-provider conformance는 후속 gate가 소유한다. |

## Tracked 구성

| 위치 | 책임 | 공개 경계 |
| --- | --- | --- |
| `packages/product-contract` | Product bootstrap·workspace·settled history·material preview, explicit retry와 세 갈래 Review를 포함한 current mutation request·response, replacement/resolved/recovery activity를 포함한 closed operation frame의 dependency-free exact type·decoder | Browser-safe `.` 하나. HTTP framing, Server domain/store, React state와 native Runtime protocol은 포함하지 않음 |
...
| `npm run test:e2e` | 실제 Vite·Express·product store·hosted MCP와 deterministic product-capable runtime을 통과하는 Chat Shell Assignment/Chat, 세 갈래 Review·replacement·settled reload, answer 전 continuity loss·explicit retry와 apply 뒤 continuation-loss desktop behavior 및 static camp browser flow |
