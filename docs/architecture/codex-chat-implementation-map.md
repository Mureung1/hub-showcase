...
ADR 0018 target에서 root v4 identity codec, external `WorkspaceRegistry` v1 codec·CAS store, candidate/bootstrap Browser lifecycle codec과 process-local operation lease coordinator가 current graph 옆의 internal seam으로 구현됐다. Registry는 canonical root와 active pointer만 보존하고 fresh root identity가 matching할 때만 available로 판정하며 malformed·future bytes를 reset하지 않는다. Generic coordinator의 non-preemptive admission과 terminal-only release는 current Assignment·Chat이 이미 소비하는 survivor다. 반면 `BootstrapCandidate`, `workspace_init` eligibility와 candidate activation union은 current public route·UI consumer가 없고 ADR 0020 이후 adopted target도 아니므로 후속 contract 대상이다.
...
| User-owned Git workspace | Current chooser directory 하나를 곧바로 Runtime cwd로 쓰며 fixed `project_root_markers=[]`, process-wide managed Skill root, app-owned source registry와 `.ay-ple/workspace-state.json`을 사용한다. Root v4 identity codec, external `WorkspaceRegistry` v1 CAS·fresh reopen, candidate/browser union과 generic coordinator seam은 internal expansion으로 구현됐다. Pre-App native Bootstrap, launch-time prepared-root validation, registry reopen, exact Git-root thread cwd와 native project context, `hub/.agents/skills/` init→`hub/skills/` source→workspace `.agents/skills/` copy, required MCP readiness 뒤 registry commit과 AY-owned checkpoint는 아직 연결되지 않았다. Candidate/browser union과 `workspace_init` branch는 연결 대상이 아니라 correction residue다. | [User-owned Git SemesterWorkspace ADR](../adr/0018-adopt-user-owned-git-semester-workspaces.md), [pre-App native Bootstrap ADR](../adr/0020-bootstrap-semester-workspaces-before-app-startup.md) |
