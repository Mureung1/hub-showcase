...
| 이 tracer가 제품 runtime으로 충분한가? | Deterministic Chromium→Vite→Express→product store/runtime trace가 explicit workspace, exact Skill+Text, private MCP proposal, 세 갈래 Review, replacement 뒤 수락, durable reject/no-apply, Review 결정 뒤 same-Turn 일반 clarification의 answer/cancel·explicit interrupt, answer 전 continuity loss의 `interrupted`·no-apply·explicit retry, default disconnect drain 뒤 final settled read, pending Review 중 실제 reload 수렴과 apply 뒤 `continuation_lost`·no-reapply를 증명한다. Workspace source/store recovery와 actual-child·local/live-provider conformance는 후속 gate가 소유한다. |

## Tracked 구성

| 위치 | 책임 | 공개 경계 |
| --- | --- | --- |
| `packages/product-contract` | Coarse operation status를 포함한 Product bootstrap·workspace·settled history·material preview, explicit retry와 세 갈래 Review를 포함한 current mutation request·response, replacement/resolved/recovery activity를 포함한 closed operation frame의 dependency-free exact type·decoder | Browser-safe `.` 하나. HTTP framing, Server domain/store, React state와 native Runtime protocol은 포함하지 않음 |
...
| `apps/server` | Chat configuration·runtime lease와 별도로 주입 가능한 SemesterWorkspace activation, current-only v2 workspace store의 `Course`·`RawMaterial`·Assignment/StatePatch/UserConfirmation/ModelingRun aggregate와 registered-material execution guard, retry ancestry·recovery invariant, codec·invariant·physical I/O·canonical byte comparison을 숨기는 internal persistence module, non-current bytes-preserving incompatible 경계, selected-source snapshot·lease, private `propose_state_patch` MCP host, Assignment/explicit retry/Chat product operation coordinator, ephemeral Plan clarification과 exact Review binding·accept/revise/reject·continuation-loss transaction, bounded text scan·digest-bound preview, loopback·Origin guarded HTTP/NDJSON, listener/runtime close ordering과 signal handling | `createServerApplication()`, `SemesterWorkspaceController`, product operation/Review coordinator, `/api/product/*`와 `/api/codex-chat/*` |
| `apps/chat-shell` | SourceSelection·자료 preview·workspace activation, Review exact response 뒤 control을 해제한 best-effort hydration과 response-loss confirmation fallback용 one-shot read, recovery용 status-aware final product bootstrap read와 read-generation freshness를 소유하는 source-centered 3-pane workbench, cumulative Assignment/Chat activity, evidence-linked 세 갈래 Review와 replacement 전환, interrupted·unknown recovery와 explicit retry, apply 뒤 continuation-loss, 일반 clarification answer/cancel, interrupt와 settled-only reload를 유지하는 오른쪽 companion | Mounted product surface는 `@ay-ple/product-contract`를 strict decode하는 fetch/NDJSON adapter와 cross-frame reducer. Chat-only conformance source는 `@ay-ple/codex-chat-runtime/contract` |
...
| `GET /api/product/bootstrap` | Account readiness와 coarse product `operationStatus(active | idle)`, 활성 workspace의 root 비노출 product snapshot, `Course`, `RawMaterial` registry, confirmed revision·settled product history를 `no-store`로 반환한다. Status를 snapshot보다 먼저 읽어 `idle` 응답을 앞선 lease 정산 뒤 final history read로 만든다. Readiness 조회 실패는 safe `unavailable`로 격리하고 pending patch·active Run, prompt identity, Skill path·native correlation, persistence version과 내부 진단값은 Browser에 노출하지 않는다. |
