...
| Runtime과 workspace는 어떻게 선택하는가? | Current Chat은 여섯 explicit absolute `CODEX_CHAT_*` path를 검증한다. Product foundation은 이 환경 seam과 별도로 Server-owned macOS chooser 결과를 `packageRoot`·`appDataRoot`와 함께 검증하고 canonical workspace를 활성화한다. 어느 경로도 legacy env, repository `.ay-ple` 또는 `process.cwd()`로 fallback하지 않는다. |
...
| `apps/server` | Chat configuration·runtime lease와 별도로 explicit SemesterWorkspace activation, workspace-local versioned store·one Course reopen, loopback·Origin guarded HTTP/NDJSON, listener/runtime close ordering과 signal handling | `createServerApplication()`, path-free `SemesterWorkspaceController`와 `/api/codex-chat/*` 네 route |
| `apps/chat-shell` | Status, 새 native conversation, strict NDJSON decode, identity reducer, AgentMessage transcript, interrupt와 same-thread follow-up; Playwright에서 fresh materialized SemesterWorkspace activation | Browser UI와 `@ay-ple/codex-chat-runtime/contract` |
...
| Product layout | Server-owned macOS chooser seam, root non-overlap, workspace-local versioned store와 one Course reopen foundation은 구현됐다. Browser activation operation, recent workspace registry와 macOS app data 기본 경로는 아직 정하지 않았다. | [Codex Runtime 격리](codex-runtime-isolation.md) |
