...
| First Assignment vertical은 닫혔는가? | 아직 아니다. Prepared Git workspace의 normal AY Chat→Interaction MCP proposal→inline Semantic Review→AY-owned actual-file apply/checkpoint와 closed `organize_sources` contract·Server operation은 구현됐지만, GUI source selection·action presentation과 exact local-provider closeout은 남아 있다. |
| 채택된 public workflow는 무엇인가? | 현재 구현은 ADR 0018·0019·0020의 prepared Git workspace, read-only source explorer·preview, normal Chat, transient Interaction request/result, AY-owned file apply와 Server의 validated `organize_sources` Product operation이다. ADR 0021의 명시적 ActionInvocation 중 Browser interaction은 아직 미구현이며 old app-owned source registry·durable Run·patch·confirmation·apply는 복원하지 않는다. |
...
| `apps/server` | Prepared launch·registry·startup coordinator, read-only active-root SourceProjection, target Product Router, Interaction Broker, generic Chat와 validated `organize_sources` Product Turn, neutral NDJSON writer와 listener·Runtime close ordering | Canonical entrypoint의 `/api/product/*`는 lifecycle·settings·source list/text/PDF preview·normal Chat·closed ActionInvocation·semantic Review·general interaction·interrupt를 제공. Action은 current safe text refs와 exact workspace-local Skill을 fresh 검증한다. Private Broker Router는 same listener loopback+credential 경계이며 academic persistence·apply는 없음 |
| `apps/chat-shell` | Candidate-free lifecycle, 3-pane source explorer·preview·AY Chat, inline semantic Review card·settlement reducer와 recovery | `@ay-ple/product-contract`를 strict decode하는 fetch/NDJSON adapter와 cross-frame reducer. Source selection은 단일 preview용 transient presentation state이며 action control은 없다. Semantic `204`는 delivery ACK이며 resolved frame만 settlement authority |
...
| `POST /api/product/actions` | Closed `organize_sources` request의 current text refs와 enabled exact workspace-local Skill을 fresh 검증한 뒤 `workspace_write`, optional settings, Skill 하나와 bounded action text로 shared Product operation NDJSON을 시작한다. |
...
| `npm run test:e2e` | Default Browser route의 prepared lifecycle, 3-pane source list·text/PDF/unsupported preview와 evidence navigation, normal Chat, general clarification·interrupt, inline Semantic Review accept·revise·reject. 현재 Browser source action control이 없고 removed app-owned academic route가 mount되지 않은 trace도 함께 검증 |
...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| App-originated ActionInvocation | Closed Product contract, validated Server route와 optional Skill Runtime input은 구현됐다. Source selection은 아직 preview-only이며 Browser의 ordered selection·action transcript/controller, action-triggered stream UI와 exact local-provider closeout은 미구현이다. | [AY–App Interaction Layer 아키텍처](ay-app-interaction-layer.md) |
