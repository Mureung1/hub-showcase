...
Canonical root `npm run dev`는 repository 기준 `../.ay-ple-dogfood` profile의 `app-data/`와 `../workspace/year-2-semester-2`의 existing workspace를 검증한 뒤 Server와 Chat Shell을 exact local Origin으로 시작한다. Fixture를 복사하지 않고 caller의 전역 `CODEX_HOME`, 또는 미설정 시 `~/.codex`를 사용하며 별도 auth state를 만들지 않는다. Internal composition script가 package, app data와 workspace root를 검증하고 Runtime은 exact workspace `cwd`로 시작한다. Python worker가 SDK initialize 후 private `ready`를 보낸 뒤에만 public factory가 resolve하며, product native thread는 active internal-ready directory와 exact `cwd`를 가지고 private MCP URL·token은 그 thread에만 결합한다.
...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| InteractionCapability seam | Thread start가 hosted private MCP URL·token config를 주입하고, `propose_state_patch`가 private proposal key, workspace·Course·revision과 durable patch/confirmation apply에 결합되며 같은 결정에 built-in `request_user_input`을 함께 사용하고 replacement가 active Review를 교체한다. Private workspace package `@ay-ple/interaction-mcp`, project `.codex/config.toml`→built STDIO Adapter→shared loopback listener의 Runtime-scoped `apps/server` Broker, startup handshake, generation별 단일 pending slot·`busy` rejection, one-call/one-held-POST와 `accept | revise | reject` 정상 result·MCP failure 경계, settled card의 read-only 유지·fresh-call append, capability-neutral Runtime의 dynamic env 전달은 아직 없다. | [InteractionCapability ADR](../adr/0019-use-mcp-interaction-capabilities-as-the-ay-app-seam.md) |
| User-owned Git workspace | Current chooser directory 하나를 곧바로 Runtime cwd로 쓰며 fixed `project_root_markers=[]`, process-wide managed Skill root, app-owned source registry와 `.ay-ple/workspace-state.json`을 사용한다. Hub-rooted Bootstrap→fresh Workspace Runtime 전환, durable `WorkspaceRegistry`, exact Git project root에 고정된 thread cwd, native project context, `hub/.agents/skills/` Bootstrap→`hub/skills/` built-in source→workspace `.agents/skills/` copy, root `workspace-state.json`과 AY-owned commit flow가 아직 연결되지 않았다. | [User-owned Git SemesterWorkspace ADR](../adr/0018-adopt-user-owned-git-semester-workspaces.md) |
