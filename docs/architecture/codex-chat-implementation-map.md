...
| `apps/server` | Current-v2 product graph, Account Readiness, Course·material·Assignment·Chat·Review HTTP, old private MCP, neutral NDJSON writer와 listener·Runtime close ordering. Target expand로 Runtime generation credential, active product Turn binding, single held call, atomic evidence resolver와 in-memory UI Adapter를 가진 Interaction Broker, opt-in same-listener internal target composition이 추가됨 | Side-effect-free `@ay-ple/server` root의 `createServerApplication()`, `listenToServerApplication()`, `bindServerApplicationListener()`와 `/api/product/*`. Internal target은 private Router→current Product Turn NDJSON→bodyless semantic result vertical을 검증하지만 current public First Assignment composition은 전환하지 않음 |
| `apps/chat-shell` | Source-centered 3-pane workbench, cumulative Assignment·Chat activity, evidence-linked old Review·replacement, explicit retry·workspace recovery, clarification·interrupt와 settled-only hydration. Internal target용 inline semantic Review card·settlement reducer가 나란히 존재함 | `@ay-ple/product-contract`를 strict decode하는 fetch/NDJSON adapter와 cross-frame reducer. Semantic `204`는 delivery ACK이고 resolved frame만 settlement authority |
...
Interaction expand 단계에서는 `@ay-ple/interaction-mcp`의 built Adapter·exact codecs와 `apps/server`의 held Interaction Broker·evidence resolver가 current graph 옆에 존재한다. Runtime은 bounded generic child environment와 thread-scoped exact MCP readiness를 제공하며 tracked trusted Git project에서 built Adapter handshake·`propose_state_patch` roster까지 검증한다. Opt-in internal target은 Broker Router를 canonical listener에 mount하고 current Product Turn NDJSON·Browser inline card·bodyless result·continuity failure를 end-to-end로 잇는다. 다만 Runtime child environment와 public First Assignment route의 joint composition은 아직 전환하지 않았으며 current `/api/product-mcp`와 academic Review/apply path가 active public behavior다.
...
ADR 0018 target에서 root v4 identity codec, external `WorkspaceRegistry` v1 codec·CAS store, prepared-workspace launch resolver, candidate-free Browser lifecycle codec과 process-local product Turn coordinator가 current graph 옆의 internal seam으로 구현됐다. Resolver는 explicit prepared root를 우선하고 no-argument start에서 active pointer를 fresh reopen하며 canonical exact Git root와 strict v4 identity만 선택한다. No-argument authoritative reopen은 registry read 전에 dead `pending` writer를 reconcile하여 acceptance 전 first-open target을 제거하거나 switch 이전 pointer를 복원한다. Registry는 canonical root와 active pointer만 보존하고 fresh root identity가 matching할 때만 available로 판정하며 malformed·future bytes를 reset하지 않는다. Server의 internal prepared-workspace startup coordinator는 shared listener·Broker generation을 Runtime보다 먼저 준비하고 exact-root config·authenticated thread·required MCP roster·fresh context 뒤에만 verified `workspaceId` CAS와 coordinator-owned active projection을 연다. Registry transaction의 synchronous acceptance보다 Runtime terminal이 먼저 오면 같은 writer lease가 prior pointer를 exact 복원하고 normal active projection을 열지 않는다. Durable `pending | accepted` writer phase와 new-file proof는 acceptance 전 process death도 first-open missing 또는 prior switch authority로 수렴시킨다. Missing/moved/reused active root는 Runtime spawn 없는 path-free `workspace_unavailable`, malformed/future registry는 bytes-preserving `registry_incompatible`, active Runtime·Adapter continuity loss는 `runtime_unavailable` recovery로 구분한다. Failed explicit relaunch 뒤 no-argument run은 previous pointer의 root를 fresh Runtime·thread·Broker generation으로 reopen한다. Failure·terminal·shutdown의 Broker → Runtime → listener cleanup은 5초 total deadline 안에서 모든 단계를 시도한다. Chat Shell의 reusable target lifecycle surface는 `starting | active | recovery_required`만 desktop Browser에 투영하고 candidate/change control을 두지 않지만 default public composition 교체는 joint cutover 전까지 수행하지 않는다. Generic coordinator의 non-preemptive admission과 terminal-only release는 current Assignment·Chat이 이미 소비하며 prepared-workspace startup은 operation lease를 만들지 않는다.
...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| InteractionCapability seam | Thread start가 hosted old private MCP URL·token config를 주입하고, current `propose_state_patch`가 private proposal key, registered source·workspace·Course·revision과 durable patch/confirmation apply에 결합되며 같은 결정에 built-in `request_user_input`을 함께 사용하고 replacement가 active Review를 교체한다. Target `@ay-ple/interaction-mcp`와 Server Broker는 strict handshake, generation credential, active product Turn binding, single pending slot·held response, once-only result, atomic bounded evidence와 continuity failure를 구현했다. Opt-in internal target에서 canonical listener·current Product Turn NDJSON·Browser inline card·settlement와 deterministic E2E까지 연결됐다. Runtime의 bounded generic child environment와 official thread-scoped MCP readiness는 tracked trusted Git project의 `.codex/config.toml`→built Adapter trace까지 구현됐다. Prepared temporary Git trace는 target Review 뒤 AY-owned actual-file checkpoint와 no-mutation failure matrix를 고정했다. Public First Assignment의 old MCP·built-in Review를 target으로 atomic cutover하는 일은 아직 남아 있다. | [InteractionCapability ADR](../adr/0019-use-mcp-interaction-capabilities-as-the-ay-app-seam.md) |
| User-owned Git workspace | Runtime package는 caller가 준 exact workspace를 native `.git` project·thread `cwd`로 고정하고 root-local `.codex/config.toml`, `AGENTS.md`, repo Skill을 native discovery하며 hostile ancestor와 Skill extra-root injection을 배제한다. Current chooser는 directory 하나를 곧바로 Runtime cwd로 쓰고 app-owned source registry와 `.ay-ple/workspace-state.json`을 사용한다. Root v4 identity codec, external `WorkspaceRegistry` v1 CAS·fresh reopen, prepared-root launch resolver, candidate-free Browser lifecycle, generic product Turn coordinator, required-readiness startup ordering과 relaunch recovery는 internal expansion으로 구현됐다. Pre-App Bootstrap Skill과 `hub/skills/` source→workspace `.agents/skills/` copy도 구현됐고 internal startup seam은 required MCP readiness 뒤 registry commit, failed switch 뒤 previous-root fresh reopen과 Runtime/Adapter recovery를 검증한다. Prepared temporary Git trace는 Bootstrap output, exact-root Adapter readiness, inline Review와 AY-owned meaningful checkpoint를 한 test surface로 연결했다. Public composition cutover는 아직 남아 있다. | [User-owned Git SemesterWorkspace ADR](../adr/0018-adopt-user-owned-git-semester-workspaces.md), [pre-App native Bootstrap ADR](../adr/0020-bootstrap-semester-workspaces-before-app-startup.md) |
