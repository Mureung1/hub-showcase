...
Canonical root `npm run dev`는 repository 기준 `../.ay-ple-dogfood`의 persistent app data와 `../workspace/year-2-semester-2`의 existing workspace를 검증한 뒤 Server와 Chat Shell을 exact local Origin으로 시작한다. Fixture를 복사하지 않고 caller의 전역 `CODEX_HOME`, 또는 미설정 시 `~/.codex`를 사용하며 별도 auth state를 만들지 않는다. Internal composition script가 package, app data와 workspace root를 검증하고 Runtime은 workspace role로 시작한다. Python worker가 SDK initialize 후 private `ready`를 보낸 뒤에만 public factory가 resolve하며, product native thread는 active internal-ready directory와 exact `cwd`를 가지고 private MCP URL·token은 그 thread에만 결합한다.
