...
| In-app Browser OAuth가 구현됐는가? | 아니다. Current Runtime·Server·Browser는 workspace-bound generic Account Readiness read만 제공하고, dogfood login은 외부 device-auth helper다. Login·cancel·logout과 pre-workspace Runtime은 채택한 목표이지 current surface가 아니다. |
...
| `apps/server` | Canonical product composition, current selected-directory activation, current-v2 store·recovery, selected-source snapshot·guard, private MCP host, product operation·Review coordinator, Account Readiness, neutral NDJSON writer, listener·Runtime close ordering | `createServerApplication()`, product composition seam과 `/api/product/*` |
...
Canonical `npm run dev -- --app-data-root <absolute-path>`는 package, app data와 current materialized/override directory를 검증한 뒤 Server와 Chat Shell을 exact local Origin으로 시작한다. Root `npm run dogfood -- --root <absolute-path>`는 repository 밖의 persistent profile과 isolated auth state를 준비·검증하고 같은 canonical composition에 `appDataRoot`와 caller-owned fixture directory를 전달한다. 이 current helper는 public scaffold나 admission 구현이 아니다. Runtime은 product bootstrap의 Account Readiness read에서 lazy start할 수 있고, Python worker가 SDK initialize 후 private `ready`를 보낸 뒤에만 public factory가 resolve한다. Product native thread는 active internal-ready directory와 exact `cwd`를 가지며 private MCP URL·token은 그 thread에만 결합한다.
...
| Workspace admission·setup | Current chooser·materializer는 임의 directory와 current v2 store를 internal `ready`로 열 뿐 Browser `SetupJourney`, 최종 확인·approved-only transaction, app-owned scaffold·`WorkspaceManifest`, workspace instruction/Skill bundle materialization·complete-tree validation, ancestor 차단·workspace-local effective-native-context gate, incomplete-setup recovery와 `Semester Ready`를 구현하지 않았다. | [ADR 0014](../adr/0014-create-app-owned-normalized-semester-workspaces.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
...
| Browser OAuth·pre-workspace Runtime | Current private bridge는 generic account read만 제공하고 Runtime factory는 workspace input을 요구한다. Login·cancel·logout, fresh ChatGPT account projection, transient attempt lifecycle과 auth-only bootstrap→workspace Runtime transition은 구현되지 않았다. | [ADR 0017](../adr/0017-use-codex-managed-browser-oauth-for-product-account-lifecycle.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
