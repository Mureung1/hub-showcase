...
이 지도에서 current canonical controller·wire type의 `ready`는 chooser/current-v2 directory가 mutation 가능한 내부 상태라는 뜻이다. 별도 v3 setup Modules은 [CONTEXT.md](../../CONTEXT.md)의 `Semester Ready`를 만들기 위한 durable envelope, lease-bound Ready transition/attestation과 projection을 구현했지만 canonical product router·startup·UI에는 아직 조합되지 않았다. 따라서 현재 dogfood의 `ready`와 v3 setup graph가 만드는 `Semester Ready`를 같은 상태로 해석하지 않는다.
...
| In-app Browser OAuth가 구현됐는가? | Runtime package에는 official managed Browser login·cancel·logout, fresh account read와 immutable `auth-only | workspace` role이 구현됐다. Server에는 app-wide `AccountRuntimeCoordinator`와 Account route adapter도 구현됐지만 setup/product router와 Browser projection에는 아직 조합되지 않아 current dogfood UI는 외부 device-auth helper를 사용한다. |
...
| Runtime와 workspace는 어떻게 선택하는가? | Canonical startup이 `packageRoot`에서 verified artifact를 찾고 explicit `appDataRoot`에서 controlled runtime directory를 계산한다. Current controller는 chooser·materializer가 넘긴 directory를 internal `ready`로 열어 Product Turn의 exact `cwd`로 쓴다. `CODEX_CHAT_WORKSPACE`는 manual-development selection override다. App-owned v3 scaffold·`WorkspaceManifest` admission, durable setup envelope·Ready relaunch와 Server의 lease-bound Ready/native/action-readiness Modules은 구현됐지만 current startup·controller·router에는 아직 조합되지 않았다. |
...
| `apps/server` | Canonical current-v2 product composition, selected-directory activation·recovery, selected-source guard, private MCP host, product operation·Review coordinator, Account Readiness, app-wide account/Runtime coordinator, high-level native project boundary, lease-bound Ready transition/attestation, setup-envelope action readiness·safe Ready presentation, neutral NDJSON writer와 listener·Runtime close ordering | `createServerApplication()`, product composition seam과 `/api/product/*`. V3 setup Modules은 아직 current router에 mount되지 않음 |
...
아직 mount되지 않은 v3 setup graph에서는 package `SetupJourney`가 prepared workspace를 Server의 lease-bound Ready transition에 넘긴다. A1 coordinator는 auth-only close → workspace Runtime start → fresh ChatGPT account read를 수행한 뒤 B callback을 호출하고, callback은 native project boundary → `active_ready` commit/strict readback을 완료한다. Server의 process-local Ready attestation은 모든 app-wide workspace transition 시작과 Account route의 성공한 fresh non-ChatGPT 관찰에서 전부 무효화된다. Durable `active_ready` pointer와 current attestation이 exact하게 일치해야 setup-envelope action readiness가 열리며, reconnect 뒤 ChatGPT 관찰만으로 attestation을 다시 만들지는 않는다.
...
| Workspace admission·setup | `@ay-ple/semester-workspace`는 v3 scaffold admission·recovery, instruction/Skill bundle, durable setup envelope·approved→prepared journey와 lease-bound Ready relaunch를 구현했다. Server도 native boundary, Ready transition/attestation, action readiness와 safe Ready projection을 구현했다. 미지원 경계는 current chooser·controller·HTTP·Chat Shell이 여전히 임의 directory/current v2를 internal `ready`로 사용하고 이 v3 graph를 production command route로 조합하지 않았다는 점이다. | [ADR 0014](../adr/0014-create-app-owned-normalized-semester-workspaces.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
| Workspace registry·ready-relaunch | App data의 durable `active_ready` pointer와 same-release relaunch logic은 package에 구현됐지만 canonical startup은 이를 읽어 v3 workspace를 재검증하거나 setup wizard를 건너뛰지 않는다. Current Browser chooser는 process input·memory의 directory를 연다. | [Codex Runtime 격리](codex-runtime-isolation.md) |
| Browser OAuth·pre-workspace Runtime | Runtime package의 managed account lifecycle과 Server의 app-wide account/Runtime coordinator·route adapter는 구현됐다. 미지원 경계는 이 graph를 setup HTTP/Browser projection, v3 Ready transaction과 current application shutdown에 하나의 composition으로 연결하는 일이다. | [ADR 0017](../adr/0017-use-codex-managed-browser-oauth-for-product-account-lifecycle.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
...
| Public npx Runtime delivery | macOS arm64 verified Runtime은 repository-local ignored artifact로 materialize할 수 있지만 public application package, embedded exact descriptor, immutable GitHub Runtime release와 resolver-owned verified cache는 없다. | [ADR 0016](../adr/0016-distribute-public-preview-with-an-exact-npx-launcher-and-verified-runtime-release.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
