...
| `apps/chat-shell` | Candidate-free lifecycle, 3-pane source explorer·preview·AY Chat, explicit `organize_sources` selection·action transcript, inline semantic Review card·settlement reducer와 recovery | `@ay-ple/product-contract`를 strict decode하는 fetch/NDJSON adapter와 shared cross-frame reducer. Preview focus와 extension-neutral ordered action selection은 독립적인 transient state이고 action 시작 시 refs·settings를 동결한다. Semantic `204`는 delivery ACK이며 resolved frame만 settlement authority |
...
| `POST /api/product/actions` | Closed `organize_sources` request의 current safe regular-file refs와 enabled exact workspace-local Skill을 initial preflight하고, `operation.preparing` 뒤 dispatch 직전에 current refs·effective Skill·prepared input을 각각 한 번 final validation한 다음 `workspace_write`, optional settings, Skill 하나와 Markdown file references를 담은 bounded action text로 shared Product operation NDJSON을 시작한다. |
...
| `npm run test:e2e` | Default Browser route의 prepared lifecycle, 3-pane source list·text/PDF/unsupported preview, preview kind와 독립적인 ordered `organize_sources` selection·frozen transcript, operation lock·retry, evidence navigation, normal Chat 격리, general clarification·interrupt와 inline Semantic Review accept·revise·reject를 검증 |
