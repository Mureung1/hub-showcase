...
| `apps/chat-shell` | Candidate-free lifecycle, 3-pane source explorer·preview·AY Chat, explicit `organize_sources` selection·action transcript, inline semantic Review card·settlement reducer와 recovery | `@ay-ple/product-contract`를 strict decode하는 fetch/NDJSON adapter와 shared cross-frame reducer. Preview focus와 text-only ordered action selection은 독립적인 transient state이고 action 시작 시 refs·settings를 동결한다. Semantic `204`는 delivery ACK이며 resolved frame만 settlement authority |
...
| `npm run test:e2e` | Default Browser route의 prepared lifecycle, 3-pane source list·text/PDF/unsupported preview, explicit ordered `organize_sources` action·frozen transcript, operation lock·retry, evidence navigation, normal Chat 격리, general clarification·interrupt와 inline Semantic Review accept·revise·reject를 검증 |
...
| Skill reader identity binding | Runtime은 startup root와 dispatch 직전 root→`SKILL.md` ancestor·leaf identity를 검증한다. 그러나 official `SkillInput`은 name·path만 운반하고 native core가 이후 path를 다시 열기 때문에 검증 handle과 actual reader inode 사이의 post-validation pathname 교체·ABA를 원자적으로 배제하지 못한다. 완전한 closure에는 native same-open identity/digest, descriptor capability 또는 immutable snapshot carrier 중 하나의 별도 architecture 결정이 필요하다. | [Runtime README](../../packages/codex-chat-runtime/README.md) |
