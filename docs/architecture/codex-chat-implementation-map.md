...
이 지도에서 current controller·wire type의 `ready`는 chooser/current-v2 directory가 mutation 가능한 내부 상태라는 뜻이다. [CONTEXT.md](../../CONTEXT.md)의 `Semester Ready`는 OAuth, app-owned scaffold, `WorkspaceManifest` validation과 active registry가 끝난 setup checkpoint이므로 현재 구현의 `ready`와 같지 않다.
...
| In-app Browser OAuth가 구현됐는가? | Runtime package에는 official managed Browser login·cancel·logout, fresh account read와 immutable `auth-only | workspace` role이 구현됐다. 다만 Server의 account transition lease·setup route와 Browser projection은 아직 조합되지 않아 current dogfood UI는 외부 device-auth helper를 사용한다. |
...
| Runtime와 workspace는 어떻게 선택하는가? | Canonical startup이 `packageRoot`에서 verified artifact를 찾고 explicit `appDataRoot`에서 controlled runtime directory를 계산한다. Current controller는 chooser·materializer가 넘긴 directory를 internal `ready`로 열어 Product Turn의 exact `cwd`로 쓴다. `CODEX_CHAT_WORKSPACE`는 manual-development selection override다. App-owned v3 scaffold·`WorkspaceManifest` admission Module은 구현됐지만 current startup·controller에는 아직 조합되지 않았다. |
...
| `apps/server` | Canonical product composition, current selected-directory activation, current-v2 store·recovery, selected-source snapshot·guard, private MCP host, product operation·Review coordinator, Account Readiness, high-level native project boundary, neutral NDJSON writer, listener·Runtime close ordering | `createServerApplication()`, product composition seam과 `/api/product/*` |
...
| Workspace admission·setup | `@ay-ple/semester-workspace`는 v3 scaffold admission·recovery와 instruction/Skill bundle materialization·complete-tree validation을 구현했고, Server native boundary는 fixed project root·controlled roots·static conflict와 Runtime의 effective config·Skill snapshot을 검증한다. 그러나 current chooser·controller·HTTP는 여전히 임의 directory와 current v2 store를 internal `ready`로 열며, Browser `SetupJourney`, 최종 확인·approved-only transaction, admission/native gate 조합, durable setup envelope와 `Semester Ready`는 구현하지 않았다. | [ADR 0014](../adr/0014-create-app-owned-normalized-semester-workspaces.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
| Workspace registry·ready-relaunch | Canonical startup과 Browser chooser는 process input·memory의 directory를 열지만 app data의 durable active pointer, `WorkspaceManifest` 재검증과 setup wizard를 건너뛰는 relaunch가 없다. | [Codex Runtime 격리](codex-runtime-isolation.md) |
| Browser OAuth·pre-workspace Runtime | Runtime package는 managed login·cancel·logout, fresh ChatGPT account projection, transient attempt lifecycle과 owner-only auth-only role을 구현했다. Server의 app-wide transition lease와 auth-only→workspace generation 교체, setup HTTP·Browser projection, Ready commit은 아직 구현되지 않았다. | [ADR 0017](../adr/0017-use-codex-managed-browser-oauth-for-product-account-lifecycle.md), [Codex Runtime 격리](codex-runtime-isolation.md) |
