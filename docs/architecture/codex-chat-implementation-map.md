...
ADR 0018 target의 root v4 identity codec과 external `WorkspaceRegistry` v1 codec·CAS store는 current graph 옆의 internal target seam으로 구현됐다. Registry는 canonical root와 active pointer만 보존하고 fresh root identity가 matching할 때만 available로 판정하며 malformed·future bytes를 reset하지 않는다. Candidate·activation·Browser lifecycle은 아직 이 seam을 current public composition에 연결하지 않는다.
