...
Canonical `npm run dev -- --app-data-root <absolute-path>`는 package, app data와 current materialized/override directory를 검증한 뒤 Server와 Chat Shell을 exact local Origin으로 시작한다. Root `npm run dogfood`는 repository 기준 `../.ay-ple-dogfood`의 persistent app data와 `../workspace/year-2-semester-2`의 existing workspace를 같은 canonical composition에 전달하며 fixture를 복사하지 않는다. 두 경로 모두 caller의 전역 `CODEX_HOME`, 또는 미설정 시 `~/.codex`를 사용하며 별도 auth state를 만들지 않는다. Runtime은 workspace role로 시작하고 Python worker가 SDK initialize 후 private `ready`를 보낸 뒤에만 public factory가 resolve한다. Product native thread는 active internal-ready directory와 exact `cwd`를 가지며 private MCP URL·token은 그 thread에만 결합한다.
