...
| `npm run test:e2e` | Canonical prepared Browser trace와 별도 donor HTML entry의 old academic regression trace. Prepared trace는 default route의 normal Chat·inline Semantic Review·lifecycle recovery를, donor trace는 rollback source의 v2 store durability만 검증 |
...
| `npm run test:first-assignment-product-actual -w @ay-ple/server` | Old academic rollback source의 exact local provider·managed Skill·private MCP→Plan→Review→terminal, v2 durable outcome과 cleanup donor conformance |
...
| `npm run trace:first-assignment-live -w @ay-ple/server -- --codex-home <isolated-auth-seed>` | Old academic rollback source의 explicit isolated auth·fresh roots opt-in live donor outcome |
