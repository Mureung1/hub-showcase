...
| Gap | 현재 사실 | 정본 |
| --- | --- | --- |
| InteractionCapability seam | Thread start가 hosted private MCP URL·token config를 주입하고, `propose_state_patch`가 private proposal key, workspace·Course·revision과 durable patch/confirmation apply에 결합되며 같은 결정에 built-in `request_user_input`을 함께 사용한다. Project `.codex/config.toml`→hub STDIO Adapter→App Broker와 dynamic env binding은 아직 없다. | [InteractionCapability ADR](../adr/0019-use-mcp-interaction-capabilities-as-the-ay-app-seam.md) |
