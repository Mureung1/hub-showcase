...
filters, generic crawling, AI extraction, multiple feeds, or deployment
automation. Its S1-lite boundary renders the fixed 601-event Foundation.25.1
snapshot plus one manually refreshed, allowlisted public source event.

## Runtime Flow

```text
administrator
  └─ Bearer-authenticated create / inspect / rotate / revoke / refresh
       └─ Express S-Lite router
            └─ long-lived Python JSON-lines bridge
                 ├─ SQLite: feed, source state, materialized ICS snapshot
                 ├─ Foundation.25.1: immutable 601-event base + Policy.15
                 └─ fixed-source HTTPS fetch: one allowlisted KNU notice

calendar client
  └─ GET or HEAD /calendar/:opaqueToken.ics
       └─ hash comparison → status check → materialized ICS render
```

The raw token and full subscription path exist in process memory while a
...
again by the status API. The LAN operator may explicitly retain the URL in a
separate mode-`0600`, Git-ignored recovery file; that file is a bearer secret,
not part of the database contract.
...
| `POST /api/subscription-feeds/slite/refresh` | Evaluate the one fixed source and atomically retain or replace the materialized snapshot; body must be exactly `{}` | `200` |
...
`synchronous=FULL`. Schema v2 stores the singleton feed, at most one source
state row, and one materialized ICS snapshot. Static Foundation data is rebuilt
in memory at process start, then the persisted source and snapshot are
recomputed and validated before the feed is restored into the delivery
service. See
[`slite-single-source-refresh.md`](slite-single-source-refresh.md) for the
S1-lite fetch, event identity, and failure-isolation contract.

Back up the database from stable persistent storage together with the
administrator credential and serving CA required by the deployment. Restore
must preserve private permissions and use a Foundation build containing the
same profile and snapshot. Deleting or losing the database permanently
invalidates the issued link because the raw token cannot be reconstructed.
The local encrypted drill is documented in
[`slite-local-backup-restore.md`](../deployment/slite-local-backup-restore.md).
...
provisioning, production secret rotation, monitoring, scheduled off-host
backup, scheduler/retry behavior, cancellation reconciliation, and observed
background calendar-client update timing remain deployment gates
