...
| `packages/runtime-codex` | Codex app-server 통합 package | `CodexRuntimeAdapter`, `CodexRawClient` wrapper type, 제품 runtime layout 사전 검증, status/smoke helper, capability slots | 생성된 app-server protocol type, direction·ID type을 보존하는 bidirectional stdio JSONL transport, actual-child fixture journal, Harness-managed repository-local runtime home, 제품 root·binary·runtime-home pair 검증, raw/debug log |
...
| 제품 Host의 App Server 요청 왕복 | `packages/runtime-codex`의 lower stdio transport는 Client request·notification과 Server request·notification을 분류하고 exact `RequestId`로 generated-schema-validated typed response를 한 번 돌려보낸다. Raw numeric identity의 exact safe-integer value와 top-level key ambiguity를 parse 전에 검증하고 request timeout 뒤 known late response를 해당 identity에만 fence한다. Actual spawn과 close는 coalesced Promise로 관측하며 Server request response/dismiss 뒤 active identity를 해제한다. Client identity tombstone은 safety-preserving hard cap으로 bounded한다. 기존 Harness `CodexRawClient`는 이 seam을 사용하지 않으며, Server request를 product-safe pending interaction으로 만드는 Host mapping은 아직 없다. Generic transport만 구현했으므로 method별 integration 단계도 승격하지 않았다. | [runtime-codex README](../../packages/runtime-codex/README.md), [Method 목록](codex-app-server-method-inventory.md), [개발 백로그](../product/ay-ple-development-backlog.md) |
