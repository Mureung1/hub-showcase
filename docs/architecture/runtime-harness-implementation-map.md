...
| `vendor/ai-sdk-provider-codex-cli` | 수정 가능한 upstream fork incubation | 없음 | Donor source·tests·fixtures·toolchain 전체와 MIT notice, exact upstream identity, local patch ledger, exact `0.144.4` pin, package-private generated snapshot, sole-ingress decoder와 adopted outbound builder. Root workspace와 production consumer에서 격리됨 |
...
| Fork 후속 patch | 진행 중 | Method-specific response decoder, donor surface pruning, transport/RPC hardening과 T0·T0-C·T0.1 conformance가 남아 있다. 순서 정본은 [개발 백로그](../product/ay-ple-development-backlog.md)다. |
...
| `npm run validate --prefix vendor/ai-sdk-provider-codex-cli` | 격리 fork의 exact Codex pin/generated snapshot, generated sole-ingress decoder와 adopted outbound builder, donor build·typecheck·format·lint와 non-live regression. Method-specific response result, raw-byte/RequestId·transport hardening과 live conformance는 아직 증명하지 않는다. |
