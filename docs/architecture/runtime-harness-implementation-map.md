...
| `vendor/ai-sdk-provider-codex-cli` | 수정 가능한 upstream fork incubation | 없음 | Donor App Server mechanics·tests·toolchain과 MIT notice, exact upstream identity, local patch ledger, exact `0.144.4` pin, package-private generated snapshot, sole-ingress decoder, adopted outbound builder·response decoder, native lifecycle identity·bootstrap/catalog/item view·turn-event router·turn-result collector. Exact pin 밖의 legacy fixture/validator, dead handwritten item/event catalog와 process-per-call Exec/CodexCli lane은 제거됐고 root workspace·production consumer에서 격리됨 |
...
| `FP-0007a` Exec surface retirement | 완료 | App Server foundation과 독립인 process-per-call Exec/CodexCli source·exports·tests·active docs를 제거했다. App Server AI SDK adapter는 native facade 전환 전까지 oracle로 남는다. |
| Fork 후속 patch | 진행 중 | Native App Server facade, 남은 AI SDK projection 제거, transport/RPC hardening과 T0·T0-C·T0.1 conformance가 남아 있다. 순서 정본은 [개발 백로그](../product/ay-ple-development-backlog.md)다. |
...
| `npm run validate --prefix vendor/ai-sdk-provider-codex-cli` | 격리 fork의 exact Codex pin/generated snapshot, generated sole-ingress decoder, adopted outbound builder·method-specific response decoder, lifecycle/item identity view, package-private turn-event router, native turn-result collector, retired Exec source/export boundary, App Server-only build·typecheck·format·lint와 non-live regression. Native facade, bounded staging, raw-byte/RequestId·transport hardening과 live conformance는 아직 증명하지 않는다. |
