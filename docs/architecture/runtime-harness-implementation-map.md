...
| `packages/runtime-codex` | Codex app-server 통합 package | `CodexRuntimeAdapter`, `CodexRawClient` wrapper type, 제품 runtime layout 사전 검증, status/smoke helper, capability slots | 생성된 app-server protocol type, direction·ID type과 adopted Client response schema를 검증하는 bidirectional stdio JSONL transport, single-consumer bounded observation queue, actual-child fixture journal, Harness-managed repository-local runtime home, 제품 root·binary·runtime-home pair 검증, raw/debug log |
...
| 제품 Host의 App Server 요청 왕복 | `packages/runtime-codex`의 lower stdio transport는 Client request·notification과 Server request·notification을 분류하고 exact `RequestId`로 generated-schema-validated typed Server response를 한 번 돌려보낸다. Adopted Client response roster의 `initialize` result도 state settlement 전에 generated schema로 검증한다. Raw numeric identity의 exact safe-integer value와 top-level key ambiguity를 parse 전에 검증하고 request timeout 뒤 known late response를 해당 identity에만 fence한다. Actual spawn과 close는 coalesced Promise로 관측하며 close가 spawn settlement보다 먼저 시작되면 shared start를 `transport_closed`로 reject하고 child cleanup을 기다린다. Force-kill 뒤에도 exit를 관측하지 못하면 `close_timeout`으로 reject한다. Server request response/dismiss 뒤 active identity를 해제하고 Client identity tombstone을 hard cap으로 bounded한다. Observation stream은 single-consumer이고 pending queue cap overflow가 connection을 안전하게 닫는다. 기존 Harness `CodexRawClient`는 이 seam을 사용하지 않으며, Server request를 product-safe pending interaction으로 만드는 Host mapping과 operation 의미별 timeout fencing은 아직 없다. Generic transport만 구현했으므로 method별 integration 단계도 승격하지 않았다. | [runtime-codex README](../../packages/runtime-codex/README.md), [Method 목록](codex-app-server-method-inventory.md), [개발 백로그](../product/ay-ple-development-backlog.md) |
| 제품 Host의 검증된 layout 사용 | `packages/runtime-codex`에 명시적 세 root, package-owned pinned binary와 app-managed runtime-home pair를 spawn 전에 검증하는 `prepareProductRuntimeLayout()`이 구현됐다. `appDataRoot`는 아직 존재하지 않아도 existing ancestor와 symlink를 canonicalize한 뒤 overlap 검사, exact root 생성과 post-create containment 검사를 수행한다. 아직 App Server child와 Host lifecycle은 이 layout을 소비하지 않으며, 기존 Harness `cwd`와 home default/override는 그대로다. | [runtime-codex README](../../packages/runtime-codex/README.md), [Runtime 격리](codex-runtime-isolation.md), [개발 백로그](../product/ay-ple-development-backlog.md) |
