...
| Human decision의 native timeout | Held Broker response가 정상이어도 current pinned default 300초를 넘기면 Codex가 MCP call을 먼저 실패시킬 수 있다. | Project config가 별도로 결정한 human-decision `tool_timeout_sec`을 명시하고 timeout path를 cancellation과 동일하게 terminal 정산한다. |
