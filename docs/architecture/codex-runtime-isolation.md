...
| Native context | Runtime의 persistent bridge와 one-shot official App Server sidecar가 모두 fixed `project_root_markers=[]`, exact workspace `cwd`와 controlled environment를 사용한다. Sidecar의 `config/read`·`skills/list` raw protocol은 Runtime-private이고 Server는 atomic high-level snapshot만 소비한다. `@ay-ple/semester-workspace`의 bundle verifier와 Server native boundary는 static·effective conflict를 검사하지만 current action/setup composition에는 아직 연결되지 않았다. | App 실행 전 native client는 hub project의 `.agents/skills/`에서 init Skill을 발견하고, App의 정상 Runtime은 exact Git root의 project config·instruction·Skill을 native discovery한다. App은 broad CLI override나 process-wide managed Skill root로 context를 대체하지 않는다. | Fixed project marker·managed Skill override 제거와 obsolete bundle verifier contraction |
