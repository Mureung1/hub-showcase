...
| Sandbox·approval | Legacy text tracer는 `deny_all + read_only`, structured product Turn은 `auto_review + workspace_write`를 explicit하게 보낸다. 두 native permission profile은 AY-PLE Review·`UserConfirmation` state와 합치지 않는다. | OS process 보안 경계, path-level filesystem immutability, final product permission conformance |
