...
| Untrusted project config 무시 | Codex는 신뢰하지 않은 project의 `.codex/config.toml`을 읽지 않으므로 Interaction MCP가 조용히 사라질 수 있다. | Exact Git root와 `workspace-write` thread start의 native trust write·config reload를 사용하고 effective MCP discovery를 확인한다. Explicit `untrusted`를 App이나 Bootstrap이 덮어쓰지 않으며 그때의 fail behavior는 후속 lifecycle 결정이 소유한다. |
