...
현재 exact official Python SDK와 production wheel, private bridge와 Node `CodexProductCapableRuntime`에는 `SkillInput`·`TextInput` product Turn, product permission, Plan·MCP activity와 deferred typed `request_user_input` answer/cancel seam이 구현돼 있다. Server 내부에는 app-issued selected-source context에 결합한 exact `propose_state_patch`, workspace-local pending `StatePatch`, same-Turn exact Review binding과 accepted/rejected `UserConfirmation` transaction도 구현됐다. Deterministic Runtime/MCP integration은 product commit이 native answer보다 먼저 끝나고 settled Assignment·patch·confirmation이 reopen되는 순서를 Browser 없이 검증한다.

아직 구현되지 않은 경계는 `ModelingInvocation` admission과 durable `ModelingRun`, product runtime에 MCP session을 주입하는 action coordinator, public activity/action HTTP와 Browser Review다. 수정 요청의 replacement, continuity loss·source/state drift recovery도 후속 lifecycle slice가 소유한다. 따라서 구현된 authority를 current text-only route가 이미 사용한다고 해석하지 않는다. [First Assignment Product-bound Codex Companion spec](../specs/2026-07-19-first-assignment-product-bound-companion.md)과 implementation tickets가 exact adaptation contract를 소유한다. Current package 책임과 확인된 gap은 [Codex Chat 구현 지도](codex-chat-implementation-map.md)와 [codex-chat-runtime README](../../packages/codex-chat-runtime/README.md), 작업 순서와 상태는 [개발 백로그](../product/ay-ple-development-backlog.md)가 소유한다. Raw capability의 저수준 근거는 [context delivery 조사](../spikes/codex-app-server-context-delivery/research.md)에 둔다.
