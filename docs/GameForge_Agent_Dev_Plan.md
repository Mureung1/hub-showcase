...
- `DiffViewer` (파일 클릭 시 확장되는 unified diff, 추가/삭제 라인 색상 구분)
- `CommitSelectionBar` ("선택 항목 커밋 (N개)" 버튼)

**작업 목록**
1. 코드 생성/리팩토링 Agent의 응답 포맷을 **파일 단위 배열**로 강제: `[{ path, changeType(new|modified|deleted), diff, suggestedCommitMessage }]` — 하나의 통짜 텍스트가 아니라 구조화된 JSON으로 받도록 프롬프트/파싱 설계
2. `GET /api/steps/{id}/file-changes` — 위 배열 조회, 프론트에서 리스트+diff 렌더링
3. 사용자가 파일별 체크박스 선택 + 커밋 메시지 수정 가능
4. `POST /api/repo/{id}/commit-batch` — 선택된 항목을 **파일(혹은 논리적 그룹) 단위로 순차 커밋 처리**, 각각 개별 커밋 메시지 사용
5. 체크 해제되어 이번에 커밋되지 않은 파일은 `pending_changes` 상태로 남아 다음 라운드에 다시 노출
6. 진행률(`progress_pct`) 계산 방식 변경: 이 두 단계에서는 "승인 완료된 파일 수 / 전체 제안된 파일 수"로 계산 (다른 단계의 체크리스트 기반 계산과 다른 룰이므로 `Step` 스키마에 `progress_mode` 필드 추가 검토)

**리스크**: diff 생성/파싱 로직이 새로 필요하고(unified diff 포맷 생성 + 프론트 렌더링), Agent 프롬프트도 "자연어 설명 + 구조화된 파일 배열"을 동시에 뽑아내야 해서 7단계 난이도가 예상보다 높아질 수 있음 — 4주 일정에서 가장 타이트한 지점이 될 가능성이 큼.
...
- **4.6 커밋 리뷰(Diff) 화면 — 이번 4주 스코프 밖.** 7~8단계(코드 생성/리팩토링)는 4주 계획에 포함되지 않으므로(마일스톤이 9단계 이전까지만 다룸), 파일 단위 원자적 커밋 로직은 다음 사이클에서 착수
...
- 반려 사유 입력 UI
- **4.6 커밋 리뷰(Diff) 화면** — 7~8단계(코드 생성/리팩토링)의 파일 단위 원자적 커밋 로직, 이번 4주는 1~6·9단계 워크플로우까지가 목표
