...
## Phase 2 백로그 (우선순위 벗어나 먼저 진행하는 경우)

**T22 (화이트노이즈 테마)**

```
docs/instructions.md 절차에 따라 T22(화이트노이즈 테마 — 낮/밤 동기화·숲·카페)를 진행해줘.
완료 조건은 checklist.md C22야. 이슈 번호는 #56이니까
커밋 메시지에 (#56) 붙이고, 마지막 커밋엔 Closes #56 넣어줘.
docs/prototype/whitenoise-themes.html을 디자인 근거로 삼아줘.
```
