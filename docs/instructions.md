...
| [backlog.md](backlog.md) | T01~T20 구현 순서·선행조건·상태 | 매 작업 시작·종료 시 |
| [checklist.md](checklist.md) | C01~C20 검증 가능한 완료 조건 | 매 작업 시작·종료 시 |
...
7. backlog.md의 해당 Task 상태를 `진행중`으로 바꾼다.
8. **구현 전략 먼저 보여주고 승인받기** — 바로 코드를 쓰지 않는다. 고칠/만들 파일과 각 파일의 역할, 데이터가 어디 있고 어떻게 흐르는지(state 위치, 요청·응답), 왜 이렇게 구현하는지(대안이 있다면 장단점도)를 먼저 설명하고 승인받은 뒤에 구현을 시작한다.
...
4. **skills.md** — 입출력 계약이 필요한 Task면 계약(S) 추가. 통일성을 위해 새 기능은 화면(무엇이 생기고 바뀌는지) → 데이터(테이블/컬럼) → 흐름(요청·응답) 순서로 먼저 설계하고, 그 결과를 계약으로 옮긴다. 순수 UI/설정 작업이면 계약은 생략(`—`).
