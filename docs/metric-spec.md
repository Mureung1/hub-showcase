...
| 제외 | 웹·모바일·게임 클라이언트가 주 업무다 |
| 제외 | 인프라 구축·운영, 데이터 파이프라인, 머신러닝 학습·서빙이 주 업무다 |
...
`postings.job_role_id`는 단일 값이다. 여러 직무를 함께 모집하는 공고는 모집단에 넣지 않는다.

직군 필드와 제목이 사업 영역만 가리키고 직무를 가리키지 않는 공고는 세 번째 입력으로 판정한다. 이 경우 판정 근거가 되는 본문 구절을 `job_role_raw`에 담는다.
