...
| `manager_meta` | `jsonb` | No | `{}` | Manager 실행 메타(pivot·셔플 시드·모델·프롬프트 버전·판정 기준·지표). SPEC-AI-002 §3.3 |
| `created_at` | `timestamptz` | No | `now()` | 생성 시간 |
| `updated_at` | `timestamptz` | No | `now()` | 수정 시간 |
...
| `disagreement_type` | `agenda_disagreement_type` | Yes | `null` | 1차 판정에서 관찰한 차이 유형 5종 |
| `revised_type` | `agenda_disagreement_type` | Yes | `null` | 재검토로 수정된 유형. 1차 판정이 타당하면 `null` |
| `confidence` | `numeric(3,2)` | Yes | `null` | 판정 확신도 0~1. **관측용이며 강제 전환에 쓰지 않는다** |
| `display_order` | `smallint` | No | `0` | 화면 표시 순서 — pivot 섹션 순 → 신규 쟁점 순 |
...
SPEC-AI-002 결정 4. 파생하지 않고 저장하는 이유는, 충돌 판정 기준(`MANAGER_CONFLICT_TYPES`)을 나중에 바꿔도 과거 Agenda의 당시 판정이 보존되어야 하기 때문이다.

### 4.8 `agenda_disagreement_type`
...
Manager가 관찰한 차이의 유형이다. 합의/충돌 판정은 이 값에서 **코드가 계산**하며, 어떤 유형을 충돌로 볼지는 설정값이다(기본 `main_answer`만).
