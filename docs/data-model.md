...
`course_catalog`만 특이하게 "공용" 테이블이에요 — 개설 과목 목록은 모든 학생이 같은 걸 보니까요. 나머지 3개는 사용자마다 따로 가지는 데이터라 전부 `user_id`로 연결돼요.
...
지금 당장 만들지 않고, T-05(3주차, 과목 검색) 때 이 구조로 시작하면 됨:
...
**참고**: `schedule_text`를 요일/교시 구조로 잘게 쪼개서 저장할지(`day`, `period` 컬럼 분리) 아니면 텍스트로 뭉쳐서 저장 후 화면에서만 파싱할지는 3주차에 결정. 지금은 원본 그대로 보존하는 쪽으로 설계해둠.

---

## 내일(Supabase 작업) 순서 제안
