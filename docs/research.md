...
- **캔들 색상(국내 관례로 확정)**: 초기 목업의 CSS 변수 기준 — **상승(양봉) `#e0453f`(빨강)**, **하락(음봉) `#2f6bd6`(파랑)**. (원본 investment_journal은 서구 관례로 `upColor:"#059669"`(초록)/`downColor:"#dc2626"`(빨강)를 썼으나, Beacon은 국내 관례로 뒤집는다.) `addSeries(CandlestickSeries, { upColor:"#e0453f", downColor:"#2f6bd6", borderUpColor:"#e0453f", borderDownColor:"#2f6bd6", wickUpColor:"#e0453f", wickDownColor:"#2f6bd6" })`.
