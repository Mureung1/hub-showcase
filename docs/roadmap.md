...
> 마지막 갱신: **2026-07-24** (WP-H 부분 검증: 거울 프레임 라이브 통과 · 멀티유저 격리/귀속/소유권 DB·코드 레벨 확인 · 웹 단일 회귀 통과. 2계정 실제 Discord 클릭만 이월).

> **2026-07-20~24 요약**: 단일 사용자 라이브 완주(G0·G1·H1) 검증 완료 → `0007` "거울 프레임" 피벗 → G2/G3 멀티유저 라우팅 코드 착지. **2026-07-23 CLI 실측으로 `0001`~`0008` 원격 적용과 4개 Edge Function ACTIVE를 확인**했고, Discord 버튼의 조건 소유자 검증을 단위 테스트와 함께 보강했다. **2026-07-24 WP-H 검증 세션**: 거울 프레임 신규 복기 라이브 통과 + 멀티유저 격리/귀속/소유권을 RLS 정책·실데이터·단위 테스트로 확인 + 웹 단일 회귀 통과. 남은 것은 **계정 2개 실제 Discord 클릭 경로**뿐(4주차 이월).
...
- ✅ **거울 프레임 라이브 검증**(2026-07-24): 신규 복기 재생성(005930 매수 trade `50e471ac`) → `plan_adherence` 생성·계획 미기록("가설·목표가·손절가·보유기간·확신도 모두 미기록") 정확히 지적·미래지시/종목 가치평가/결과론 없음, `cited_trade_ids` 4건 인용, `ai_usage_events` 6→7 적재(F2).
- ✅ **단일계정 감시→알림→기록 라이브 루프**(2026-07-24): 웹에서 조건 생성(즉시 `active`) → `monitor` 트리거 → 조건 충족(KIS 실시세 250,000) → `alerts` 행 기록(소유자 `user_id`) + **Discord 알림 발송** → 사용자 매수 버튼 원클릭 → `trades` insert(@250,000·`source=discord_button`·소유자 `user_id` 귀속). 감시 파이프라인 전 구간 라이브 확인.
- 🟡 **멀티유저 격리 검증**(2026-07-24): RLS own-row 정책 9개 테이블 활성화(`auth.uid()=user_id`)·`user_id` NOT NULL·`trades`가 2개 `user_id`로 분리 저장·버튼 소유권 단위 테스트 3건 통과로 **격리·귀속·소유권을 정책/데이터/코드 레벨에서 확인**. 단, 계정 2개·채널 2개의 **교차 경로(각자 채널 수신·타인 버튼 거부)는 실제 클릭 미실시**(NOT VERIFIED, 계정 1개만 가용) → 4주차 이월.
...
- **복기 스키마 개편(0009)**: `reviews.timing`→`execution` 개명·확장(진입/청산이 계획 규칙을 따랐나=통제 가능한 행동) + `reflection_prompt` 신설(성찰 질문 한 줄, 질문형만·매매 지시 금지). `review-agent` 출력 스키마/프롬프트·`TradeDetailPage`·prd 동기화.
- **이연**: 면책 고지 상시 노출(AppLayout/복기 카드/알림/랜딩)은 일부만 반영 — 배포 세션에서 점검.

### WP-H. 라이브 E2E + 문서 마감 (2026-07-24 부분 완료, 잔여 4주차 이월)

1. ✅ **거울 프레임 라이브 검증**(07-24): 복기 재생성 → 미래지시·종목평가·결과론 문구 없음 + `plan_adherence` 생성 확인. 계획 미기록 trade에 "계획 모두 미기록" 정확히 지적, `cited_trade_ids` 4건 인용.
2. 🟡 **멀티유저 라이브 E2E**(07-24 부분): RLS own-row 9테이블·`user_id` NOT NULL·`trades` 2개 `user_id` 분리·버튼 소유권 단위 3건으로 격리·귀속·소유권 확인. 단일계정 감시→알림→기록 루프는 라이브 완주(항목 3). **계정 2개 교차 클릭(각자 채널 수신·타인 버튼 거부)만 미실시 → 4주차 이월**(계정 1개만 가용).
3. ✅ **단일 사용자 회귀 + 감시 라이브 루프**(07-24): 대시보드(관심종목 라이브 시세·최근기록) → 종목 페이지(KIS 차트·마커·조건·기록) → `/trade/:id` 계획 필드 수정·거울 프레임 복기·인용 기록 → 히스토리 그리드(완주 루프 카운트) → 설정 Discord 연동(연동됨) 전 경로 정상, 콘솔 에러 0. **감시 라이브**: 웹 조건 생성 → `monitor` → 조건 충족 → `alerts` + Discord 알림 → 매수 버튼 원클릭 → `trades` 기록(소유자 귀속) 완주.
4. ✅ 본 문서 §1 스냅샷 갱신(07-24). Notion 3주차 보드 갱신은 인증 필요 — 별도 처리.
