...
| Conformance | Checkpoint `bc2e5355`의 production bridge exact-local gate가 official `MockResponsesServer`와 native `0.144.4`를 관통해 T0, native item identity/FIFO, authoritative terminal, interrupt, follow-up, reap과 same-home effective `never + readOnly` state를 검증했다. Deterministic fake와 exact-local은 green이고, explicit disposable provider/auth 부재로 live provider는 blocked다. |
