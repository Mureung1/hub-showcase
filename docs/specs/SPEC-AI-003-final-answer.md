...
**판단 주체를 아예 표기하지 않는다** (2026-07-31 확정).

초안은 "§12.5의 분류 규칙을 지킨다"였다. 즉 자동 통과와 사용자 판단을 **구분해서** 표기하라는 뜻이었으나, 구현 중 **구분 자체를 없애는** 쪽으로 바꿨다.

| | 분기가 있으면 | 분기가 없으면 |
|---|---|---|
| 정확성 | 분기가 틀리면 T-019.6과 같은 결함 | **틀릴 수가 없다** |
| 정보 | 자동/수동 구분이 노트에 보인다 | `resolutionReason`이 DB에 남으므로 손실이 아니다 |

**대체 노트는 fallback이고, fallback에서는 정교함보다 정확함이 우선이다.** 쓸 수 있는 문구를 쓰지 않는 것은 거짓을 만들지 않는다. T-019.6에서 `auto_single_source`를 "내 결정 반영"으로 표기한 결함이 정확히 이 분기에서 나왔다.
...
### 8.1 두 경로 — 전달 방식이 다르다 (2026-07-31 확정)

| 경로 | 스트림 상태 | 전달 방식 |
|---|---|---|
| **충돌 0건** | 아직 열려 있다 | **SSE** — `final_answer.progress` → `final_answer.done` |
| **충돌 있음** | 사용자 판단 중에 닫혔다 | **PATCH 즉시 반환 + web이 GET 폴링** |

**충돌 있는 경로에서 SSE를 다시 열지 않는다.** 인증·heartbeat·종료 판정·화해를 다시 다뤄야 하는데, 얻는 것이 작다.

**PATCH는 생성을 기다리지 않고 즉시 반환한다.** 생성이 30~60초 걸리므로 응답을 붙잡고 있으면 HTTP 타임아웃 위험이 있고 사용자는 아무 표시 없이 대기하게 된다. 대신 "생성이 시작됐다"를 알리고 web이 `GET`으로 폴링한다.

**폴링이 SSE보다 나은 이유가 여기서는 분명하다.**

- `GET .../final-answer`는 새로고침 복원용으로 어차피 필요하다 — **경로가 하나로 줄어든다**
- 결과가 **하나뿐**이라 쟁점별 조기 표시 같은 세밀함이 필요 없다. 스피너 하나면 충분하다
- 폴링 간격 3~5초 × 30~60초 = 10~20회. 부담이 없다

`final_answer.progress`는 **충돌 0건 경로에서만** 의미가 있다. 충돌 있는 경로의 진행 표시는 web이 고정 문구로 한다.
...
| `QuestionStreamEventSchema` | `final_answer.progress`·`final_answer.done` 추가 (충돌 0건 경로 전용) |
| PATCH 응답 | **변경하지 않는다.** FinalAnswer를 싣지 않고 web이 GET 폴링한다 (§8.1) |
...
| 일자 | 내용 |
|---|---|
| 2026-07-31 | **§5.2 대체 노트에서 판단 주체 표기를 제거** (T-020.1 구현 판단 반영). 초안은 자동/수동을 구분해 표기하도록 했으나, 그 분기가 T-019.6 결함의 발생 지점이었다. fallback에서는 정교함보다 정확함이 우선이므로 **구분 자체를 없앤다** — `resolutionReason`이 DB에 남아 정보 손실도 아니다 |
| 2026-07-31 | **§8.1 신설 — 두 경로의 전달 방식 확정.** 초판이 "PATCH 응답으로 받거나 GET으로 조회"라고만 적어 구현 판단을 남겨뒀다. 충돌 0건은 SSE, 충돌 있음은 **PATCH 즉시 반환 + GET 폴링**으로 확정. SSE 재개방은 인증·화해를 다시 다뤄야 해 비용 대비 이득이 작고, `GET`은 복원용으로 어차피 필요하므로 경로가 하나로 줄어든다. PATCH 응답 계약은 바꾸지 않는다 |
