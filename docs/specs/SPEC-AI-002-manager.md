...
| **단계 6 (판정)** | **100초** | 실측 p90 65~66초. 45초로는 **호출의 40~45%가 초과**해 재시도되어 지연이 배가됐다 |

45초는 원래 SPEC-AI-001의 3사 호출 기준을 그대로 가져온 값이고, 긴 추론을 동반하는 단계 6의 특성에 맞지 않았다. **타임아웃을 올려 지연을 줄이는 역설**이며, 근거는 §14.5의 분위수 실측이다.
...
#### 14.5.5 범위 밖 발견 — 타임아웃이 지연의 숨은 주범

**단계 6 호출의 40~45%가 45초 타임아웃을 초과한다.** 초과하면 재시도되어 호출 수와 지연이 배가된다. T-019.3에서 관측된 88,234ms는 "45초 타임아웃 + 43초 재시도"였을 가능성이 높다.

p90이 65~66초이므로 **타임아웃 100초면 대부분이 재시도 없이 통과**한다. §2.4를 개정했다.
