...
한 회사에서만 나타나는 표현은 차원이 아니라 그 회사의 특징이며 편차 해석이 다룬다. 승격 임계값은 `taxonomy_policy_version`으로 관리한다.
...
| 검사 | 내용 |
| --- | --- |
| 분모 일치 | 모집단이 범위·기간 정의와 일치한다 |
| 중복 제거 | `posting_version_id` 단위로 중복이 제거되었다 |
| 표본 판정 | `sample_status`가 정책 버전의 임계값과 일치한다 |
| 재계산 일치 | 독립 재계산 결과가 저장값과 일치한다 |
| 버전 일치 | `taxonomy_version`, `metric_policy_version`이 실행 컨텍스트와 일치한다 |
| 적용 가능성 | 적용 불가로 표시된 차원·지표 조합이 계산되지 않았다 |

검사 절차와 판정 형식은 [에이전트 설계](agent-design.md)를 따른다.
