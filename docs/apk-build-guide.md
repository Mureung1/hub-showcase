...
- **로그인**: 이메일/비밀번호 로그인은 서버 API 호출이라 앱에서 그대로 동작합니다. 구글 OAuth는 임베디드
  웹뷰를 구글이 막는 경우가 있어, 앱에서 매끄럽게 쓰려면 추후 네이티브 구글 로그인(또는 시스템 브라우저 +
  딥링크) 전환이 필요합니다 — 데모 APK 단계에선 이메일/비밀번호를 권장합니다.
...
- [ ] 로그인: **이메일/비밀번호**로 동작 확인(구글 OAuth는 임베디드 웹뷰 제약 있음 — 6번 참고)
