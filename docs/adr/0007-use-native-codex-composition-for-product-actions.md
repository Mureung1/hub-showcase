...
- 반복 가능한 학업 작업은 versioned `ModelingRecipe`로 정의한다. Recipe는 사용할 Skill, parameterized prompt template, argument contract와 output schema를 묶는 정적 정의다.
- 학생-facing action은 Recipe를 선택하고 입력을 모으는 UI 명령이다. 별도 영속 `ProductAction` 객체로 만들지 않는다.
...
- 첫 구현 gap은 ModelingInvocation을 만들고 native Codex input으로 번역한 뒤 ModelingRun과 구조화 결과를 연결하는 composer다.
