...
- Root `npm run dev -- --app-data-root <absolute-path>`가 유일한 supported local development composition entrypoint다. 이 composition은 `packageRoot`에서 verified Runtime artifact를 찾고 explicit `appDataRoot` 아래의 app-managed `HOME`, `CODEX_HOME`, `CODEX_SQLITE_HOME`, temporary/runtime state를 계산한다. Caller가 여섯 `CODEX_CHAT_*` path를 조립하지 않으며 ambient home, repository-local state 또는 `process.cwd()`로 fallback하지 않는다. Repository의 dogfood helper는 외부 persistent profile을 준비·검증한 뒤 이 canonical composition에 위임할 수 있지만 별도 Server·Runtime topology나 compatibility entrypoint를 만들지 않는다.
