...
- Startup은 shared listener bind와 private Broker route·binding 준비가 성공한 뒤 prepared SemesterWorkspace의 Runtime을 시작한다. Exact Git root native thread를 먼저 시작해 native trust write와 project config reload를 일으키고, complete effective static declaration과 live Adapter lifecycle을 각각 검증한 뒤 root·identity를 확인해야만 registry active pointer를 commit한다. Listener bind·Broker preparation이 실패하면 Runtime을 spawn하지 않고, stale token·binding이나 closed Broker의 request는 fail closed한다.
- Adapter는 startup 때 짧은 authenticated handshake를 완료한 직후 authenticated `lifecycle_open` HTTP request를 하나 열고, Broker의 `lifecycle_accepted` prefix를 받은 뒤에도 response를 generation 수명 동안 유지한다. 이 private lifecycle client에는 response-body timeout을 두지 않으며 explicit abort, Broker EOF 또는 process termination만 channel을 닫는다. Adapter는 이 live channel이 승인된 뒤에만 MCP initialize를 성공시킨다. 이후 capability MCP call 하나는 별도의 authenticated Broker HTTP POST 하나로 운반하고, Broker는 request를 Browser UI에 투영한 뒤 정상 사용자 result 또는 MCP failure가 정산될 때까지 그 response를 pending으로 유지하며 terminal response를 정확히 한 번만 쓴다.
...
- Browser에는 App-owned opaque interaction identity만 투영한다. Browser answer는 현재 pending call 하나만 settle하며 duplicate·late answer는 두 번째 result를 만들지 않는다. Turn interrupt에 따른 MCP cancellation·STDIO EOF·capability HTTP abort·live lifecycle channel loss, Browser disconnect, Runtime terminal·replacement와 App shutdown은 정상 result를 만들지 않고 pending call을 MCP failure path로 terminal 정산한다.
...
- `@ay-ple/codex-chat-runtime`은 capability-neutral하게 Codex child environment를 전달하고 native effective config를 bounded projection으로 제공한다. `@ay-ple/interaction-mcp`를 import하거나 `propose_state_patch` schema, Broker endpoint, live Adapter health와 UI lifecycle을 소유하지 않는다. Runtime은 `waitForMcpServerReady` 같은 readiness port를 제공하거나 official MCP status를 poll하지 않는다.
- App 실행 전 native Bootstrap Skill은 Git-tracked `<SemesterWorkspace>/.codex/config.toml`에 AY-PLE Interaction MCP의 정적 project declaration을 설치한다. 이 declaration은 exact SemesterWorkspace root에서 `hub/packages/interaction-mcp`의 built STDIO entrypoint까지 계산한 상대 `command`, 전달할 세 environment variable 이름, capability allowlist와 `required = true`를 기록하고 `args`, static `env`, MCP server `cwd`, `tool_timeout_sec`, `disabled_tools`와 explicit `enabled`는 생략한다. Startup이 읽는 complete effective shape에서는 각각 empty `args`, empty static `env`, `cwd = null`, `tool_timeout_sec = null`, empty `disabled_tools`와 `enabled = true`여야 한다. Current pinned launcher는 생략된 server `cwd`를 Workspace Runtime의 exact root `cwd`로 fallback하고 native MCP tool timeout 300초를 적용한다. Exact executable filename과 env 이름은 implementation spec이 고정한다.
...
- STDIO Adapter는 process 시작이나 handshake만으로 initialize를 성공시키지 않는다. Environment binding을 검증하고 현재 App-side Broker와 인증된 handshake를 완료한 뒤 persistent lifecycle request를 열어 `lifecycle_accepted`까지 받아야 MCP initialize를 성공시킨다. Endpoint·token 누락, Broker offline, authentication 실패나 capability mismatch는 required MCP initialization failure이며 prepared-workspace startup을 실패시킨다. STDIN EOF·error와 termination signal은 handshake와 lifecycle creation을 함께 abort하며, 중단 뒤 늦은 handshake가 새 lifecycle을 열지 않는다.
- App startup은 Runtime의 effective config projection에서 expected `ay_ple_interaction` declaration의 complete static shape를 검증하고, Broker-owned `adapterStatus`에서 live lifecycle acceptance를 확인한다. Explicit `untrusted`처럼 project config 자체가 무시되어 declaration이 보이지 않거나 command·args·unsourced env names·`cwd`·timeout·static env·enabled/required·tool allowlist가 하나라도 다르거나 `disabled_tools`가 non-empty이면 실패한다. 다른 user MCP server의 valid `env_vars = [{ name, source = "local" | "remote" }]`는 source를 잃지 않고 projection하되 Required Interaction declaration의 세 항목은 모두 `source = null`이어야 한다. Native Codex가 allowlist 뒤 denylist를 적용하므로 expected tool을 다시 끄는 declaration은 허용하지 않는다. `adapterStatus`는 unexpected lifecycle channel loss를 동기적으로 latch해 active commit과 active generation을 차단하며, expected Broker close나 App shutdown은 Adapter loss로 보고하지 않는다.
...
- Prepared-workspace startup이 trust·declaration·live lifecycle·root identity를 검증한 그 exact thread를 Product Turn이 이어서 사용한다. 별도의 Product thread나 health-authority thread를 다시 만들지 않으므로 registry commit을 승인한 Adapter와 실제 사용자 Turn의 Adapter가 갈라지지 않는다.
