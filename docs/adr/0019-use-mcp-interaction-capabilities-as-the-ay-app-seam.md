...
- `propose_state_patch`는 첫 InteractionCapability로 유지한다. 요청은 사용자가 검토할 변경 요약·비교·선택적인 `EvidenceRef`를 담고, 결과는 `accept | revise | reject`와 필요한 경우 feedback을 반환한다. `StatePatch`는 이 호출 동안의 transient presentation payload이고 `UserConfirmation`은 별도 durable App entity가 아니라 그 호출의 사용자 result다.
