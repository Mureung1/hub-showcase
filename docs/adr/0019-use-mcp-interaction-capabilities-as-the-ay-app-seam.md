...
- Adapter는 startup 때 짧은 authenticated handshake 하나를 완료하고, 이후 capability MCP call 하나를 authenticated Broker HTTP POST 하나로 운반한다. Broker는 request를 Browser UI에 투영한 뒤 사용자 result 또는 명시적인 cancellation·failure가 정산될 때까지 그 HTTP response를 pending으로 유지하고 terminal response를 정확히 한 번만 쓴다.
- Browser에는 App-owned opaque interaction identity만 투영한다. Browser answer는 현재 pending call 하나만 settle하며 duplicate·late answer는 두 번째 result를 만들지 않는다. MCP cancellation·STDIO EOF·HTTP abort, Browser disconnect, Runtime terminal·replacement와 App shutdown은 pending call을 terminal 정산한다.
- Broker는 `202` acknowledgement 뒤 polling, callback URL, persistent WebSocket session, durable outbox나 response replay journal을 만들지 않는다. Terminal response가 Adapter에 전달되기 전에 continuity를 잃으면 call은 성공으로 추정하지 않고 실패·cancellation으로 끝낸다. AY가 계속하려면 fresh capability call을 보내며 App은 이전 사용자 선택을 workspace에 대신 적용하지 않는다.
- Current pinned Codex의 MCP tool call 기본 timeout은 300초다. Human Review를 이 기본값에 맡기지 않고 project MCP declaration에 명시적인 human-decision `tool_timeout_sec`을 둔다. Exact seconds는 별도 후속 결정으로 고정한다.
