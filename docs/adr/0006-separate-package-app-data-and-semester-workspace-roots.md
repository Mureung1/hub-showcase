...
- 과거 Runtime Harness의 저장 경로나 local residue를 새 authority로 채택하거나 통째로 이관하지 않는다. Runtime은 canonical appData에 재생성·검증하고 사용자가 소유한 실제 SemesterWorkspace만 보존한다. `../.ay-ple-dogfood/`, `../.ay-ple-dev-workspaces/`와 `packageRoot` 안의 Runtime `.artifacts/`는 새 layout의 end-to-end smoke 뒤 scoped cleanup한다.
...
Current development composition이 verified Runtime byte를 package-local ignored `.artifacts/`에 materialize하고 별도 `../.ay-ple-dogfood/`를 쓰는 것은 전환 전 구현 사실이다. Adopted target에서는 Runtime dependency를 `../.ay-ple/runtime/`에 두고 source·build output과 실행 payload를 분리한다. 중단한 public target의 배치 결정과 당시 구현 근거는 historical ADR 0016이 보존한다.
