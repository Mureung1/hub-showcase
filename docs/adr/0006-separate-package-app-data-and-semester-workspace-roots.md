...
부분 대체됨: [ADR 0014 — SemesterWorkspace를 app-owned normalized scaffold로 생성한다](0014-create-app-owned-normalized-semester-workspaces.md)가 사용자가 고른 기존 폴더를 `workspaceRoot`로 직접 채택하는 admission·identity 의미를 대체한다. 이 ADR의 세 root 분리와 수명·복구 책임은 유지한다.
