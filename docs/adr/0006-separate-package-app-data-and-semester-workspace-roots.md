...
| `appDataRoot` | Canonical sibling `../.ay-ple/`의 cross-workspace 운영 metadata·config, Runtime payload, cache·temp와 `WorkspaceRegistry` | 전역 Codex account·config·session, 학기 identity, workspace 내부 구조·Git history, 확인된 학업 사실과 사용자 원본, pending interaction·operation 상태 |
...
- Pending InteractionCapability, product operation과 Adapter health는 현재 process의 Runtime generation에만 결합한다. App은 이를 durable recovery record로 저장하지 않고 terminal event에서 정산하며, restart 뒤 사라진 operation의 결과를 추정하지 않는다.
