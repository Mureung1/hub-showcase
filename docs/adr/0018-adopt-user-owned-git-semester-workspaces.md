...
- Active SemesterWorkspace가 없을 때 AY-PLE은 `hub/`를 exact `cwd`로 쓰는 **Bootstrap Runtime·thread**를 연다. 이 thread는 `hub/.agents/skills/`의 Bootstrap Skill을 native discovery하는 setup context이며 SemesterWorkspace의 학업 thread가 아니다.
- Bootstrap Skill이 선택한 candidate directory에서 Git·최소 workspace file·built-in Skill copy와 첫 checkpoint를 준비하면 App은 명시적인 activation transition을 시작한다. Candidate는 이 transition이 성공하기 전에는 durable active SemesterWorkspace가 아니다. Exact handoff transport는 후속 lifecycle spec이 소유한다.
- Activation transition은 Bootstrap Runtime·thread를 종료하고 [ADR 0019](0019-use-mcp-interaction-capabilities-as-the-ay-app-seam.md)의 shared listener·Broker binding을 준비한 뒤, 같은 전역 Codex account를 사용하는 새 Workspace Runtime·thread를 exact candidate root와 `workspace-write` permission으로 시작한다. Current pinned App Server가 trust 미지정 Git root를 native user config에 기록하고 같은 start에서 project config를 reload하며, 명시적 `untrusted`는 보존한다. Required MCP handshake를 포함해 새 Runtime이 준비되면 canonical root를 `WorkspaceRegistry`의 known·active workspace로 기록한다. Bootstrap thread를 target root로 resume하거나 cwd만 바꾸지 않는다. App이 소유하는 것은 이 두 Runtime phase와 cwd 전환이며, Git·file·Skill 설치 동작은 Bootstrap Skill이 소유한다.
...
- 앱은 workspace와 독립된 appData에서 Codex account 연결을 먼저 완료할 수 있다. 연결 뒤 active workspace가 없으면 hub-rooted Bootstrap Runtime을 열고, 초기화·activation이 끝나면 선택한 root를 exact `cwd`로 사용하는 Workspace Runtime으로 전환한다.
...
- 처음 여는 workspace에서는 AY의 instruction-based init Skill을 실행한다. 이 Skill이 일반 file·Git 도구로 Git 초기화, 최소 `AGENTS.md`, `workspace-state.json`, built-in Skill copy, 정적인 `.codex/config.toml`과 첫 checkpoint를 준비한다. Exact deterministic generation이 실제로 필요해지기 전에는 별도 scaffold script를 두지 않는다.
...
- Bootstrap은 `.codex/config.toml`에 hub-owned `@ay-ple/interaction-mcp` package의 built STDIO entrypoint, 전달할 env 이름·capability allowlist, explicit human-decision `tool_timeout_sec` 같은 정적 declaration만 설치한다. Entry point `command`는 exact SemesterWorkspace root에서 current `hub/` artifact까지 계산한 상대경로이며 MCP server `cwd`는 생략한다. Current pinned local STDIO launcher는 이 경우 Workspace Runtime의 exact root `cwd`를 fallback으로 사용한다. App endpoint·token·Runtime binding은 기록하지 않으며 App이 Codex child environment로 공급한다. 정상 Runtime은 이 project config를 native loading하고 App은 MCP config를 CLI·thread override로 다시 주입하지 않는다.
