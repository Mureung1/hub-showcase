...
- ActionInvocation의 `WorkspaceFileRef`는 실행 시 고정한 relative path 목록을 current-path semantics로 전달하며 file bytes나 content version을 고정하지 않는다. Version-bound 근거가 필요한 Review에는 exact digest와 locator를 가진 `EvidenceRef`를 사용한다. App은 path reference와 actual reader 사이를 snapshot이나 열린 handle로 이어 붙이지 않고, AY·Skill이 실제 file을 읽고 작업 시점의 drift를 판단한다.
