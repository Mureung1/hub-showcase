...
- Activation transition은 Bootstrap Runtime·thread를 종료하고 같은 전역 Codex account를 사용하는 새 Workspace Runtime·thread를 exact candidate root에서 시작한다. 새 Runtime이 준비되면 canonical root를 `WorkspaceRegistry`의 known·active workspace로 기록한다. Bootstrap thread를 target root로 resume하거나 cwd만 바꾸지 않는다. App이 소유하는 것은 이 두 Runtime phase와 cwd 전환이며, Git·file·Skill 설치 동작은 Bootstrap Skill이 소유한다.
