...
- Native Bootstrap은 사용자가 지정한 directory를 독립 Git root로 준비하고 root `workspace-state.json`, 최소 `AGENTS.md`, `hub/skills/` catalog의 모든 valid built-in을 담은 workspace-local `.agents/skills/` copy, 정적인 `.codex/config.toml`과 review 가능한 Git checkpoint를 만든다. Catalog 전체 validation과 destination conflict planning은 workspace mutation보다 먼저 끝내며 invalid Skill root나 divergent installed destination이 하나라도 있으면 일부 built-in이나 다른 managed file을 쓰지 않고 전체 실행을 fail closed한다. 이 validation은 installability와 identity를 위한 최소 구조에 한정하고 별도 Skill schema framework나 content lint를 만들지 않는다. Existing repository의 history, remote, user bytes와 dirty working tree를 존중한다.
- Current development lifecycle에서 built-in Skill materialization은 fresh SemesterWorkspace를 대상으로 한다. Existing installed tree가 source와 다르면 Bootstrap은 original bytes를 보존하고 no-clobber conflict로 중단하며 이를 refresh·replace·merge lifecycle로 확장하지 않는다.
...
- Explicit `--workspace` 선택이 전체 startup readiness를 통과하면 registry transaction은 같은 canonical root의 이전 `workspaceId` binding을 새 verified identity로 CAS 교체하고 다른 workspace entry를 보존한다. 이는 Bootstrap이 같은 경로에 fresh SemesterWorkspace를 만든 경우에도 App이 소유하는 launch-time selection 결과다. 반대로 인자 없는 reopen은 registry root와 fresh identity가 불일치하면 `identity_mismatch`로 fail closed하며 stale binding을 추측해 고치지 않는다.
...
- App startup은 `hub/skills/` source를 읽거나 installed built-in roster·bytes를 재검증하지 않는다. Workspace-local native Skill discovery는 Runtime context로 사용하고, specific built-in의 availability는 이를 요구하는 ActionInvocation이 실행 직전에 fail closed로 판정한다.
...
- Authenticated Interaction MCP handshake와 required server·tool readiness가 성공한 뒤에만 root를 `WorkspaceRegistry`의 known entry와 active pointer로 commit한다. Transaction acceptance 전 실패하면 기존 registry authority와 bytes를 보존하고 degraded AY-PLE mode로 계속하지 않는다.
