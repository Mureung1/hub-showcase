...
- [ ] 라우트 골격 구성: `/`, `/onboarding/:step`, `/onboarding/complete`, `/home`, `/subsidies/:id`
- [ ] `AppShell` 적용: 모바일 full, PC에서는 390x844 프레임(center + radius + shadow)
- [ ] 와이어프레임 스타일 포팅: `src/styles/tokens.css` 기준으로 `app.css` 구조 정리, Pretendard 로드
- [ ] 온보딩 상태 저장 구조 설계: Context 또는 sessionStorage 스키마 정의 (업종/지역/구군/직원수/연매출)
- [ ] 온보딩 화면 구현(1~4): 각 step의 선택/입력 완료 전 `다음` 비활성 처리
- [ ] 완료 화면 구현: 입력 요약(업종/지역/직원수/연매출) + 홈 이동 CTA
- [ ] 홈 화면 mock 구현: 헤더, 정렬 칩(전체/마감임박/지원금액/신규), 리스트 라벨, 카드, 하단 탭 placeholder
- [ ] 계획 수립 Agent 산출물 정리: 작업 분해/우선순위/완료 기준 템플릿 문서화

월요일 완료 기준:
- mock 데이터만으로 `Welcome -> 온보딩 4스텝 -> 완료 -> 홈` 흐름이 끊기지 않고 동작한다.
- 모바일 화면 기준(390px)에서 UI가 와이어프레임 톤과 구조를 유지한다.

### 화요일 TODO (모바일 상세 + 데이터 계층/API 계약)
- [ ] 상세 화면 구현(모바일): 상단 정보, 요약 카드(금액/매칭/마감/방식), 자격/서류/신청방법 섹션
- [ ] 외부 신청 CTA 연결(mock): 하단 고정 버튼 + 접수처 링크 동작
- [ ] 공용 컴포넌트 분리: `SubsidyCard`, 상태 박스(로딩/에러/빈결과), D-day 뱃지 유틸
- [ ] TanStack Query 세팅: query client, 목록/상세 query 훅, 제출 mutation 훅 골격
- [ ] API client 래퍼 추가: mock fallback 가능한 `getSubsidies`, `getSubsidy`, `submitProfile` 인터페이스
- [ ] 공유 타입 보강: `profiles/match` 요청·응답 타입을 `shared/`에 정의
- [ ] 홈 정렬 동작 구현(mock 기준): 매칭도/마감임박/지원금액 정렬 토글 확인
- [ ] 기능 검증 Agent 산출물 초안: 월/화 구현 범위에 대한 점검 체크리스트 작성

화요일 완료 기준:
- 모바일에서 `홈 리스트 -> 상세 -> 외부 신청 링크` 흐름이 동작한다.
- FE 데이터 계층(Query + API client + shared contract)이 준비되어 수/목 DB 연동으로 자연스럽게 전환 가능하다.
