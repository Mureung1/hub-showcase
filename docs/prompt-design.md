...
LLM 응답을 받은 뒤 프론트엔드 또는 서버에서 다음을 확인한다.
...
```text
이전 응답은 지정된 JSON 스키마를 지키지 않았다.
설명 문장이나 Markdown을 제거하고, 아래 스키마에 맞는 JSON만 다시 반환해.
입력에 없는 내용은 만들지 말고, 불확실하면 status를 "unclear"로 표시해.
```
