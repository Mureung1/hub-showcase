...
| T24 | 감각 강도 조절 + 테마별 배경음 (이슈 #59) | 3 | T22 | C24 | — | 완료 |
| T25 | 같이 있어요 (장식용 동시접속 표시, 이슈 #60) | 3 | — | C25 | — | 완료 |
| T26 | 통계 화면 (이번 주 완료 현황, 이슈 #61) | 3 | — | C26 | — | 완료 |
...
### T24 — 감각 강도 조절 + 테마별 배경음
- Phase 2 백로그 항목. `docs/prototype/feature-proposals.html` 03번 프레임(다이얼 디자인) 기반, 색·소리·움직임 세기를 사용자가 직접 맞추게 한다.
- 메인 다이얼("차분하게 ↔ 생기있게")이 캐릭터·테마 장식 채도와 애니메이션(타이머 전환 효과)을 함께 조절. 소리는 다이얼과 별도인 작은 스피커 아이콘으로 켜고 끄되, 볼륨 자체는 다이얼 값을 따라간다(컨트롤 개수를 늘리지 않기 위해).
- 테마별 배경음(Mixkit 무료 라이선스 mp3, `public/sounds/day.mp3`·`night.mp3`·`forest.mp3`·`cafe.mp3`) 반복 재생.
- 선행: T22(화이트노이즈 테마, 완료). 순수 화면/클라이언트 상태 작업이라 S 계약은 생략.
- 종료: C24.

### T25 — 같이 있어요 (장식용 동시접속 표시)
- Phase 2 백로그 항목. `docs/prototype/feature-proposals.html` 04번 프레임 기반. One-Focus View에 작은 점 몇 개(그 중 하나는 "나")와 "지금 N명이 같이 집중 중이에요" 문구를 보여준다. 이름·채팅 없음을 문구로 명시.
- 실제 동시접속자 수가 아니라 장식용(260802 결정 — 진짜 구현은 T21급 멀티유저 인증 구조가 필요). 숫자는 날짜 기준으로 하루 동안 고정되는 값을 씀(매번 랜덤하게 바뀌지 않게).
- "혼자 할래요" 링크로 표시를 끌 수 있고, localStorage로 새로고침해도 유지된다.
- 구현 시 프로토타입과 다르게 간 부분: 캐릭터가 화면 하단을 크게 차지해서 프로토타입처럼 화면 중앙에 헤드라인+점 묶음으로 배치하면 캐릭터에 가려짐. 대신 상단 좌측(SensoryControl 반대쪽)에 점+숫자+닫기 버튼만 있는 작은 알약으로 압축, "이름도 채팅도 없다"는 문구는 hover 툴팁으로 이동.
- 선행: 없음. 순수 화면/클라이언트 상태 작업이라 S 계약은 생략.
- 종료: C25.

### T26 — 통계 화면 (이번 주 완료 현황)
- Phase 2 백로그 항목. `docs/prototype/design-board.html` 07번 프레임("대시보드 대신 숫자 하나") 기반. 이번 주(최근 7일) 중 며칠 완료했는지 큰 숫자 + 요일별 점 7개로 보여준다.
- Steps DB의 `Done=true`·`CompletedAt` 실데이터 기준(mock 아님). 그래프·표 등 대시보드 형태는 만들지 않는다(디자인 원칙).
- 입력 화면에 작은 링크로 진입점을 두고, 홈으로 돌아올 수 있게 한다.
- 선행: 없음(`CompletedAt`은 T10에서 이미 기록 중).
- 종료: C26.
