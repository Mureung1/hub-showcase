...
남은 개발(Feat-2 잔여 ~ Feat-5)을 구현 단위(T01~T22)로 나눈 것. 각 Task는 선행조건이 `완료`일 때만 착수한다. 상태는 `대기` | `진행중` | `완료` | `BLOCKED(사유)`.
...
| T04 | Full Screen Timer 지속성 (새로고침 내구성) | 3 | T03 | C04 | — | 완료 |
...
| T08 | 이유 칩 + 제안/수락/거절 UI | 4 | T06 | C08 | S2 | 완료 |
| T09 | outcome 기록 (완료 즉시 + 다음 방문 일괄) | 4 | T05, T08 | C09 | S4 | 완료 |
| T10 | 개인화 (최근 로그를 판단 프롬프트에 주입) | 4 | T06, T09 | C10 | S2, S3 | 완료 |
...
| T14 | Brain Dump 일정 확인 멀티턴 (기한 미정 항목 최대 2턴 질문 후 확정) | 2 | T02 | C14 | S1 | 완료 |
| T15 | 타이머 종료 시 완료 확인 + Agent 판단 연장 | 3 | T04 | C15 | S6 | 완료 |
| T16 | Brain Dump 음성 입력 | 2 | — | C16 | — | 대기 |
| T17 | 마이크로스텝 검토·삭제 화면 | 3 | T02 | C17 | — | 완료 |
...
| T21 | Notion Public Integration + OAuth 연결 (이슈 #57) | 2 | T18 | C21 | — | 대기 |
| T22 | 화이트노이즈 테마 (낮/밤 동기화·숲·카페) | 3 | — | C22 | — | 완료 |
| T23 | 다시 시작해도 괜찮아 (복귀 환영 메시지, 이슈 #58) | 3 | — | C23 | — | 완료 |
...
- 선행: T10.
- 종료: C11. 계약 S5.
- 260802 결정: Phase 2 백로그(디자인 위주)를 먼저 진행하기로 해서 보류. Phase 2 끝나면 재개.
...
- 종료: C13. 계약 S1.
- 260802 결정: T06이 이미 Solar로 완료됐고 지금은 Phase 2 백로그를 먼저 진행하기로 해서 보류. Phase 2 끝나면 재개.
...
- 선행: 없음.
- 종료: C16.
- 260802 결정: Phase 2 백로그를 먼저 진행하기로 해서 보류. Phase 2 끝나면 재개.
...
### T20 — 타이머 일시정지 + 재개 사유 기록
- `FocusTimer` 진행 중 일시정지 버튼을 누르면 카운트다운이 멈춘다. 전화·다른 일 등 할 일 자체와 무관한 이유로 잠깐 멈추는 용도로, Agent가 tool을 제안하는 "힘들어" 루프(S2)와는 별개다.
- 일시정지 중엔 이유를 선택적으로 적을 수 있고, "다시 시작"을 누르면 멈춘 시간만큼 제외하고 원래 남은 시간부터 이어진다(T04의 `stepStartedAt` 재계산 방식 재사용 — 멈춘 시간만큼 `stepStartedAt`을 뒤로 밀어서 경과 시간 계산에서 빠지게 함).
...
- 260802 결정: 이슈 등록·완료조건 정의까지만 지금 하고, 실제 구현(OAuth 코드 작성)은 Phase 2 백로그 완료 후 별도로 진행.

### T22 — 화이트노이즈 테마 (낮/밤 동기화·숲·카페)
- Phase 2 백로그(dev-plan.md) 항목이지만 우선순위(A→B→D→E→C)를 벗어나 지금 먼저 진행하기로 결정(260802, 디자인 작업 먼저 하고 싶다는 사용자 요청).
- One-Focus View·타이머 화면에서 배경 테마를 낮/밤 동기화·숲·카페 중에서 고를 수 있게 한다. 캐릭터 위치·크기는 그대로 두고 배경색·주변 장식(해/달, 나뭇잎·새, 커피잔·김)만 바뀐다. 실제 오디오 재생은 범위 밖(캡션 문구로만 분위기 표현) — `docs/prototype/whitenoise-themes.html` 참고.
- "낮/밤 동기화" 테마는 실제 현재 시각(주간/야간)에 따라 자동으로 낮/밤 톤이 바뀐다. 선택한 테마는 localStorage로 새로고침해도 유지된다.
- 선행: 없음. 순수 화면/클라이언트 상태 작업이라 S 계약은 생략.
- 종료: C22.

### T23 — 다시 시작해도 괜찮아 (복귀 환영 메시지)
- Phase 2 백로그 항목. 며칠 만에 다시 접속했을 때 스트릭이 끊긴 걸 벌점처럼 보여주지 않고, 복귀 자체를 반갑게 맞이하는 문구를 입력 화면에 보여준다("3일 만이네, 반가워").
- 마지막 방문일을 localStorage에 기록해 오늘과 차이를 계산한다(Notion 쿼리 없이 클라이언트에서만 판단). 오늘 처음 접속이면(차이 없음) 문구를 보여주지 않는다.
- 선행: 없음. 순수 화면/클라이언트 상태 작업이라 S 계약은 생략.
- 종료: C23.
