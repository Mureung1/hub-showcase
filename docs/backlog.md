...
| 3 | 에러 처리 강화 | GET 실패 시 화면에 표시, POST 실패 시 재시도 버튼, 네트워크 끊김 케이스 | P0 | 수 | 대기 | [#8](https://github.com/snael0510-coder/hub/issues/8) |
...
| 요일 | 목표 |
|---|---|
| **월 (오늘)** | 주간 계획 수립 + 이슈 등록 + 진도 점검(완료 확인) |
| **화** | Task 1, 2 — 테스트 프레임워크 세팅 + TDD로 체크인 여부 로직 구현 |
| **수** | Task 3, 4 — 에러 처리 강화 + 아키텍처 다이어그램(mermaid) |
| **목** | Task 5, 6 — 테스트코드 생성 Skill + 코드 검증 Agent 제작, 이번 주 기능 검증 |
