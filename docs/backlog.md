...
| 배포 | 데모용 배포. FE `https://articles-client-nine.vercel.app`(Vercel), BE `https://articles-server-lc3l.onrender.com`(Render). 2026-07-29 배포 환경 점검으로 실제 동작 확인, 상세는 `docs/deploy-verification.md` 참고 | P1 | Done |
...
| 바텀시트 저장/닫기 분리 (신규) | 기존엔 바텀시트를 어떤 경로(닫기 버튼/오버레이 클릭)로 닫아도 `handleCloseSheet`가 무조건 `saveDecision`을 호출해 자동 저장됐다. 사용자 요청으로 "닫기=자동 저장" 동작을 없애고, 하단 버튼을 "인사이트 노트에 저장"(명시적 확정 클릭 시에만 `POST /api/decisions`)으로 바꾸고, 우측 상단에 저장 없이 닫기만 하는 `✕` 버튼(`aria-label="저장하지 않고 닫기"`)을 신설. 오버레이 클릭도 저장 없는 닫기로 통일(모달 바깥 클릭=취소가 일반적인 UX 관례라는 판단, 사용자 확인 없이 결정). `BottomSheet.jsx`의 `onClose` prop을 `onSave`/`onDismiss` 둘로 분리, `ReaderDetail.jsx`의 `handleCloseSheet`도 `handleSaveDecision`(기존 저장 로직 그대로)과 `handleDismissSheet`(단순히 `setPendingDecision(null)`)로 분리했다. 저장 없이 닫은 경우의 완독 로깅은 새 로직을 추가하지 않고 기존 IntersectionObserver 기반 unmount 폴백(`decisionId: null`)에 자연스럽게 위임(판단 버튼을 아예 안 누른 경우와 동일 취급). `BottomSheet.test.jsx`에 저장/닫기 각각의 호출 분리를 검증하는 케이스 추가, client 32개(신규 2개 포함) 테스트 전체 통과(2026-07-29 완료) | P1 | Done |
...
>
> **2026-07-29 갱신 (바텀시트 저장/닫기 분리 구현 완료)**: 사용자가 브라우저
> 실사용 중 "닫기를 누르면 자동으로 인사이트 노트에 저장되는" 기존 동작이
> 의도와 다르다는 피드백을 줘, 바텀시트 하단 버튼을 "인사이트 노트에
> 저장"(명시적 클릭 시에만 저장)으로 바꾸고 우측 상단에 저장 없이 닫는
> `✕` 버튼을 신설했다. 오버레이 클릭도 저장 없는 닫기로 통일하기로는
> 사용자 확인 없이 일반적인 모달 UX 관례에 따라 판단해 결정. `BottomSheet`
> 의 `onClose`를 `onSave`/`onDismiss`로, `ReaderDetail`의 `handleCloseSheet`를
> `handleSaveDecision`/`handleDismissSheet`로 각각 분리했고, 저장 없이 닫은
> 경우의 완독 로깅은 별도 로직 추가 없이 기존 unmount 폴백에 위임했다.
> `BottomSheet.test.jsx`에 저장/닫기 분리 검증 케이스를 추가해 client
> 테스트가 30개→32개로 늘었고 lint/전체 테스트 통과 확인. 이번 갱신 결과
> 4주차 표는 17개 중 10개 완료(Done)/7개 Todo.
