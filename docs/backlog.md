...
- [P0] D2 매칭 로직: 모음소에서 수신자 선정 — 설계 확정(2026-07-21): ① 한 편지가 한 번 추천에 쓰이면 모음소 후보에서 제외(1:1 소진, "이음" 컨셉에 맞춤) ② 이미 나와 매칭된 적 있는 작성자는 재매칭 후보에서 제외("낯선 이와의 새로운 연결" 취지 유지) ③ 모음소에 카테고리별 후보가 임계치 이상 쌓이기 전엔 매칭을 시도하지 않음(정확한 숫자 미정 — 실서비스 오픈을 기간이 아닌 편지 개수 기준으로 게이팅하기 위함). 챌린지 데모용으로는 임계치 대신 시드 데이터(실제 글+LLM 생성 글 20~30통, 카테고리당 3~4통)로 채워서 확인 예정. (기존 "선호 기반"은 B2 미구현으로 카테고리 기반 매칭 유지)
