...
- [x] **T-15 k6 부하 테스트 — 오버셀 증명** `server` ★기술 셀링포인트
  - 락 미적용(`UNSAFE_STOCK=1` 플래그) vs 적용 비교 — 재고 3·동시 100에서 21건 성공(오버셀 +18) → 정확히 3건
  - 판정 근거는 응답 집계가 아니라 DB 불변식(`verify:load`) — 오버셀은 200 응답으로 조용히 통과하기 때문
  - 결과 기록: [최적화.md](./최적화.md) §7.4
  - DoD 충족: 전후 비교 결과를 docs/에 기록
- [x] **T-16 데모 시나리오 정리 + 폴리싱**
  - 리허설 대본: [데모시나리오.md](./데모시나리오.md) — 3분 핵심 루프 + 동시성 하이라이트, 예상 질문, 실패 복구
  - 발표 직전 준비 절차: [시연준비.md](./시연준비.md) — 딜 마감 경과로 목록이 비는 문제 대응
  - 대본의 명령·기대 출력을 실제 실행해 일치 확인
...
- ~~위치 쿼리 최적화~~ → **완료**: PostGIS(geography + GiST)로 전환, 전후 측정 기록 [최적화.md](./최적화.md) §7.2. Redis Geo는 계속 범위 밖
