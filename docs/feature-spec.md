...
- 승인된 병원 정보의 검색 노출을 DB 상태로 강제하는 기능은 P2에서 적용합니다.

### 4.4 검색

- P1에서는 준비된 mock 병원 목록을 병원명, 시·도, 시·군·구와 대표 진료과로 검색합니다.
- `내 주변` 영역은 MVP에서 미리 준비한 병원 데이터를 보여줍니다.
- PostgreSQL 기반 검색 API와 `approval_status = approved` 필터는 P2입니다.
...
| 메서드 | 경로 | 역할 |
|---|---|---|
| `GET` | `/api/hospitals` | P2: PostgreSQL 기반 병원명·지역·진료과 검색과 승인 병원 제한 |
...
- PostgreSQL 기반 병원 검색 API와 승인 병원 노출 제한
- 실제 알림톡·SMS 중계 API
- 소셜 로그인
...
15. P1에서는 승인되지 않은 병원이 대기열을 운영할 수 없고 mock 병원 목록으로 검색 흐름을 검증합니다. P2에서는 실제 DB 검색 결과를 승인 병원으로 제한합니다.
