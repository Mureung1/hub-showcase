...
| 3주차 | 분석 결과 기반 회고 초안 흐름 구현 | 사용자가 분석 결과를 바탕으로 회고 초안과 Markdown 결과를 확인할 수 있음 |
...
| T-24 | Week4 | P0 | Later | 사용자 테스트 및 피드백 반영 | 실제 사용자 피드백을 받아 우선순위가 높은 개선점을 반영한다. |
| T-25 | Week4 | P0 | Later | UX 개선 | 입력, 로딩, 결과, 오류, 빈 상태 흐름이 더 자연스럽게 개선된다. |
| T-26 | Week4 | P0 | Later | 성능 최적화 | 이미지, 번들 크기, API 요청, 로딩 상태를 점검하고 개선한다. |
| T-27 | Week4 | P1 | Later | 내부 UI 유틸 라이브러리 제작 | 버튼, 카드, 상태 배지, 로딩 UI를 직접 만든 재사용 컴포넌트로 정리한다. |
| T-28 | Week4 | P1 | Later | 내부 분석 유틸 라이브러리 제작 | Repository URL 파싱, 기여도 계산, commit message 정규화 로직을 직접 만든 유틸로 분리한다. |
| T-29 | Week4 | P1 | Later | 접근성 점검 | 키보드 접근, aria label, 색상 대비, 오류 안내 문구를 점검한다. |
| T-30 | Week4 | P2 | Later | 최종 배포와 발표 자료 갱신 | 데모 링크, README, 발표 자료, 회고 문서를 최신 상태로 정리한다. |
