...
- 최초 실행 공유는 getPendingShare()가 한 번만 소비하는 메모리 대기열에 넣는다.
- 실행 중 재진입 공유는 shareIntentReceived 이벤트로만 전달한다. 따라서 같은 id가 대기열과 이벤트에 중복 전달되지 않는다.
- 재진입 이벤트가 JavaScript listener 등록 전 도착하면 Capacitor의 notifyListeners(..., true)가 이벤트 payload를 보존한다. 첫 listener 등록 시 보존된 payload를 전달하고 제거하므로 listener는 각 보존 이벤트를 한 번 받는다.
- finishShare()는 공유 결과 완료 시 현재 Android Activity에 종료를 요청한다.

AndroidSharePluginTest는 길이 제한, 실제 Capacitor listener 보존·전달과 초기/재진입 전달 분리를 검증한다. MainActivityShareIntentTest는 실제 onNewIntent 재진입이 보존 이벤트를 발행하고 대기열을 비우는지 검증한다. AndroidSharePluginApiTest는 실제 getPendingShare()·finishShare()가 PluginCall.resolve와 Activity 종료 요청을 수행하는지 검증한다.
...
현재 가능한 가장 가까운 네이티브 검증은 Kotlin 단위 테스트다. PluginCall의 실제 resolve, Capacitor의 실제 listener 보존, Activity.finish() 요청을 모두 호출한다. WebView에 등록된 JavaScript listener가 수신하는 종단 간 검증은 Android 에뮬레이터에서 후속 TypeScript 공유 어댑터가 연결된 뒤 수행한다.
...
2. 실행 중 앱에 공유한 입력이 listener 등록 전에도 shareIntentReceived로 한 번 수신되고 대기열에는 없는지 확인한다.
3. 4097자 text, 501자 title, text/html, ACTION_VIEW 입력이 전달되지 않는지 확인한다.
4. finishShare() 뒤 공유 원본 앱으로 돌아가는지 확인한다.
