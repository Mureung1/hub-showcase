...
| Web Storage API             | 브라우저 내장                                   | 로컬 데이터 저장     | MVP 선택       | 계정·서버 없이 저장과 재발견 가설을 가장 빠르고 저렴하게 검증할 수 있다.                               | IndexedDB, Supabase                                      | 텍스트 데이터만 저장하고 `schemaVersion`과 교체 가능한 저장 어댑터를 둔다.                                |
...
| Supabase JS                 | 2.110.1                                         | BaaS 클라이언트      | 설치됨, 미연동 | Auth, Postgres, Edge Functions를 프론트엔드에서 일관된 클라이언트로 다룰 수 있다.                      | Firebase, 직접 Express API, Appwrite                     | 현재 MVP의 선행 조건이 아니다. 계정·기기 간 동기화 가치가 검증된 뒤 연결한다.                             |
...
2. `Web Storage API`: `schemaVersion`이 있는 로컬 저장 어댑터로 현재 MVP의 저장·재로딩 흐름을 완성한다.
3. `Testing Library`, `jsdom`: 저장, 수정·삭제, 검색, `꺼내보기` 수직 슬라이스를 사용자 행동 기준으로 검증한다.
4. `@wanteddev/wds`: 현재 화면과 디자인 토큰을 유지한다. MVP 중 UI 시스템 전환 작업은 만들지 않는다.
5. `MiniSearch`, `Fuse.js`: 실제 사용자 검증에서 현재 결정적 검색이 실패한 원인을 측정한 뒤 비교한다.
6. `Supabase JS`, `TanStack Query`, `Zod`: 원격 저장·계정·기기 간 동기화 가치가 검증된 뒤 저장 어댑터 교체 작업에서 연결한다.
7. `GSAP`, `Motion`, `Zustand`, `dnd-kit`: 현재 구현을 유지하되 MVP 핵심 흐름에 새 요구가 생기지 않으면 범위를 확장하지 않는다.
...
- `React Router`, `TanStack Query`, `Supabase JS`, `Zustand`, `Motion`, `dnd-kit`, `React Hook Form`, `Sonner`: 설치되어 있지만 현재 로컬 우선 MVP의 선행 조건이 아니다.
