...
**완료 기준**
- [ ] `buildJobDisplay`(`src/lib/gapAnalysis.js`)의 checklist 항목에 카테고리별 원본 요구값(예: `foreignLanguage`의 `job.foreign_lang_test`)을 함께 실어서 노출 — 현재 checklist 항목은 `{category, label, ok, detail}`뿐이라 `detail`은 사람이 읽는 한글 문장이라 이걸 재파싱해서 시험명을 뽑는 방식은 문구가 바뀌면 깨지므로 채택하지 않음
- [ ] 어학 미충족 시 → 시험 종류별(TOEIC / TOEFL / OPIc / TOEIC Speaking, 데이터셋에 존재하는 4종 전부) 공식 사이트 링크 매핑 테이블 작성, 위에서 노출한 원본 요구 시험명 기준으로 정확한 링크 연결
- [ ] 경력 미충족 시 → 사람인(saramin.co.kr) 채용정보 페이지로 고정 링크
- [ ] 자격증 미충족 시 → 큐넷(Q-net, 한국산업인력공단) 단일 링크로 고정. 링크 바로 옆에 "정확한 시행기관은 자격증마다 다를 수 있음"을 작은 캡션(muted 톤)으로 표시 — 27종 전부가 큐넷 소관은 아니라는 걸 사용자가 알 수 있게
- [ ] 학력·전공 항목에는 참고링크 자체를 렌더링하지 않음 (학력무관/전공무관처럼 조건이 없는 케이스 포함)
- [ ] 이미 충족(✓)된 항목에는 참고링크를 노출하지 않음 — 미충족(✕) 항목에만 표시
- [ ] 참고링크는 새 탭으로 열림 (`target="_blank" rel="noopener noreferrer"`)
- [ ] 링크 URL에 사용자의 스펙 값(학력·경력·자격증 등 개인 입력값)이 쿼리 파라미터로 들어가지 않음 — 고정 URL만 사용
- [ ] 기존 `.checklist-detail`/`.checklist-detail-fail` 레이아웃 안에 자연스럽게 들어가도록 배치, 별도 팝업이나 모달을 새로 만들지 않음
- [ ] 결과 화면과 마이페이지(북마크) 상세 모달 양쪽 진입점에서 동일하게 동작 (`JobDetailModal` 하나를 공유하므로 자연히 만족되는지 확인)
- [ ] 데스크톱·모바일 뷰포트 양쪽에서 링크 클릭 영역·줄바꿈이 깨지지 않는지 확인
- [ ] 862건 공고를 순회해 어학 4종 각각 최소 1건, 경력 미충족 케이스, 자격증 미충족 케이스에서 링크가 각각 올바른 대상으로 연결되는지 확인
