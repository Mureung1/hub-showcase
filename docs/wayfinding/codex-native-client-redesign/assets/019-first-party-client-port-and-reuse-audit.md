...
이 문서는 exact pin `767822446c7a594caa19609ca435281a9ec67e0d`의 first-party client와 UI projection을 기준으로 `CodexAppServerConnection → CodexConversationRuntime` 목표와 현재 `packages/runtime-codex` 구현의 disposition을 교정한다. 조사 입력인 `artifacts/pro-bridge/0714-chat/question.md`와 `artifacts/pro-bridge/0714-chat/answer.md`는 사용자 제공 원문으로 보존하고, 아래 판정은 exact source를 다시 대조해 기록했다.

## 판정

두 module 경계는 유지하되 내부 알고리즘을 줄인다.

| 유지하는 경계 | Source-guided baseline |
| --- | --- |
...
이 항목이 실제 AY-PLE 요구로 다시 필요해지면 upstream baseline이 아니라 별도 deviation으로 근거·비용·oracle을 기록한다.

## Exact-pin first-party behavior

| 근거 owner | 확인한 동작 | TypeScript port에 주는 제약 |
| --- | --- | --- |
...
Python SDK는 exact commit의 source behavior reference다. 배포 metadata의 native runtime pin이 npm pin과 자동으로 같다는 뜻은 아니므로 현재 goal에서 production dependency로 채택하지 않는다. Generated TypeScript·JSON Schema는 계속 wire shape만 소유하고 ordering·identity·terminal 의미를 소유하지 않는다.

### Validation failure 경계

| Failure | Baseline disposition |
| --- | --- |
...
Generated validation 자체는 JavaScript external boundary의 hardening이지만 failure scope도 first-party active routing보다 강한 global policy로 자동 확대하지 않는다.
...
| --- | --- | --- | --- |
| `src/internal/codex-app-server-protocol/generated/**`, response-contract maps와 generation provenance | 그대로 보존 + validator owner 개조 | Exact-pin generated types/schema, typed result/error response contract | 현재 initialize-only Client response validator 범위를 T0 adopted method까지 확장하고 notification validation을 추가한다. Generator는 destructive output replacement 전에 safe staging·verification을 둔다. |
...
| `src/raw-client.ts`, `src/adapter.ts`, `src/status.ts`, `src/smoke.ts` | developer-only Runtime Harness에 보존 | 현재 server/Inspector의 single-run diagnostic path와 live smoke | `String(id)` routing, raw/unbounded notification queue와 broad shallow wrappers를 production Connection/Runtime 적합성 근거로 승격하지 않는다. Foundation primitive를 이 class 위에 계층화하지 않는다. |
| `src/testing/fake-codex-app-server.ts` | developer-only Runtime Harness에 보존 | 현재 adapter/server parity test와 journal/temp cleanup | Response-first one-shot scenario를 T0/T0-C/T0.1 conformance oracle로 사용하지 않는다. |
| `src/testing/fake-codex-stdio-transport.ts` | upstream behavior에 맞춰 scaffolding을 추출·개조 | Actual child spawn, journal, temp directory와 cleanup/fault injection pattern | Legacy transport scenario strings, tombstone/reuse/`dismiss()` 의미는 버린다. 새 fake는 TypeScript build/typecheck 경계 안의 typed T0/T0-C/T0.1 scenario table을 사용한다. |
...
| Tracked `dist` 및 stale artifacts | generated build output로만 취급 | Guarded clean build 결과 | Source owner나 compatibility evidence로 사용하지 않고 교체 뒤 removed symbol이 없는지 검사한다. |
...
## 기존 결정의 current disposition

| 기존 결정 | 보존 | Supersede·축소 |
| --- | --- | --- |
...
| Ticket 014 | Replace-don't-layer, Runtime Harness/native data 보호, generated/schema/layout/JSONL primitive 선별 이식과 T0·T0-C·T0.1 이후 legacy 제거 | Late sink, process-lifetime tombstone, actor poison/long-lived retention을 migration gate에서 제거한다. 이 문서의 code-unit disposition이 current plan이다. |

## 교정된 implementation 순서
...
- Lifetime tombstone, permanent poison, global causal ordering이나 automatic reconciliation을 baseline에 다시 넣지 않았는가?
- Current Runtime Harness와 native app-data를 보존하면서 legacy Host를 새 Runtime 위의 compatibility layer로 남기지 않는가?
- T0·T0-C·T0.1 Interface가 후속 multi-turn·streaming·control·read/resume·activity tracer를 막지 않는가?

이 감사는 source·current code·문서의 design disposition을 고정한다. Runtime 구현 완료나 integration 승격을 주장하지 않으며 `codex-method-decisions.json`과 generated inventory를 변경하지 않는다.
