...
이 문서는 [첫 tracer와 module seam을 선택한다](../tickets/008-choose-first-tracer-and-module-seams.md)가 첫 supported Server request를 선택할 때 사용하는 exact-pin 근거다. 네 method는 committed stable schema와 generated inventory에 모두 존재하지만, first-party source가 제품 도입 순서를 정해 주지는 않는다.
...
| [첫 tracer와 module seam을 선택한다](../tickets/008-choose-first-tracer-and-module-seams.md)의 결정 | AY-PLE의 첫 variant와 좁은 acceptance | upstream 구현 사실의 재정의 |
...
다만 [첫 tracer와 module seam을 선택한다](../tickets/008-choose-first-tracer-and-module-seams.md)의 후속 **`T0.1` responder tracer**로는 command approval을 조건부 권고한다. Regular request가 command·cwd와 optional reason을 제공해 file-change transcript reconstruction, permission-profile 교집합, user-input form/expiry보다 적은 주변 Module로 exact request routing과 once-only response를 검증할 수 있기 때문이다.
...
- Safe caller decision은 `approve_once`, `decline`, `cancel`만 허용하고 각각 generated `accept`, `decline`, `cancel`로 mapping한다.
