...
| `cwd = workspaceRoot` 결합에서 검증된 path intent | external preparation/harness로 추출·개조 | Prepared process launcher `cwd`와 `thread/start.cwd` workspace capability를 분리한다. `CODEX_HOME` 등 launcher environment는 preparation/launcher policy이며 Runtime protocol semantics가 아니다. | 새 preparation/Connection composition gate |
...
| Timeout·close 뒤 request contract와 registry 즉시 제거 | Active waiter는 first response/error 또는 disconnect settlement에서 remove-once한다. Request observer timeout의 route disposition은 first-party가 직접 정하지 않으므로 spec이 bounded operation-local policy로 명시하며 lifetime history·permanent sink를 만들지 않는다. Active entry가 없어진 뒤 도착한 response는 map-miss no-op이고 unknown outcome은 해당 operation 결과에만 남는다. |
