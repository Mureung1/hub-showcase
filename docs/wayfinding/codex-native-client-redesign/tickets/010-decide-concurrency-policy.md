...
Cross-thread public independence는 별도 required `T0-C` tracer가 `A pending → B completes → A completes`를 deterministic fake child와 필요한 live gate에서 증명한다. T0의 max-one/no-queue public slot은 이 gate 전까지만 유지하고, 이를 내부 Connection ingress·demux·actor progression의 global lock으로 구현하지 않는다. `T0-C`는 product adapter 채택 여부와 무관한 runtime foundation conformance gate다.

`T0-C`와 T0.1은 서로를 대체하지 않는 independent foundation conformance slice며 현재 semantic blocker가 없다. `/to-tickets`는 true code blocker가 발견되지 않으면 common foundation 뒤 sibling frontier로 두고, 단순한 risk preference로 인위적인 dependency edge를 만들지 않는다. Future product adapter의 capability prerequisite는 current foundation completion 조건이 아니며 별도 product goal/map에서 결정한다.
