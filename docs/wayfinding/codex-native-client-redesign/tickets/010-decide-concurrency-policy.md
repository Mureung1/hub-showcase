...
| 선택지 | 동작 | 판단 |
| --- | --- | --- |
| A. Idle-only semantic start | Idle `ThreadActor`만 semantic `startTurn`을 admit한다. Starting·active state의 두 번째 start는 `RequestId` 할당과 wire write 전에 typed conflict로 끝내고, active input은 별도 `turn/steer` tracer가 exact `expectedTurnId`와 함께 채택한다. | 권고. First-party TUI method routing과 일치하고 response `B`와 actual `A`가 갈라지는 active-start branch를 현재 tracer의 partial adoption 밖에 둔다. Source가 강제하는 사실이 아니라 semantic runtime policy다. |
| B. Native overloaded start | Active state에도 raw `turn/start`를 보내고 regular active turn에는 implicit steer, 다른 active kind·empty input에는 settings·error side effect와 incomplete `B` lifecycle을 그대로 projection한다. | Exact server branch를 넓게 보존하지만 deep semantic Interface가 raw method overload와 orphan-like response identity를 떠안는다. 채택하면 active-second-start를 별도 required tracer와 conformance matrix로 올려야 한다. |
...
이 첫 결정은 reusable same-thread surface의 method domain을 정한다. T0는 새 thread에 첫 turn만 시작하므로 어느 선택지도 현재 T0 wire path를 바꾸지 않는다.
...
- Same-thread 선택지 A를 고르면 reusable thread surface는 idle `ThreadActor`에서만 `turn/start`를 보내고, `turn/steer`는 exact `expectedTurnId`를 쓰는 별도 active-input tracer에서 채택한다. 선택지 B를 고르면 active second-start branch 전체가 별도 required tracer가 된다.
...
| `turn/start` | `raw-wrapper` / `baseline` | T0·`T0-C` idle branch required; reusable active branch는 Same-thread HITL 결과에 따름 | `ThreadActor`; active routing source와 TUI explicit steer precedent |
...
첫 결정은 Same-thread 선택지 A처럼 semantic `startTurn`을 idle-only partial adoption으로 둘지다. 추천은 A다. B는 pinned raw branch를 넓게 보존하지만 response identity와 running identity가 갈라지는 behavior를 runtime Interface에 포함하고, C는 source에 없는 queue policy를 만든다. 이 결정을 받은 뒤 public cross-thread 선택지 A의 `T0-C` 시점을 다음 한 질문으로 확인한다. Product queue·steer UX는 [첫 AY-PLE adapter tracer와 runtime readiness gate를 결정한다](018-decide-first-ayple-adapter-tracer.md) ticket에 남긴다.
