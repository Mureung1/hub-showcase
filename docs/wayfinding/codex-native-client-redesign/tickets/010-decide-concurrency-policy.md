...
| B. Native overloaded start | Active state에도 raw `turn/start`를 보내고 regular active turn에는 implicit steer, 다른 active kind·empty input에는 settings·error side effect와 incomplete `B` lifecycle을 그대로 projection한다. Response `B`는 request submission acknowledgement일 뿐 새 active turn authority가 아니며 observation은 `A`에 계속 귀속한다. | Intentional server branch를 직접 보존하는 선택이다. 채택하면 active-second-start와 settings·error 세 branch를 별도 required tracer와 conformance matrix로 올리고, `B`를 turn handle로 노출하지 않는 Interface가 필요하다. |
