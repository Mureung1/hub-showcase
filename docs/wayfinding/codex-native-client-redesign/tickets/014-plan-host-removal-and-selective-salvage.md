...
- Actor/mailbox는 package-private implementation 선택으로만 남기고 public 이행 invariant나 retention gate로 고정하지 않는다.
...
External appData와 repository `.ay-ple/runtime-codex`의 native state·rollout/history·native identity는 code-unit 분류가 아니라 모든 제거 단계가 지키는 비파괴 invariant다. 새 Connection/Runtime contract는 기존 class를 다섯 번째 재사용 범주로 승격하지 않고, 위에서 검증한 primitive를 새 owner 뒤에 조합해 active exact demux, current-pending settlement, method-specific early FIFO와 native `ThreadId`별 projection을 구현한다.
...
4. Native identity·`ThreadId`별 projection을 가진 `CodexConversationRuntime`과 T0를 구현한다. Actor/mailbox는 package-private 선택이며 public 이행 invariant나 ledger owner로 고정하지 않는다. Stage별로 통과한 verification selector만 먼저 `implemented`로 기록한다.
5. T0-C와 T0.1을 공통 T0 뒤의 sibling tracer로 구현한다. T0-C는 처음부터 native `ThreadId`별 projection을 사용하는 `./conversation` public surface에서 cross-thread independence를 증명하며, T0-only global operation slot을 임시 architecture로 만들지 않는다. 둘 다와 full repository regression이 green이 된 뒤 Host/layout/transport legacy surface와 self-oracle을 제거하고 `legacy-client-host` membership을 0으로 만든다.
...
- Ticket 015는 superseding ADR 0010, ADR 0008의 historical disposition과 ADR index 정렬을 이미 완료했다. 이 plan은 그 결정을 다시 열지 않는다.
- Old spec의 `wontfix`, 미구현 Tickets 004–010의 `wontfix`·`Next actor: none`, Tickets 001–003의 `completed` evidence 보존도 완료됐다. Old artifact를 새 결정의 source로 개조하거나 이동하지 않고 `/to-spec`·`/to-tickets` 뒤 exact replacement link만 보강한다.
...
- AY-PLE adapter, browser transport, product policy와 첫 product tracer는 이 이행 DAG와 readiness condition 밖이다.
...
아래 review는 process-lifetime tombstone·actor sink와 장기 retention을 이행 gate로 사용하던 이전 계획을 검토한 역사적 결과다. 현재 교정의 Source·Standards·Spec 결과는 이 ticket의 별도 checkpoint가 소유하며 아래 actor·sink 관련 판정을 current 이행 contract로 사용하지 않는다.
