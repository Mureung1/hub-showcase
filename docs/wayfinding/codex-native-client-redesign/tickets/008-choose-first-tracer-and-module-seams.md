...
후속 사용자 verdict는 이 Answer의 T0 runtime 결정을 유지하면서 현재 map의 production dependency direction을 `CodexAppServerConnection → CodexConversationRuntime`까지로 한정했다. 위 `AYPLE adapter` row와 app-side mapping은 future downstream boundary를 설명한 역사적 문맥일 뿐 현재 foundation spec의 owner, readiness prerequisite나 implementation 대상이 아니다. Three-root validation과 launcher `cwd`도 Runtime protocol semantics가 아니라 external preparation·harness capability다.

`runNewConversation()` sketch는 T0 composite conformance를 가장 작은 호출로 관찰하기 위한 임시 facade이며 최종 Chat Interface의 one-shot ceiling이 아니다. Resulting foundation Interface는 native thread lifetime 위에 multi-turn, streaming, interrupt/steer, `thread/read`·`thread/resume`, activity와 추가 Server request tracer를 source-guided 방식으로 더할 수 있어야 한다. 이 scope correction은 T0의 method roster, identity authority, convergence, safe result와 T0.1 선택을 다시 열지 않는다.
