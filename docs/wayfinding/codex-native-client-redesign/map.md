...
- [Pinned method lifecycle fact table을 만든다](tickets/004-build-method-lifecycle-fact-table.md) — Public shape·exact-pin source/test·live observation·inference·product decision을 분리했다. Native scope correlation은 global journal을 요구하지 않으며, active second-start의 불완전 lifecycle과 thread-scoped `SystemError`, turn terminal 뒤 late child를 method별 사실로 고정하고 concurrency·retention·recovery 정책은 후속 ticket에 남겼다.
- [First-party connection·ingress architecture pattern을 지도화한다](tickets/005-map-first-party-rust-architecture-patterns.md) — Production client에는 stdio child path가 없으므로 AY-PLE이 external process lifecycle을 소유한다. Single connection/ingress authority와 waiter-independent drain은 채택하되 upstream의 unbounded queue, silent anomaly, transport별 lossless와 shutdown result gap은 contract로 복사하지 않고 seam·delivery·recovery 결정으로 넘겼다.
- [Core·TUI·exec conversation ownership pattern을 지도화한다](tickets/006-map-first-party-conversation-ownership.md) — First-party에는 별도 shared conversation kernel이 없고 core deep thread state와 surface별 projection이 있다. Native identity와 per-thread ownership, live/snapshot·terminal/drain 경계는 후속 design pressure로 보존하되 active second-start, TUI bootstrap staging과 Exec backfill 같은 surface/version-specific 동작을 generic contract로 승격하지 않는다.
