...
- [First-party client 근거로 port와 재사용 계획을 교정한다](tickets/019-correct-first-party-client-port-and-reuse.md) — Python external stdio client, Rust facade와 TUI projection/pending lifecycle을 주요 동작 근거로 보완하고 현재 code를 보존·추출/개조·Harness-only·제거로 분류했다. Lifetime tombstone·permanent poison·global semantic arbiter를 기준선에서 제거했다.
