...
- [ADR 0008을 대체할 architecture decision을 기록한다](tickets/015-record-superseding-architecture-decision.md) — ADR 0010으로 Connection → ConversationRuntime 운영 경계와 first-party client behavior port 원칙을 채택하고, 기존 Host는 호환성 없이 역사화하며 제품 adapter는 foundation 준비 조건 밖에 둔다.
- [First-party client 근거로 port와 재사용 계획을 교정한다](tickets/019-correct-first-party-client-port-and-reuse.md) — Python external stdio client, Rust facade와 TUI projection/pending lifecycle을 primary behavior evidence로 보완하고 current code를 보존·추출/개조·Harness-only·제거로 분류했다. Lifetime tombstone·permanent poison·global semantic arbiter를 baseline에서 제거했다.
...
- 자동 restart·mutation replay·범용 reconciliation framework. Source-guided `thread/read`·`thread/resume` tracer가 explicit reconciliation을 후속 채택하는 것은 금지하지 않는다.
