...
- [Source conformance verification matrix를 결정한다](tickets/013-decide-source-conformance-verification.md) — Generated shape, exact-pin source/tests, unit, fake child와 live binary의 authority를 분리하고 versioned coverage ledger, fail-safe generation·drift check와 runtime-first product readiness gate를 확정한다.
