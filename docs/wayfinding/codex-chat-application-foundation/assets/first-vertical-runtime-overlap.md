...
> **2026-07-19 후속 범위 정정:** [Codex 실행 권한과 AY-PLE 제품 확인 경계](codex-execution-permission-boundary.md)에 따라 아래 source 관찰은 보존하되, product `read-only extraction`을 `Sandbox.read_only`·no-network·unexpected-request reject와 동일시하지 않는다. Default `accept`는 native permission disposition evidence이지 AY-PLE product blocker가 아니며, reject patch나 narrow port를 미리 요구한 문구는 superseded다.
...
- `Sandbox.read_only`는 exact generated policy에서 `networkAccess: false`이고 current exact-local test도 이 effective policy를 관찰한다. Exact Python client의 low-level default handler가 synthetic command/file approval을 `accept`한다는 사실도 확인했다. 두 사실은 current permission behavior를 설명하지만 proposal-only AY-PLE product invariant의 충족 여부를 결정하지 않는다.
...
| Current Codex execution permission posture | `ApprovalMode.deny_all`은 native `never`, `Sandbox.read_only`는 `ReadOnlySandboxPolicy(networkAccess=false)`로 mapping된다 ([approval](../../../../packages/codex-chat-runtime/python/openai-codex/sdk/python/src/openai_codex/_approval_mode.py#L13-L35), [sandbox](../../../../packages/codex-chat-runtime/python/openai-codex/sdk/python/src/openai_codex/_sandbox.py#L15-L73), [policy type](../../../../packages/codex-chat-runtime/python/openai-codex/sdk/python/src/openai_codex/generated/v2_all.py#L3459-L3464)). | Current exact-local test는 bridge override의 effective `never`·`readOnly`·`networkAccess:false`를 확인한다 ([policy probe](../../../../packages/codex-chat-runtime/src/local-provider.actual.test.ts#L129-L140)). SDK low-level default handler는 synthetic command/file approval을 `accept`한다 ([handler](../../../../packages/codex-chat-runtime/python/openai-codex/sdk/python/src/openai_codex/client.py#L773-L779)). | 두 관찰 모두 008의 native permission profile·request projection disposition 입력이다. Current bridge override는 자동 survivor가 아니며 default `accept`만으로 narrow port를 요구하지 않는다. | 선택한 native profile과 실제 action trace가 public SDK seam으로 표현되지 않을 때만 precise residual을 확인한다. |
...
3. Codex는 exact `cwd`에서 selected native permission profile이 허용하는 filesystem access를 통해 source를 읽는다. 006의 historical probe는 current fixed `read_only` profile을 사용했다.
...
### Python SDK default approval 관찰의 범위
...
`approvalPolicy=never`가 정상 경로에서 request를 만들지 않는다는 가정과 policy를 어긴 synthetic request에 대한 low-level fallback은 서로 다른 사실이다. 같은 exact pin의 first-party `codex exec`가 request를 명시적으로 reject하는 동작은 conformance donor로 남지만, 자동으로 product target이 되지는 않는다. 006 negative control은 current SDK fallback을 관찰하는 데 성공했다. 008은 먼저 필요한 native permission profile·설정 소유자·request UX를 정하고, 그 뒤 public seam에 실제 gap이 남을 때만 upstream extension이나 narrow port를 검토한다.
...
| Fixed `deny_all`·`read_only`를 장기 제품 permission profile로 간주 | `probe-needed` | Effective current policy와 synthetic default response는 관찰됐지만 제품 action에 필요한 native profile·사용자 선택은 정하지 않았다. 008에서 fixed override를 `keep | replace | delete | frozen fallback`으로 판정한다. |
...
반면 exact exec initial input은 prompt text와 local images로 구성되고 structured `SkillInput`/selected TXT receipt API를 직접 노출하지 않는다. CLI JSONL event와 process exit를 004의 native acceptance·unknown contract로 정확히 map할 수 있는지도 이 문서에서 증명하지 않았다. 그러므로 `codex exec`의 request rejection은 permission behavior donor일 뿐, adopted Python SDK runtime과 local-web product surface를 대신하는 후보가 아니다.
...
| Native permission observation | Current fixed profile과 effective sandbox, synthetic unexpected request response, write/network marker를 각각 기록 | 008에서 desired native profile·설정 소유자와 current override disposition을 먼저 결정하고 그 뒤 actual seam gap만 판정 |
...
005는 confirmed runtime residual을 승인하지 않는다. Source shape만으로 확정할 수 있는 것은 Recipe version/args, `SourceSelection` receipt, product `ModelingRun` correlation과 final JSON/reference validation이 AY-PLE product adaptation이라는 책임 경계다. Path/link behavior와 process topology는 006 trace가 검증했고, current permission override·SDK fallback과 exact adapter survivor는 008이 판정한다. 그 전에는 Module이나 API shape를 정하지 않는다.
