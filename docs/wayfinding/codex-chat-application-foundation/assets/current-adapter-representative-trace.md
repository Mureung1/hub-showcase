...
- 유일한 failed required invariant는 unexpected command/file approval이다. Exact high-level Python SDK의 default handler는 injected request 두 family에 `accept`를 반환했다. Effective `never + readOnly + networkAccess:false`와 side-effect marker 비발생은 별도로 충족됐지만, [004의 “예상하지 않은 request를 자동 승인하지 않는다”](../tickets/004-first-assignment-runtime-envelope.md#required-runtime-envelope)는 충족되지 않았다.
- 최종 prototype status는 `confirmed_residual`, exit code `2`다. 이는 006을 해결하는 유효한 falsifying result이며, 008이 adopted Python SDK path 안에서 public extension·lower-level seam·narrow port와 current custom survivor를 판정한다.
...
| Unexpected approval | Injected command approval과 file-change approval에 current SDK default handler가 모두 `accept`를 반환했다. Fake request라 실제 side effect는 없었지만 required fail-closed invariant는 실패했다. | `residual` |
...
| Policy assumption을 위반해 request가 도착했을 때 client response | command·file 모두 default `accept`; fail-closed invariant 실패 |

006은 SDK patch나 production adapter를 수정하지 않았다. 008은 required read-only first target을 단순 deferred하지 않고, adopted SDK path에서 explicit reject를 제공할 가장 얇은 disposition을 선택해야 한다. 장기 write·network capability와 interactive approval UX는 이 residual을 고친 뒤 별도 product need로 재검토한다.
...
| `npm run prototype:first-vertical -w @ay-ple/codex-chat-runtime` | `confirmed_residual`, exit `2` | Deterministic, bundle, frozen actual/local, exact-local semantic·permission과 live 3회 실행·sanitized report·cleanup을 한 command로 수행 |
...
| Unexpected approval handling | High-level SDK default `accept`가 required invariant를 실패 | Public SDK extension, lower-level seam 또는 provenance·test를 갖춘 narrow port 중 가장 얇은 fail-closed disposition |
...
- Unexpected approval residual을 prototype 안에서 수정하거나 008의 disposition을 선결하지 않았다.
