...
Source만으로 좁혀진 중요한 current-pin gap은 high-level Python SDK가 unexpected command/file approval을 default `accept`하면서 fail-closed handler injection을 제공하지 않는다는 점이다. 이를 permission residual로 확정할지, 같은 pin의 fail-closed `codex exec`나 narrow port가 해결하는지는 006–007이 검증한다. 두 TXT·Recipe·schema trace, accepted crash·fresh restart와 deterministic·exact-child·3회 live reproducibility는 아직 `probe-needed`이며 새 ticket은 만들지 않는다.
