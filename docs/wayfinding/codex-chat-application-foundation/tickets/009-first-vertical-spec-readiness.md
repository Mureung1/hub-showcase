...
MCP call·result와 proposal·Review는 별도 job UI가 아니라 같은 Chat transcript의 native·product activity로 누적한다. Exact pin은 custom MCP config와 tool call의 started·completed·result·error lifecycle을 이미 소유하지만 current bridge는 Agent message 외 MCP item을 투영하지 않으므로, app-managed MCP lifecycle·proposal persistence와 MCP Chat projection은 resulting product integration이 소유할 adaptation이다. [023](023-state-patch-review-interaction-donor.md)의 donor 판정에 따라 first vertical은 `propose_state_patch` call과 origin native Turn을 Review 동안 열어 두지 않는다. Tool이 durable pending `StatePatch`와 stable patch identity를 반환한 뒤 App-owned `StatePatch` persistence와 decision reconciliation이 reload·Server/runtime restart·stale·duplicate·unknown outcome을 정산하고, `UserConfirmation`만 confirmed `SemesterModel` apply authority를 가진다. Agent continuation이 필요할 때만 decision settlement 뒤 같은 native Thread에 새 follow-up Turn을 시작한다. Native pending prompt의 presentation behavior는 adaptation하지만 Plan clarification·MCP action·Codex approval을 product authority로 재사용하거나 generic Review workflow state machine을 먼저 만들지 않는다.
...
- [023](023-state-patch-review-interaction-donor.md)에서 선택한 durable `StatePatch`·`UserConfirmation`과 필요 시 같은 Thread의 follow-up Turn lifecycle, native pending interaction에서 adaptation할 presentation·identity·resume behavior
