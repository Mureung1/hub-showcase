...
- Runtime sufficiency는 general Chat completeness와 다르다. First vertical은 Account Readiness, explicit `cwd`, exact `SkillInput`, selected source Markdown path·arguments를 담은 bounded `TextInput`, custom MCP proposal, Plan mode `request_user_input`, terminal·interrupt·unknown outcome와 process settlement 중 채택된 observable outcome만 소유한다. `MentionInput`과 optional `outputSchema`는 native capability inventory일 뿐 first-vertical input이나 proposal payload가 아니다.
