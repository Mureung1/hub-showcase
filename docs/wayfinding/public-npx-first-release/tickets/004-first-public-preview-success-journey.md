...
- `Semester Ready`는 Course, RawMaterial, live model turn이나 학업 action을 요구하지 않으며 AY가 학기를 이해했다는 뜻도 아니다. 완료 화면이 가리키는 다음 여정은 `첫 자료 가져오기`이며, current release에서의 disabled·active availability는 [Ticket 012](012-semester-ready-first-action.md)가 소유한다.
...
- First-run golden path와 별도로 `ready-relaunch`를 필수 release gate로 둔다. `Semester Ready`에서 정상 종료한 뒤 같은 public `npx` 명령을 다시 실행하면 exact Runtime cache와 격리 OAuth session을 재사용하고 workspace registry의 active `SemesterWorkspace`를 다시 찾아 `WorkspaceManifest` validation 후 setup wizard 없이 post-Ready entry surface를 연다. 그 exact 화면은 [Ticket 012](012-semester-ready-first-action.md)의 compact semester status center가 소유한다.
- `ready-relaunch`는 scaffold나 기본값을 중복 생성·재작성하지 않는다. Setup 중 중단되면 마지막 durable safe 단계에서 재개하고, 불완전 scaffold는 Ready로 합성하지 않고 recovery 상태로 보낸다. Exact target `mkdir` 뒤 durable ownership evidence 전 interruption처럼 App이 소유권을 증명할 수 없는 leaf는 자동 resume·discard하지 않는 preserved manual-recovery collision이며, 사용자가 확인한 뒤 다른 target을 선택할 수 있다. 충돌 경로나 사용자 파일을 자동 덮어쓰기·삭제하지 않는다.
