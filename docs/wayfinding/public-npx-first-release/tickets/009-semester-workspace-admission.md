...
이 판정을 입력으로 Ticket 010은 App-owned `inspect`·`apply` 위의 Browser `SetupJourney`, 최종 확인·approved-only durability, workspace instruction/Skill bundle과 repository-only thin smoke adapter를 결정했다. Mandatory setup에는 Codex Skill이나 live Turn이 없다. Ticket 011은 matching setup evidence·registry·resume/discard와 process interruption의 exact protocol을, Ticket 012는 Course-free admitted workspace의 Browser-safe 완료 경험을 각각 결정한다. Setup progress·registry와 `Semester Ready` composition은 이 ticket의 schema authority가 아니다.
