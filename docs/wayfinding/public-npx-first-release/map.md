...
- `SemesterWorkspace`는 AY-PLE이 생성하고 schema·validation·deterministic migration을 소유하는 정규화된 학기 공간이다. 기존 자료 폴더는 workspace로 직접 열 대상이 아니라 후속 `ImportSource`로 다루며 setup Skill이 canonical schema를 소유하지 않는다. [ADR 0014](../../adr/0014-create-app-owned-normalized-semester-workspaces.md)가 adopted format·admission authority를 소유하고, [Ticket 010](tickets/010-resumable-setup-authority.md)은 그 app-owned operation을 사용하는 setup orchestration·defaults만 결정한다.
