...
- `SemesterWorkspace`는 AY-PLE이 생성하고 schema·validation·deterministic migration을 소유하는 정규화된 학기 공간이다. 기존 자료 폴더는 workspace로 직접 열 대상이 아니라 후속 `ImportSource`로 다루며 setup Skill이 canonical schema를 소유하지 않는다. 정확한 설계는 [첫 public release의 SemesterWorkspace schema·scaffold 경계를 정한다](tickets/009-semester-workspace-admission.md)와 [재개 가능한 setup과 Installer bundle의 authority를 정한다](tickets/010-resumable-setup-authority.md)가 나눠 소유한다.
- `Semester Ready`의 학생용 표현은 `학기 공간 준비 완료`이며 Codex account 연결과 app-owned scaffold·기본 설정·validation이 끝난 setup checkpoint다. Course·RawMaterial·live model turn·학업 action이나 AY의 학기 이해를 뜻하지 않고 다음 `첫 자료 가져오기` 여정의 진입점만 제공한다.
...
- Public 인증은 AY-PLE `appDataRoot`에 격리한 ChatGPT browser OAuth만 지원하며 API key·access token·device-code와 전역 `~/.codex` credential import를 제외한다. Matching login completion과 fresh `account/read`가 성공해야 `Codex 연결됨`으로 표시하고, 취소·실패·만료는 workspace 생성 전의 재시도 가능한 `로그인 필요` 상태로 수렴한다.
- First-run 성공 뒤 같은 public `npx` 명령이 cache·OAuth session·workspace registry를 재사용해 `WorkspaceManifest`를 재검증하고 setup wizard 없이 workbench를 여는 `ready-relaunch`를 별도 필수 release gate로 둔다. 중단된 setup은 안전하게 재개하거나 recovery로 수렴하고 incomplete state·중복 scaffold·자동 덮어쓰기·삭제를 성공으로 취급하지 않는다.
