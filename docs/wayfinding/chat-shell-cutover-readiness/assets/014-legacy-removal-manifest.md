...
| Entrypoint smoke | 설정 없는 `npm run dev`와 exact six-root configured `npm run dev` | 전자는 Server+Shell이 시작하고 safe `unavailable`, 후자는 verified status/runtime path를 사용한다. 어느 경우도 Inspector/legacy process를 시작하지 않는다([entrypoint contract](../tickets/002-extension-envelope.md#L53-L59)). |
...
016은 다음 negative checks를 명시해야 한다.
