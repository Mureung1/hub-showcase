...
- 상태: 완료·역사 gate decision; final authorization·safety contract는 [Codex Chat-only runtime cutover spec](../../../specs/2026-07-17-codex-chat-only-cutover.md)이 supersede한다.
- 판정일: 2026-07-17
...
> `/to-spec` live-tree 재검증에서 executable ownership spike, 네 deleted workspace의 ignored residue, current survivor stale emit과 pre/post worktree binding gap을 추가로 발견했다. 최종 contract는 일곱 exact deletion root, current clean-build, durable deletion evidence, fresh post-delete attempt와 same-SHA retry만 허용한다. 아래의 세-root·6-symlink·exact-source-in-spec·clean-status-empty·pre-cutover rollback 문구는 resolved decision의 이전 snapshot이며 실행 authority가 아니다. Reviewed program source·serialization은 candidate-verification implementation ticket이 소유한다.
