...
역산 결과를 자소서·포트폴리오·면접으로 배정한 화면입니다. 범위(직무 전체·기업군·개별 공고)를 고르면 내용이 조정됩니다. 준비 현황 카드(보유·필수 미보유 수, 체크 시 실시간 갱신), [요구 항목 · 근거 · 필요 산출물/활동 · 활용처 · 보유] 체크리스트(역산 편차 항목 강조), 포트폴리오 전략(강조점 카드·기업군별 소개 순서), 자소서 전략(문제→해결→성장 서사 구조·예시 문장), 면접 전략(예상 질문·꼬리질문 트리·답변 포인트)을 포함하며, 각 전략 카드는 체크리스트 항목과 연결 표시됩니다.
