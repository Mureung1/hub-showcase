...
5. 관련 인사이트 카드가 노출된다.
6. 사용자가 원문을 새 탭에서 열어본다.

### MVP 성공 기준

MVP는 많은 기능을 제공하는 것이 아니라, 저장한 인사이트를 상황 기반으로 다시 발견하는 흐름이 자연스럽게 작동하는지를 보여줘야 한다.
...
사용자는 현재 상황이나 필요한 용도를 입력해 저장된 인사이트 중 관련 있는 항목을 추천받을 수 있다.
...
- 추천 상황 버튼
- 자유 입력
- MiniSearch 또는 Fuse.js 기반 검색/추천
- 최대 6개의 인사이트 카드 결과
- 원문 새 탭 열기
...
4. 아맞다는 저장된 인사이트 중 관련 있어 보이는 항목을 카드로 보여준다.
5. 사용자는 필요한 카드를 새 탭으로 열어 원문을 확인한다.
...
- MiniSearch 또는 Fuse.js
- 제목, 메모, 카테고리, URL, 도메인 기반 검색/추천
- AI와 LLM은 MVP에서 제외
