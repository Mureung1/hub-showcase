...
CareerSignal은 채용공고의 기준선과 편차를 심층 해설해, 직무·기업군·개별 공고가 실제로 원하는 수준과 준비 방법을 알려주는 진로탐색 리서치 에이전트입니다.

사용자는 관심 직무를 선택하고, 서비스는 채용공고에 반복되는 기술·경험·자격요건·우대사항을 분석합니다. 기술 이름의 빈도와 함께 구현 범위, 필수·우대 우선순위, 기업군별 강조점을 정리해 학습·프로젝트·지원 준비의 다음 행동으로 연결합니다. 반복되는 요구사항과 기업군·공고별 편차를 심층 해설하고, 자소서·포트폴리오·면접에서 지원자가 무엇을 보여줘야 하는지까지 제시합니다.
...
사용자는 직무를 고르면 통계 분석 → 채용공고 해설 → 합격 전략 → 준비 로드맵 순으로, 전체에서 기업군·개별 공고로 좁혀가며 준비 기준과 다음 행동을 받습니다. 각 단계는 같은 채용 데이터 버전에서 생성·검증된 결과로 이어집니다. 서비스는 채용공고와 근거 자료가 갱신될 때 분석 결과를 함께 갱신하고, 일반 조회에서는 저장된 결과를 제공합니다.
...
전체 뷰는 이 직무의 공통 기대치(baseline)를 보여 주고, 기업군이나 개별 공고를 고르면 그 기준 위에서 더 높게 또는 추가로 요구되는 부분을 편차로 강조해 보여 줍니다. 개별 공고를 선택하면 원문과 함께 편차 해설·근거 문장·신뢰도를 확인합니다. 사용자는 이 회사·직무가 실제로 원하는 인재상을 근거와 함께 읽어 냅니다.

### 5.3 합격 전략: 체크리스트·자소서·포트폴리오·면접

사용자는 공고 해설 결과를 자소서·포트폴리오·면접으로 연결한 체크리스트를 받습니다. 목표 기업군·개별 공고로 범위를 좁히면 체크리스트와 활용처별 전략이 조정됩니다. 각 항목의 보유·미보유 체크는 준비 현황과 준비 로드맵에 반영되며, 포트폴리오·자소서·면접 전략의 내용은 바꾸지 않습니다.
...
핵심 기능은 채용공고를 통계적으로 분석하고 직무 기준선 대비 편차를 심층 해설해, 지원자가 자소서·포트폴리오·면접에서 보여줘야 할 것을 정의한 뒤, 그것을 채울 학습·프로젝트 로드맵을 제시하는 네 단계로 이어집니다. 각 단계의 뷰는 전체 → 기업군 → 개별 공고로 좁혀집니다. 각 화면의 정보 구성은 9장(화면 목록)에서, 데이터·분석 방법 등 구현 세부는 [설계 문서](architecture.md)에서 다룹니다.
...
전체 뷰는 직무 공통 기대치(baseline)를, 기업군·개별 공고 뷰는 그 기준 위에서 더 높거나 추가로 요구되는 편차를 강조해 보여 줍니다. 편차는 회사 공식 자료와 공공·표준 자료로 근거·해설을 보강하고, 근거가 약하면 신뢰도를 낮춰 표시합니다. 각 편차에는 "같은 직무 공고 중 몇 %가 비슷한 표현을 쓰는지" 지표를 붙입니다. 기업군은 사람이 정한 태그로 묶습니다. LLM 에이전트는 채용 데이터 갱신 시 범위별 해설을 생성·검증하고 서비스는 저장된 결과를 제공합니다.

### 6.3 합격 전략

공고 해설 결과의 각 요구 항목을 자소서·포트폴리오·면접 중 맞는 곳으로 보냅니다. 산출물로 증명 가능한 항목(배포 URL·README·GitHub 작업 기록 등)은 포트폴리오 강조점으로, 가치관·과정·성장 서사형 항목은 자소서 소재로, 이론·원리형이거나 편차로 강조된 심화 항목은 면접 예상 꼬리질문으로 보냅니다. 한 항목이 여러 곳에 동시에 들어갈 수 있습니다. 결과는 [요구 항목 · 필요 산출물/활동 · 활용처(자소서/포트폴리오/면접) · 보유 여부] 체크리스트와 함께, 활용처별로 무엇을 써야 하는지(자소서 소재, 포트폴리오 강조점, 예상 면접 질문)를 정리해 제공합니다. 목표 기업군·회사를 좁히면 맞춤 전략으로 조정됩니다.
...
프로토타입은 핵심 기능의 전체 흐름(직무 선택 → 통계 분석 → 채용공고 해설 → 합격 전략 → 준비 로드맵)을 백엔드 직무 기준으로 시연하는 **HTML/CSS 정적 프로토타입**입니다. 실제 분석 로직 없이 정보 구조와 화면 흐름을 검증합니다.
...
- 채용공고 해설 — 직무 기준선과 편차, 숨은 요구, 근거 문장, 신뢰도
...
사용자가 직무 선택부터 통계 분석·채용공고 해설·합격 전략·준비 로드맵까지 확인하는 정적 화면으로, 어떤 정보를 어떤 순서로 보여줄지 검증합니다.
...
- 채용공고 해설: 직무 기준선 대비 편차·숨은 요구·근거·해설을 전체·기업군·개별 공고 단위로 생성·검증해 저장
...
- 분석 실행: 데이터 변경의 영향 범위를 계산하고 다섯 에이전트의 결과를 검증된 버전으로 활성화
...
- 비로그인 사용자의 채용공고 원문 입력과 개별 공고 해설, 동일 원문 분석 결과 재사용
...
해당 직무에서 반복되는 요구를 리얼리티 체크 중심의 10개 블록으로 보여 줍니다. 화면 순서는 리얼리티(①KPI ②직무 외 요구 범위 ③필수 인플레이션 ④숨은 난이도) → 기술(⑤기술 빈도 ⑥조합·구현 수준 ⑦증가/유지/감소 추이) → 해설(⑧라벨 vs 현실 ⑨기업군 성향 히트맵) → 데이터(⑩요구 항목 전체표, 기본 접힘)입니다. ⑩은 채용공고 해설의 입력과 같은 데이터이며, 각 블록에는 표본 수와 신뢰도를 함께 표기합니다.

### 9.3 채용공고 해설 화면

전체 baseline, 기업군 편차, 개별 공고의 세 섹션을 항상 표시하고, 기업군 선택이 아래 두 섹션의 내용을 바꿉니다. baseline 카드는 통계 근거(등장률·필수율)를 병기하고, 편차 카드는 편차 없음 항목까지 명시합니다. 개별 공고는 목록(검색·건수 표기)에서 선택하며, 원문의 모든 문장에 세 종류 주석 — 직무 공통(회색 번호+라벨) · 숨은 의미(파란 하이라이트) · 회사 특징(노란 하이라이트) — 을 달고, 종합 해설과 함께 주석 종류별 해설 탭으로 보여 줍니다.

### 9.4 합격 전략 화면

공고 해설 결과를 자소서·포트폴리오·면접으로 배정한 화면입니다. 범위(직무 전체·기업군·개별 공고)를 고르면 내용이 조정됩니다. 준비 현황 카드(보유·필수 미보유 수, 체크 시 실시간 갱신), [요구 항목 · 근거 · 필요 산출물/활동 · 활용처 · 보유] 체크리스트(편차 항목 강조), 포트폴리오 전략(강조점 카드·기업군별 소개 순서), 자소서 전략(문제→해결→성장 서사 구조·예시 문장), 면접 전략(예상 질문·꼬리질문 트리·답변 포인트)을 포함하며, 각 전략 카드는 체크리스트 항목과 연결 표시됩니다. 체크리스트에는 산출물로 증명하는 프로젝트형·서사형 항목과 함께, 면접에서 검증되는 학습형 항목(CS·이론)이 포함됩니다.

### 9.5 준비 로드맵 화면

미보유 항목을 채우는 준비를 두 트랙으로 보여 줍니다. 프로젝트 로드맵(순서형 타임라인 — 기간·우선순위·산출물·채워지는 항목·추천 이유)과 학습 로드맵(병행형 — CS·프레임워크 주제별 "어디까지" 깊이 기준, 면접 검증 연결)입니다. 범위(직무 전체·기업군·개별 공고)를 고르면 전체 내용이 조정되고, 화면 안의 체크리스트에서 보유를 체크하면 브라우저 상태에 반영되며 로드맵 재구성은 적용 버튼으로 반영됩니다. 보유 항목은 완료로 표시됩니다.
...
flowchart TD
    ...
    Y -->|있음| Z["개별 공고 해설 결과 조회"]
    Y -->|없음| O["온디맨드 개별 분석 체인"]
    O --> Z
    Z --> D
```

일반 흐름은 전체 → 기업군 → 개별 공고로 좁혀가며 저장된 분석 결과를 조회합니다. 직접 입력 흐름은 동일 분석을 재사용하거나 공고 추출·채용공고 해설·합격 전략·준비 로드맵으로 구성된 온디맨드 개별 분석 체인을 실행합니다.
