...
- 매장명, 위치, 업종, 주력 상품(시그니처 메뉴) 등 우리 가게의 기본 정보를 셋업합니다.

**② 트렌드 확인 (대시보드 홈)**

- ①에서 입력한 업종 정보를 기준으로 실시간 수집된 틱톡/인스타그램의 트렌드 데이터를 확인합니다.
- 검색량 추이는 꺾은선 차트로, 추천 해시태그는 컬러풀한 형태로 노출됩니다.

**③ 기획 세팅 (파이프라인)**

- 릴스 생성 파이프라인 화면으로 넘어가, 오늘 영상의 방향성을 체크리스트 형태로 각각 선택합니다.
...
| 단계          | 화면        | 사용자 행동            | 시스템 반응                             |
| ------------- | ----------- | ---------------------- | --------------------------------------- |
| ① 정보 세팅   | 대시보드 홈 | 매장 정보 입력 후 저장 | StoreInfo DB 저장                       |
| ② 트렌드 확인 | 대시보드 홈 | 업종별 트렌드 확인     | 차트·해시태그 동적 렌더링               |
| ③ 기획 세팅   | 파이프라인  | 목적/분위기 클릭 선택  | 기획 파라미터 설정                      |
| ④ 사진 업로드 | 파이프라인  | 사진 드래그 앤 드롭    | 이미지 서버 업로드                      |
| ⑤ AI 가동     | 파이프라인  | "AI 생성 시작" 클릭    | 4단계 파이프라인 순차 실행              |
| ⑥ 발행        | 파이프라인  | 미리보기 확인 후 발행  | SNS 플랫폼 업로드 & GeneratedReels 기록 |
...
```mermaid
graph LR
    A[대시보드: 디테일한 정보 입력] --> B[실시간 트렌드 분석 확인]
    B --> C[파이프라인: 릴스 기획 방향성 선택]
