...
4. 시나리오 문서이면 관련 자료를 읽고 인과, 동기, 긴장, 정보 공개, 선택과
   분기를 검토한다. 더 나은 구조는 원안 기반 Draft에 섞지 않고 별도 권고로
   제시한다.
5. 개요서에는 상세 내용을 복제하지 않고 요약과 상대경로 링크만 제안한다.
6. 확정 문서가 아니라 Approval Queue 항목으로 제안한다.
...
4. 시나리오 변경이면 요청에 따른 Draft와 추가 구조 개선 권고를 분리한다.
   사용자가 권고를 선택하면 원본을 재확인해 Draft를 갱신하고 다시 승인받는다.
5. 변경안은 `workspace/projects/<project_slug>/approvals/approval_queue.md`에 올릴 형식으로 작성한다.
6. 요청이 여러 문서 역할에 걸치거나 기존 문서를 분리해야 하면 경로별 작업
