...
미보유 항목을 채우는 준비를 두 트랙으로 보여 줍니다. 프로젝트 로드맵(순서형 타임라인 — 기간·우선순위·산출물·채워지는 항목·추천 이유)과 학습 로드맵(병행형 — CS·프레임워크 주제별 "어디까지" 깊이 기준, 면접 검증 연결)입니다. 범위(직무 전체·기업군·개별 공고)를 고르면 전체 내용이 조정되고, 화면 안의 체크리스트에서 보유를 체크하면 즉시 저장되며 로드맵 재구성은 적용 버튼으로 반영됩니다. 보유 항목은 완료로 표시됩니다.
