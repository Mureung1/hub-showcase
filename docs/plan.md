...
사용자는 직무를 고르면 통계 → 역산 → 합격 조건 → 로드맵 순으로, 전체에서 기업군·개별 공고로 좁혀가며 준비 기준과 다음 행동을 받습니다. 각 단계는 같은 채용 데이터 버전에서 생성·검증된 결과로 이어집니다. 서비스는 채용공고와 근거 자료가 갱신될 때 분석 결과를 함께 갱신하고, 일반 조회에서는 저장된 결과를 제공합니다.
...
사용자는 미체크 항목을 채우기 위한 학습 주제와 프로젝트를 추천받습니다. 각 추천은 그것을 하면 어떤 미체크 항목이 채워지는지 연결해 보여 주고, 필수 항목을 먼저 채우도록 우선순위를 매깁니다. 목표 기업군을 고르면 강조점이 조정됩니다. 체크 상태는 미리 생성된 전략과 프로젝트 후보의 완료 여부와 순서를 조정하며 새로운 분석을 생성하지 않습니다. 사용자는 막연한 공부 목록이 아니라 우선순위가 있는 학습·프로젝트 로드맵을 얻습니다.
...
전체 뷰는 직군 공통 기대치(baseline)를, 기업군·개별 공고 뷰는 그 기준 위에서 더 높거나 추가로 요구되는 편차를 강조해 보여 줍니다. 편차는 회사 공식 자료와 공공·표준 자료로 근거·해석을 보강하고, 근거가 약하면 신뢰도를 낮춰 표시합니다. 각 편차에는 "같은 직군 공고 중 몇 %가 비슷한 표현을 쓰는지" 지표를 붙입니다. 기업군은 사람이 정한 태그로 묶습니다. LLM 에이전트는 채용 데이터 갱신 시 범위별 해석을 생성·검증하고 서비스는 저장된 결과를 제공합니다.
...
체크리스트의 미체크 항목을 채우기 위한 학습 주제와 프로젝트를 제시합니다. 항목별 전략과 여러 항목을 함께 채우는 프로젝트 후보를 미리 구성하고, 사용자의 체크 상태에 따라 필수 항목·선행 관계·충족 범위를 계산해 순서를 조정합니다. 이미 완료한 항목도 함께 보여, 사용자가 채운 것과 남은 것을 한눈에 봅니다.
...
- 인재상 역산: baseline 대비 편차·근거·해석을 전체·기업군·개별 공고 단위로 생성·검증해 저장
- 합격 조건 정의: 범위별 자소서·포트폴리오·면접 활용처 배정 체크리스트와 전략 저장
...
- 백엔드 외 직군(프론트엔드·AI 엔지니어·데이터 엔지니어·풀스택·DevOps·모바일·정보보안·게임 개발자)의 채용 데이터와 범위별 분석 산출물 구축
- 실제 채용공고 데이터 수집·갱신과 연도별 추이 분석
- 비로그인 사용자의 채용공고 원문 입력과 개별 역산, 동일 원문 분석 결과 재사용

로그인·사용자별 영구 저장은 공고 입력과 분리합니다. 개인화 Gap 분석, 로그인·저장, 자동 클러스터링, 비공식 외부 자료 연동, 권고/선택 구분, 직무 자유 입력은 별도 확장 후보입니다.
...
전체 baseline, 기업군 편차, 개별 공고의 세 섹션을 항상 표시하고, 기업군 선택이 아래 두 섹션의 내용을 바꿉니다. baseline 카드는 통계 근거(등장률·필수율)를 병기하고, 편차 카드는 편차 없음 항목까지 명시합니다. 개별 공고는 목록(검색·건수 표기)에서 선택하며, 원문의 모든 문장에 세 종류 주석 — 직무 공통(회색 번호+라벨) · 숨은 의미(파란 하이라이트) · 회사 특징(노란 하이라이트) — 을 달고, 종합 해석과 함께 주석 종류별 해설 탭으로 보여 줍니다.

### 9.4 합격 조건 정의 화면

역산 결과를 자소서·포트폴리오·면접으로 배정한 화면입니다. 범위(직무 전체·기업군·개별 공고)를 고르면 내용이 조정됩니다. 준비 현황 카드(보유·필수 미보유 수, 체크 시 실시간 갱신), [요구 항목 · 근거 · 필요 산출물/활동 · 활용처 · 보유] 체크리스트(역산 편차 항목 강조), 포트폴리오 전략(강조점 카드·기업군별 소개 순서), 자소서 전략(문제→해결→성장 서사 구조·예시 문장), 면접 전략(예상 질문·꼬리질문 트리·답변 포인트)을 포함하며, 각 전략 카드는 체크리스트 항목과 연결 표시됩니다. 체크리스트에는 산출물로 증명하는 프로젝트형·서사형 항목과 함께, 면접에서 검증되는 학습형 항목(CS·이론)이 포함됩니다.
...
일반 흐름은 전체 → 기업군 → 개별 공고로 좁혀가며 저장된 분석 결과를 조회합니다. 직접 입력 흐름은 동일 분석을 재사용하거나 온디맨드 개별 역산을 실행한 뒤 합격 조건과 로드맵으로 이어집니다.
