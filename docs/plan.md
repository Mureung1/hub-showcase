...
초기 HTML/CSS 프로토타입은 화면 방향을 확정한 뒤 로컬 자료로 보관했다. 현재 구현의 기준은 `client/src/`와 [화면·데이터 흐름](screen-flow.md)이다.
...
## 데이터 저장 원칙 (멘토링 피드백 반영, 3주차)
감정 기록은 일반 서비스 데이터보다 민감하다. 회원가입·클라우드 저장 자체가 사용 장벽이 될 수 있고, 로그인 기능을 추가하는 것만으로 신뢰 문제가 해결되지는 않는다. 기능을 늘리기보다 사용자가 저장 방식을 선택할 수 있게 하는 쪽이 이 서비스 주제와 맞는다.

- **게스트 모드 (기본)**: 회원가입 없이 시작. 기록은 브라우저 저장 공간에만 저장하고 다른 기기와 동기화하지 않음. 전체 삭제·내보내기 제공.
- **AI 정리 (명시적 선택)**: AI 호출 시 입력 원문이 서버로 전송된다는 걸 사용 전에 안내. 실패 시 기본 문구로 대체, 호출 횟수 제한.
- **로그인·클라우드 동기화 (선택 기능)**: 로그인 없이도 핵심 기능을 쓸 수 있어야 함. 로그인하면 사용자가 고른 기록만 동기화. `user_id`·RLS·계정 간 데이터 격리 검증이 함께 필요.
