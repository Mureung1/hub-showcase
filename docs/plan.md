...
## 0.1 이번 주 방향 (2026-07-20 갱신)

배포된 파일럿 피드백(설문이 딱딱함·결과가 전문용어 덩어리·판정 근거 불투명)과 주간 미션(핵심 기능 1개·아키텍처 시각화·TDD·Agent 산출물)을 반영한 이번 주 작업 방향:

- **이해도 개선**: (a) 결과 추천 카드에 **쉬운 한 줄 정의 + 일상 행동예시**를 노출(오늘 반영), (b) 설문 문항을 쉬운 말로 재작성 + 예시·툴팁 보강(채점 매핑 불변), (c) 설문답→지표→추천 연결을 보여주는 **판정 근거·한계 신뢰 패널**.
- **핵심 기능 — 하루 스케줄 생성기**: 24h에서 필수시간(수면·수업·식사)을 빼고 남는 시간에 추천 학습법을 배치(`lib/schedule.js` `buildDayPlan`, 오늘 최소 슬라이스). 이후 완성 + 주간 타임테이블 골격.
- **LLM 간이 MBTI 추정 채팅([ADR-008](./decisions.md)):** 설문이 안 맞는 사용자가 외부 LLM과 동의 후 대화 → 간이 추정 MBTI를 `matchMethods`로 연결. 공식 판정 아님(P-B 유지), backend 프록시·폴백·키 사용자 제공.
- **MBTI 방어 + 매칭 근거 보강**: MBTI 신뢰도 한계는 투명 고지(신뢰 장치)로, 대신 **기질↔공부법 연결고리의 근거**([matching-criteria.md](./matching-criteria.md)·`evidence-catalog.md`)를 유형·기질별로 명확화(예: SJ/ENTJ=계획·분산).
- 근거 논문 보강은 **인지과학 공부법 쪽**(인출·분산·메타인지)에 집중한다(MBTI 타당성 옹호가 아니라 방법 효과의 근거).

세부 작업은 GitHub 이슈로 분해해 관리한다(오늘 등록분 포함).
...
#### 제품 의미 재정의 (2026-07-22, ADR-009)

이 제품은 성격풀이가 아니라 **자기조절 회고 도구**다. 사용자 흐름의 의미는 다음 한 줄로 압축된다:

> MBTI를 친숙한 **입구**로 삼아 → 실제 공부 행동·피로 신호를 행동지표로 **정리**하고 → 오늘 바로 시도할 방법·회복 루틴으로 **바꾸고** → 해본 결과를 예측과 **대조**해 "나에게 맞는 방식"을 스스로 **검증**하며 → 그 과정을 **개인 회고 리포트**(로컬 전용)로 남겨 회고·포트폴리오에 쓴다.

결과 화면·랜딩은 이 의미를 **명시(의미 프레임)**해 "그래서 내가 무엇을 얻었고 다음에 뭘 하는지"를 사용자가 알 수 있게 한다. 사용자가 준 일상 근거는 간이 추정 채팅에서 `observedSignals`로 관찰돼 판정 근거를 **보완**한다(대체 아님, Gemini만, ADR-008 확장).

**재점검에서 드러난 엣지 케이스와 대응:**
- (a) 공식 결과 없이 탐색만 한 사용자 → 산출물이 빈약했다 → 의미 프레임 + 개인 회고 리포트로 보완.
- (b) 재방문 사용자 → "지난번 대비" 맥락이 없었다 → 회고 리포트가 로컬 실행·피드백·보정오차 이력을 보여준다.
- (c) 결과를 저장·회고·공유할 형태가 없었다 → 회고 리포트 + 클립보드 복사(포트폴리오/회고용).

**의도적으로 채택하지 않은 것(유사 서비스 차용 제외):** 랭킹·좋아요·그룹 경쟁, 알림·PIP, 레벨 경쟁형 게이미피케이션 — 하드룰(비교·경쟁·알림 금지)과 충돌하므로 표현 방식만 참고한다.
