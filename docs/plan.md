...
| 플랫폼 관리자 | 약식 문의 수락·거절과 상세 신청 승인·거절. 병원 이용 중지는 확장 |

환자용 화면과 병원 관리자용 화면은 분리하지만 인증은 공통 Supabase Auth를 사용하고 역할·전화번호·상태는 `profiles`에서 관리합니다. MVP 플랫폼 관리자 화면은 간단 입점 문의와 상세 신청의 수락·승인·거절을 제공합니다.
...
3. 병원이 설정한 환자 분류별 인원 또는 총인원을 선택해 가족 단위 원격 웨이팅을 등록합니다.
...
2. 병원 직원이 전화번호와 병원이 설정한 분류별 인원 또는 총인원을 입력해 현장 웨이팅을 등록합니다.
...
1. 병원 관리자가 이메일 확인과 로그인을 마친 뒤 병원명, 대표 진료과, 주소와 대표 전화번호로 간단 입점 문의를 제출합니다.
2. 약식 플랫폼 관리자가 문의를 수락하면 병원 레코드가 생성되고 상세 검증 신청이 열립니다.
3. 병원 관리자가 사업자등록정보, 요양기관기호와 증빙 이미지를 제출합니다.
4. 플랫폼 관리자가 mock 검증 결과와 신청 정보를 확인해 승인하거나 거절합니다. P1 검색 화면은 준비된 mock 병원 데이터를 사용하고, 실제 DB 승인 상태에 따른 노출 제한은 P2에서 적용합니다.
5. 병원은 매일 하나의 통합 대기열을 열고 평균 진료시간과 원격 접수 한도를 설정합니다.
6. 직원은 원격·현장 환자를 같은 순서에서 보고 현장 접수, 도착 처리, 호출, 보류, 취소와 순서 조정을 수행합니다.
...
- 접수번호는 병원별 운영일마다 `1`부터 시작해 가족 웨이팅 단위로 1씩 증가합니다.
...
- 병원명·지역·진료과를 대상으로 한 mock 병원 검색과 `내 주변` mock 영역
- 병원 입점 신청, 증빙 mock 업로드와 플랫폼 관리자의 상세 승인·거절
...
- PostgreSQL 기반 병원 검색 API와 `approval_status = approved` 노출 제한
