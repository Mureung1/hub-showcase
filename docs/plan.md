...
- **2단계 (다중 사용자)**: Supabase Auth·RLS 적용, 사용자별 Discord 연결과 알림 발송으로 확장 (셀프 연동 G1 + 사용자별 라우팅 G2/G3 구현 — [roadmap.md](roadmap.md) WP-G)

### 서비스 원칙 — 법적 경계와 "거울 프레임"

AI가 매매를 다루는 이상 **자본시장법**상 규제(투자자문업·유사투자자문업)를 먼저 짚었다. 요지:

- **투자자문업**: 금융투자상품의 가치·**투자판단**(종목·취득/처분·수량·가격·시기)에 관한 자문을 특정인의 개별성을 반영해 **영업으로** 제공 → 등록제(자본금 요건).
- **유사투자자문업**: 불특정 다수 대상 투자판단·가치 **조언** + **대가 수취** → 신고제.
- 단순 금융 지식·정보 제공은 투자조언에 해당하지 않는다(금융위 유권해석).

**Beacon의 안전 지대**: ① **사후성** — 이미 실행한 *자기* 매매의 기록 회고(조언이 아니라 도구) ② **무대가** — 무료 ③ **객관 사실 통지** — 자기가 설정한 조건의 충족을 알릴 뿐.

이를 제품 규칙으로 못 박은 것이 **"거울 프레임"**이다. AI는 매매를 추천·평가하지 않고 **사용자 자신의 과거 행동을 비추는 거울**로 동작한다:

1. **미래 매매 지시 금지** — "사라/팔아라/기다려라" 불가. 과거 사실 서술과 질문형만.
2. **종목·시장 가치평가 금지** — "고평가다/오를 것" 불가. 분석 대상은 시장이 아니라 **사용자의 행동**.
3. **결과론 평가 → 계획 대비 평가** — 사후 수익률로 "좋은/나쁜 매매" 판정 금지. "계획(목표가·손절가·가설)을 지켰는가 / 계획이 없었는가"를 비춘다.
4. **용어 교체** — 법률상 '투자판단'과 겹치는 "판단"을 피하고 "관찰/돌아보기/복기 노트"로.
5. **면책 고지 상시 노출** — "Beacon은 투자자문·매매 권유 서비스가 아니며, 모든 투자 판단과 책임은 사용자 본인에게 있습니다."

이 원칙이 데이터·에이전트에 반영된 결과: 매매 기록에 **진입 계획 필드**(가설·목표가·손절가·보유기간·확신도)를 두어 "계획 대비 실행"을 비출 수 있게 하고, 복기 출력의 `repeated_mistake`('실수'라는 가치어)를 **`behavior_pattern`(행동 패턴)**으로 바꾸고 **`plan_adherence`(계획 준수)** 축을 신설했다. (스키마·프롬프트 반영은 [prd.md](prd.md) §2·§6, `0007` 마이그레이션.) 아래 §5~§6의 "반복 실수 피드백" 류 표현은 모두 이 거울 프레임 안에서 **행동 패턴의 사실 서술**로 읽는다.
