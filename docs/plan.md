...
- 분해 결과와 각 퀘스트의 진행 상태(완료·미완료·멈춤)를 영속 저장해, 앱을 다시 열어도 어디까지 했고 어디서 멈췄는지 기억한다.
- AI 응답 실패나 지연 시 대표 도전 유형 템플릿을 제공한다.
- 사용자가 특정 단계에서 멈추면, 저장된 진행 상태를 근거로 해당 지점을 더 작은 퀘스트로 재분해할 수 있다.
...
- 사진과 메모를 선택적으로 첨부하면 보너스 보상을 지급하고 성취 기록을 영속 저장한다.
- 완료 이력과 진행 상태를 저장해, 재접속 시에도 이어서 도전을 계속할 수 있다.
...
> 진행 상태·완료 이력을 저장(T)하기 때문에, 앱을 닫았다 다시 열어도 어디서 멈췄는지 기억하고 그 지점부터 재분해·재개할 수 있다. 분해해 주고 끝나는 일반 AI 도구와 달리, 결과가 살아남아 실행 루프에 연결되는 것이 One-Step의 차별점이다.
