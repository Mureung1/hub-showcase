...
1. 현재 가중치와 경계값을 표로 감사한다.
2. `rules-v1` 결과와 추천 피드백에 동일한 알고리즘 버전이 기록되는지 회귀 테스트한다.
3. 문헌 근거 카탈로그의 claim·population·outcome·boundary 필드를 작성한다.
4. 사용자 1~2명 사용성 검증에서 단정감, 이해도, 실행 가능성을 확인한다.
5. 로그인·서버 저장은 데이터 거버넌스 게이트를 통과한 뒤 별도 기획한다.
6. 대학 플랫폼과 OpenAI 기능은 공식 인터페이스와 구체적 사용자 가치가 확인될 때 재평가한다.
