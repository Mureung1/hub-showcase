...
개인 메모는 이번 계약에 포함한다. 공유 기능과 조리 팁은 후속 범위로 둔다.
...
- 메모리 세션 저장소는 로컬 개발에서만 사용하며 운영 저장소는 데이터베이스 선택 시 확정한다.
...
삭제·복원 API는 정책만 확정하고 핵심 흐름 완료 후 구현한다. 공유 API와 관련 응답 필드는 다음 주 공유 기능 설계 시 추가한다. 조리 팁은 후속 범위로 유지한다.
