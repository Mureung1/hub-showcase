...
```mermaid
flowchart LR
    photo["📷 사진 있음"] --> ai1["Gemini 식별"] --> db["식약처 DB 조회"] --> clamp["현실 범위 보정"] --> card
    text["⌨️ 메뉴명만"] --> ai2["Gemini 표준 1인분 추정"] --> clamp2["표준 1인분 보정"] --> card
    label["🏷️ 영양성분표 스캔"] --> ai3["Gemini 표 추출<br/><i>추정 아님</i>"] --> card

    card["결과 카드<br/>(같은 상태 머신)"] --> save["저장 → dataStore"]

    classDef nodb fill:#FFF4E5,stroke:#FF9F1C
    class text,ai2,clamp2 nodb
```

> 주황색 경로(메뉴명만)는 **식약처 DB를 타지 않는다** — AI 추정치를 표준 1인분 범위로 보정만 한다.
> 그래서 출처 배지가 `추정`/`공식`으로 나오고, 사진 경로의 `식약처DB`보다 신뢰도가 한 단계 낮다. → 발견 3번
...
### 🟠 3. 텍스트 분석은 식약처 DB를 안 탄다 (화면에 티가 안 남)

사진 경로는 DB 조회 → 환산 → 보정을 거치지만, 메뉴명만 입력한 경로는 **AI 추정치 + 표준 1인분 보정**이 전부다. 출처 배지(`추정`/`공식` vs `식약처DB`)로만 구분되는데, 입력 필드 라벨은 "사진 없이 분석 가능"이라 정확도 차이를 알리지 않는다.
→ 선택지: 텍스트 경로에도 DB 캐스케이드를 태우거나, 입력 시점에 정확도 차이를 한 줄로 안내.
