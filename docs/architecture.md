...
기획 → 디자인·프로토타입(mock) → 프론트(`product`, 샘플) → 백+DB(`server`, rule 통계·API 계약) → AI 에이전트(LLM 역산·문구, 샘플) → MVP 배포 → 확장(백엔드 외 직군 데이터·실데이터 수집).

rule 뼈대를 먼저 만들고 그 위에 LLM 에이전트를 얹습니다. LLM 도입과 실데이터 수집은 분리해, 에이전트는 샘플 데이터 위에서 먼저 검증합니다.
