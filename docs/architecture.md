...
| 데이터·에이전트 실행 흐름 | 데이터 변경, 영향 범위 계산, 여섯 에이전트 실행과 버전 활성화 | 이 문서 5장 |
...
flowchart TB
    ...
    SOURCES["기업 채용 페이지·공식 자료<br/>공공 표준·외부 전략 자료"] --> COLLECTOR
...
분석 에이전트는 공통 지식 조회 계층과 앞 단계 산출물을 조회해 결과와 검증된 관계를 같은 분석 버전에 저장한다. 데이터 수집 에이전트는 원문·출처를 저장하고, 지식 구축 에이전트는 원문 기반 노드·관계를 구축하며, 통계 분석·채용공고 해석·합격 전략·준비 로드맵 에이전트는 자기 산출물에서 파생된 관계를 기록한다. 각 결과 원통은 같은 분석 산출물 저장 영역의 단계별 결과를 나타낸다. 일반 사용자 조회와 체크 상태 조합은 검증을 통과한 활성 분석 버전만 사용한다.
...
| 데이터 수집 에이전트 | 자료 발견·수집·평가, 원문·출처 메타 저장 | 사용 | 데이터 갱신 | `agent/` |
| 지식 구축 에이전트 | 원문 정제, Wiki·벡터 인덱스·그래프 구축 | 사용 | 데이터 갱신 | `agent/` |
...
sequenceDiagram
    ...
    participant Collector as 데이터 수집 에이전트
    participant Knowledge as 지식 구축 에이전트
    ...
    Orchestrator->>Collector: 영향 자료 수집
    Collector->>DB: Raw·출처 메타 저장
    Orchestrator->>Knowledge: 지식 갱신
    Knowledge->>DB: Wiki·벡터·그래프 저장
    ...
    Note over Interpretation,Roadmap: 근거가 부족하면 데이터 수집 에이전트에 조사 요청 후 재조회
...
이 다이어그램은 채용공고 변경으로 여섯 에이전트가 모두 필요한 전체 실행을 나타낸다. 분석 오케스트레이터는 변경 유형에 따라 영향이 없는 단계를 생략한다.

채용공고 해석·합격 전략·준비 로드맵 에이전트는 필요한 근거가 부족하면 데이터 수집 에이전트에 조사 요청을 전달한다. 새 자료가 저장되면 요청한 에이전트가 동일 실행 안에서 검색과 생성을 반복한다. 반복 횟수와 검색 비용은 실행 정책으로 제한한다.
...
| 변경 데이터 | 데이터 수집·지식 구축 | 통계 분석 | 채용공고 해석 | 합격 전략 | 준비 로드맵 |
| --- | --- | --- | --- | --- | --- |
