...
## 2026-07-30 (showcase AI 에이전트 활용 서술 보강)
- 멘토가 개발자의 판단 과정과 AI 활용 경계를 빠르게 이해할 수 있도록 `showcase/showcase.json`의 개발 여정·AI 협업·에이전트 운영 설명을 보강했다.
- 범용 AI 대비 제품의 존재 이유, AI 생성과 결정적 시스템의 분리, 역할 중심 멀티에이전트 실험에서 작업 중심 운영으로 전환한 이유, 자동 검증과 사람 문구 검수를 분리한 고민을 기존 작업 기록에 근거해 명시했다.
- 개인 말투 기능의 관계별 mock 회귀 발견을 구체 사례로 추가하고, 도구 목록은 이름 나열 대신 각 도구가 맡은 책임이 드러나도록 정리했다. 기존 전시 이미지 경로는 변경하지 않았다.
- 사용자가 제공한 Google Drive 소개 자료를 `presentationUrl`로 추가했다.
- 프론트엔드·백엔드/배포·DB·AI 모델·검색·캐릭터·검증 도구별 선택 이유와 trade-off가 드러나도록 `techHighlights`를 재구성했다.
- 범용 AI 대비 우위는 아직 외부 사용자 과업으로 검증하지 않았고, Gemini holdout은 통과했지만 모델 간 지연·실비용 비교는 기록하지 않았다는 한계를 함께 명시했다.
