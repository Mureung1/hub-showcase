...
상태: 구현 완료 (2026-07-22). `checkinService.test.js` 통과, curl로 라우트 e2e 확인, `[검증테스트]` 더미 행 정리 완료.
...
- [x] 삭제 버튼 클릭 시 확인창 표시
- [x] 확인하면 목록 화면으로 이동
- [x] 삭제된 기록이 목록/캘린더에서 사라짐
- [ ] 새로고침해도 다시 안 나타남 — 브라우저 수동 확인은 아직 안 함(서버 라우트는 curl e2e로 확인)
- [x] 존재하지 않는 id 요청 시 404 + 화면에 에러 메시지
- [x] `[검증테스트]` 더미 행 정리 완료
- [x] `checkinService.test.js`에 `deleteCheckin` 테스트 통과
