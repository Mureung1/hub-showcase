...
현재 Inspector의 시간대 막대는 값을 꾸며 만든 추이가 아니다. 선택 분기의 실제 시간대
집계값을 선택하면 `명/분기`로 보여 주고, 막대 높이는 **같은 상권 안에서 시간대끼리
비교하기 위한 상대값**임을 함께 표시한다. 따라서 사용자는 막대의 모양만 보고 다른
상권과 절대 인구를 비교하지 않는다.

원천의 특정 시간 구간이 없으면 API의 `flow_time_buckets`와 legacy `flow_by_time` 모두
`null`을 유지한다. `0명`으로 바꾸지 않으며 화면에서는 `데이터 없음`으로 분리한다.

분기 선택기는 점포·개폐업 원천이 존재하는 모든 실제 분기를 보여 준다. 매출과
길단위인구까지 모두 있는 분기는 `전체 분석`, 점포·개폐업만 있는 분기는
`점포·개폐업`으로 구분한다. 기본값은 가능한 가장 최신의 `전체 분석` 분기이며, 부족한
매출·유동인구를 0으로 채워 더 최신 분기처럼 보이지 않는다.
...
- 현재 `상권 입지 점수`는 선택한 점포 한 곳의 개별 점수가 아니라, 선택 점포가 속한 상권의 공식 집계값이다. 반경은 주변 점포 목록과 경쟁 현황의 조회 기준이며, 좌표별 점수 기능은 별도 분석 과제로 둔다.
...
- 제품 화면의 현재 분석 기준은 서울시 공식 상권 경계 하나로 고정한다. 반경·지도 위치 이동 API는 향후 공간 분석 기능을 위한 서버 contract로 유지하지만, 점포·경쟁·상권 지표가 서로 다른 공간 기준으로 보이는 문제를 막기 위해 일반 사용자 UI에는 노출하지 않는다.
