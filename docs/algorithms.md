...
### 핵심 목표
- **임박 재료 최우선 해결**: 다른 어떤 기준보다 유통기한 임박 식자재의 소진을 최우선.
- **최소 끼니 수로 최대 소진**: 임박 재료 5가지를 5개 요리로 따로 소진하는 것보다, 2개 요리로 전부 소진하는 조합을 우선 추천.

### 알고리즘 (크기 오름차순 Set Cover 브루트포스)

```text
INPUT : threshold, K_max(=3)
