...
8. **요청 속도 제한**: `/v1/chat`은 계정(로그인 사용자) 단위로 분당 3회·시간당 10회로 제한한다([기획서](docs/wiki/기획서.md) 3.8). 데모 범위라 별도 라이브러리 없이 메모리 슬라이딩 윈도우로 구현하고, 한도값은 `CHAT_RATE_PER_MINUTE`·`CHAT_RATE_PER_HOUR` 환경변수로 조정한다. IP가 아니라 계정 기준이다.
