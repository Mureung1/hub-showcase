...
- **완료**: 화면 뼈대(6개 화면 조건부 렌더링, `docs/etc/component-tree.md`), Notion 연동 기반(`app/lib/notion.js`, T01), Brain Dump category 확장 + Notion 저장(T02), One-Focus View 실데이터 연결(T03) — `/api/steps`로 Notion에서 오늘 할 일을 읽어와 순회하고, 완료 시 `/api/steps/complete`로 Done 갱신. AgentLog DB 생성 + 기록/조회 lib(T05) — `app/lib/agentlog.js`의 `logStruggle`/`getRecentLogs`. "힘들어" 루프 판단 API(T06) — `POST /api/struggle`, cold start prior·거절 tool 제외·최근 로그 기반 판단.
- **진행/예정**: T07(재판단 시간 게이트)·T08(이유 칩+수락/거절 UI)부터. Full Screen Timer 지속성(T04)은 뒤로 미룸(Agent 루프 우선).
