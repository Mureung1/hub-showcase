...
- **완료**: 화면 뼈대(6개 화면 조건부 렌더링, `docs/etc/component-tree.md`), Notion 연동 기반(`app/lib/notion.js`, T01), Brain Dump category 확장 + Notion 저장(T02), One-Focus View 실데이터 연결(T03) — `/api/steps`로 Notion에서 오늘 할 일을 읽어와 순회하고, 완료 시 `/api/steps/complete`로 Done 갱신. AgentLog DB 생성 + 기록/조회 lib(T05) — `app/lib/agentlog.js`의 `logStruggle`/`getRecentLogs`. "힘들어" 루프 판단 API + 재판단 시간 게이트(T06·T07) — `POST /api/struggle`. 이유 칩 + 제안/수락/거절 UI(T08) — `onStruggle`이 `ReasonChips` → `ProposalCard` → tool 실행/재판단으로 연결됨, 수락/거절마다 `/api/agent-log`로 실제 기록. outcome 기록(T09) — encourage/shrink_step으로 이어서 완료 시 `markOutcomeDone`, Brain Dump 시작 시 `sweepStaleLogs`로 어제까지의 pending 일괄 정리.
- **진행/예정**: T10(개인화 — `recentLogs` 실제 연결)부터. Full Screen Timer 지속성(T04)은 뒤로 미룸(Agent 루프 우선).
- **아직 mock/미완**: `/api/struggle` 호출 시 `recentLogs`가 아직 항상 빈 배열(T10에서 `getRecentLogs` 실제 연결 예정).
