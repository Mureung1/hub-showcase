...
The differentiating features (per `docs/plan.md`) are the gap-analysis engine (AND-matching against 5 requirement categories, with "what-if I improve X" simulation) and the visualization result screen (donut chart, priority bar chart, insight popup, filterable job list, per-job checklist modal). Computer literacy (컴퓨터활용능력) is collected but excluded from pass/fail judgment — it's a "nice to have" signal only.
...
- `src/routes/health.routes.js` — `GET /api/health`, does a trivial `SELECT 1` through `better-sqlite3` to prove the DB connection is alive. This is the only real endpoint so far; `jobs`/`gap-analysis` routes described in `docs/checklist.md` don't exist yet.
- `src/db/connection.js` — opens `better-sqlite3` at `DB_PATH` (WAL mode). No schema/migrations yet. `server/data/jobs_data.csv` (862 real JOB-ALIO postings + rule-synthesized spec fields — see `docs/checklist.md`'s "확정 사항") is the real seed source; a `jobs` table (single table mirroring the CSV columns directly, per the week-2 issue plan) plus an `analysis_results` table and a seed script to load the CSV still need to be written. `src/services/` is the intended home for `gapAnalysisService.js` but is currently empty.
...
### Docs and prototype
- `docs/plan.md` is the authoritative product spec: user flow (landing → filter → spec input → loading → insight popup → results, plus the optional login/bookmark track), the screen table, and the branching insight-popup copy by scenario (4 branches: single-item gap / all requirements met / multi-item gap / zero matching postings). Read this before building new screens so flows match the intended state machine. Kept up to date with what's actually shipped — was originally `plan_3.md`, renamed and rewritten 2026-07-30 once it had drifted from the live app (old resume-banner wording, missing login/bookmark/guide/reference-link features, only 3 insight-popup branches).
- `docs/checklist.md` is a merged, fully-completed task-breakdown/issue-tracker doc (originally three separate files: `checklist_2.md`'s numbered 0–4 breakdown, `checklist_3_login_bookmark.md`'s login/bookmark detail, `checklist_4_week3_4_issues.md`'s #15–28 issue list — combined into one file 2026-07-30, kept as history/rationale reference, not an active task list). Each part's "확정 사항" block (data source, AND-matching rule, single-item-gap-only improvement counting, no weighting, no region/salary filter, Supabase Auth decision, etc.) should be treated as settled — don't re-litigate these without new justification. Part 1's 2-table schema sketch (`jobs` + `job_requirements`, camelCase fields) predates the current plan; the actual week-2 issue breakdown uses a single `jobs` table with the CSV's own snake_case column names instead (see below).
...
- **Auth & bookmarks (login/bookmark extension track only, decided 2026-07-21)**: Supabase Auth (client-side `@supabase/supabase-js`) for signup/login/logout/session, **not** Express's own session+cookie system as originally planned — avoids the cross-origin cookie complexity (`sameSite=None; Secure` + CORS `credentials:true`) that a local dev proxy can't reproduce and that would otherwise only surface at deploy time. The `bookmarks` table lives in Supabase's own hosted Postgres, not the local `better-sqlite3` file — avoids losing account/bookmark data if the deploy platform wipes the filesystem on redeploy. Express still gets a `requireSupabaseAuth` middleware to verify the Supabase-issued JWT (sent via `Authorization: Bearer`, not a cookie) on `/api/bookmarks/*`. This decision is scoped to the login/bookmark extension track (#19/#20) only — `jobs` and `analysis_results` stay exactly as they are in local SQLite, untouched. See `docs/checklist.md`'s Part 2 for the full breakdown.
