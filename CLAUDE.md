...
- **직접적이고 구체적으로** 답한다. 두루뭉술한 요약·일반론 대신 이 코드베이스의 실제 값·파일·함수로 답한다.
- **중간 단계를 항상 보여준다.** 결론만 던지지 말고, 거기까지 어떻게 왔는지(데이터 흐름·호출 순서·변환 과정)를 단계로 드러낸다.
- **생성·수정한 코드의 구조를 설명한다.** 어떤 부분이 무슨 역할이고 서로 어떻게 연결되는지, 왜 이렇게 짰는지를 함께 준다.
- **직관적으로 이해되게 한다.** "왜 이런 결과가 나오는지"를 원인→결과로 풀어서, 코드를 안 읽어도 감이 잡히게 설명한다.
- **다른 선택지도 제시한다.** 택한 방법 외에 어떤 대안이 있었는지 밝히고, **왜 이 방법은 되고 다른 방법은 안 됐는지**(트레이드오프·제약·실패 원인)를 근거로 비교한다.
