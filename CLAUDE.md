...
**Current state: scaffolding plus raw seed data, no feature logic yet.** Routing, the Express skeleton, and the dev-server wiring all work end-to-end, and the real seed CSV now lives in the repo (`server/data/jobs_data.csv`), but the gap-analysis engine, DB schema, real forms, and charts have not been ported from `prototype/demo_12.html` yet. Real development starts in week 2 per the team's plan — don't assume any business logic exists beyond what's described below.
...
- `prototype/demo_12.html` is a **static, self-contained HTML/CSS/JS mockup** of the full app (filters, spec form, donut/bar charts, job cards, modal) — not wired to React, and the primary reference for porting real UI/logic. Supersedes `prototype/demo_11.html`, which was removed 2026-07-20 after a design pivot (see below). Highlights:
  ...
  - `:root` CSS variables (`--gray-900`, `--green-bg`/`--green-text` per status tag, etc.) are read by name as strings from JS (`STATUS_STYLE`, `JOBTYPE_STYLE`, `donutSvg`) — **keep variable names identical** when porting into `src/styles/tokens.css`; only values are free to change. **Design direction changed 2026-07-20**, based on a reference public-service site: single indigo/violet accent (`--accent: #5b52e8`) on a lavender-tinted background (`--gray-50`/`--gray-200`/`--paper`-less full-bleed page), replacing demo_11's mint/warm-cream/dark-accent tone. The landing hero is now a 2-column split (copy + a donut-chart result preview card), and demo_11's floating boxed `.site-frame` "window" card was dropped — pages are full-bleed now, not a centered card on a colored backdrop. Design tokens still haven't been ported into React — every screen built so far, including the landing/home screen (#14), still uses a temporary dark+violet placeholder hardcoded in `src/App.css` (an unrelated placeholder purple, not demo_12's real token values) — issue #22 is the tracked work to port demo_12's actual tokens over this placeholder.
