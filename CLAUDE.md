...
- **Storage**: 프로토타입 단계는 목데이터 + localStorage(초안). 기획서상 목표는 Supabase(Auth + PostgreSQL + Storage)이며, 도입 시점에 이 문서에 먼저 기록하고 진행한다.
...
- **LLM 피드백(기획서 §3.4)**: 온디맨드 호출만(작성 중 버튼 또는 발행 직전), 유저당 일일 호출 제한. 응답은 자유 텍스트가 아니라 **섹션별 코멘트 JSON 배열**로 강제하고, frontend는 이를 사람 코멘트와 동일한 UI(`is_ai` 라벨만 다름)로 렌더링한다. 피드백 관점은 4가지로 한정: 구조 완결성 / 구체성 / 예외 처리 질문 생성 / 역기획 관점(요약 vs 의도 분석). 게임 사실관계 평가는 시키지 않는다.
