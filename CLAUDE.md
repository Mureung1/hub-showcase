...
## 작업 방식
- 라이브러리·데이터 출처·문서 구조·리서치 범위처럼 여러 타당한 선택지가 있는 판단은 스스로 정하지 않는다. 근거와 추천안을 먼저 제시하되 최종 선택은 사용자가 하도록 먼저 질문한다(관련 스킬: `ask-before-deciding`).
- 새 기능이나 여러 파일에 걸친 작업은 코드부터 짜지 않고 설계서(기능 단위: 완료기준·크기·빠진것·장단점·대안·수직정의·화면·테이블·API) 또는 구현 전략(작업 단위: 파일과 역할·데이터 흐름·이유와 대안)을 먼저 제시하고 검토받는다(관련 스킬: `design-first`, 재사용 프롬프트: @docs/prompt-templates.md). 오탈자 수정 등 자명한 작업엔 강제하지 않는다.
- 설계서·구현 전략 어디에서든 장단점과 별개로 **한계점**(이 방식이 못 하는 것, 어디서 깨지는지)을 반드시 명시한다 — 장단점 항목에 뭉뚱그리지 않는다. 한계를 알아야 사용자가 스스로 다른 방법을 떠올릴 수 있기 때문이다.
...
- API 키·DB 접속 정보(`DATABASE_URL`, `ANTHROPIC_API_KEY` 등) 같은 비밀값을 코드에 하드코딩하거나 커밋하지 않기 — `.env`에만 두고 `.gitignore`로 제외, `.env.example`엔 실제 값 대신 자리표시자만 남긴다

## 참고
- 기획서: @docs/기획서.md
- 경쟁사 분석: @docs/경쟁사분석.md
- 디자인(Claude Design 실제 화면 기준, 화면 검토 전이라 바뀔 수 있음): @docs/design.md
- 설계 먼저 요청할 때 쓰는 프롬프트 템플릿: @docs/prompt-templates.md
