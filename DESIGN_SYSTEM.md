...
| `inline-loading` (실제 앱 `LoadingIndicator`) | 데이터 fetch가 진행 중일 때 보여주는 로딩 표시 — 냉장고를 들여다보는 끼니 영상(`로딩-애니메이션.mp4`) + `text-secondary` 기니 1인칭 카피("기니가 냉장고 재료로 만들 요리를 찾는 중..."). `fullScreen` prop으로 두 가지 변형: 기본은 영상 `w-10` + 카피를 가로로 중앙 정렬한 한 줄 표시(홈 화면 fetch 중), `fullScreen`은 화면 전체를 덮는 영상 위에 카피가 하단에 떠 있는 형태(`?loading=1` 개발용 미리보기). `empty-state-card`(결과 0건 전용, CTA 있음)와 달리 카드·CTA 없이 로딩 상태만 알림 — "확인 중"과 "결과 없음"이 시각적으로 헷갈리지 않게 분리 | `text-secondary` |
