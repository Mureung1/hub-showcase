...
| `inline-loading` | 데이터 fetch가 진행 중일 때 보여주는 한 줄 표시 — 마스코트 소형 아이콘(`마스코트-끼니 - 여백 줄임.png`, `w-10`) + `text-secondary` 기니 1인칭 카피("기니가 냉장고 재료로 만들 요리를 찾는 중...")를 가로로 중앙 정렬. `empty-state-card`(결과 0건 전용, CTA 있음)와 달리 카드·CTA 없이 로딩 상태만 알림 — "확인 중"과 "결과 없음"이 시각적으로 헷갈리지 않게 분리. 홈 화면 추천 로딩 중에 사용. 실제로 "찾는 동작"을 보여주는 커스텀 일러스트는 아직 없음(정적 이미지 + 카피만으로 표현) — 캐릭터 최종본 확정 후 후속 작업으로 교체 예정(`checklist.md` 백로그 참고) | `text-secondary` |
