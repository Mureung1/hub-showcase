...
| `c56e1c3` | Gemini 사용량 소진 시 폴백 — 데모 채점(라벨) + 큐레이션 콘텐츠 |
| `f31227f` | 설정 내 정보에 프로필 연결 + 뉴스 기록 축별 점수 구분 표시 |
| `15d4536` | 캡처 업로드 UI 잔손질 — 스피너 회전 + 채운 칸 펼치기 |
| `30b9a48` | 폴백 발동 분류(friendlyError) 테스트 추가 |
...
| **화** | ✅ 완료(`f31227f`) | ① 설정 내 정보에 직업·관심 분야 연결(옛 프로필 "-" 해소) ② 학습 기록 카드에 뉴스는 자체 축(핵심 포착·사실 정확성·압축) 라벨로 구분 표시 |
| **수** | ✅ 완료(`15d4536`) | 카톡 캡처 경로 실제 이미지로 e2e 검증(200·vision 정상). UI 잔손질: 캡처 스피너 회전, 채운 who/tension 칸 자동 펼침 |
| **목** | ✅ 완료(`30b9a48`) | 폴백 발동 분류(friendlyError) 테스트 4개 추가. tsc=0·vitest 143개·build 전부 통과. `30b9a48` main push → 배포 |
...
### 미해결
- ~~설정 `내 정보 → 목표: -` 비어 있음~~ → **해결(`f31227f`)**: 직업·관심 분야를 직접 표시.
- ~~뉴스 기록 축별 점수 구분 표시~~ → **해결(`f31227f`)**: `History.tsx`가 `modeOf`로 갈라 뉴스는
  `fromScores`+`NEWS_AXES`(핵심 포착·사실 정확성·압축), 대화·메일은 `AXES`(맥락·격식·전략) 라벨로 표시.
- ~~캡처 업로드 경로 실제 이미지 미검증~~ → **해결(`15d4536`)**: 카톡 대화 이미지로 e2e 확인
  (직접 `/api/capture` 200·vision 정확, 실제 파일 업로드 핸들러도 title/who/tension 자동 채움).
