...
> 키·DB 없이도 부팅됩니다. DB 없으면 상태는 localStorage 폴백, 키 없으면 채점만 503 안내.
...
1. **Vercel → New Project → 이 GitHub 리포(`zer8m/uncoach`) import**
   - 기존 프록시(`uncoach-pi`)와는 별개의 새 프로젝트로 만든다.
...
> **함수 실행 시간**: 채점(`/api/score`)은 Claude adaptive thinking으로 수 초~수십 초 걸릴 수 있어
> route에 `maxDuration = 60`을 뒀다. Vercel **Hobby(무료)** 는 함수 최대 실행이 제한적이라
> 긴 채점이 잘리면 **Pro** 로 올리거나 `SCORING_MODEL`을 더 빠른 모델(`claude-sonnet-5`)로 낮춘다.

## 라이브 프로토타입과의 관계

- 현재 라이브(https://unco-965ab.web.app)는 **커스텀 DSL 프로토타입**(리포 루트 `index.html`, Firebase Hosting).
- 이 `web/` 는 그 후속인 **정식 풀스택 재구현**. 병렬로 두고 완성되면 전환한다.
