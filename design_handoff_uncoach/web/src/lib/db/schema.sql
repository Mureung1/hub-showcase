...
-- app_state 는 owner 키로 저장: 로그인 전 device_id, 로그인 후 'u:{userId}'.
create table if not exists app_state (
  device_id  text primary key,      -- owner 키(호환 위해 컬럼명 유지)
  data       jsonb not null,
  updated_at timestamptz not null default now()
);

-- ── 계정(옵션) — 익명(기기)으로 시작 후 원하면 계정으로 승격 ──────────────
create table if not exists users (
  id         text primary key,      -- uuid
  email      text unique not null,
  pass_hash  text not null,         -- 'salt:hash' (scrypt)
  created_at timestamptz not null default now()
);

create table if not exists auth_sessions (
  token      text primary key,      -- 랜덤 opaque 토큰(쿠키)
