 ...
 *  → 사장님이 눈앞에서 [사장님 확인] 버튼 탭 → server가 1장 차감
 *  (다음 단계 개선: 사장님 PIN 6자리 입력으로 신원 확인 강화)
 */
export default function TicketRedeemPage() {
  ...
  async function handleRedeem() {
    ...
    setError("");
    try {
      const result = await redeemTicket(ticketId); // server가 차감 판단
        ...
        {done ? (
          ...
          <>
            <div className="redeem-guide">
              📱 이 화면을 <strong>사장님께 보여드리고</strong>, 사장님이 직접 확인 버튼을 눌러주세요.
            </div>
            {error && <p className="redeem-error">{error}</p>}
            <button className="redeem-btn" onClick={handleRedeem} disabled={busy || remaining <= 0}>
              {busy ? "처리 중이에요…" : "사장님 확인 (1장 사용)"}
            </button>
