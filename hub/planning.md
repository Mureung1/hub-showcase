...
건강 상태에 맞는 식품 정보 부재와 영양제 오남용으로 **부작용 위험에 노출된 소비자**가, 자신에게 꼭 필요한 건강기능식품 성분을 분석받아, 기업 규모(대기업·중소기업·소상공인)와 상관없이 **성분과 품질이 우수한 기능성 제품과 신뢰성 있게 연결되지 못하는 문제**.
...
4. **[제품 추천 화면] 객관적 비교 및 추천**
    - 에이전트가 기업 규모(대기업·중소기업·소상공인)에 상관없이, 추천된 기능성 원료를 안심하고 섭취할 수 있도록 우수한 **건강기능식품 목록**을 성분/가격 비교 카드 형태로 나열한다.
5. **[상세/스토어 연결] 구매 페이지 이동**
    - 소비자는 마음에 드는 제품의 상세 검증 데이터(HACCP, 시험성적서 등)를 확인한 후, **[스마트스토어로 이동]** 버튼을 눌러 실제 구매를 완료한다.
...
1. **기능 A: 개인 상태 맞춤형 기능성 성분 분석 및 건기식 추천**
    - **필수성**: 몸 상태(설문)에 맞는 성분 처방 및 가공 제품 매칭이라는 본질적 기능.
    - **구현**: 신체 증상 기반 기능성 원료 매핑 알고리즘 + 기업 규모 무관, 성분/품질 기준 건강기능식품 DB 필터링 노출.
...
flowchart TD
    ...
    Safety -->|중복 확인 완료| Recommend[5. 추천 화면: 건기식 카드 목록]
...
| 4 | 제품 추천 화면 | 제품 비교/추천 (기업 규모 무관, 성분 기준) | 제품 카드 목록(이미지, 성분/가격 비교, 업체명) |
