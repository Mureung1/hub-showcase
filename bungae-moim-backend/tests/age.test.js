...
// isAdult는 (pg가 주는 로컬 자정 Date와 맞추려고) now의 로컬 게터(getFullYear 등)를 쓴다.
// 그래서 여기서도 절대 시각(문자열+오프셋)이 아니라 로컬 구성요소로 Date를 만들어야
// 어느 머신의 타임존에서 실행해도 "그 지역의 2026년 7월 20일 정오"로 일관되게 해석된다.
// (절대 시각으로 만들면 로컬 게터로 읽었을 때 타임존에 따라 날짜/시각이 달라져 버린다.)
