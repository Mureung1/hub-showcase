...
POSTINGS: tuple[dict[str, Any], ...] = (
    {
        ...
        "summary": "점검을 많이 해 본 사람보다 찾은 것을 정확히 전달하는 사람을 찾습니다. 기준선 항목은 대체로 공통 기대치 그대로이고, 시큐어 코딩 관점의 검토가 이 공고의 변별점입니다.",
    ...
    },
    {
        ...
        "summary": "규제와 절차가 업무의 형태를 정하는 팀입니다. 탐지와 장비 운영은 기준선의 연장이고, 금융 규제 대응과 사고 대응 절차가 실질 변별점입니다.",
    ...
    },
    {
            ...
            ("우대사항", (
                ("보안관제 또는 로그 분석 업무를 경험해 보신 분", "soc-siem", "foundation",
                 ("mark", "로그 분석 — 이 기업군의 단골 우대",
                  "서비스 규모가 크면 경보의 양 자체가 문제입니다. 정상 기준선을 정의해 본 기록이 있으면 우대 이상으로 읽힙니다.",
                  "mid", "같은 직군 80%")),
        ...
        "summary": "개발 조직 옆에서 함께 일하는 보안 엔지니어를 찾습니다. 기준선 항목이 그대로 통하되 코드 관점의 설명력과 로그 분석이 추가 요구입니다.",
        ...
        "sections": (
            ...
            ("우대사항", (
                ...
                ("정보보안기사 또는 ISMS-P 인증심사원 자격을 보유하신 분", None, "foundation",
                 ("base", "정보보안 기초 이론과 자격",
                  "심사원 자격은 신입에게 현실적이지 않습니다. 기사 범위의 기초 이론까지가 실질 기준선입니다.")),
                ("고객사 대상 문서 작성과 발표 경험이 있으신 분", None, "foundation", None),
            )),
        ),
        "summary": "찾는 일보다 증명하는 일의 비중이 큰 자리입니다. 진단과 보고서는 기준선 그대로이고, 관리체계 요구사항 이해가 이 공고의 실질 변별점입니다.",
        "summary_ratio": "편차 1건 · 기준선 일치 4건",
    },
    {
        "nn": "05",
        ...
        "sections": (
            ...
            ("우대사항", (
                ("웹·클라이언트 취약점 진단을 경험해 보신 분", "web-vuln-assessment", "application",
                 ("base", "웹 취약점 점검 수행",
                  "게임사라도 웹 진단은 기준선 그대로입니다. 클라이언트가 붙었을 뿐 준비는 그대로 통합니다.")),
        ...
        "summary": "실시간 서비스에서 정상과 이상을 가르는 기준을 세울 사람을 찾습니다. 진단은 기준선 그대로이고, 이상 행위 식별과 트래픽 대응이 변별점입니다.",
...
BASELINE_ITEMS: tuple[tuple[str, str, str, str], ...] = (
    ...
    ("security-cert", "privacy-compliance", "정보보안 기초 이론과 자격",
     "정보보안기사 등 자격을 우대로 명시하는 공고가 꾸준합니다. 자격 자체보다 그 범위의 기초 이론이 기준선입니다."),
)
...
CONCEPTS: tuple[tuple[str, str, str, str, str, tuple[str, ...], str, bool], ...] = (
    ("web-vuln-lab", "웹 취약점 진단 실습", "점검 환경을 직접 세우고 돌려 보기",
     "기준선 · 최근 공고 전량이 취약점 점검을 요구합니다", "실습 환경 구성 기록 + 점검 항목별 결과 정리",
     ("portfolio",), "project", True),
    ("diagnosis-report", "진단 보고서 작성", "위험도와 조치 우선순위까지",
     "기준선 · 진단 업무의 산출물이 보고서입니다", "진단 보고서 1건 + 조치 확인 이력",
     ("portfolio", "interview"), "project", True),
    ("log-analysis", "보안 로그 분석 실습", "정상과 이상을 가르는 기준 세우기",
     "기준선 · 로그를 읽는 감각은 전 기업군 공통입니다", "공개 로그 데이터 분석 노트 + 판단 근거",
     ("portfolio", "interview"), "project", True),
    ...
    ("network-lab", "네트워크 보안 실습 환경", "구간을 나누고 정책을 적용해 보기",
     "기준선 · 네트워크 기본기는 사실상 전제 조건입니다", "실습 구성도 + 정책 적용 전후 확인 기록",
     ("portfolio",), "project", True),
    ...
    ("privacy-checklist", "개인정보 점검표 작성", "법령 항목을 업무 절차로 옮기기",
     "기준선 · 개인정보 요구는 전 기업군에 반복됩니다", "처리 흐름도 + 항목별 점검표",
     ("portfolio",), "project", True),
    ...
    ("security-ethics", "보안 담당자의 책임 서사", "권한을 다루는 태도",
     "기준선 · 보안 직무는 태도 검증 비중이 큽니다", "권한·정보 취급 원칙을 담은 서술 준비",
     ("essay",), "story", True),
...
ROADMAP_STEPS: tuple[tuple[int, str, int, str, str, str, str, str, tuple[str, ...]], ...] = (
    (1, "STEP 01 · 3주", 3, "vhigh", "점검 환경을 직접 세우고 돌려 보기",
     ...
     "실습 환경 구성도 + 항목별 점검 결과 기록", "기준선 항목이 채워지지 않으면 다른 준비가 평가에 닿지 않습니다.",
     ("실습 환경", "점검 항목", "결과 기록")),
    ...
    (3, "STEP 03 · 2주", 2, "high", "로그를 읽고 기준을 세우기",
     "공개 로그 데이터로 정상 기준선을 정의하고, 이상이라고 판단한 근거를 남기세요.",
     "로그 분석 노트 + 판단 근거 + 오탐 사례", "탐지는 도구가 아니라 기준의 문제입니다.",
     ("기준선 정의", "이상 판단", "오탐")),
    (4, "STEP 04 · 2주", 2, "mid", "기업군에 맞춰 마무리하기",
     "지원 기업군의 편차 항목을 채우고 개인정보 점검표와 소개 순서를 다시 배치하세요.",
     "편차 항목 산출물 + 기업군 맞춤 소개 순서", "필수가 채워진 뒤의 마무리입니다. 순서만 바꿔도 인상이 달라집니다.",
     ("편차 보강", "점검표", "소개 순서")),
)
...
WIKI: dict[str, dict[str, Any]] = {
    ...
    "incident-response": {
        ...
        "interview": ["정상 기준선은 어떻게 정했나", "오탐을 줄이려면 무엇을 바꾸겠나"],
        "sequence": ["로그 수집", "기준선 정의", "탐지 규칙 초안", "대응 절차 정리"],
    },
    "compliance-design": {
