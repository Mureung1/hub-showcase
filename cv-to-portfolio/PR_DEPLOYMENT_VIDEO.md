...
- React 정적 화면과 `/api`를 한 도메인에서 제공하는 Sites Worker 배포 구성
...
그래서 배포용 Worker가 정적 React 자산과 `/api` 라우트를 같은 도메인에서 처리하게 했습니다.
React는 여전히 상대 경로 `/api`만 사용하므로 환경별 분기 코드가 필요하지 않습니다.

Supabase secret과 Anthropic key는 Worker 환경 변수에만 있고 브라우저 번들에는 포함되지 않습니다.
...
- 배포 사이트는 현재 소유자 접근 정책입니다. 외부 심사자가 직접 접속해야 한다면 저장소의
  인증·인가 범위를 확인한 뒤 공개 접근 정책을 선택해야 합니다.
