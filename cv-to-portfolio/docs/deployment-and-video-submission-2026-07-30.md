...
> 배포 확인일: 2026-07-30  
> 배포 사이트: [CV2PF](https://cv2pf-jd-portfolio.isuka-01.chatgpt.site)  
...
| 새 요청에서 목록 조회 | PASS | 저장 UUID `cc4bcf8f-a3e3-45bd-b60d-72fcab9e9a94`가 목록에 존재 |
...
현재 배포 사이트는 소유자 접근 정책을 사용한다. 제출용 Showcase에는 실제 배포 URL과 실제
데모 영상 URL을 연결했다.
...
- 로컬 Express 구조를 읽고 동일한 API 계약을 운영 Worker로 옮기기
...
- 브라우저에는 Supabase secret과 Anthropic key를 넣지 않고 Worker 환경 변수로만 전달했다.
