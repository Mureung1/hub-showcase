...
| `--rose` / `--rose-line` / `--rose-ink` | `#E4BFB9` / `#D9A9A3` / `#8C5A54` | 캐릭터, 강조 배지 등 포인트 컬러 (버튼 배경으로는 안 씀) |
| `--lavender` / `--lavender-line` / `--lavender-ink` | `#E1D9EC` / `#D2C6E6` / `#7A6B94` | 휴식/차분함 관련 요소 전용 (해당 의미가 있을 때만) |
| `--sky` / `--sky-line` / `--sky-ink` | `#DCE8ED` / `#C4D9E1` / `#5C7986` | 집중/타이머 관련 요소 전용 (해당 의미가 있을 때만) |
| `--peach` | `#F5E1CE` | 포인트 배경 (배지, 하이라이트 바탕) |
| `--sage` / `--sage-ink` | `#E3EADD` / `#5F7A5A` | 슬라이드 전용 — 완료/긍정 상태 |

**버튼 색 규칙**: 주요 액션(화면에서 다음으로 넘어가는 버튼) = `--ink` 배경 + 흰 글씨. 보조/대안 액션 = 흰 배경 + `--cream-line` 테두리 + `--ink` 글씨. 파스텔(rose/lavender/sky)은 버튼 배경으로 쓰지 않고, 캐릭터·배지·타이머처럼 그 색의 의미와 실제로 관련 있는 요소에만 쓴다. 한 화면에 서로 다른 파스텔 버튼을 섞지 않는다.
