...
- 한 단계(또는 기능 전체) 작업이 끝나면, 사용자가 묻기 전에 먼저 커밋 단위와 메시지를 제안한다. `git status`/`git diff --stat`로 실제 변경 파일을 확인해 서로 독립적으로 의미가 통하는 단위(예: BE 라우트/마이그레이션 vs FE 페이지, 기능 vs 문서)가 있는지 보고, 근거 없이 쪼개지 말고 하나의 기능으로 엮여 있으면 단일 커밋을 추천한다.
- `git log`의 최근 커밋 메시지 스타일(`feat:`/`fix:`/`docs:` prefix + 한글 설명, 제목 위주 1줄)에 맞춰 메시지를 추천한다.
- `frontend/.env`처럼 시크릿이 든 파일(`.gitignore`의 `.env*` 대상)은 스테이징에서 반드시 제외하도록 안내한다.
