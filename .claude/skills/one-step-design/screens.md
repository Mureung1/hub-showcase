...
> **이 매핑은 Flutter 앱에 이미 적용됐다**(`lib/core/theme/`). 아래 표는 `docs/prototype/`(HTML 프로토타입)의 색이 왜 Flutter 쪽과 다른지를 설명하는 **대조표**다. 프로토타입은 화면 흐름 레퍼런스로만 남아 있고 색은 갱신하지 않는다.

`docs/prototype/`의 색은 아래처럼 대응된다.
