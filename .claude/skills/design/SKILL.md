---
name: design
description: 프로젝트 디자인 규칙(이 스킬에 내장된 CSS 변수와 스타일 규칙)을 그대로 적용해 UI 컴포넌트/페이지/스타일을 만듭니다. 사용자가 /design으로 명시적으로 호출할 때 사용하세요.
---

# 디자인 규칙 적용 스킬

이 스킬은 아래에 정의된 CSS 변수와 스타일 규칙을 그대로 가져와
...
1. **기준 규칙 파악**
   아래 "CSS 변수", "사용 규칙", "Do / Don't" 섹션을 확인해 현재 확정된 CSS 변수 전체
   (색상/폰트/모서리/여백/카드/모션)를 파악한다.

2. **하드코딩 대신 변수 사용**
   요청받은 UI를 만들 때 색상 hex, px 값 등을 직접 쓰지 않고 아래 CSS 변수
...
3. **변수 블록 재사용/삽입**
   ...
   - 임의로 새로운 색상/간격 값을 추가하지 않는다. 필요한 값이 아래 목록에 없다면
