...
토큰 원본:
- 프로토타입: `prototype/src/index.css` (`:root` + 다크 오버라이드)
...
- 브랜드: `--accent` `--accent-bg` `--accent-border` (에이전트 판단·CTA·강조)
- 시맨틱: `--green`/`--green-bg`, `--red`/`--red-bg`
- **시장 관례색(국내)**: 상승·매수 = `--up`(빨강), 하락·매도 = `--down`(파랑), 관망 = `--hold`(앰버)
- 라운드: `--r-card`(16) `--r-cell`(12) `--r-pill`(999)
- 그림자: `--shadow`
...
- AI 에이전트의 판단·개입은 **보라(accent)**로 시각적으로 구분한다.
...
- [ ] 라이트/다크 모두에서 대비가 충분하다 (다크는 `prefers-color-scheme`로 자동).
