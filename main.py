...
app = FastAPI(title="Civil Litigation AI Agent API", version="4.0")
...
def ask_agent(request: ChatRequest):
    try:
        ...
        # 🚀 검색된 판례에 현재 사건 유형(case_type)에 맞는 가중치를 더해줌 (도메인 분리 적용)
        weights = load_weights()
        ctype = case_type or ai_agent.extract_live_facts(query).get("case_type", "기타 사건")
        domain_weights = weights["references"].get(ctype, {})
        
        for doc in searched_context:
            doc['base_sim_debug'] = doc.get('similarity', 0)
            if doc['title'] in domain_weights:
                doc['similarity'] = min(99, doc['similarity'] + domain_weights[doc['title']].get('direct_score', 0))
...
def delete_case(case_id: str, user_id: str = "default_user"):
    ...
    if target_case and target_case.get("rating", 0) > 0:
        ...
        ctype = target_case.get("extracted_data", {}).get("title", "기타 사건")
        ...
        if p_change != 0 and "related_laws" in target_case and ctype in weights["references"]:
            for idx, law in enumerate(target_case["related_laws"]):
                ltitle = law.get("title", "")
                if ltitle in weights["references"][ctype]:
                    weights["references"][ctype][ltitle]["direct_score"] -= int(p_change * (1.0 if idx == 0 else 0.5))
...
def update_card_feedback(feedback: CardFeedbackData):
    ...
    ctype = feedback.case_type
    
    # 🚀 [수정] 사건 유형(Domain) 방을 먼저 만들어 줍니다.
    if ctype not in weights["references"]:
        weights["references"][ctype] = {}
    if ltitle not in weights["references"][ctype]: 
        weights["references"][ctype][ltitle] = {"direct_score": 0, "picked_count": 0}
        
    p_change = 10 if feedback.is_useful else -10
    
    if feedback.is_cancel:
        weights["references"][ctype][ltitle]["direct_score"] -= p_change
        weights["references"][ctype][ltitle]["picked_count"] = max(0, weights["references"][ctype][ltitle]["picked_count"] - 1)
    else:
        weights["references"][ctype][ltitle]["direct_score"] += p_change
        weights["references"][ctype][ltitle]["picked_count"] += 1
...
def get_agent_stats():
    ...
    # 🚀 [수정] 분리된 도메인(사건 유형) 구조에 맞게 통계 합산 로직 변경
    law_scores = []
    prec_scores = []
    total_card_votes = 0
    
    for ctype, laws in references.items():
        for k, v in laws.items():
            if "법령" in k: law_scores.append(v.get("direct_score", 0))
            if "판례" in k: prec_scores.append(v.get("direct_score", 0))
            total_card_votes += v.get("picked_count", 0)
