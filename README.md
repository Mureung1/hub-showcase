...
OpenAI 요청은 15초, Gemini 영상 분석 요청은 30초 후 중단하며 자동으로 재시도하지 않는다. 직접 입력은 공백 제거 후 최대 20,000자, 인증된 사용자별 구조화 요청은 10분에 10회로 제한한다. Timeout이나 제공자 장애가 발생하면 입력을 유지한 채 재시도할 수 있는 오류를 반환한다.
