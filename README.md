...
임의의 공개 제3자 YouTube 영상은 MVP에서 자막을 자동 수집하지 않는다. 공식 YouTube Data API의 메타데이터만으로는 레시피 원문을 얻을 수 없고 자막에는 OAuth 및 영상 편집자 권한이 필요하므로 비공식 스크래핑이나 내부 엔드포인트를 사용하지 않는다. YouTube URL 수집이 실패하면 기존 `URL_FETCH_FAILED` 422 응답과 직접 입력 안내를 사용하며, 이를 위한 OAuth, API 키 또는 환경 변수는 추가하지 않는다.
