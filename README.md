...
시나리오 관련 요청은 산출물에 따라 메인 에이전트와 전용 서브에이전트의
개선안 처리 방식이 다르다.

| 요청 | 담당 | 개선안 처리 | 공개 방식 | 승인 전 결과 |
|---|---|---|---|---|
| 일반 시나리오 자료 검토·문서 작성·수정 | 메인 Codex | 원안 기반 Draft를 보존하고 더 나은 전개·분기를 별도 권고 | `Scenario Improvement Review` | 선택 후에도 갱신된 `pending` 승인안으로 재검토 |
| 플레이어 노출 인게임 스크립트 작성 | `scenario_writer` 서브에이전트 | 더 나은 구조를 하나의 최적안에 직접 반영 | 구조 변경은 `NR-*`, 구체 창작은 `CW-*` | 상위 시나리오·대본·링크를 묶은 `restructure` 승인안 |

### Main-Agent Scenario Review

- 메인 Codex는 시나리오 자료를 검토하거나 일반 시나리오 문서를 신규 작성·
  수정할 때 Project Brief, 게임 개요, 관련 확정 시나리오·세계관·시스템 문서를
  읽고 사건 구조를 검토한다.
- 인과, 인물 동기, 긴장과 속도, 정보 공개, 선택의 의미, 분기 비용·합류와
  Outcome을 확인하고 더 나은 안이 있을 때만 `Scenario Improvement Review`에
  별도 권고로 제시한다.
...
- 저장 요청이 있으면 같은 프로젝트의 Approval Queue에 `pending`으로 기록한다.

#### 사용 흐름

1. 사용자가 프로젝트, 상위 시나리오, 대상 챕터·Phase와 범위를 지정한다.
...
6. 문서 후반의 `NR-*` 로그에는 원본과 달라진 구조, 변경 이유, 기대 효과,
   ...
   `CW-*` 각주로 공개한다.
7. 메인 Codex는 작가 브리프, 구조 변경, 창작 각주, `TBD`, 충돌과 의존성을
   검토한다. 상위 시나리오 변경이 있으면 상위 문서, 대본과 링크 갱신을 하나의
   `restructure` 승인 항목으로 저장한다.
8. 세계관 정사나 시스템 규칙 변경이 필요한 부분은 별도 고위험 승인 항목으로
   분리하고 대본에는 `TBD`로 남긴다. 선행 항목 적용 후 대본을 재확인하고
   다시 승인받는다.
9. 사용자가 `restructure` 항목을 명시적으로 승인하면 적용 직전 모든 원본을
...
Chapter 2 Phase 1을 인게임 스크립트 승인 초안으로 작성해줘.
