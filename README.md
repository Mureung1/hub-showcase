...
1. **AI 플래닝** — 주제·마감일·인원·첨부(사진/PDF)·기피 기간을 입력하면 Claude API가 역할 정의 + 마일스톤 + 태스크를 생성 (재생성 3회, 확정 후 잠금)
...
│   ├── screens/               # 홈, 로그인/가입, 위저드, 계획 검토, join,
│   │                          #   설문, 배정 결과, 대시보드, 프로젝트 진행, 프로젝트 관리
...
| 날짜 | 목표 |
|------|------|
| 7.13 (월) | 계획 수립 Agent 제작(✅ 완료) + React 화면 골격 — 홈(서비스 소개·플래너/역할 배정 에이전트 안내), 로그인/회원가입(UI), 사이드바 3탭(대시보드/프로젝트 진행/프로젝트 관리) |
