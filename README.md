...
- `profile` 자체는 Supabase에 저장은 되지만, 프론트는 그걸 다시 조회하지 않고 `App.jsx`의 메모리 상태로만 들고 있다가 자소서 초안 요청 시 그대로 재전송한다(T10에서 단순함을 우선해 결정). 그래서 정보입력 이후 브라우저를 새로고침하면 프로필이 날아가고 처음부터 다시 입력해야 한다 — `step`/`selectedJob`도 마찬가지로 메모리에만 있고 라우터가 없어서, 이 부분만 따로 고쳐도 새로고침 복원 문제 자체는 풀리지 않는다. 라우팅/상태 복원을 새로 설계해야 하는 더 큰 변경이라 이번 4주 범위 밖으로 남겨둔다.
