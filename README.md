...
두 채널은 성질이 달라 측정 방식을 의도적으로 분리했습니다.

| 채널 | 역할 | 대상 | 쿠폰 | 측정 |
|---|---|---|---|---|
| 단골 문자 | **전환** | 수신동의 단골(식별됨) | 1인 1코드 | 사용률·귀속 매출 실측 |
| 인스타·X | **도달** | 불특정 다수(공개) | 발급 불가 | 게시 여부·permalink |

공개 채널은 수신자 목록이 없어 개인별 코드를 발급할 수 없습니다. 대신 캡션에
`📍 매장에서 이 게시물 보여주세요`를 붙여 오프라인 귀속 경로를 남깁니다
(`packages/shared/src/sns.ts`).
