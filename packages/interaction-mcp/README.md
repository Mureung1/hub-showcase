...
| `protocolVersion: 1` handshake, capability call/result와 safe error envelope | Browser route·frame, active workspace·Turn 선택 |
| `dist/stdio.js`의 environment validation, authenticated handshake와 one-call→one-held-POST mapping | Broker listener, pending slot, evidence resolution과 UI settlement |
...
세 값과 authenticated handshake가 유효하기 전에는 MCP initialize가 성공하지 않는다. Capability call마다 HTTP POST 하나만 열며 terminal Broker response를 같은 tool call에 반환한다.
...
Build는 Node shebang을 가진 `dist/stdio.js`를 만들고 executable mode를 설정한다. Test는 real built process와 mock loopback Broker로 initialize → tools/list → held call → structured result, handshake 거절, Broker failure, transport loss와 cancellation을 검증한다. Package-root verification은 default-condition export, shebang과 mode를 반복 build 뒤 확인한다.
