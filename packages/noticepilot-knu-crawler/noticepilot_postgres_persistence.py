...
MIGRATIONS_RELATIVE_DIRECTORY = "migrations/postgresql"
...
def migration_version_for_path(migration_path: Path) -> str:
    """Return the repository-owned version encoded in a migration filename."""
    match = re.fullmatch(r"([0-9]{4}_[a-z0-9_]+)\.sql", migration_path.name)
    if not match:
        raise PostgresPersistenceValidationError(
            f"unsupported PostgreSQL migration filename: {migration_path.name}"
        )
    return match.group(1)


def discover_migrations(migrations_directory: Path) -> list[Path]:
    """Discover the single ordered PostgreSQL migration authority.

    Down migrations intentionally remain outside the forward-only runtime
    authority.  A duplicate numeric prefix is fail-closed before any database
    work begins.
    """
    directory = Path(migrations_directory)
    if not directory.is_dir():
        raise PostgresPersistenceValidationError("PostgreSQL migrations directory is missing")
    paths = [
    ...
    for path in paths:
        version = migration_version_for_path(path)
        prefix = version.split("_", 1)[0]
        if prefix in versions:
            raise PostgresPersistenceValidationError(
                f"duplicate PostgreSQL migration number {prefix}: "
                f"{versions[prefix].name}, {path.name}"
            )
        versions[prefix] = path
    return [versions[key] for key in sorted(versions)]


def validate_migration_sql(sql: str, *, migration_version: str = MIGRATION_VERSION) -> None:
    required_fragments = {"jsonb", "timestamptz"}
...
def execute_migration(connection: Any, migration_path: Path) -> str:
    ...
    version = migration_version_for_path(migration_path)
    sql = migration_path.read_text(encoding="utf-8")
    validate_migration_sql(sql, migration_version=version)
    ...
    try:
            ...
            if recorded is not None:
                ...
                if recorded_checksum != checksum:
                    raise PostgresPersistenceError(
                        f"migration checksum conflict for {version}: "
        ...
        elif not migration_table_exists:
            raise PostgresPersistenceError(
                f"cannot apply {version} before {MIGRATION_VERSION} establishes the migration ledger"
            )
...
def apply_migrations(connection: Any, migrations_directory: Path) -> dict[str, str]:
    """Apply every repository-owned forward migration in filename order."""
    return {
        migration_version_for_path(path): execute_migration(connection, path)
        for path in discover_migrations(migrations_directory)
    }
