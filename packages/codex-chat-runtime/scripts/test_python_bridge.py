...
class ProtocolUnitTests(unittest.TestCase):
    ...
    def test_keeps_account_read_and_rejects_removed_account_commands(self) -> None:
        command = decode_command_line(
            b'{"bridgeRequestId":"read","command":"read_account"}\n'
        )
        self.assertEqual(command.command, "read_account")
        ...
        for value in removed:
            with self.subTest(command=value["command"]):
                with self.assertRaisesRegex(ProtocolViolation, "unknown_command"):
                    decode_command_line(
...
class PythonBridgeActualChildTests(unittest.TestCase):
    ...
    def test_fresh_account_projection_keeps_only_safe_states(self) -> None:
        cases = (
            ("signed_out", "signed_out"),
            ("chatgpt", "chatgpt"),
            ("api_key", "unsupported"),
        )
        for native_state, expected_state in cases:
            with self.subTest(native_state=native_state):
                with tempfile.TemporaryDirectory(
                    prefix="ay-ple-python-bridge-account-read-"
                ) as temp:
                    root = Path(temp)
    ...
    def test_removed_account_command_is_unknown_and_never_reaches_app_server(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory(
