...
- Cancelled async waiter가 아직 도착하지 않은 request를 나중에 dequeue해 버리지 않는다. 이미 시작한 blocking delivery는 shield한 뒤 exact pending object를 route 앞에 복원하고, terminal·close가 먼저 정산한 binding은 되살리지 않는다.
