...
- Answer/cancel admission과 explicit interrupt admission은 전용 settlement lock에서 첫 wire write까지 atomic하게 경쟁한다. Interrupt가 먼저면 pending binding을 해제해 answer/cancel이 `interaction_not_pending`으로 끝나고, answer/cancel이 먼저면 그 response가 interrupt보다 wire에서 앞선다. Response writer는 write 완료를 settlement 완료로 해석하지 않고 matching native `serverRequest/resolved`가 request-local barrier를 해제할 때까지 기다린다. SDK가 내부 처리한 모든 server request ID는 1,024개 bounded tracker에 resolution 또는 connection cleanup까지 남고, approval과 user-input cleanup 뒤 matching acknowledgement도 request-local 정산과 함께 소비해 global notification route에 다시 적재하지 않는다. Duplicate ID나 tracker overflow는 transport를 fail closed한다.
...
- `scripts/test_plan_interaction.py`가 purpose-built OS child를 public `AsyncCodex`로 통과해 request-before-`turn/start` response, pending 중 account response·turn notification liveness, answer/cancel 뒤 same-Turn terminal과 raw ID 비노출을 검증한다. Delayed native resolution trace는 response write 뒤에도 public settlement가 pending이고 exact `serverRequest/resolved` 뒤에만 완료되며 matching acknowledgement 뒤 global route 사용량이 0인지 고정한다. Default command approval과 terminal cleanup 뒤 늦은 user-input acknowledgement trace도 같은 global usage 0을 검증한다.
