...
- exact source commit, immutable unpatched manifest와 complete ordered patch stack `ffc43da6e5e7a146016404db54968d37d849b778e5e9b04db680cac4124fc1c9`
...
- bundle-local import path, native executable, `codex-cli 0.144.4`와 complete tree-roster digest `4b72a60735d6b6d1489bab9fa937889f296ba2268c3fc0c433ba84ca10b36b7a`
...
Worker는 official public `AsyncCodex`, `AsyncThread`, `AsyncTurnHandle`만 conversation baseline으로 사용한다. Process-local live handle을 native `threadId`·`turnId`로 보관할 뿐 native thread를 archive/delete하거나 AY-PLE ID로 remap하지 않는다. Turn start는 process-wide Skill root 설정과 함께 직렬화하고, internal text regression은 빈 root를, product Turn은 exact managed `SKILL.md`의 directory 하나를 적용한 뒤 native Turn을 시작한다. Internal text operation의 `deny_all + read_only`는 exact Runtime regression oracle일 뿐 public Server·Browser policy가 아니다. Canonical product Turn은 bounded `TextInput` 하나, Plan `collaborationMode`, `ApprovalMode.auto_review + Sandbox.workspace_write`를 explicit하게 전달하고 Skill은 action caller가 요청한 경우에만 exact `SkillInput` 하나를 추가한다. Codex permission은 AY-PLE Review·`UserConfirmation`과 별도 상태다.

Product Turn의 model과 reasoning effort는 app input이 아니다. Policy disposition은 `adapt`다. Pinned first-party client가 native `thread/start`에서 `configured → advertised default → first available`로 결정한 effective model·reasoning effort를 0007이 high-level Thread에 보존하고 Bridge가 exact Plan `collaborationMode.settings`에 그대로 사용한다. Bridge는 `model/list`를 다시 호출하지 않으며 effective model이 없으면 Turn acceptance 전에 fail closed한다. Caller가 보낸 legacy model·reasoning private field는 strict decoder가 거부한다.
...
`createCodexChatRuntime({ runtimeRoot, workspace, environment })`는 verified full bundle 외의 실행 경로를 갖지 않는다. Canonical product composition이 `packageRoot`에서 `runtimeRoot`를 찾고 explicit `appDataRoot`에서 `home`, `codexHome`, `codexSqliteHome`, `tempDirectory`를 계산한다. Runtime factory는 이 absolute·writable·서로 다른 directory와 workspace symlink을 거부하고 canonical path로 고정한 뒤 bundled Python worker를 시작하며, private `ready`로 SDK initialize 완료를 확인해야 resolve한다. Node-only `CodexProductCapableRuntime`은 `startThread({ workspace, mcp })`로 ready `SemesterWorkspace`의 canonical cwd와 authenticated loopback MCP를 product thread에만 결합한다. `readAccountReadiness`, optional Skill·Text를 받는 `startProductTurn`, `answerUserInput`, `cancelUserInput`을 제공하며 native identity를 다시 만들거나 Browser/product state 또는 model selection을 소유하지 않는다. Internal no-argument text methods는 exact lifecycle regression에 남을 수 있지만 public HTTP·Browser compatibility contract가 아니다. `terminal`은 첫 process-wide failure에서 한 번 resolve하고 reject하지 않으며 late subscriber도 같은 error를 받는다. 정상 `close()`만으로는 settle하지 않는다.
...
| `npm run test:node-unit -w @ay-ple/codex-chat-runtime` | Bundle 없이 full-manifest verifier fixture, private output byte-framing·strict decode, per-route/aggregate queue와 writer accounting, bounded stderr, internal native conversation parser와 deterministic product fake lifecycle을 검사한다. |
