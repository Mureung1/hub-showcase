...
Official OpenAI Codex Python SDK를 재사용하는 AY-PLE product companion Runtime package다. Exact SDK source·generated contract·provenance, response-last correction·bounded notification routing·initialize notification opt-out·strict correlated response classification·Plan user-input seam·native thread effective setting·standalone Skill root를 포함한 ordered eight-patch stack, macOS arm64용 standalone production bundle과 persistent Python bridge에 더해 hardened Node supervisor를 구현한다. Node production·testing surface는 exact workspace 하나를 받는 `CodexWorkspaceRuntime`으로 닫혀 있고 fresh Account Readiness는 이 Runtime의 read-only capability다. AY-PLE-owned Node contract·private bridge·patch roster에는 managed Browser login·logout이나 pre-workspace Runtime이 없다.
...
| `src/runtime-contract.ts` | Node product caller가 쓰는 private product-thread input, structured Skill·Text input, Account Readiness, model catalog, opaque user-input answer/cancel, effective config·Skill observation과 role-free `CodexWorkspaceRuntime` boundary. Managed absolute Skill path와 workspace·loopback MCP credential은 이 Node-only boundary에만 있고 browser-safe contract에는 포함되지 않는다. |
| `src/native-context-probe.ts`, `src/native-context-probe-protocol.ts` | Verified bundle의 exact native executable을 `--config project_root_markers=[] app-server --listen stdio://`로 one-shot 실행하고 `initialize(capabilities.experimentalApi=true) → initialized → config/read → skills/list`를 strict bounded decode한 뒤 process group을 완전히 reap한다. Raw App Server envelope와 generated protocol shape는 package-private다. |
...
Ordered SDK surface는 `Thread.run/turn(..., collaboration_mode=...)`, `AsyncCodex.next_user_input()`, request-local `answer()`·`cancel()`, native thread effective setting read와 `set_skill_extra_roots()`를 제공한다. Sole reader는 pending answer 동안 ingress를 계속 drain하며 global 32개·Turn당 1개 bound, 단일 bounded writer를 통한 overflow·동일 Turn 충돌의 affected-Turn interrupt, waiter 없는 fire-and-forget control과 direct settlement reserve를 소유한다. Async waiter는 하나의 in-flight collector reservation을 공유하므로 cancellation 뒤에도 exact request를 보존하고 반복 cancellation이 shared executor worker를 누적하지 않는다. Reserved token의 sync claim이 terminal settlement와 같은 condition lock에서 경쟁하므로 terminal이 먼저 정산한 collector result는 public request로 전달되지 않는다. Explicit interrupt admission과 answer/cancel은 첫 wire write까지 one-settlement로 경쟁한다. Duplicate·late conflict, resolved·terminal·close·transport cleanup과 internal control/response write half-close도 current/future waiter를 fail closed한다. Private Python bridge와 Node runtime은 이 seam을 opaque `interactionId`와 typed product activity로 projection한다. Server product operation coordinator는 이를 private MCP proposal과 public Review HTTP에 결합하며 Browser 표현은 후속 UI slice가 소유한다.

Worker는 official public `AsyncCodex`, `AsyncThread`, `AsyncTurnHandle`만 conversation baseline으로 사용한다. `CodexConfig.config_overrides`에 `project_root_markers=[]`를 고정해 persistent native process도 one-shot context probe와 같은 project-root boundary를 사용한다. Process-local live handle을 native `threadId`·`turnId`로 보관할 뿐 native thread를 archive/delete하거나 AY-PLE ID로 remap하지 않는다. Turn start는 process-wide Skill root 설정과 함께 직렬화하고, internal text regression은 빈 root를, product Turn은 exact managed `SKILL.md`의 directory 하나를 적용한 뒤 native Turn을 시작한다. Internal text operation의 `deny_all + read_only`는 exact Runtime regression oracle일 뿐 public Server·Browser policy가 아니다. Canonical product Turn은 bounded `TextInput` 하나와 Plan `collaborationMode`를 전달하며 action caller가 `permissionProfile`을 반드시 고른다. `workspace_write`는 `ApprovalMode.auto_review + Sandbox.workspace_write`, `read_only`는 `ApprovalMode.deny_all + Sandbox.read_only`로 mapping한다. Skill은 action caller가 요청한 경우에만 exact `SkillInput` 하나를 추가한다. Codex permission은 AY-PLE Review·`UserConfirmation`과 별도 상태다.
...
| `start_product_turn` | `bridgeRequestId`, `threadId`, bounded `text`, optional exact Skill pair와 optional complete model·reasoning·service tier triple | managed Skill root와 native effective 또는 selected model setting을 적용한 `{ threadId, turnId }` acceptance 뒤 curated product activity |
...
`createCodexChatRuntime({ runtimeRoot, workspace, environment })`는 current product의 workspace-only factory call이며 role-free `CodexWorkspaceRuntime`을 반환한다. Canonical personal product composition이 `packageRoot`에서 `runtimeRoot`를 찾고 explicit `appDataRoot`에서 `home`, `codexSqliteHome`, `tempDirectory`를 계산하며 `codexHome`은 caller의 전역 `CODEX_HOME` 또는 `~/.codex`를 사용한다. Runtime factory는 이 absolute·writable·서로 다른 directory와 workspace root의 symlink·unsafe relation을 거부하고 canonical path로 고정한 뒤 bundled Python worker를 시작하며, private `ready`로 SDK initialize 완료를 확인해야 resolve한다. Workspace root는 persistent worker와 native-context sidecar의 exact `cwd`로 함께 고정하고 App-owned admission 여부는 Server·workspace Module이 소유한다.
...
Provider-free gate는 서로 다른 failure locality별로 구분한다. Response-last, bounded-router, bridge와 Node adversarial gate는 `CodexConfig.launch_args_override`로 시작한 purpose-built OS child를 사용한다. Bridge fake journal은 initialize/initialized, fixed `project_root_markers=[]`, explicit `never + readOnly`, native identity, acceptance-first replay, interrupt와 no archive/delete를 확인한다. Native-context gate는 package-private one-shot protocol supervisor를 adversarial child로 검사하고 verified native `0.144.4`를 실제로 시작해 provider 없이 effective config·Skill projection과 full reap을 확인한다. Exact local-provider gate는 tracked official `MockResponsesServer`·SSE helper와 verified native `0.144.4`를 production Node→bundled Python→official SDK 경로로 연결하고, managed config를 test-only opt-in으로 차단한 격리 home에서 T0, interrupt, same-thread follow-up과 full process-group reap을 확인한다. 종료 뒤 같은 native thread를 exact `thread/resume`으로 읽어 persisted `approvalPolicy: never`와 `sandbox: { type: readOnly, networkAccess: false }`도 검증한다. Synthetic schema-valid approval에 대한 low-level default `accept`는 current pin의 관찰 사실이지만, 그 자체로 AY-PLE 제품 확인 실패나 반드시 추가할 fail-closed layer를 뜻하지 않는다.

Exact product fixture는 같은 official local provider와 production bundle을 real Server product seam에 연결해 managed Skill·selected source·representative Python/command·hosted MCP·Plan Review·same-Turn terminal·durable outcome을 검증한다. Opt-in isolated live-provider command와 auth prerequisite은 [Server README](../../apps/server/README.md)가 소유하며, complete Assignment action과 clean shutdown을 green으로 확인했다.
