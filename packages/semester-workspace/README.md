...
`@ay-ple/semester-workspace`는 app-owned v3 `SemesterWorkspace`의 strict codec, filesystem admission과 workspace instruction/Skill bundle을 소유하는 Node-only Module이다. 현재 구현은 fresh workspace의 admission/recovery, current v2 compatibility decoder와 exact bundle materialization까지 닫았으며, durable setup envelope와 `Semester Ready` 조합은 아직 구현하지 않는다.
