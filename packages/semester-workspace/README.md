...
| Root classification | Valid v4는 strict decode하고, version marker가 있는 current-v2·historical v3는 payload를 academic schema로 해석하지 않은 채 legacy status로 분류한다. Malformed·future·oversized bytes는 `incompatible`이다. Classification은 입력 bytes를 rewrite하지 않는다. |
...
Existing current-v2, historical v3, malformed와 future files는 automatic migration·delete 대상이 아니다. Canonical Server startup은 strict v4만 열고 unsupported bytes를 incompatible prepared root로 보존한다. Pre-App Bootstrap과 AY가 workspace file·Git authority를 소유한다.
