...
| `src/runtime-archive-directory-capability.ts` | Dedicated child의 kernel-held cwd를 directory capability로 유지한다. Exact `(dev, ino, uid, mode)` handshake와 direct-leaf create·open·hash operation만 허용해 ancestor replacement·symlink·hardlink·rename race에서 no-follow/no-clobber 경계를 보존한다. |

## 내부 경계

- Package root `src/index.ts`는 아직 `RuntimeReleaseScaffold`만 노출한다. D1 authority와 archive extraction Module은 package entrypoint에서 re-export하지 않는 source-internal seam이다.
...
- `RuntimeResolver`는 frozen Interface이며 구현체가 없다. HTTP transport, download/resume journal, cache generation publish·repair와 `VerifiedRuntime` 반환은 후속 slice에 남아 있다.
...
- Extraction은 이미 검증된 archive와 admission·cache mutation authority를 입력으로 받으며 network를 호출하거나 archive fallback·자동 downgrade를 수행하지 않는다.
