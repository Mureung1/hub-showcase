...
AI에게 작업을 맡길 때 쓰는 4단계 사이클. 매번 복붙하지 않도록 각 도구의 슬래시 커맨드로 등록했다.
...
| 4. 구현 검증 | Codex | `/implementation-verification` | `.codex/prompts/implementation-verification.md` |

- 2번의 지적 사항을 사람이 결정해 설계에 반영한 뒤 3번으로 넘어간다.
- 4번에서 실패한 항목은 3번으로 되돌아간다.

## Claude Code 슬래시

`.claude/commands/`에 있는 파일은 저장소에 커밋되어 바로 작동한다. Claude Code에서 `/design-review`, `/implementation-request`를 치면 된다.
