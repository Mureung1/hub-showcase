...
- **`/camp-pr`** — manage the repo's camp PR flow. It creates, updates, labels, and merges fork-local integration PRs from `codex/w<week>d<day>` to `swh3467:N180_하성욱`; it only creates, updates, or merges the upstream camp submission PR to `connect-AIAgentChallenge-26-1/hub:N180_하성욱` when the user explicitly asks for camp submission.
