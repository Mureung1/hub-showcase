...
export async function analyzeWithOpenAI({ projectTitle, rawText }, options = {}) {
  ...
  const model = options.model || process.env.MODU_BRAIN_OPENAI_MODEL;
  ...
  if (!model) {
    throw new ContextAnalysisApiError(
      503,
      "PROVIDER_CONFIGURATION_ERROR",
      "OpenAI provider를 사용하려면 MODU_BRAIN_OPENAI_MODEL을 명시해야 합니다.",
      { missing: ["MODU_BRAIN_OPENAI_MODEL"] },
    );
  }

  const client = options.client || new OpenAI({ apiKey, timeout: options.timeoutMs || 45_000 });
