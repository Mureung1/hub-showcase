...
export const AGENT_PROMPTS: Record<string, AgentPromptConfig> = {
  "Requirements Agent": {
    ...
    gatherContext: () => docContent(0), // 00_Analysis_Report.md, if an analysis ran
...
- [ ] SO 이름 — 담을 필드, 어떤 클래스가 참조하는지`,
  ...
  "Documentation Agent": {
    ...
    // steps 7-8 (code gen/refactoring) don't have Markdown docs yet, so
    // docContent() on those simply returns "" and gets skipped below.
    gatherContext: async () => {
      const sections = await Promise.all(
        [1, 2, 3, 4, 5, 6, 7, 8].map(async (id) => ({ id, content: await docContent(id) }))
      );
      return sections
        .filter((s) => s.content)
        .map((s) => `### Step ${s.id}\n${s.content}`)
    ...
    buildSystemInstruction: (context, questionCount) => `당신은 GameForge Agent의 "Documentation Agent"입니다.
1~8단계에서 만들어진 모든 문서를 종합해서, 하나의 최종 문서(요구사항 명세서 +
게임 기획서 + 시스템 설계서를 정리한 요약)로 정리하는 역할입니다. 새로운 내용을
지어내지 말고, 이미 있는 문서들을 정리·요약하세요.

## 지금까지의 산출물 (1~8단계)
${context || NO_DOC}

## 지금까지 진행 상황
지금까지 질문을 ${questionCount}개 했습니다. 대략 2~4개면 충분합니다 — 이미 내용은
다 있으니, 강조하고 싶은 부분이나 누락된 게 없는지 정도만 확인하면 됩니다.
