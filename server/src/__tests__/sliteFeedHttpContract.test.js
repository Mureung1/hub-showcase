...
function refreshDto(overrides = {}) {
  return {
    schemaVersion: 'noticepilot.sliteSourceRefresh.v0.1',
    feedId: FEED_ID,
    sourceId: 'knu-721-2436',
    outcome: 'updated',
    previousEventCount: 601,
    eventCount: 602,
    snapshotChanged: true,
    checkedAt: '2026-07-20T12:34:56+09:00',
    ...overrides,
  }
}
...
test('SL-02A source refresh authenticates before parsing and exposes only its exact DTO', async (t) => {
  let calls = 0
  let fail = false
  let result = refreshDto()
  const gateway = {
    async refreshSliteFeed() {
      calls += 1
      if (fail) throw new ReferenceFeedGatewayError('bridge_failed')
      return result
    },
  }
  const server = await startTestServer(createApp(sliteOptions(gateway)))
  t.after(() => server.close())
  const endpoint = `${server.baseUrl}/api/subscription-feeds/slite/refresh`

  const unauthenticatedMalformed = await fetch(endpoint, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: '{',
  })
  assert.equal(unauthenticatedMalformed.status, 401)
  assert.equal(unauthenticatedMalformed.headers.get('cache-control'), 'no-store')
  assert.equal(calls, 0)

  const authenticatedMalformed = await fetch(endpoint, {
    method: 'POST',
    headers: adminHeaders({ 'content-type': 'application/json' }),
    body: '{',
  })
  ...
  assert.deepEqual(await refreshed.json(), refreshDto())
  assert.equal(calls, 1)

  result = { ...refreshDto(), unexpected: true }
  const invalidGatewayDto = await fetch(endpoint, {
    method: 'POST',
    headers: adminHeaders({ 'content-type': 'application/json' }),
    body: '{}',
  })
  assert.equal(invalidGatewayDto.status, 503)
  assert.equal(invalidGatewayDto.headers.get('cache-control'), 'no-store')
  ...
  assert.deepEqual(await unavailable.json(), {
    error: {
      type: 'slite_feed_unavailable',
      message: 'The S-Lite subscription feed is unavailable.',
    },
  })
})
