...
function getGraduationStatus(args, ctx) {
  const { targets, completedCourses = [], basketCourses = [] } = ctx;
  const creditSummary = calculateCreditSummary(targets, completedCourses, basketCourses);

  const track = ctx.track ?? args?.track ?? null;
  let trackRequirements = null;
  if (track) {
    trackRequirements = evaluateTrackRequirements([...completedCourses, ...basketCourses], track);
  }
 ...
 * 총학점에만 더하고, 전공/교양 세부 목표는 현재까지의 completedCourses/basketCourses
 * 기준 그대로 사용한다.
 */
function simulatePlan(args, ctx) {
  const { targets, completedCourses = [], basketCourses = [], currentSemester } = ctx;
  ...
  const plan = calculateSemesterPlan({
    currentSemester,
    combinedTotal: creditSummary.total.current + additionalTotal,
    combinedMajor: creditSummary.major.current,
    combinedGeneral: creditSummary.general.current,
...
function recommendCourses(args, ctx) {
  ...
  const { completedCourses = [], basketCourses = [] } = ctx;
  const alreadyTakenNames = new Set([...completedCourses, ...basketCourses].map((c) => c.name));
 ...
 * @param {Object} ctx - POST /api/chat 요청 body에서 온 상태 (targets, completedCourses, basketCourses, track, currentSemester)
 */
