...
export function isSliteSourceRefreshDto(value) {
  return (
    hasExactKeys(value, REFRESH_KEYS) &&
    value.schemaVersion === 'noticepilot.sliteSourceRefresh.v0.1' &&
    /^feed_[0-9a-f]{32}$/.test(value.feedId) &&
    value.sourceId === 'knu-721-2436' &&
    ['updated', 'unchanged'].includes(value.outcome) &&
    Number.isInteger(value.previousEventCount) &&
    value.previousEventCount >= 0 &&
    Number.isInteger(value.eventCount) &&
    value.eventCount >= 0 &&
...
export class PythonSliteFeedGateway extends PythonReferenceFeedGateway {
  ...
  async refreshSliteFeed() {
    return requireDto(
      await this._request('refresh_slite', {}),
      isSliteSourceRefreshDto,
    )
  }
