/**
 * 구·군(기초자치단체) 매핑. 시·도 목록(REGIONS) 자체는 크롤러 지역 추출과 표기를 통일하기
 * 위해 `@hub/shared`로 옮겼다 — 여기선 재노출만 한다.
 * 기준: 2023-07-01 군위군 대구 편입 + 2026-07-01 전남광주통합특별시 출범(광주광역시+전라남도
 * 통합) 반영, 전국 시·군·구 전체 포함.
 * (세종특별자치시는 기초자치단체가 없어 단일 항목으로 처리)
 */
export { REGIONS, type Region } from '@hub/shared'

/** step3 구·군 매핑 — 시·도별 전체 목록 */
...
/** 시·도 선택값에 해당하는 구·군 목록 (없으면 fallback) */
export function getDistricts(region: string): string[] {
  return DISTRICTS[region] ?? ['해당 지역 전체']
}
