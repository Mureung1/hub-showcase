...
매칭 방식은 전체 경로(디렉토리 포함)를 소문자로 바꾼 뒤 위 단어들이 부분 문자열로 포함되는지만 확인한다. 단어 경계(word boundary) 기준으로 엄격하게 매칭하면 `PaymentService.js`, `UserController.js`처럼 접두어와 붙어 있는 camelCase 파일명(9장 출력 예시의 `PaymentService.js` 포함)이 매칭되지 않는 문제가 있어, 대소문자 무시 단순 포함 검사를 채택한다. 대신 `deviceService`처럼 의도치 않은 부분 일치도 보너스를 받을 수 있다는 트레이드오프가 있다 — MVP에서는 오탐보다 미탐(실제 서비스/컨트롤러 파일을 놓치는 것)이 더 나쁘다고 보고 이 방식을 택함.

두 값은 그대로 더한다(정규화 없음). 이 임계값(15줄/300줄, 보너스 0.3)은 MVP 기본값이며, 실제 유저 레포 데이터를 보고 Phase 2에서 조정 가능하다.

## 줄 수 계산 방식 (바이트 기반 근사)

Candidate File 전체의 점수를 매기려면 전체의 줄 수가 필요하지만, 원문(raw content)은 최종 선정된 3~5개 파일에 대해서만 가져온다(10장 GitHub API 호출 방식 참고). 따라서 스코어링 단계에서는 원문을 추가로 조회하지 않고, Git Trees API 응답에 이미 포함된 blob의 `size`(바이트) 값을 줄 수로 환산한 근사치를 사용한다.

```
근사 줄 수 = size(byte) / 평균 바이트/줄 상수(MVP 기본값 30)
```

이 근사치는 15줄/300줄 임계값 판정에만 쓰이는 1차 필터용이며, 실제 파일 내용과 정확히 일치하지 않을 수 있다(들여쓰기/공백 비중에 따라 오차 발생). 최종 선정된 3~5개 파일은 어차피 원문을 가져오므로, 그 시점에 실제 줄 수를 다시 세어 `score`/`score_reason`에 반영해 사용자에게 보여지는 근거 문구는 정확한 값을 쓴다(순위를 정하는 1차 스코어링과, 최종 후보의 표시용 점수는 서로 다른 시점의 값을 쓸 수 있음).
