...
export const featuredOfficial: FeaturedOfficialChallengeDto = Object.freeze({
  ...
  dailyMinutes: 30, entryPoints: 39000, participation: null, source: 'mock',
  sections: {
    ...
    howItWorks: ['신청', '포인트 차감', '참가 완료', '일일 학습', '인증', '완료'].map((title, index) => ({ title, description: `${index + 1}단계`, state: index === 0 ? 'current' as const : 'upcoming' as const })),
