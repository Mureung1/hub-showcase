  ...
  function renderDeleteConfirmation() {
    return (
      <div className="category-manager__delete-confirmation">
        <p>이 카테고리를 삭제할까요?</p>
        <p>연결된 인사이트는 삭제하지 않고 미분류로 옮겨요.</p>
        {failureReason ? (
    ...
    return (
      <>
        ...
        <Button
          ...
          type="submit"
        >
          {mode.type === 'create' ? '카테고리 만들기' : '변경 내용 저장하기'}
        </Button>
      </>
    );
  }
...
function getDescription(mode: CategoryManagerMode) {
  switch (mode.type) {
    case 'create':
      return '이름과 색상을 정해 새 카테고리를 만들어요.';
    case 'delete':
      return '삭제하기 전에 연결된 인사이트가 어디로 이동하는지 확인해 주세요.';
    case 'edit':
      return '이름이나 색상을 바꾸면 연결된 카드와 필터에도 반영돼요.';
    default:
      return '카테고리를 만들거나 이름과 색상을 바꿀 수 있어요.';
  }
}
