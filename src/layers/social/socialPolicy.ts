...
export function filterPublicQuestShards(_profile: UserProfile, shards: SocialQuestShard[]) {
  if (!_profile.publicQuestMode) return [];
  return shards.filter((shard) => shard.visibility === "anonymous_public");
}
