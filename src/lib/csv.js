...
//
// [레거시 로컬 데이터 처리 방침] 프로필/신체정보(db.js)와 끼니 기록(db.js의 meals 함수들)이 전부
// Supabase로 옮겨간 뒤로, 이 파일이 의존하는 mealStore.js/dailyRecord.js/records.js의 localStorage
// 저장분은 CSV 내보내기/가져오기 "안에서만" 계속 쓰이는 자체 완결형 레거시 서브시스템이다(내보내기는
// 이 로컬 데이터를 읽고, 가져오기는 이 로컬 데이터에 다시 쓴다 — 화면에 보이는 라이브 데이터는 전부
// Supabase에서 온다). 마이그레이션 이전 계정(임의 문자열 아이디, guest_xxxx)으로 남아있는 옛
// localStorage 데이터는 의도적으로 옮기지 않고 그대로 둔다(정책: 무시) — 그 옛 아이디들은 지금의
// Supabase auth uid(user.id)와 연결할 방법이 전혀 없어서(로그인 시스템 자체가 통째로 교체됐다),
// "이 옛 데이터가 지금 로그인한 이 사람 것"이라고 안전하게 판단해 옮길 근거가 없기 때문이다. 다만
// 데이터 자체는 삭제하지 않는다 — 과거에 CSV로 내보내 둔 백업을 다시 가져오기 하려는 사용자가 있을
// 수 있어서, 읽기 경로는 계속 살려둔다.
//
