...
//
// [현재 상태] saveRecord()를 부르는 곳은 이제 없다(예전엔 Analyze.jsx가 저장 때마다 같이 불렀지만,
// 끼니 저장이 Supabase meals 테이블로 옮겨가며 제거했다) — 즉 이 파일의 실제 저장소(records:<userId>)는
// 더는 늘어나지 않는 읽기 전용 과거 데이터다. getAllRecords()는 Calendar.jsx가 옛 날짜 폴백으로만
// 계속 읽는다. toDateKey()만은 순수 날짜 포맷 함수라 지금도 여러 화면에서 범용으로 쓴다. 레거시 로컬
// 데이터 처리 방침은 csv.js 상단 주석 참고.
