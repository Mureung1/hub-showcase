...
describe('nutrientCriteria', () => {
  ...
  it('부동소수점 경계 회귀 방지: 나눗셈이 아니라 곱셈으로 판정한다(리뷰에서 발견한 버그)', () => {
    // 86*0.8은 부동소수점으로도 정확히 68.8이지만, 68.8/86은 0.7999999999999999로 반올림된다.
    // 나눗셈 우선으로 판정하면 이 케이스가 "미충족"으로 잘못 나온다.
    expect(86 * 0.8).toBe(68.8) // 전제 확인
    expect(68.8 / 86).not.toBe(0.8) // 전제 확인 — 나눗셈은 정확히 0.8이 아니다
    expect(isMet('protein', 68.8, 86, 0.8)).toBe(true)
  })
})
