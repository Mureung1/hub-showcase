...
function ReactBasicsLessonPage() {
  ...
  useEffect(() => {
    ...
    const loadQuiz = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`퀴즈 데이터를 불러오지 못했습니다. (${response.status})`);
        }
        const data = (await response.json()) as QuizData[];
        if (!active) return;
        setQuizLoadState({ status: "ready", data });
      } catch (error) {
        if (!active || controller.signal.aborted) return;
        setQuizLoadState({
          status: "error",
  ...
  const deleteNote = (targetIndex: number) => {
    setNotes((current) => current.filter((_, idx) => idx !== targetIndex));
  };
    ...
    <main className={styles.page}>
      <section className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <div className={styles.viewTabs} aria-label="화면 뷰 선택">
            <button
              type="button"
              className={`${styles.tabButton} ${viewMode === "lesson" ? styles.tabButtonActive : ""}`}
            ...
            <button
              type="button"
              className={`${styles.tabButton} ${viewMode === "quiz" ? styles.tabButtonActive : ""}`}
          ...
          <article className={styles.panel}>
            ...
            <div className={styles.cardStrip}>
              <LessonCard
                ...
                classNameCard={styles.lessonCard}
                classNameActive={styles.lessonCardActive}
              />
              <LessonCard
                title="화면의 현재값을 저장한다"
                label="상태"
            ...
            <ul className={styles.noteList} aria-label="실습 메모">
              {notes.map((note, index) => (
                <NoteItem
                  ...
                  classNameItem={styles.noteItem}
                  classNameIndex={""}
                  classNameText={styles.noteItemText}
                  classNameDelete={styles.deleteButton}
                />
              ))}
            </ul>
          </article>
      ...
      {viewMode === "quiz" && (
        <section className={styles.grid}>
          <article className={styles.panel} style={{ gridColumn: "span 3" }}>
            <div className={styles.panelHead}>
              <p className={styles.panelLabel}>자가 진단 퀴즈</p>
              <h2>React 핵심 개념 테스트</h2>
            </div>
            <div className={styles.quizBox}>
              {quizLoadState.status === "loading" && <p>퀴즈 데이터를 불러오는 중입니다.</p>}
              {quizLoadState.status === "error" && <p role="alert">{quizLoadState.message}</p>}
              {quizLoadState.status === "ready" &&
                quizLoadState.data.map((quiz) => {
                  const selectedOpt = selectedAnswers[quiz.id];
                  const hasAnswered = selectedOpt !== undefined;
                  return (
                    <div key={quiz.id} className={styles.focusCard} style={{ marginBottom: "16px" }}>
                      <h3 className={styles.quizQuestion}>Q{quiz.id}. {quiz.question}</h3>
                      <div className={styles.quizOptions} style={{ marginTop: "12px" }}>
                        {quiz.options.map((opt, optIdx) => {
                          ...
                          if (hasAnswered) {
                            ...
                            else if (isSelected) optClass = `${styles.quizOption} ${styles.quizOptionIncorrect}`;
                          }
                          return (
                            <button
                              key={optIdx}
