...
function NavDot({ active }) {
  return (
    <span
      className={`h-1.5 w-1.5 rounded-[2px] ${active ? 'bg-accent' : 'bg-nav-inactive'}`}
    />
  )
}
  ...
  return (
    <aside className="sticky top-0 flex h-screen w-[238px] flex-none flex-col gap-[30px] border-r border-border px-[18px] py-[26px]">
      <div className="flex items-center gap-2.5 px-2">
        ...
        <span className="font-display text-[19px] font-bold tracking-[.14em]">
          REFIT
        </span>
      </div>
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span
                    className={`text-sm ${isActive ? 'font-semibold text-text' : 'font-medium text-text-secondary'}`}
                  >
                    {item.label}
                  </span>
                </>
      ...
      <div className="mt-auto rounded-md border border-border bg-panel p-3.5">
        <div className="font-display mb-[9px] text-[10px] tracking-[.16em] text-text-secondary">
          THIS WEEK
        </div>
        <div className="flex items-baseline gap-1.5">
          <span className="font-display text-[30px] font-bold text-accent">
            {weekProgress.completed}
          </span>
          <span className="text-[13px] text-text-secondary">
            / {weekProgress.total} 세션 완료
          </span>
        </div>
