...
export function BottomNavigation({ activeTab, onChange, onAdd }: BottomNavigationProps) {
  return (
    <nav className="scheduler-bottom-nav" aria-label="주요 메뉴">
      ...
      <button type="button" className="nav-add" aria-label="선택한 날짜에 새 일정 추가" onClick={onAdd}><i>+</i></button>
      <button type="button" className={activeTab === 'friends' ? 'active' : ''} aria-pressed={activeTab === 'friends'} onClick={() => onChange('friends')}><i className="nav-friends" /><span>친구</span></button>
      <button type="button" className={activeTab === 'profile' ? 'active' : ''} aria-pressed={activeTab === 'profile'} onClick={() => onChange('profile')}><i className="nav-profile" /><span>마이</span></button>
    </nav>
  )
}
