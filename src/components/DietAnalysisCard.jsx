// 달력 탭 상단 "AI 식습관 분석" 카드(PRD 4주차 3절). 버튼을 눌러야만 실행되고(자동 실행 금지),
// 같은 기간 옵션에 대한 분석은 하루 1회만 실제 호출한다 — 그래서 화면 어디에도 "다시 분석"류의
...
import { GEMINI_TEMPERATURE } from '../lib/geminiSchemas.js'
import { buildDietAnalysisPrompt, isDietAnalysisValid } from '../lib/prompts/dietAnalysis.js'
...
export default function DietAnalysisCard() {
  ...
  const [analysis, setAnalysis] = useState(null) // 캐시됐거나 방금 만든 결과
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      let text = await geminiCompleteWithRetry({ prompt, temperature: GEMINI_TEMPERATURE.dietAnalysis })
      // 4문장 미만이면 1회만 자동 재요청한다(FR-3.2) — 그래도 미달이면 오류로 안내한다.
      if (!isDietAnalysisValid(text)) {
        text = await geminiCompleteWithRetry({ prompt, temperature: GEMINI_TEMPERATURE.dietAnalysis })
      }
      if (!isDietAnalysisValid(text)) {
        throw new Error('분석 결과를 제대로 받지 못했어요. 잠시 후 다시 시도해주세요.')
      }
  ...
  return (
    <Card>
      ...
      {!analyzing && !analysis && summary === undefined && <Skeleton height={60} radius={radius.md} />}

      {!analyzing && !analysis && summary === null && (
        <p style={{ margin: 0, color: colors.textSub, fontSize: font.size.sm, textAlign: 'center', padding: `${spacing.md}px 0` }}>
          기록이 더 쌓이면 분석해드릴게요
        </p>
      )}

      {!analyzing && !analysis && summary && <AppButton onClick={runAnalysis}>분석하기</AppButton>}
