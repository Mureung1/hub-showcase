-- Align AI task mutations with the user-visible task status boundary.
-- Only AI tasks in not_started may be edited or deleted. Active and applied
-- runs remain locked independently, while a credential failure that restores
-- not_started remains recoverable.
...
begin
  ...
  if has_applied_run or has_active_run then
