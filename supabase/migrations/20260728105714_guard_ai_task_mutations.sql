-- TeamFlow AI-assigned task mutation guard.
...
begin
  if to_regclass('public.tasks') is null
    ...
    raise exception 'TEAMFLOW_PROJECT_PREFLIGHT_FAILED: expected TeamFlow AI task schema';
  end if;
end;
$$;

create function private.guard_ai_task_initial_status()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  where member.id = new.assignee_id
    and member.project_id = new.project_id;

  if assignee_kind = 'ai' and new.status <> 'not_started' then
...
create trigger tasks_guard_ai_initial_status
before insert on public.tasks
for each row execute function private.guard_ai_task_initial_status();
...
set search_path = ''
as $$
  select private.update_task(p_task_id, p_patch);
$$;
...
begin
  ...
  select exists (
    ...
    from public.ai_runs run
    where run.task_id = target_task.id
      and run.status in ('running', 'pending_review', 'applied')
  ) into has_locked_run;

  if has_locked_run then
    raise exception 'TEAMFLOW_CONFLICT:AI_TASK_LOCKED';
  end if;
...
set search_path = ''
as $$
  select private.delete_task(p_task_id);
$$;

revoke update (title, assignee_id, due_date, status, description, updated_at)
  on public.tasks from authenticated;
revoke delete on table public.tasks from authenticated;

revoke all on function private.guard_ai_task_initial_status() from public, anon, authenticated;
revoke all on function private.update_task(uuid, jsonb) from public, anon, authenticated;
revoke all on function private.delete_task(uuid) from public, anon, authenticated;
revoke all on function public.update_task(uuid, jsonb) from public, anon, authenticated;
revoke all on function public.delete_task(uuid) from public, anon, authenticated;

grant execute on function private.update_task(uuid, jsonb) to authenticated;
grant execute on function private.delete_task(uuid) to authenticated;
