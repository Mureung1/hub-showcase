...
alter table public.store_invitations
  drop constraint if exists store_invitations_hourly_wage_check,
  add constraint store_invitations_hourly_wage_check
    check (hourly_wage is null or hourly_wage >= 0);

alter table public.store_invitations
  drop constraint if exists store_invitations_default_time_order,
  add constraint store_invitations_default_time_order
    check (
