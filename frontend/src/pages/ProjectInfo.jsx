...
const ProjectInfo = () => {
  const problems = [
    {
      ...
      number: '01',
      title: '정산의 번거로움',
      description: '파티장은 매달 정산을 요청하고 입금 확인을 반복해야 하며, 파티원도 매달 이체를 반복해야 합니다.'
    },
    {
      id: 2,
      number: '02',
  ...
  const targets = [
    '2개 이상의 구독 서비스를 이용하며 합리적인 소비를 지향하는 청년',
  ...
  const coreFeatures = [
    {
      ...
      tag: 'Feature 01',
      title: '원클릭 정산',
      description: '결제일에 맞춰 파티장에게는 정산 시작 알림을, 파티원에게는 간편 이체 링크가 담긴 알림을 이메일과 푸시로 보냅니다. 카카오톡 초대 링크로 그룹원을 구글 로그인만으로 자동 등록하고, 필요하면 카카오톡으로 정산 요청도 추가로 보낼 수 있습니다. \'이체하기\'를 누르면 토스/카카오페이 딥링크로 바로 연결되어 이체할 수 있습니다.'
    },
    {
      id: 2,
      tag: 'Feature 02',
  ...
  const scenarios = [
    {
      ...
      role: '파티장(계정주)',
      steps: [
        '구글 로그인으로 SUBZIP에 가입하고, 구독 서비스, 총 결제 금액, 결제일, 파티원 수와 정산금을 받을 계좌번호, 예금주명을 등록한다.',
        '초대 링크를 카카오톡 공유 기능으로 그룹원들에게 전달한다.',
        '결제일이 되면 푸시 알림을 확인하고, 필요하면 카카오톡 공유 기능으로 정산 요청 메시지를 파티원들에게 추가로 전송한다.',
        '구독 서비스에 대한 만족도 설문을 완료하고, 개인 만족도 리포트를 기반으로 구독 유지 및 변경 여부를 결정한다.',
        '파티원의 이체가 확인되면 정산 상태를 \'확인 완료\'로 변경한다.',
        '정산이 완료된 후 그룹 만족도 리포트를 확인하고, 파티원들과 구독 유지 여부를 논의한다. (MVP 이후)'
      ]
    },
    {
      id: 'invited-member',
      role: '초대받은 신규 파티원',
      steps: [
        '파티장이 카카오톡으로 공유한 초대 링크를 클릭한다.',
        '구글 로그인/가입을 진행하면, 로그인 완료와 동시에 해당 그룹에 파티원으로 자동 등록되어 리디렉션된다.',
        '이후로는 회원 파티원과 동일한 흐름을 따른다.'
      ]
    },
    {
      id: 'member',
      role: '회원 파티원',
      steps: [
        '결제일이 되면 이메일/푸시 알림(또는 파티장이 보낸 카카오톡 공유 메시지)을 통해 토스/카카오페이로 이동하여 이체한다.',
        '구독 서비스에 대한 만족도 설문을 완료하고, 개인 만족도 리포트를 기반으로 구독 유지 및 변경 여부를 결정한다.',
        '정산이 완료된 후 그룹 만족도 리포트를 확인하고, 파티원들과 구독 유지 여부를 논의한다. (MVP 이후)'
      ]
    },
    {
      ...
      steps: [
        ...
        '구독 서비스에 대한 만족도 설문에 참여하여 그룹 데이터에 기여하고, 초대 링크를 통해 SUBZIP에 가입할 수 있다.',
        '이후로는 회원 파티원과 동일한 흐름을 따른다.'
      ]
    }
  ];
  ...
  return (
    <div className="project-intro-container">
      <header className="intro-header">
        <h1 className="intro-subtitle">매달 반복되는 정산, 이제는 한 번에 끝내보세요</h1>
      ...
      <section className="scenario-section">
        ...
        {scenarios.map((scenario) => (
          <div key={scenario.id} className="scenario-group">
            <h3 className="scenario-role">
              {scenario.role}
              {scenario.badge && <span className="scenario-badge">{scenario.badge}</span>}
            </h3>
