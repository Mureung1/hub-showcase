...
export function useAssessmentFlow() {
  ...
  // AI 추정이 폴백(백엔드 꺼짐·실패)되면 조용히 넘어가지 않고 이 안내를 화면에 띄운다(② 방지책).
  ...
  useEffect(() => {
    ...
    saveResult({
      ...
      profile: {
        ...
        mbtiEstimated,
        mbtiSource,
        estimatedMeta: estimatedMeta ?? null,
        createdAt,
      },
      studyAnswers,
      stressAnswers,
  ...
  // 보충 모드(③): 확정 유형은 유지하고, AI 대화에서 관찰된 근거 메타만 붙인다(공식·추정 공통).
  function applySupplementMeta(meta = null) {
    if (meta) {
      setEstimatedMeta(meta);
    }
    setStep(2);
