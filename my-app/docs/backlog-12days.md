...
- 기간: 2026-07-13(월)부터 시작, **월/화/수/목만 작업**, 하루 3~4시간. 원래 12세션·07-30 마감이었으나 범위 확장으로 **총 18세션·2026-08-11(화)까지**로 늘어남 (아래 "검토 필요" 참고)
- 인원: 혼자 개발
...
이번 주 요구사항(Supabase/테스트/Skill/Agent/다이어그램)을 원래 계획에 다 얹으면서 완료 시점이 **2026-07-30 → 2026-08-11**로 밀렸습니다. 2026-07-20 재조정으로 기능 강화 스프린트(옛 Sprint 6, Day 16~17)를 범위에서 제외하고, 프론트-백엔드 연동(Sprint 4)을 테스트/Skill/Agent 산출물(Sprint 5)보다 앞으로 당겨서 일정을 2일 단축했습니다. 만약 남은 요구사항(테스트/TDD, Skill, Agent, 다이어그램)에 별도의 확정 마감이 있다면 이 일정으로도 못 맞출 수 있으니 그 경우 다시 확인이 필요합니다.
...
| Sprint 4 — 프론트-백엔드 연동 | 07-28 ~ 07-29 | Day 10~11 | 목업 훅을 실제 API 호출로 전면 교체 |
| Sprint 5 — 테스트 & Agent 산출물 | 07-30 ~ 08-05 | Day 12~15 | 테스트/TDD, 테스트코드 생성 Skill, 코드 검증 Agent, 아키텍처 다이어그램 |
| Sprint 6 — LLM 연동 | 08-06 ~ 08-10 | Day 16~17 | LLM 폴백 구조 설계 + 생성 기능 전반 연결 |
| Sprint 7 — 통합 점검 | 08-11 | Day 18 | e2e 테스트, 예외 처리, 워크플로우 문서, README 정리 |

**2026-07-20 재조정**: 기능 강화 스프린트(옛 Sprint 6, Day 16~17)는 범위에서 제외. 프론트-백엔드 연동(옛 Sprint 5)을 테스트/Skill/Agent 산출물(옛 Sprint 4)보다 앞으로 당김 — 백엔드 API가 Sprint 3에서 이미 다 갖춰졌기 때문에 순서를 바꿔도 막히는 의존관계가 없고, 프론트 연동을 먼저 끝내두면 이후 테스트 대상 범위(프론트+백엔드)가 더 명확해짐.
