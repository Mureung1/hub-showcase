...
3. 목표 직무 입력
   - 사용자가 스펙 등록 화면에서 희망 직무를 함께 입력합니다.
...
사용자는 처음에는 홈 화면에서 서비스를 이해하고, 회원가입과 로그인 후 핵심 기능을 순서대로 이용합니다. 1차 MVP의 핵심 반복 흐름은 `스펙 등록 → AI 분석 → 미션 추천 → 수행 → 피드백 → 포트폴리오`입니다.
...
4주차 프론트엔드 범위는 결과물 업로드, AI 피드백, 포트폴리오 화면과 최종 UI 정리 중심으로 점검했습니다.

### 완료된 화면

- `/upload`: 결과물 링크, 설명, 파일 UI, 제출 후 피드백 이동
- `/feedback`: 제출 결과물 기반 AI 피드백, 개선점, 포트폴리오 반영 포인트
- `/portfolio`: 미션, 결과물, 피드백을 프로젝트 경험으로 정리
- `/me`: 회원 기본 정보, 스펙, AI 분석 요약 확인

### UI 정리

- 공통 버튼 스타일을 `cm-button` 계열 클래스로 정리했습니다.
- 입력 폼은 `cm-field`, `cm-input`, `cm-textarea`, `cm-select` 계열로 통일했습니다.
- 빈 상태 화면은 `cm-empty-state`로 정리했습니다.
- Home, MyPage, SpecRegister, Analysis 주요 2열 레이아웃은 모바일에서 1열로 접히도록 보강했습니다.

### 검증

- `npm run lint` 기준 정적 검사
- `npm run build` 기준 Vite 프로덕션 빌드
