...
현재 서비스는 **모집, 참여·취소, 방장의 주문·픽업 상태 변경, 참여자의 수령 완료 기록**까지 실제 MySQL에 저장한다. 실제 계좌 이체와 상품 주문·전달 자체는 서비스 밖에서 이뤄지지만, 누가 어느 단계까지 완료했는지는 화면과 API에서 확인할 수 있다.
...
1. 정원이 차면 자동으로 모집을 마감한다.
2. 방장이 `주문 완료`, `픽업 가능`, `공구 완료`를 순서대로 누르면 참여자 화면에도 상태가 바뀐다.
3. 참여자는 `픽업 대기` 상태에서 `수령 완료`를 직접 누른다. 모든 참여자의 수령 기록이 있어야 방장이 공구를 완료할 수 있다.
4. 입금 완료 현황과 대기자 자동 승격은 다음 기능으로 구현한다.
