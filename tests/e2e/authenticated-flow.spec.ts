...
test("login, persist two analyses, inspect evidence, share, refresh, and revoke", async ({
  ...
  try {
    ...
    await expect(
      page.getByRole("heading", { name: projectTitle, level: 1 }),
    ).toBeVisible();
    ...
    await importPastedContext(page, {
      title: "첫 기획 회의",
      content:
        "공모전 팀은 7월 12일 기획 회의에서 MVP 범위를 논의했다. 사용자 인터뷰 8명 중 6명이 첫 화면에서 서비스 목적을 이해하지 못했고 발표까지 2주밖에 남지 않았기 때문에, 팀은 MVP 대상을 모든 협업팀이 아니라 대학생 팀 프로젝트로 좁히기로 결정했다. 민지는 저장된 기록을 새로고침 뒤에도 확인할 수 있어야 한다고 말했다. 다음 회의에서는 대학생 3명에게 새 소개 문구를 다시 검증한다. 모든 참여자는 실패한 실행이 최근 성공 결과를 덮어쓰면 안 된다는 데 동의했다.",
    });

    await page.getByRole("tab", { name: "분석 실행", exact: true }).click();
    ...
    await expect(
      page.getByText(/사용자 인터뷰 8명 중 6명.*발표까지 2주.*때문입니다/),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("dialog").getByText(
        "사용자 인터뷰 8명 중 6명이 첫 화면에서 서비스 목적을 이해하지 못했고 발표까지 2주밖에 남지 않았기 때문에, 팀은 MVP 대상을 모든 협업팀이 아니라 대학생 팀 프로젝트로 좁히기로 결정했다.",
      ),
    ).toBeVisible();
    await page
      .getByRole("dialog")
    ...
    await page.getByRole("tab", { name: "분석 이력", exact: true }).click();
    const firstPersistedHistory = page.getByRole("complementary", {
      name: "분석 실행 이력",
    });
    await expect(
      firstPersistedHistory.getByText("성공", { exact: true }),
    ).toHaveCount(1);
    await expect(
      page.getByText(/사용자 인터뷰 8명 중 6명.*발표까지 2주.*때문입니다/),
    ).toBeVisible();
    const persistedEvidenceButton = page
      .getByRole("button", { name: /근거 \d+개/ })
      .first();
    await persistedEvidenceButton.click();
    await expect(
      page.getByRole("dialog").getByText(
        "사용자 인터뷰 8명 중 6명이 첫 화면에서 서비스 목적을 이해하지 못했고 발표까지 2주밖에 남지 않았기 때문에, 팀은 MVP 대상을 모든 협업팀이 아니라 대학생 팀 프로젝트로 좁히기로 결정했다.",
      ),
    ...
    await page.getByRole("tab", { name: "분석 실행", exact: true }).click();
    await expect(page.locator('[data-testid^="analysis-source-"]')).toHaveCount(2);
    await page.getByRole("tab", { name: "분석 이력", exact: true }).click();
    const persistedHistory = page.getByRole("complementary", {
      name: "분석 실행 이력",
    });
    await expect(
      persistedHistory.getByText("성공", { exact: true }),
    ).toHaveCount(2);
    await expect(
      page.getByRole("heading", { name: "최근 분석의 변화 서사" }),
    ).toBeVisible();
    ...
    await expect(page.getByRole("group", { name: "생각 그래프" })).toBeVisible();
    for (const width of [375, 768, 1440]) {
      await assertResponsiveKnowledgeMap(page, width);
    }
    ...
    await expect(
      sharedPage.getByRole("heading", { name: "공유된 분석 요약", level: 1 }),
    ).toBeVisible();
    await expect(
      sharedPage.getByRole("note", { name: "공유 공개 범위" }),
    ).toContainText("요약 공개 모드");
    await expect(sharedPage.getByText(projectTitle, { exact: false })).toHaveCount(0);
    await expect(sharedPage.getByText("첫 기획 회의", { exact: false })).toHaveCount(0);
    await expect(sharedPage.getByText("멘토 후속 피드백", { exact: false })).toHaveCount(0);
    await expect(sharedPage.getByText("민지", { exact: false })).toHaveCount(0);
    await expect(sharedPage.getByText("유나", { exact: false })).toHaveCount(0);
    await expect(sharedPage.getByText("현우", { exact: false })).toHaveCount(0);
    await expect(
...
async function importPastedContext(
  ...
  const importDisclosure = page.locator("details.context-import-disclosure");
  if (!(await importDisclosure.evaluate((element) => (element as HTMLDetailsElement).open))) {
...
async function assertResponsiveKnowledgeMap(page: Page, width: number) {
  await page.setViewportSize({ width, height: width === 375 ? 812 : 900 });
  const graphToggle = page.getByRole("button", { name: "그래프 보기" });
  if (await graphToggle.getAttribute("aria-pressed") !== "true") await graphToggle.click();
  ...
  const stageBox = await canvas.locator(".brain-stage-wrap").boundingBox();
  const inspectorBox = await canvas.locator(".brain-inspector").boundingBox();
