...
function toNotionProperties(entry) {
  return {
    ...
    // proposed_reason은 S3 계약대로 Text 속성. Notion DB는 title 속성이 하나 꼭 있어야 해서,
    // 그건 'label'이라는 별도 표시용 속성으로 분리해 짧은 요약만 담는다.
    proposed_reason: { rich_text: [{ text: { content: entry.proposed_reason } }] },
    label: {
      title: [{ text: { content: `${entry.reason_chip} → ${entry.proposed_tool}` } }],
    },
...
function fromNotionPage(page) {
  ...
  return {
    ...
    proposed_reason: p.proposed_reason?.rich_text?.[0]?.plain_text ?? "",
