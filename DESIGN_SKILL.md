...
- 카테고리와 태그는 구분선을 둔 하나의 그룹에 모두 표시합니다. `분류 ·`, `영양 ·`, `가공 ·` 접두어로 역할을 구분합니다.
- 태그 영역은 두 줄 이상의 최소 높이를 확보하고 카드 본문은 세로 flex로 구성해 모든 카드 높이와 하단 CTA 위치를 맞춥니다.
...
D-3 이상 또는 장기 보관 확인일은 강한 경고색을 사용하지 않습니다. 앱이 제안한 날짜는 실제 소비기한이 아닌 `권장 사용 날짜`로 표현하고, 통조림·인스턴트·조미료 같은 장기 보관 품목은 `보유 상태 확인일`로 표현합니다.
