...
재료 추가는 모달 또는 전용 패널에서 동일한 입력 흐름을 사용합니다. 사용자가 먼저 아는 정보부터 입력하고, 분류와 보관 방법을 선택하면 시스템이 권장 날짜와 태그를 제안합니다.

입력 순서는 반드시 다음을 유지합니다.

1. 재료명
2. 수량
3. 재료 분류
4. 보관 방법
5. 권장 사용 날짜 또는 보유 상태 확인일
6. 추천용 재료 태그
7. 제출 버튼

- 카테고리에 허용되지 않는 보관 방법은 선택지에 노출하지 않습니다.
- 자동 제안 날짜에는 `자동 제안` 상태와 산정 기준을 표시합니다.
- 사용자가 날짜를 직접 수정한 뒤 분류나 보관 방법을 바꾸면 입력값을 덮어쓰지 않습니다. 새 권장 날짜와 `권장값 적용` 액션만 제공합니다.
- 포장지의 소비기한과 실제 재료 상태를 우선하라는 안내를 항상 함께 표시합니다.
- 소금·후추 등 기본 양념은 이 폼에 등록하지 않고 레시피 상세의 기본 양념 보유 설정에서 관리합니다.
...
2. D-day + 남은 권장 사용기간
3. 보관 방법
4. 사용자가 선택해 저장한 추천용 재료 태그
5. `이 재료로 요리 찾기` 액션

#### Ingredient Tile Layout

- Header 영역은 `[재료 아이콘] [재료명+수량 / 기한 / 보관 방법] [더보기]` 구조를 사용합니다.
- 재료명과 수량은 같은 기준선에 배치하고 카드에서 가장 먼저 읽히게 합니다.
- 기한 정보는 D-day와 남은 기간을 한 줄로 묶어 중복 문장을 만들지 않습니다.
- 보관 방법은 기한 아래의 보조 텍스트로 표시하며 별도의 강한 배지를 사용하지 않습니다.
- 태그 영역에는 재료의 `tags` 필드에 저장된 태그명만 표시합니다. 카테고리, 기한 상태, 가공 여부를 자동으로 합성하거나 `분류 ·`, `영양 ·`, `가공 ·` 접두어를 붙이지 않습니다.
- 저장된 태그가 없으면 태그 영역을 표시하지 않습니다. 카드 본문은 세로 flex로 구성해 카드 높이와 하단 CTA 위치를 맞춥니다.
- 모바일에서는 텍스트 겹침이 없도록 Header 내부에 `min-width: 0`과 유연한 간격을 적용합니다.
- 카드는 프로젝트 토큰인 `16px` radius와 부드러운 그림자를 유지합니다. 첨부 명세의 구조 원칙은 따르되 별도의 8px radius나 다른 브랜드 색상을 도입하지 않습니다.
...
- 카드 배경: `--warning-soft`
- D-day 배지: `--warning`
- D-day 배지와 카드 경고색으로만 강조하며 `먼저 사용`을 태그 영역에 추가하지 않습니다.

D-3 이상 또는 장기 보관 확인일은 강한 경고색을 사용하지 않습니다. D-6 이상으로 여유가 있는 재료는 카드 배경과 테두리를 바꾸지 않고 D-day 배지와 남은 기간 문구만 초록색으로 표시합니다. 앱이 제안한 날짜는 실제 소비기한이 아닌 `권장 사용 날짜`로 표현하고, 통조림·인스턴트·조미료 같은 장기 보관 품목은 `보유 상태 확인일`로 표현합니다.

### Basic Pantry

- 소금, 후추, 식용유, 고춧가루, 간장, 설탕, 식초, 다진 마늘은 기본 보유로 가정합니다.
- 기본 양념은 내 냉장고 카드와 전체 재료 개수에 포함하지 않습니다.
- 레시피 상세에서 해당 레시피에 필요한 기본 양념만 표시합니다.
- 각 항목은 `보유 중`과 `없어요`를 즉시 전환할 수 있어야 합니다.
- `없어요` 상태는 부족 재료, 장보기 목록, 추천 점수에 일관되게 반영합니다.
- 상태 버튼은 보유 시 green soft, 없음 시 orange soft를 사용하며 선택을 막지 않습니다.
