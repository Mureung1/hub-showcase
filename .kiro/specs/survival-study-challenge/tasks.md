...
- [x] 2. Drizzle 스키마 및 마이그레이션 (데이터 계층)
  ...
  - [x] 2.7 얇은 raw SQL 마이그레이션: pgcrypto 확장 + handle_new_user 트리거
  ...
  - [x] 2.8 raw SQL: RLS 정책 + Storage 버킷/정책 + v_leaderboard 뷰
  ...
  - [x] 2.10 신규 스키마를 원격 DB에 적용 및 검증 (원격 전용)
...
- [x] 3. 포인트 시스템 트랜잭션 함수 (무결성 기반)
  - [x] 3.1 debitPoints / creditPoints 구현
...
- [x] 4. 챌린지 개설/참가 도메인 함수
  - [x] 4.1 createUserChallenge 구현
  ...
  - [x] 4.3 joinOfficialChallenge 및 handlePaymentWebhook 구현
  ...
  - [x] 4.8 챌린지 개설/참가 단위 테스트
    - 필수 항목 누락/범위 초과 오류, 현금 참가비 거부, 공개 목록 노출, 잔액 부족/모집 마감/중복 거부, 결제 거부 경로 검증
    - _Requirements: 4.2, 4.3, 4.5, 3.2, 5.2, 5.3_

- [x] 5. 체크포인트 - 데이터 계층·포인트·참가 흐름 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. 인증 도메인 함수 (일일 목표·타이머·생존 인증)
  - [x] 6.1 submitDailyGoal 및 recordTimerSession 구현
  ...
  - [x] 6.2 submitVerification 구현 (게이팅)
...
- [x] 7. 일일 탈락 처리 및 부활권
  - [x] 7.1 processDailyEliminations 구현
...
- [x] 8. 체크포인트 - 인증 및 탈락 흐름 테스트 통과 확인
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. 종료 정산 및 보상 분배
  - [x] 9.1 settleChallenge 구현 (완주자 확정 + 분배 + 보존식 검증)
  ...
  - [x] 9.2 완주 배지·학습 리포트·완주 보상 포인트 지급
