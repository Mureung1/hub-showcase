  ...
  it("원격 접수를 저장·조회·미루기·취소하고 전체 테스트 데이터를 롤백한다", async () => {
    ...
    await expect(
      ...
      `,
        ...
        await expect(
          service.register(accountId, hospitalId, {
            inputMode: "total_only",
            totalCount: 1,
          }),
        ).rejects.toMatchObject({ code: "ACTIVE_REMOTE_WAITING_EXISTS" });
        await expect(
          service.register(secondAccountId, hospitalId, {
            inputMode: "total_only",
            totalCount: 3,
          }),
        ...
        await expect(service.defer(accountId)).rejects.toMatchObject({
          code: "PATIENT_DEFER_ALREADY_USED",
        });
    ...
    const result = await databasePool.query("SELECT id FROM public.waiting_entries WHERE id = $1", [
      waitingId,
    ]);
