...
import type { WaitingEntry, WaitingEntryCount, WaitingRepository } from "../repositories/waitingRepository.js";
...
export interface PatientWaitingOperations {
  ...
  register(accountId: string, hospitalId: string, input: PatientRegistrationInput): Promise<QueuePosition>;
...
export class PatientWaitingService implements PatientWaitingOperations {
  ...
  async getHospitalConfig(hospitalId: string): Promise<MockPatientConfig> {
    return this.transactionManager.run(async (executor) => {
      const { queue, configuration } = await this.getQueueContext(executor, hospitalId);
      return {
  ...
  async register(accountId: string, hospitalId: string, input: PatientRegistrationInput): Promise<QueuePosition> {
    return this.transactionManager.run(async (executor) => {
      ...
      if (await this.waitingRepository.findActiveRemoteByAccount(executor, accountId)) {
        throw new ApiError(409, "ACTIVE_REMOTE_WAITING_EXISTS", "이미 진행 중인 원격 웨이팅이 있습니다.");
      }
      if (input.inputMode !== configuration.inputMode) {
        throw new ApiError(409, "PATIENT_INPUT_MODE_CHANGED", "병원의 현재 환자 인원 입력 방식을 다시 확인해 주세요.");
      }

      const patientCount = calculateRegistrationPatientCount(input);
      const countInputs = input.inputMode === "categorized"
        ? this.validateCategoryCounts(input.patientCounts, configuration.categories.map(({ id }) => id))
        : [];
      ...
      const remotePatientCount = await this.waitingRepository.sumActiveRemotePatients(executor, queue.id);
  ...
  async defer(accountId: string): Promise<QueuePosition> {
    return this.transactionManager.run(async (executor) => {
      ...
      if (current.patientDeferCount > 0) {
        throw new ApiError(409, "PATIENT_DEFER_ALREADY_USED", "순서 미루기는 한 번만 사용할 수 있습니다.");
      }
      if (!(["remote_waiting", "entry_requested"] as const).includes(current.status as "remote_waiting" | "entry_requested")) {
        throw new ApiError(409, "PATIENT_DEFER_UNAVAILABLE", "현재 상태에서는 순서를 미룰 수 없습니다.");
      }
      const updated = await this.waitingRepository.deferRemoteToEnd(executor, current.id, current.version);
  ...
  async cancel(accountId: string): Promise<QueuePosition> {
    return this.transactionManager.run(async (executor) => {
      const current = await this.requireOwnedActiveRemote(executor, accountId);
      if (!(["remote_waiting", "entry_requested"] as const).includes(current.status as "remote_waiting" | "entry_requested")) {
        throw new ApiError(409, "PATIENT_CANCEL_UNAVAILABLE", "현장 접수 후에는 병원에 문의해 주세요.");
      }
  ...
  private async requireOwnedActiveRemote(executor: DatabaseExecutor, accountId: string) {
    await this.requirePatientProfile(executor, accountId);
    const waiting = await this.waitingRepository.findActiveRemoteByAccount(executor, accountId);
    if (!waiting) throw new ApiError(404, "ACTIVE_WAITING_NOT_FOUND", "진행 중인 원격 웨이팅이 없습니다.");
    return waiting;
  }
  ...
  private async getQueueContext(executor: DatabaseExecutor, hospitalId: string) {
    ...
    const queue = await this.dailyQueueRepository.findByHospitalAndDate(executor, hospitalId, this.options.getClinicDate());
    if (!queue) throw new ApiError(404, "DAILY_QUEUE_NOT_FOUND", "오늘 대기열이 준비되지 않았습니다.");
    const configuration = await this.categoryRepository.findConfigurationById(executor, queue.categorySetId);
  ...
  private validateCategoryCounts(patientCounts: PatientCounts, allowedIds: string[]) {
    ...
    return Object.entries(patientCounts).filter(([, count]) => count > 0)
      .map(([patientCategoryId, count]) => ({ patientCategoryId, count }));
  }

  private async buildPosition(executor: DatabaseExecutor, waitingId: string): Promise<QueuePosition> {
    ...
    const configuration = await this.categoryRepository.findConfigurationById(executor, queue.categorySetId);
    if (!configuration) throw new ApiError(409, "CATEGORY_CONFIGURATION_INVALID", "환자 분류 설정을 확인해 주세요.");
    ...
    const positions = calculateQueuePositions(entries.map((entry) => this.toQueueEntry(entry, counts, configuration.inputMode, configuration.categories)));
  ...
  private toQueueEntry(waiting: WaitingEntry, counts: WaitingEntryCount[], inputMode: QueueEntry["inputMode"], categories: QueueEntry["categorySnapshot"]): QueueEntry {
    return {
      id: waiting.id,
      ticketNumber: waiting.ticketNumber,
      source: waiting.source,
      inputMode,
      patientCounts: Object.fromEntries(counts.filter((row) => row.waitingEntryId === waiting.id).map((row) => [row.patientCategoryId, row.count])),
