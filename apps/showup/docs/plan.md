...
> 개발 가능일 = 10일 (금요일 3일 + 공휴일 1일 제외), 남은 개발일 = 5일.
...
| 6일차 | 7/16 | 목 | ✅ | 위험도 계산·riskStats 연동 + **1주차 발표·데모 (18:30)** |
| 7일차 | 7/17 | 금 | ❌ | **공휴일** (작업 없음, 멘토링만 진행) |
...
| 13일차 | 7/27 | 월 | ✅ | 배포·스테이징·발표 자료 완성 |
| 14일차 | 7/28 | 화 | ✅ | **최종 마무리** |

**개발일 요약**: 10일 (1~9일차 완료, 5일 남음)
**PR 제출 가능일**: 10일차(수), 11일차(목), 13일차(월), 14일차(목)

Phase 1.5(통계 차트)·Phase 2(본인 조회/정정·삭제 요청/이의제기)·Phase 3(플랫폼 연동)는 7/28 이후.
