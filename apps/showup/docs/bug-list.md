...
| # | 세션 | 파일:줄 | 문제 | 상태 |
|---|---|---|---|---|
| 1 | FE | CustomerDetail.tsx:25 | `useState<any>` 타입 미사용 — Customer 타입 적용 필요 | ✅ 수정 (`aad82332`) |
| 2 | FE | CustomerDetail.tsx:61,124 | `occurredAt as any` 타입 캐스팅 — Timestamp 변환 로직 필요 | ✅ 수정 (`aad82332`) |

## P1 - 배포 전 권장 수정 ✅ 완료

| # | 세션 | 파일:줄 | 문제 | 상태 |
|---|---|---|---|---|
| 3 | FE | src/mock/ | mock 데이터 잔존 (import 0건이나 파일 존재) — cleanup 대상 | ✅ 수정 (`aad82332`) |
| 4 | FE | services/storeFlow.ts | 데드 코드 (import 0건) — 삭제 대상 | ✅ 수정 (`db0f89d0`) |

## P2 - 데모/발표 전 확인

| # | 세션 | 항목 | 문제 | 상태 |
|---|---|---|---|---|
|| 5 | BE | Cloud Functions 배포 | onWrite 트리거가 프로덕션에서 동작하는지 미확인 | ✅ Spark 요금제 불가 → riskRefresh.ts 클라이언트 갱신으로 대체 완료 |
...
| 8 | FE | Lighthouse 실측 | 코드 스플리팅 적용했으나 실제 Lighthouse 점수 미측정 | ⚠️ 미측정 (코드 스플리팅 적용됨, 메인 9.82KB) |
