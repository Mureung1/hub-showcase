...
현재 `Reservations.tsx` 는 예약 리스트를 받은 후 각 `customerId` 로 `getCustomer()` 를 호출한다.
...
**MVP 결정**: 현재 N+1 상태를 유지. 예약 수가 적은 소규모 가게 기준에서는 비용 영향 미미.
...
- 7/28 Cloud Functions 배포 후 대시보드/예약 집계를 서버에서 처리해 읽기 비용 추가 절감.
