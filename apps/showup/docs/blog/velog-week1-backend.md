...
> 역사적 회고 문서. 아래의 "예정"·"현재"·기대 결과는 1주차 작성 시점 기록이다. 2026-07-28 현재 운영 상태와 검증 결과는 [ShowUp README](../../README.md), [작업 체크리스트](../checklist.md)를 기준으로 한다.
...
### 침투 테스트 시나리오 (1주차 당시 설계, 에뮬레이터 테스트 예정)

보안 세션에서 8개 시나리오를 정의했습니다. 1주차 작성 시점에는 문서로 정의된 상태였고, 이후 실행 가능한 회귀 테스트로 정리되었습니다. 당시 기대 결과는 다음과 같습니다:
...
1주차 당시 상태는 기반 세팅이 완료된 단계였고, Cloud Function 기반 riskStats 자동 갱신, 침투 테스트 자동화, 인증 연동은 2주차 예정이었습니다. 현재 Spark MVP는 클라이언트 `riskRefresh.ts`를 사용하며, 프로덕션 Functions는 배포하지 않았습니다.
