...
| Source action | Text source만 source-list order로 최대 16개 선택하고 `선택한 자료 정리하기`를 눌렀을 때만 exact ordered path refs와 현재 Codex settings를 동결해 `/api/product/actions`의 `organize_sources`를 시작한다. Preview content·digest는 동결하거나 action request에 넣지 않는다. PDF·unsupported source와 limit 초과 선택은 accessible reason과 함께 닫고, successful reload 뒤에는 current text source와의 교집합만 보존한다. Action은 별도 transcript entry로 표시하고 기존 Product operation stream·Review·clarification·interrupt·terminal controller를 재사용하며 terminal 뒤 selection을 자동으로 지우지 않는다. |
