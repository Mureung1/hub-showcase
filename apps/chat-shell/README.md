...
| Product application state | Bootstrap의 Account readiness, coarse `operationStatus`, active workspace와 settled-only history 전체를 한 owner가 보존한다. Workspace mutation과 settled Review·continuity recovery 뒤 authoritative bootstrap을 다시 읽으며, `active`이면 Server drain 시간값을 복사하지 않고 `idle` 응답의 final read까지 poll한다. Pending prompt·private correlation·persistence metadata는 Browser 상태로 만들지 않는다. Reject reload는 no-apply confirmation을, recovery reload는 settled Run outcome만 다시 열고 confirmed model과 revision을 임의로 바꾸지 않는다. |
