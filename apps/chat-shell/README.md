...
App 시작 전에 준비한 user-owned Git SemesterWorkspace에서 read-only source explorer·preview, 명시적 `model_semester` ActionInvocation, Official OpenAI Codex Python SDK 기반 normal AY Chat과 inline Semantic Review를 제공하는 desktop AY-PLE shell이다. Browser는 absolute workspace path, source registry·copy, private Broker credential, native identity와 apply ledger를 소유하지 않는다.
...
| Source action | Source list의 safe regular file을 preview kind와 무관하게 source-list order로 최대 16개 선택하고 `선택한 자료로 학기 정보 정리하기`를 눌렀을 때만 exact ordered path refs와 현재 Codex settings를 동결해 `/api/product/actions`의 `model_semester`를 시작한다. Preview content·digest는 동결하거나 action request에 넣지 않는다. PDF와 앱에서 preview할 수 없는 source도 선택할 수 있고, limit 초과 선택만 accessible reason과 함께 닫는다. Successful reload 뒤에는 current safe source와의 교집합을 보존한다. Action은 별도 transcript entry로 표시하고 기존 Product operation stream·Review·clarification·interrupt·terminal controller를 재사용하며 terminal 뒤 selection을 자동으로 지우지 않는다. |
| Semantic Review | Product Turn decoder와 transcript reducer는 `review.requested | review.resolved | review.failed`만 public Review wire로 사용한다. Pending card는 ordered change, AY가 제시한 bounded `SourceCitation`과 `accept | revise | reject`만 표시하고 composer·새 Turn을 잠근다. Citation은 relative file path, excerpt와 optional location hint를 보여 주며 App이 excerpt를 인증했다는 뜻으로 표현하지 않는다. Bodyless `204`는 held call 전달 ACK일 뿐이며 resolved frame이 settlement authority다. Settled·failed card는 controls 없이 남고 `revise` 뒤 fresh call은 새 card로 append한다. Browser는 ledger·credential·Runtime binding을 소유하지 않는다. |
| Product Chat | Free-form composer는 `/api/product/chat/messages`만 사용하고 explorer action selection을 암묵적으로 전송하지 않는다. 일반 clarification은 normal Chat의 후속 Turn으로 이어 간다. Native user-input frame이 도착한 경우의 별도 binding·answer/cancel transport는 보존하지만 current Default-mode Product Turn의 일반 기능으로 가정하지 않으며, active operation 중 conflicting send는 닫는다. |
...
| Failure boundary | Invalid JSON·UTF-8·contract shape, operation/activity 또는 Review binding mismatch, invalid citation, missing terminal과 post-terminal frame은 raw payload 없이 safe stream failure로 닫는다. Wrong·late·stale Review response를 성공처럼 투영하지 않는다. |
| Workbench | 1440–1920px desktop에서 자료 explorer와 action control, 가장 넓은 source preview, mounted AY Chat dock을 3-pane으로 제공한다. Action 시작은 닫힌 Chat dock을 열고, Review citation의 file action은 현재 safe source preview를 열어 사용자가 AY의 excerpt·location hint를 원문과 비교할 수 있게 한다. 현재 contract는 digest match, excerpt highlight나 exact 위치 이동을 보장하지 않는다. Chat dock을 다시 접어도 controller와 stream을 유지한다. |
...
Unit suite는 target-only operation decoder, lifecycle·source projection, preview kind와 독립적인 ordered action selection, SourceCitation을 포함한 exact Semantic Review binding과 Browser NDJSON parser를 검증한다. Default Playwright trace는 실제 Vite·target Express composition과 deterministic Runtime에서 active prepared lifecycle, source list·text/PDF/unsupported preview, PDF·unsupported file까지 포함한 explicit `model_semester` selection·frozen request·action transcript, operation lock·retry, AY citation 표시·file preview 열기, normal AY Chat 격리, native user-input compatibility·interrupt와 semantic proposal의 accept·revise·reject settlement를 검증한다. Action workbench와 Workspace lifecycle E2E는 1440px·1920px desktop을 검증하며, 후자는 registry reopen·explicit relaunch recovery와 no-root/Runtime failure를 닫는다. Root `test:prepared-workspace-product-actual`은 같은 public action contract를 fresh Bootstrap workspace의 exact Runtime·selected-only read·same-Turn Review·AY-owned checkpoint와 full process cleanup으로 보완하고, `test:product-entrypoint`는 canonical OS process graph·SIGINT를 독립 검증한다.
