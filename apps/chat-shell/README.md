...
| SemesterWorkspace | `/api/product/bootstrap`의 browser-safe product outcome으로 active workspace·Course·material registry를 hydrate하고 loading, inactive, no Course, empty, incompatible와 Server failure를 학생용 copy로 구분한다. Persistence store version은 Browser 계약으로 사용하지 않는다. |
