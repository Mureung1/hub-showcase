...
| Product Chat | Free-form composer는 `/api/product/chat/messages`만 사용하며 Account와 ready workspace가 준비되면 Course·ModelingRun 생성 전에도 일반 대화를 시작할 수 있다. 일반 Plan clarification은 Review와 별도 binding 및 answer/cancel route로 같은 Turn을 이어가며 학업 상태를 만들지 않는다. Active operation 중 conflicting action·send·source mutation은 닫는다. |
