...
| Product application state | Bootstrap의 Account readiness, active workspace와 settled-only history 전체를 한 owner가 보존한다. Workspace mutation과 settled Review 뒤 authoritative bootstrap을 다시 읽으며, pending prompt·private correlation·persistence metadata는 Browser 상태로 만들지 않는다. Reject reload는 no-apply confirmation을 다시 열되 confirmed model과 revision을 바꾸지 않는다. |
...
| Lifecycle | Sidebar hide/show는 mounted product controller와 stream을 유지한다. Reload는 settled bootstrap만 다시 열고 이전 transcript, unanswered prompt나 replacement 대기 상태를 복원하지 않는다. Accepted operation interrupt는 public operation binding으로 요청하고 authoritative terminal까지 stream을 소비한다. |
