...
| Workspace recovery | Ready workspace의 `cleanup_required | source_conflict | store_conflict`를 읽는 동안 자료 선택, Assignment와 Chat mutation을 닫는다. Source conflict는 원본 TXT를 보존한 채 `현재 TXT를 새 기준으로 채택`하는 explicit rebaseline만 열고, cleanup/store conflict는 같은 workspace의 명시적 재활성화로 authority를 다시 확인한다. 성공 결과는 일반 refresh와 source rebaseline을 구분해 표시하며 incompatible store는 기존 read-only 안내를 유지한다. |
