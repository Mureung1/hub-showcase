...
`WorkspaceLifecycleView`는 Workspace/Runtime/registry recovery, no-root 안내와 failed explicit relaunch 뒤 previous-workspace reopen을 1440px·1920px desktop Chromium에서 검증한다. Browser entry는 production `index.html` 하나이며 prepared-workspace lifecycle과 AY Chat만 시작한다.
...
| Workbench | 1440–1920px desktop에서 lifecycle, full-width AY Chat, Review·clarification control과 keyboard focus를 제공한다. |
