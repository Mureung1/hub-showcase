...
| Source action | Text source만 source-list order로 최대 16개 선택하고 `선택한 자료 정리하기`를 눌렀을 때만 exact ordered refs와 현재 Codex settings를 동결해 `/api/product/actions`의 `organize_sources`를 시작한다. PDF·unsupported source와 limit 초과 선택은 accessible reason과 함께 닫고, successful reload 뒤에는 current text source와의 교집합만 보존한다. Action은 별도 transcript entry로 표시하고 기존 Product operation stream·Review·clarification·interrupt·terminal controller를 재사용하며 terminal 뒤 selection을 자동으로 지우지 않는다. |
...
Unit suite는 target-only operation decoder, lifecycle·source projection, ordered action selection, exact Semantic Review binding과 Browser NDJSON parser를 검증한다. Default Playwright trace는 실제 Vite·target Express composition과 deterministic Runtime에서 active prepared lifecycle, source list·text/PDF/unsupported preview, explicit `organize_sources` selection·frozen request·action transcript, operation lock·retry, evidence navigation, normal AY Chat 격리, 일반 clarification·interrupt와 semantic proposal의 accept·revise·reject settlement를 검증한다. Action workbench와 Workspace lifecycle E2E는 1440px·1920px desktop을 검증하며, 후자는 registry reopen·explicit relaunch recovery와 no-root/Runtime failure를 닫는다. Root `test:product-entrypoint`의 canonical OS process graph·SIGINT gate와 Server prepared startup/application tests가 startup·listener·process-tree cleanup을 독립적으로 보완한다.
