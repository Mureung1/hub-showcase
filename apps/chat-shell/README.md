...
| Product application state | Bootstrap의 Account readiness, coarse `operationStatus`, active workspace와 settled-only history 전체를 한 owner가 보존한다. Review 수락·거절 ACK와 finite response loss는 `active` 응답도 허용하는 one-shot authoritative read로 durable decision을 확인하고, mount·workspace mutation·stream loss·terminal hydration·reload는 Server drain 시간값을 복사하지 않는 `active → idle` final read를 사용한다. Pending prompt·private correlation·persistence metadata는 Browser 상태로 만들지 않는다. Reject reload는 no-apply confirmation을, recovery reload는 settled Run outcome만 다시 열고 confirmed model과 revision을 임의로 바꾸지 않는다. |
