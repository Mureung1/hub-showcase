...
| Conversation | 명시적인 새 대화 action으로 native `threadId` 하나를 만들고 화면의 diagnostic metadata로 유지한다. Authoritative terminal과 HTTP stream settlement 뒤 같은 thread에 새 native turn을 이어간다. Reload 뒤 resume하거나 별도 ref로 remap하지 않는다. |
