...
export function NotesPage() {
  ...
  const [activeId, setActiveId] = useState(projectNotes[0]?.id ?? null)
  ...
  return (
    <section className={styles.notesPage} aria-label="공유 노트">
      <aside className={styles.noteList}>
        <header><div><h1>공유 노트</h1><button type="button" onClick={() => setShowTemplates(true)} aria-label="새 노트 만들기"><Plus size={16} /></button></div><label><Search size={13} /><span className="visually-hidden">노트 검색</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="노트 검색" /></label></header>
        <div className={styles.noteItems}>{filtered.map((note) => { const author = state.members.find((member) => member.id === note.authorId); const snippet = note.content.split('\n').find((line) => line.trim() && !/^[#>*-]/.test(line))?.replace(/[*`]/g, '') ?? ''; return <button type="button" className={note.id === activeId ? styles.activeNote : ''} key={note.id} onClick={() => { setActiveId(note.id); setPreview(false) }}><strong>{note.title || '제목 없음'}</strong><span>{author ? <Avatar member={author} /> : null}<em>{formatShortDate(note.updatedAt)}</em><small>{snippet}</small></span></button> })}{filtered.length === 0 ? <p>검색 결과가 없습니다.</p> : null}</div>
      </aside>
      <main className={styles.editorPane}>
        {activeNote ? <><div className={styles.toolbar}>{preview ? <span className={styles.previewTag}>미리보기</span> : <div>{toolbar.map(({ label, icon: Icon, before, after }) => <button type="button" key={label} title={label} aria-label={label} onClick={() => insert(before, after)}><Icon size={15} /></button>)}</div>}<div>{saveStatus ? <span className={saveStatus === 'saved' ? styles.saved : ''}>{saveStatus === 'saved' ? '저장됨' : '저장 중…'}</span> : null}<button type="button" onClick={() => setPreview((value) => !value)}>{preview ? <PenLine size={14} /> : <Eye size={14} />}{preview ? '편집으로 돌아가기' : '미리보기'}</button></div></div><div className={styles.editorScroll}><article className={styles.noteDocument}><header>{preview ? <h1>{activeNote.title || '제목 없음'}</h1> : <input value={activeNote.title} onChange={(event) => update({ title: event.target.value })} placeholder="제목 없음" />}<span>{(() => { const author = state.members.find((member) => member.id === activeNote.authorId); return author ? <Avatar member={author} /> : null })()}<em>{formatShortDate(activeNote.updatedAt)}</em></span></header><div className={styles.documentBody}>{preview ? <MarkdownPreview content={activeNote.content} /> : <textarea ref={textareaRef} value={activeNote.content} onChange={(event) => update({ content: event.target.value })} placeholder="# 제목&#10;&#10;내용을 자유롭게 입력하세요." />}</div></article></div></> : <div className={styles.noNote}><p>노트를 선택하거나 새로 만드세요</p><button type="button" onClick={() => setShowTemplates(true)}><Plus size={15} />새 노트</button></div>}
      </main>
      {showTemplates ? <NoteTemplateModal onSelect={addTemplate} onClose={() => setShowTemplates(false)} /> : null}
    </section>
  )
}
