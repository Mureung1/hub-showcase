...
export interface MockStanceTemplate {
  ...
  sectionIds: string[];
  /**
   * 원문에서 실제로 잘라낸 부분 문자열(§11 검증 2·3번). 지어내지 않는다.
   * 각 quote는 이 stance의 provider 섹션 content의 부분 문자열이어야 한다.
   */
  quotes: string[];
}

export interface MockAgendaTemplate {
  kind: "consensus" | "conflict" | "single_source";
  title: string;
  summary: string;
  stances: MockStanceTemplate[];
  /** Consensus 합의 내용 — conflict는 사용자 판단 시 채워지므로 null */
  selectedContent: string | null;
  /** 재검토 요청 시 반환되는 Mock Manager 재검색 응답 본문 (conflict 전용) */
 ...
 *
 * SPEC-AI-001 8.1 확장: order(표시 순서)·kind(유형 라벨)를 함께 채운다.
 * kind는 자유 문자열이며 고정 유형 집합이 아니다(분류 기준은 SPEC-AI-002).
 */
...
/**
 * 성공한 SourceAnswer의 StructuredContent.summary (SPEC-AI-001 8.1 신규).
 * 실제 파이프라인에서는 provider 응답을 정규화해 채우며, provider가 주지 못하면
 * null이다(data-model 1.6). Mock은 세 Provider 모두 성공 경로이므로 값을 채운다.
 */
export const mockSummaryByProvider: Record<Provider, string> = {
  claude:
    "RLS는 켜는 것 자체가 아니라 정책을 어떻게 쓰느냐가 보안 수준을 결정하므로, 기본 거부로 시작해 정책을 마이그레이션으로 관리하고 다른 계정으로 검증하라.",
  openai:
    "RLS 활성화와 정책 작성을 한 작업 단위로 다루고, 목적별로 정책을 나눠 정의하며 정책 조건 컬럼의 인덱스로 성능까지 함께 챙겨라.",
  gemini:
    "RLS는 DB 계층에서 행 단위 접근을 강제하는 수단이므로, 테이블별 활성화 상태를 확인하고 역할·키 경계를 지키며 배포 전 체크리스트로 점검하라.",
};
    ...
    stances: [
      {
        ...
        quotes: ["모든 public 스키마 테이블에 RLS를 기본 ON으로 켜고"],
      },
      {
        provider: "openai",
        text: "RLS 활성화와 정책 작성을 하나의 작업 단위로 취급",
        sectionIds: ["openai-s1"],
        quotes: ["RLS 활성화와 정책 작성은 하나의 작업 단위로 취급"],
      },
      {
        provider: "gemini",
        text: "테이블별로 RLS를 켜고 생성 직후 상태를 확인",
        sectionIds: ["gemini-s2"],
        quotes: ["생성 직후 상태를 확인하는 습관이 필요합니다"],
      },
    ],
  },
  {
    ...
    stances: [
      {
        ...
        quotes: ["정책은 SQL 마이그레이션 파일로 작성해 버전 관리하는 것을 권장합니다"],
      },
      {
        provider: "openai",
        ...
        quotes: [
          "처음이라면 Supabase 대시보드의 Policies 화면에서 제공하는 템플릿으로 시작하는 것이 빠릅니다",
        ],
      },
      {
        provider: "gemini",
        ...
        quotes: [
          "정책은 SQL 파일로 관리하되, select/insert/update/delete를 나눠 정의하는 것을 권장합니다",
        ],
      },
    ],
  },
    ...
    stances: [
      {
        ...
        quotes: ["service_role 키는 RLS를 우회하므로 반드시 서버 환경에서만 사용해야 합니다"],
      },
      {
        provider: "openai",
        ...
        quotes: [
          "익명 사용자(anon)와 인증 사용자(authenticated) 역할을 구분해 정책을 작성해야 의도치 않은 공개를 막을 수 있습니다",
        ],
      },
      {
        provider: "gemini",
 ...
 * 입장만 담는다 (0.5 Should).
 * SPEC-AI-002 §3.4·§9.2에서 단일 소스 Agenda 정책이 확정됐다: kind="single_source"로
 * auto_single_source(자동 통과)한다. 다만 여러 AI의 비교가 아니므로 "합의/Consensus"
 * 표현과 auto_consensus 라벨은 여전히 쓰지 않는다(domain-policy 5.3·SPEC-AI-001 §6.1
 * 고정: 단일 소스를 다중 AI 합의로 표현 금지).
 */
