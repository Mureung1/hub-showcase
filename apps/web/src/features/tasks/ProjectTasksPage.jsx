  ...
  return (
    <section className={workspace.scrollPage} aria-labelledby="tasks-title">
      <div className={workspace.container}>
        <header className={workspace.pageHeader}><div><p>{project.name} · 할 일</p><h1 id="tasks-title">할 일 관리</h1></div><button className={workspace.primaryButton} type="button" onClick={openTaskCreate}><Plus size={15} />새 할 일</button></header>
        <div className={styles.stats}>{filters.map((status) => { const [color, background] = statColors[status]; const active = filter === status; return <button key={status} type="button" className={active ? styles.statActive : ''} style={{ '--stat-color': color, '--stat-background': background }} onClick={() => setFilter(status)}><span>{status === 'all' ? '전체' : TASK_STATUS_LABEL[status]}</span><strong className={workspace.mono}>{count(status)}</strong></button> })}</div>
        <div className={styles.toolbar}>
          <div className={styles.viewSwitch}><button type="button" className={view === 'list' ? styles.activeView : ''} onClick={() => setView('list')}>목록</button><button type="button" className={view === 'board' ? styles.activeView : ''} onClick={() => setView('board')}>보드</button></div>
