...
export function TaskCreateModal({ projectId, members, onClose }) {
  ...
  const assignableMembers = members.filter(isAssignableMember)
  const collaborators = assignableMembers.filter((member) => member.kind === 'user')
  const aiAgents = assignableMembers.filter((member) => member.kind === 'ai' || member.isAi)
  const [values, setValues] = useState({ title: '', assigneeId: assignableMembers[0]?.id ?? '', dueDate: '', status: TASK_STATUS.NOT_STARTED, description: '' })
  ...
  return (
    <Modal title="새 할 일 추가" onClose={onClose} width={480} footer={<><button type="button" className={`${forms.footerButton} ${forms.cancelButton}`} onClick={onClose} disabled={submitting}>취소</button><button type="submit" form="new-task-form" className={`${forms.footerButton} ${forms.submitButton}`} disabled={submitting || !values.title.trim() || !values.assigneeId || !values.dueDate}>{submitting ? '등록 중...' : '할 일 추가'}</button></>}>
...
function isAssignableMember(member) {
  return member.kind === 'user' || member.kind === 'ai' || member.isAi
}
