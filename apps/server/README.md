...
Internal `semester-workspace-store` module이 current-v2 JSON codec·aggregate invariant, store path의 physical read/write, canonical byte comparison과 temporary-file rename을 한 interface 뒤에 소유한다. `SemesterWorkspaceController`는 이 module의 open·write·match 결과를 기존 serialized product transaction 안에서 사용하며 transaction ordering, active in-memory authority와 store 교체 시점을 계속 단독 소유한다. Persistence module은 별도 mutable cache나 lock을 만들지 않는다.
