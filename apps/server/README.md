...
`organize_sources`는 dynamic manifest가 아닌 Server-owned 단일 action definition이다. Strict request decode와 shared lease 예약 뒤 account/settings, 선택한 `1..16`개 current text ref와 invocation의 effective Skill을 initial preflight한다. `operation.preparing` 전달이 끝난 뒤 Runtime dispatch 직전에 current refs·effective Skill·prepared input을 각각 한 번 final validation한다. Exact workspace의 enabled `.agents/skills/ay-ple-first-assignment`와 canonical non-symlink `SKILL.md`만 허용하고, `workspace_write`, optional current settings, resolved Skill 하나와 `32 KiB` 이하 fixed action text 하나를 startup-approved thread의 Product Turn에 전달한다. Initial File·Skill·Runtime context failure는 Turn과 NDJSON을 열기 전에 safe JSON으로 닫고, `operation.preparing` 대기 중 생긴 drift는 Runtime 호출 없이 safe `action_context_stale | action_context_invalid | action_unavailable | product_unavailable` streamed `failed` terminal로 닫는다. Preflight disconnect는 Turn을 시작하지 않고 shutdown은 final revalidation까지 같은 signal을 abort한다. Accepted action은 normal Chat과 같은 activity, general clarification, Semantic Review, interrupt·disconnect와 terminal projection을 사용한다.
