...
`organize_sources`는 dynamic manifest가 아닌 Server-owned 단일 action definition이다. Strict request decode와 shared lease 예약 뒤 account/settings, 선택한 `1..16`개 current safe regular-file ref와 invocation의 effective Skill을 initial preflight한다. `operation.preparing` 전달이 끝난 뒤 Runtime dispatch 직전에 current refs·effective Skill·prepared input을 각각 한 번 final validation한다. Exact workspace의 enabled `.agents/skills/ay-ple-first-assignment`와 canonical non-symlink `SKILL.md`만 허용하고, `workspace_write`, optional current settings, resolved Skill 하나와 basename label·workspace-relative destination의 Markdown file-reference 목록을 담은 `32 KiB` 이하 fixed action text 하나를 startup-approved thread의 Product Turn에 전달한다. Initial File·Skill·Runtime context failure는 Turn과 NDJSON을 열기 전에 safe JSON으로 닫고, `operation.preparing` 대기 중 생긴 drift는 Runtime 호출 없이 safe `action_context_stale | action_context_invalid | action_unavailable | product_unavailable` streamed `failed` terminal로 닫는다. Preflight disconnect는 Turn을 시작하지 않고 shutdown은 final revalidation까지 같은 signal을 abort한다. Accepted action은 normal Chat과 같은 activity, general clarification, Semantic Review, interrupt·disconnect와 terminal projection을 사용한다.
...
| `POST /api/product/actions` | Closed `organize_sources` request의 current safe regular-file refs와 exact workspace-local Skill을 fresh 검증한 뒤 Markdown file references를 담은 shared Product operation NDJSON을 시작한다. |
...
Prepared-workspace product actual은 native Bootstrap Skill의 exact CLI를 fresh temporary Git root에서 실행한 뒤, 그 root의 tracked v4 identity·`AGENTS.md`·project MCP declaration과 installed First Assignment Skill을 사용한다. Public `organize_sources`에는 fixture의 두 selected regular-file ref를 보내고, exact verified Runtime·isolated TypeScript First Assignment conformance provider가 workspace Skill body와 Markdown file-ref text, selected-only actual-file read, project MCP call과 `auto_review`를 관찰한다. 같은 native Turn의 revise→fresh Review→accept 흐름은 Review 전 bytes·index·HEAD 불변, accept 뒤 AY 역할의 explicit intended-path mutation·Git checkpoint와 unrelated dirty·untracked 보존을 검증하고, 두 번째 reject는 no-mutation으로 끝낸다. 기존 direct Adapter trace는 evidence digest·quote·path·symlink failure, busy, Turn interrupt, Browser disconnect와 Runtime·Adapter loss가 normal result·file mutation·checkpoint 없이 정산되는지 보완한다. Teardown은 credential 재사용 거절, Adapter held response, exact Node·Python·native process group, provider listener, Server listener와 temporary root를 완전히 닫고 production startup resolver·registry나 workspace 밖 bytes를 바꾸지 않는다.
