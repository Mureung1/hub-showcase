...
2. Call \`propose_state_patch\` exactly once with the evidence-backed Assignment proposal.
...
5. After the response, finish with a short result message in this same Turn.

Do not replace either tool call with prose and do not finish before the Review response. The
application, not the model or filesystem, owns confirmed-state changes.
`
