  ...
  return (
    <div className="staff-shell">
      ...
      <aside className="staff-sidebar">
        <nav>
          <a className="is-active" href="#queue"><UsersRound size={20} />통합 대기열</a>
          <button type="button" onClick={() => setShowCategorySettings(true)}>
            <Settings2 size={20} />환자 분류 설정
          </button>
          <button type="button" onClick={onOpenHospitalManagement}>
            <Building2 size={20} />병원 관리
          </button>
        </nav>
      ...
      <main className="staff-main" id="queue">
        ...
        {queueStatusError && (
          <div className="notice notice--error" role="alert">
            <CircleX size={20} />
            <div><strong>원격 접수 상태를 변경하지 못했습니다</strong><p>{queueStatusError}</p></div>
          </div>
        )}
        <section className="queue-metrics" aria-label="대기열 요약">
          <div><UsersRound /><span>대기 환자</span><strong>{totalPatients}명</strong></div>
          <div>
            <Wifi /><span>원격</span>
            <strong>{activeQueueRows.filter(({ entry }) => entry.source === "remote").length}건</strong>
          </div>
          <div>
            <UserCheck /><span>현장</span>
            <strong>{activeQueueRows.filter(({ entry }) => entry.source === "onsite").length}건</strong>
          </div>
          <div><Clock3 /><span>평균 진료</span><strong>10분</strong></div>
        </section>
        {queueStatus === "paused" && (
