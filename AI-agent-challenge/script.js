  ...
  balanced: [
    {
      ...
      level: "쉬움",
      steps: ["두부는 키친타월로 물기를 제거한 뒤 먹기 좋은 크기로 자릅니다.", "팬에 김치를 볶고 두부를 넣어 3분 정도 더 익힙니다.", "밥 위에 볶은 두부 김치를 올리고 계란프라이를 얹습니다.", "대파가 있으면 잘게 썰어 마지막에 올려 향을 더합니다."],
    ...
    },
    {
      ...
      level: "매우 쉬움",
      steps: ["따뜻한 밥을 그릇에 담습니다.", "계란프라이를 반숙으로 익혀 밥 위에 올립니다.", "간장과 참기름을 넣고 골고루 비빕니다."],
    ...
    },
    {
      ...
      level: "보통",
      steps: ["냄비에 김치와 물을 넣고 8분 정도 끓입니다.", "두부와 양파를 넣고 중불에서 더 끓입니다.", "간을 보고 부족하면 소금이나 국간장을 조금 추가합니다."],
...
function setMessage(text) {
  ...
  if (!text) return;
  window.setTimeout(() => { ingredientMessage.textContent = ""; }, 2200);
}
...
function setIngredientFormOpen(open) {
  ...
  if (open) ingredientInput.focus();
}

function resetForm(options = {}) {
  ingredientForm.reset();
  storageInput.value = activeStorage === "all" ? "fridge" : activeStorage;
  categoryInput.value = "단백질";
  expiryInput.value = addDays(5);
  editingIngredientId = null;
  formPanelTitle.textContent = "새 재료 등록";
  submitIngredient.textContent = "재료 등록";
  if (options.close) setIngredientFormOpen(false);
}

function getVisibleIngredients() {
  if (activeStorage === "all") return ingredients;
  return ingredients.filter((ingredient) => ingredient.storage === activeStorage);
}

function renderIngredients() {
  const visibleIngredients = getVisibleIngredients();
  ingredientTiles.innerHTML = visibleIngredients.length
        ...
        return `
          <article class="ingredient-tile ${isUrgent ? "urgent" : ""}">
            ...
            <div class="tile-badges">
              <span class="storage-badge ${ingredient.storage}">${storageLabels[ingredient.storage]}</span>
    ...
    : `<div class="empty-board">${storageLabels[activeStorage]} 보드에 등록된 재료가 없습니다.<br>+ 재료 추가로 집 재료를 채워보세요.</div>`;
  ...
  boardTitle.textContent = `${storageLabels[activeStorage]} 재료 보드`;
  boardCount.textContent = `${visibleIngredients.length}개`;
}

function renderMenus() {
  const menus = menusByFilter[selectedFilter];
  menuCards.innerHTML = menus.map((menu) => {
    const used = menu.used.map((item) => `<span class="chip">${item}</span>`).join("");
    const missing = menu.missing.length ? menu.missing.map((item) => `<span class="chip missing">${item}</span>`).join("") : `<span class="chip">부족 재료 없음</span>`;
    return `
      <article class="menu-card ${selectedMenuId === menu.id ? "selected" : ""}" data-menu-id="${menu.id}">
        <span class="menu-badge">${menu.badge}</span>
        <h3>${menu.name}</h3>
        <div class="menu-meta">
          <div class="meta-box"><span>조리 시간</span><strong>${menu.time}</strong></div>
          <div class="meta-box"><span>영양 균형</span><strong>${menu.balance}</strong></div>
        </div>
...
function renderRecipe(menu) {
  recipeDetail.innerHTML = `
    <div class="recipe-header">
      <div><p class="eyebrow">레시피 상세</p><h2>${menu.name}</h2><p class="summary-text">${menu.summary}</p></div>
      <div class="recipe-stats">
        <div class="stat-card"><span class="label">조리 시간</span><strong>${menu.time}</strong></div>
        <div class="stat-card"><span class="label">난이도</span><strong>${menu.level}</strong></div>
      </div>
    </div>
    <ol class="steps">${menu.steps.map((step, index) => `<li><span class="step-number">${index + 1}</span><span>${step}</span></li>`).join("")}</ol>
    ${menu.missing.length ? `<div class="substitute-box"><strong>대체 재료 안내</strong><br>${menu.substitutes}</div>` : ""}
  `;
}
...
function renderShopping(menu) {
  ...
  shoppingPanel.innerHTML = `
    <div class="section-title"><div><p class="eyebrow">Step 4</p><h2>부족 재료 구매 추천</h2></div></div>
    ${missingItems.length ? `<div class="shopping-list">${missingItems.map((item) => `<div class="shopping-card"><h3>${item}</h3><p>${menu.name}에 넣으면 맛과 완성도가 올라가는 추천 구매 재료입니다.</p><button class="buy-button" type="button">더미 구매 버튼</button></div>`).join("")}</div>` : `<div class="empty-shopping">이 메뉴는 현재 재료만으로 만들 수 있습니다.</div>`}
  `;
}
...
toggleIngredientForm.addEventListener("click", () => {
  ...
  if (editingIngredientId) resetForm();
  setIngredientFormOpen(true);
});

closeIngredientForm.addEventListener("click", () => resetForm({ close: true }));
...
ingredientForm.addEventListener("submit", (event) => {
  ...
  const duplicated = ingredients.some((ingredient) => ingredient.name === name && ingredient.id !== editingIngredientId);
  if (duplicated) {
  ...
  if (editingIngredientId) {
    ...
    setMessage("재료 정보를 수정했습니다.");
  } else {
    ingredients.unshift({ id: nextIngredientId, name, quantity, storage, category, expiry });
    nextIngredientId += 1;
    setMessage("집 재료 보드에 새 재료를 추가했습니다.");
  }

  if (activeStorage !== "all" && activeStorage !== storage) activeStorage = storage;
...
storageTabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    updateStorageTabs();
    if (!editingIngredientId) resetForm({ close: true });
    renderIngredients();
  });
});
