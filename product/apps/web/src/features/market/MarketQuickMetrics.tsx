...
export function MarketQuickMetrics({
  ...
  const flow = formatCount(analysis?.raw.total_flow ?? null, "명");
  const categoryStoreCount = analysis ? `${analysis.raw.category_store_count}개` : null;
    ...
    <section className="market-quick-metrics" aria-label="이 지역 한눈에 보기">
      ...
      <div className="quick-metrics-list">
        ...
        <div>
          ...
          <span>개·폐업</span>
          <b>{netTurnover === null ? metricValue(null, analysisState) : `순증 ${netTurnover > 0 ? "+" : ""}${netTurnover}개`}</b>
        </div>
      </div>
    </section>
  );
