...
const STORE_RAW_POINT_MIN_ZOOM = STORE_CLUSTER_MAX_ZOOM;
const STORE_ICON_MIN_ZOOM = 16.15;
const STORE_LABEL_MIN_ZOOM = 16.8;
const STORE_RAW_POINT_MAX_ZOOM = 17;
...
function StoreClusterLayers() {
  const fadeOut: ExpressionSpecification = ["interpolate", ["linear"], ["zoom"], 10, 0.94, 14.5, 0.9, 14.8, 0.6, 14.98, 0.2];
  return <>
...
function StoreRawPointLayers({ densityMode, hasFocusedStore, selectedFeatureId }: { densityMode: boolean; hasFocusedStore: boolean; selectedFeatureId: string }) {
  const pointOpacity: number | ExpressionSpecification = hasFocusedStore ? 0.5 : ["interpolate", ["linear"], ["zoom"], 15, 0.08, 15.25, densityMode ? 0.62 : 0.72, 16.2, 0.82, 16.55, 0.58, 16.85, 0.18, 17, 0];
...
export function StorePointSource({ data, selectedFeatureId, densityMode, hasFocusedStore }: { data: ReturnType<typeof createStoreFeatureCollection>; selectedFeatureId: string; densityMode: boolean; hasFocusedStore: boolean }) {
  return <>
    <Source id={STORE_POINT_SOURCE_ID} type="geojson" data={data} cluster clusterMaxZoom={STORE_CLUSTER_MAX_ZOOM} clusterRadius={64} clusterMinPoints={3} maxzoom={18} buffer={64} />
    <StoreClusterLayers />
