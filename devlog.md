...
**8. 레시피 tip에 참고 영상 링크 추가**
- 오류: 없음 — 기능 추가 요청
- 요청: "닭가슴살 스테이크 소스 다른버전있거든 참고하라고 tip에 추가하고싶어" + 유튜브 링크
- 검토·적용: Agent가 해당 영상(타임스탬프 33:23)을 확인해보니 실제로는 "소스 버전"이 아니라 황진선 셰프의 다른 스타일(맑은 국물) 레시피였음을 발견해 먼저 사실관계를 확인·보고 → 사용자가 "시간대 설명 없이 링크만 넣어달라"고 정리해줘서 `kangryeo-chicken-breast-steak`의 `tip` 필드에 링크만 추가
- 성공 확인: 로컬에서 레시피 상세 화면의 Tip 문구에 링크가 정상적으로 표시됨, `npm run seed:recipes`로 배포 DB에도 반영