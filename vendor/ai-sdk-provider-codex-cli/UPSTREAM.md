...
App Server `LanguageModelV4` adapter는 이 patch에서 제거하지 않는다. 이 checkpoint에서는 `AppServerLanguageModel → TurnStreamController → AppServerTurnEventRouter → AppServerTurnResultCollector`가 thread start/resume, request-context-before-turn-start, turn binding, authoritative terminal과 cleanup을 끝까지 조립하는 유일한 executable consumer이고 live smoke도 이 경로를 사용한다. 따라서 first-party Python의 `CodexClient → Thread → TurnHandle → TurnResult` 책임 분리를 따르는 native facade와 facade-level fake orchestration oracle이 이 역할을 인수한 뒤에만 App Server provider/emitter/projection과 AI SDK dependency를 제거한다. Full T0/T0-C/T0.1 unit·actual-child fake와 필요한 live conformance는 transport hardening 뒤의 별도 gate다.

## Native facade orchestration takeover

Fork patch `FP-0007b`는 pinned Python external client의 `CodexClient → Thread → TurnHandle → TurnResult` 책임 분리를 TypeScript package-private facade로 옮긴다. `thread/start`와 `thread/resume`의 successful response `thread.id`, `turn/start` response `turn.id`가 handle identity authority다. `NativeCodexThread.run()`은 `turn()`과 `NativeCodexTurnHandle.run()`의 convenience이며, high-level `run()`만 authoritative failed terminal의 message를 `Error`로 승격한다. Raw terminal result가 필요한 temporary AI SDK adapter는 handle의 internal completion을 소비해 기존 failed finish projection을 유지한다.

`NativeCodexTurnHandle`은 기존 `AppServerTurnEventRouter`와 `AppServerTurnResultCollector`를 다시 구현하지 않고 조립한다. Router subscription과 request context registration은 `turn/start` 전에 이루어지고, per-thread lock은 response identity와 context binding까지만 보호한다. Collector를 만든 뒤 response `turn.id`를 router에 bind하므로 response 전 ingress FIFO로 staged된 item·usage·terminal도 같은 result로 replay된다. Matching terminal은 result를 한 번 정산하고 listener와 bound context를 정리하며, `turn/start` failure는 pending context와 listener를 정리한다. 현재 fork가 이미 채택한 pre-response terminal staging은 pinned Python router보다 강한 기존 hardening이므로 유지한다.

AI SDK `TurnStreamController`는 이 patch 이후 subscription, request-context lifecycle, turn correlation, collector와 terminal promise를 따로 소유하지 않는다. `NativeCodexTurnHandle`이 전달한 correlated event와 raw completion을 `AppServerAiSdkProjection`이 AI SDK stream part와 finish metadata로만 변환한다. Facade-level in-memory fake는 response-first/notification-first 수렴, start/resume response identity, pre-response completed AgentMessage, context ordering/cleanup, foreign thread·turn isolation, terminal 후 duplicate/late result·projection no-op와 failed-run rejection을 검증한다.

이는 actual child를 시작하지 않는 orchestration checkpoint다. Initialize/initialized, sole JSONL ingress와 terminal을 한 child scenario로 묶은 T0, A pending → B complete → A complete T0-C, command approval lease T0.1, bounded staging, disconnect settlement, safe directional `RequestId`, raw-byte framing, writer와 close/reap hardening은 증명하지 않는다. Native facade와 projection은 package root에서 export하지 않으며 root workspace, production pin·ledger와 기존 Host도 바꾸지 않는다.

## Native turn notification stream

Fork patch `FP-0008a`는 AI SDK projection을 제거하기 전에 확인된 실행 의존성을 먼저 해소한다. `NativeCodexThread.turn()`은 authoritative `turn/start` response 뒤에 handle을 반환하지만, response 전 notification은 이미 router에서 replay될 수 있다. Constructor-time observer만 남기고 projection을 삭제하면 caller가 그 event를 소비할 수 없으므로, `NativeCodexTurnHandle` 생성 시점부터 연결되는 package-private FIFO와 single-consumer `stream()`을 추가했다.

이는 pinned Python external client의 pending-turn notification queue와 `TurnHandle.stream()` 책임을 TypeScript async iterator로 옮긴 것이다. Matching direct `turnId` 또는 nested `turn.id`를 가진 notification만 ingress order로 전달하고 authoritative `turn/completed` 뒤 stream을 닫는다. Turn ID가 없는 thread-scoped notification과 Server request는 stream에 섞지 않는다. Server request는 기존 RPC handler 경로에 남아 있으며 original ID once-only lease는 T0.1 hardening이 소유한다. Foreign turn, terminal 뒤 late/duplicate event는 기존 correlated handle cleanup으로 제외된다.

Handle 반환 전 terminal까지 도착한 sequence도 fork가 이미 request 전부터 소유권을 등록하므로 이후 drain할 수 있다. 이는 pinned Python router가 unregistered early terminal을 버리는 동작을 일반화한 것이 아니라, 기존 fork의 pre-response terminal regression을 보존한 local hardening이다. Consumer가 iterator를 일찍 닫으면 stream sink만 버리고 active turn을 자동 interrupt하지 않으며 collector와 request context는 matching terminal까지 유지된다. Queue는 아직 unbounded이고 process disconnect가 stream/completion을 함께 실패시키는 경로도 없으므로 transport hardening이나 T0 completion으로 간주하지 않는다.

Temporary `TurnStreamController`는 이 stream을 즉시 claim하고 terminal까지 drain해 compatibility path가 같은 notification을 duplicate backlog로 남기지 않게 한다. AI stream-part projection은 notification과 Server request가 섞인 donor ingress order를 보존하기 위해 기존 synchronous correlated-event observer를 유지한다. Native completion과 notification drain을 함께 기다린 뒤 finish를 내보내며, cross-kind regression은 `item/started`가 뒤이은 command approval Server request보다 먼저 projection되는지 고정한다. Raw-chunk observer도 유지한다. 이 연결은 surface를 아직 제거하지 않고 다음 contraction 전에 executable donor path가 native backlog lifecycle을 실제로 소유하게 한 것이다.
...
| `FP-0007b` native facade                  | 완료   | Package-private client/thread/turn handle이 nominal orchestration을 소유하고 AI SDK controller는 projection만 담당한다.       |
| `FP-0008a` native turn stream             | 완료   | Response 전 matching notification을 보존하는 single-consumer FIFO stream을 handle에 추가하고 Server request와 분리한다.       |
| Production integration                    | 미착수 | Root workspace, `packages/runtime-codex`, Server와 Inspector는 이 fork를 import하거나 실행하지 않는다.                        |

`FP-0008a` 이후에도 App Server AI SDK event/public surface, request·Server request 중심 handwritten model, generic `request<T>()`·`notify()`, bounded staging과 transport hardening이 남아 있다. 다음 contraction은 native facade와 stream fake oracle을 유지하면서 temporary AI SDK surface의 실제 dependency를 제거하는 작업이다. 이 문서와 patch ledger는 fork-local provenance와 현재 구현 경계만 기록하며, 제품 task order와 completion status는 [AY-PLE 개발 백로그](../../docs/product/ay-ple-development-backlog.md)가 소유한다. 기존 AY-PLE spec이나 Wayfinder는 fork 내부 acceptance criterion으로 사용하지 않는다.
...
`FP-0007a` checkpoint에서는 App Server non-live unit/integration test 357개가 통과했고 opt-in live test 1개는 skip 상태를 유지했다. 제거된 Exec/CodexCli test 127개는 retired production lane과 duplicate alias regression이며 App Server AI SDK integration 2개, language-model 53개, provider 19개, router/controller/emitter와 RPC regression은 보존했다. Exact pin/generated verification, build, retired source/root declaration boundary verification, typecheck, format, lint와 현재 active Markdown 3개 validation은 green이다. Private fork declaration hash는 `d146eeb0f629bbc3fa22835e1f14130ebfbe3d4da1c3540bb4ed4a86f5658e9a`로 의도적으로 바뀌었고 runtime value export는 9개, dry-run package roster는 OpenAI license·notice를 포함한 7개 entry다. Root `npm test`, `npm run typecheck`, `npm run build`와 Inspector lint도 final rerun에서 green이며 independent Source·Standards·Spec review는 각각 0 findings로 수렴했다. `validate:examples:app-server`는 package-local Codex를 시작하는 opt-in live gate라는 분류를 유지하며 process-per-call code만 제거한 이번 patch에서는 실행하지 않는다. Bounded staging, transport hardening, native T0·T0-C·T0.1 actual-child/live conformance와 live smoke는 아직 증명하지 않았다.

`FP-0007b` checkpoint에서는 App Server non-live unit/integration test 368개가 통과했고 opt-in live test 1개는 skip 상태를 유지했다. Exact pin/generated verification, build, private boundary verification, typecheck, format, lint와 active Markdown validation은 green이다. Public declaration은 clean `FP-0007a` rebuild와 byte-identical한 `d146eeb0f629bbc3fa22835e1f14130ebfbe3d4da1c3540bb4ed4a86f5658e9a`를 유지했고 runtime value export 9개와 OpenAI license·notice를 포함한 dry-run package entry 7개도 변하지 않았다. Root `npm test`, `npm run typecheck`, `npm run build`와 Inspector lint는 final rerun에서 green이다. Independent review에서 발견한 replay terminal 후 late event, projection first-settlement, duplicate start, disposed-before-start와 verification record 문제는 code·test·문서에 환류했고 Source·Standards·Spec review는 각각 0 findings로 수렴했다. 이 patch는 in-memory orchestration owner를 이동하지만 actual child·auth/provider를 새로 검증하지 않으므로 `validate:examples:app-server`와 live smoke는 실행하지 않았다. Native streaming, bounded staging, transport hardening, Server request lease와 native T0·T0-C·T0.1 actual-child/live conformance는 후속 patch로 남아 있다.

`FP-0008a` checkpoint에서는 native facade regression 7개와 controller drain·cross-kind ordering regression 2개를 추가해 non-live unit/integration test 377개가 통과했고 opt-in live test 1개는 skip 상태를 유지했다. Exact pin/generated verification, build, private boundary, typecheck, format, lint와 active Markdown validation은 green이다. Root `npm test`, `npm run typecheck`, `npm run build`와 Inspector lint도 green이다. Public declaration/runtime export와 dry-run package roster는 바꾸지 않았다. 이 patch는 in-memory notification stream과 A/B interleaving만 검증하므로 실제 Codex child, auth/provider, `validate:examples:app-server`와 live smoke는 실행하지 않았다. Independent review에서 발견한 mixed notification·Server request projection order, 중복 turn-ID 추출, bypass fake의 exact generated fixture 누락과 formatting 문제를 code·test에 환류했고 Source·Standards·Spec review는 각각 0 findings로 수렴했다.
