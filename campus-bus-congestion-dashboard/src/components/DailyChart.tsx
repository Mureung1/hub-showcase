...
interface DailyChartProps { stop: Stop | undefined; hour: number; }

export function DailyChart({ stop, hour }: DailyChartProps) {
  ...
  return (
    <section className={styles.card} aria-labelledby="daily-title">
      <div className={styles.header}><div className={styles.headerText}><div className={styles.eyebrow}>DAILY PATTERN</div><h2 id="daily-title" className={styles.title}>{stop ? stop.name : '정류장'} 혼잡도</h2></div><div className={styles.badge}>현재 {badge} 기준</div></div>
      <div className={styles.chart} role="img" aria-label={`${stop?.name ?? '선택한 정류장'}의 24시간 혼잡도 막대 그래프`}>
