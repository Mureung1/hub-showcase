...
- [x] **Task 9: [BE/FE] 가설 유지 / 수정 / 폐기 기능 구현** (대시보드 측)
  - *상세:* `PATCH /api/projects/:id/hypotheses/:hid`로 `hypotheses.status` 갱신 (컬럼은 스키마에 이미 존재해 마이그레이션 불필요). 허용값 화이트리스트(검토 전/유지/수정/폐기) 검증 + `project_id` 소속 확인 후 갱신된 행을 반환하고, FE는 클라이언트가 보낸 값이 아니라 **응답의 확정값**으로 해당 행만 교체. 같은 판단을 재클릭하면 `검토 전`으로 되돌아가는 토글로 동작.
  - *AI 제안 vs 사용자 확정 분리:* `verification_status`(AI 제안, 배지)와 `status`(사용자 확정)는 별개 컬럼이므로 행에서 "AI 권고: …"와 "내 판단: …"을 시각적으로 분리 표시. 사용자가 AI 권고와 다르게 판단하는 것도 정상 동작으로 허용.
  - *범위 결정:* 상세 화면이 아직 플레이스홀더(Task 10 미완)라 **이번엔 대시보드 판단 UI만** 구현. 상세 화면 측 판단 지정은 Task 10에서 상세 화면을 지을 때 함께 붙인다. `수정` 판단은 여기서 **상태 플래그만** 세팅하며, 실제 원인/결과 편집·버전 저장은 Task 10/12에서 연결(Task 9가 다른 Task에 의존하지 않고 자체 완결되도록).
  - *완료 조건:* 판단 지정 시 DB의 `status`가 갱신되고 화면에 즉시 반영되며, 새로고침 후에도 유지됨.

- [x] **Task 10: [FE] 가설 상세 화면 + 참조 시스템 + 사이드 드로어**
  - *상세:* 검증결과 문단 렌더 시 `[1]` 마커를 **클릭 가능한 참조 링크로 파싱**하여 `citations`의 `evidence_tag_id`와 연결(`citations`에 대응 없는 마커는 일반 텍스트로 렌더 — 환각 방어 원칙 유지). 참조 번호 클릭 ➡️ 사이드 드로어에 **해당 태그 + 전사문 발췌 + 화자 + 출처 인터뷰명** 제시. 수정 방향성 · 핵심 근거 블록 표시. 뒤로가기로 대시보드 복귀.
  - *상세 화면 측 판단:* Task 9에서 미룬 대로, 유지/수정/폐기 버튼을 여기서도 붙임(대시보드와 동일 PATCH 재사용).
...
- [x] **Task 16: [BE] Gemini 응답 검증 (환각 방어)**
