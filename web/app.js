...
(function () {
  ...
  /**
   * 수집할 논문 수. `?limit=`으로 덮어쓸 수 있고, 서버가 `MAX_LIMIT`(10)으로 자른다.
   *
   * **3이 아니라 8인 이유** — `limit`은 수집량과 판단 후보 폭을 동시에 정한다(#80).
   * 3편을 받아오면 judge 프롬프트의 "3~5편을 고른다"를 만족시키는 방법이 전부 고르는
   * 것뿐이라 `excluded`가 비고, 그러면 "제외된 N편 보기"가 화면에서 통째로 사라진다.
   * 제외 사유는 이 서비스가 절대 버리지 않기로 한 셋 중 하나라 그럴 수 없다.
   *
   * 대가는 비용이다. 논문 1편당 LLM을 4회 이상 부르므로 8편이면 20~28회가 나간다
   * (무료 티어 RPM 15 · RPD 500). **터미널 개발은 여전히 `--limit 3` 이하로 돈다** —
   * 이 값은 사람이 보는 화면에만 적용된다.
   */
