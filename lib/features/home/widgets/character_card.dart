...
class CharacterHero extends StatelessWidget {
  ...
  /// 지평선은 세로 비율(위에서 70% 지점)이라 크롭과 무관하게 제자리를 지키므로
  ...
  ///
  /// 배경 도트아트는 아래 30%가 잔디다 — 백드롭이 히어로를 통째로 채우므로
  /// (예전처럼 여백만큼 작지 않다) 지평선은 바닥에서 270 × 30% = **81dp**이고
  /// 잔디 밴드는 y 189~270이다. 발밑을 그보다 **아래**에 두면 캐릭터가
