...
### Backend
- [x] `GET /records?year=&month=` 월별 목록 API

### Frontend
- [x] 월 이동 (이전/다음)
- [x] 기록 있는 날 썸네일 표시
- [x] 기록 없는 날 빈 칸
- [x] 날짜 클릭 → 기록 상세 모달/페이지

**검증**
- [x] 기록 3일 이상 데이터로 썸네일 정확히 표시 (데모 계정 7일치 시드 데이터로 확인)
- [x] 월 경계(1일·말일) 전환 정상 (7월→6월 전환 시 데이터 없음 정상 표시 확인)
- [x] 타인 기록은 캘린더에 노출되지 않음 (demo-friend 계정으로 확인, 서로의 기록 미노출)
