...
- [ ] 공공데이터포털 레시피로 데이터 소스 교체 — **보류**. 후보(식품안전나라 `COOKRCP01`)를 조사해보니 재료가 비정형 문자열(파싱 필요), 가격 정보 없음, 유튜브 영상 없음이 겹쳐서 이번엔 보류하고 큐레이션한 `mockRecipes.js` 유지. 가격 정보 포함된 다른 데이터셋이 나오면 재검토 (`task.md` P2)
...
- [ ] 홈 화면 음식종류·시간 필터 (`plan.md` 기능 A에 명시돼 있으나 아직 미구현 — `Home.jsx`엔 필터 UI 자체가 없음)
- [ ] 모바일 반응형 레이아웃 점검
